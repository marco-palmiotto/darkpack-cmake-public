...
void Diagonalizer::applyDiagonalization(std::initializer_list<complex> massMatrix,
                                        std::initializer_list<complex*> transfer, std::initializer_list<real*> masses)
{
...
void Diagonalizer::applyBiDiagonalization(std::initializer_list<complex> massMatrix,
...
Diagonalizer::Diagonalizer(std::initializer_list<complex> massMatrix, bool t_bidiag)
        : m_data(massMatrix), m_N(std::sqrt(m_data.size())), m_bidiag(t_bidiag), m_computed(false)
{
...
std::vector<Diagonalizer::real> const& Diagonalizer::mass()
{
  updateDiagonalization();
  return m_mass;
}
...
Diagonalizer::real Diagonalizer::det(std::vector<Diagonalizer::complex> const& mat) const
{
  return REAL(mat[index(0, 0)] * mat[index(1, 1)] - mat[index(1, 0)] * mat[index(0, 1)]);
}
...
void Diagonalizer::diagonalize(std::vector<complex> massMatrix, std::vector<complex>& transfer, std::vector<real>& mass,
                               bool bidiagonalization)
{
  ...
  if (!bidiagonalization)
  {
    ...
    for (size_t i = 0; i != N; ++i)
      ...
      {
        massMatrix[index(i, j)] = (massMatrix[index(i, j)] + massMatrix[index(j, i)]) / 2.;
...
void Diagonalizer::swap(const size_t i, const size_t j, std::vector<complex>& transfer, std::vector<real>& mass)
{
...
void Diagonalizer::sort(std::vector<complex>& transfer, std::vector<real>& mass)
{
...
void Diagonalizer::sort(std::vector<complex>& transfer, std::vector<complex>& transfer2, std::vector<real>& mass)
{
  for (size_t i = 0; i != mass.size(); ++i)
  {
    size_t i_min = i;
...
void Diagonalizer::bidiagonalize()
{
  ...
  {
    assert(ABS(m_mass[i] - mass2[i]) / ((m_mass[i] == 0) ? 1 : m_mass[i]) < precision);
...
gsl_matrix_complex* Diagonalizer::getGSLMassMatrix(std::vector<complex> const& massMatrix) const
{
  gsl_matrix_complex* matrix = gsl_matrix_complex_alloc(m_N, m_N);
  for (size_t i = 0; i != m_N; ++i)
    for (size_t j = 0; j != m_N; ++j)
      gsl_matrix_complex_set(matrix, i, j,
                             gsl_complex_rect(REAL(massMatrix[index(i, j)]), IMAG(massMatrix[index(i, j)])));

  return matrix;
}

void Diagonalizer::loadGSLMatrix(gsl_matrix_complex const* matrix, std::vector<complex>& target)
{
  ...
  for (size_t i = 0; i != m_N; ++i)
    for (size_t j = 0; j != m_N; ++j)
    {
      ...
      target[index(i, j)] = {GSL_REAL(gslcomplex), GSL_IMAG(gslcomplex)};
    }
}

void Diagonalizer::loadGSLVector(gsl_vector const* vect, std::vector<real>& target)
{
...
void Diagonalizer::positiveDiagonal(std::vector<complex>& transfer)
{
  for (size_t j = 0; j != m_N; ++j)
  {
...
std::vector<Diagonalizer::complex> Diagonalizer::hermitian(std::vector<complex> const& init) const
{
...
std::vector<Diagonalizer::complex> Diagonalizer::dot(std::vector<complex> const& A, std::vector<complex> const& B) const
{
