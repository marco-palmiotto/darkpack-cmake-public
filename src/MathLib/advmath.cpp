...
real_t integrate_gauss(real_t xmin, real_t xmax, std::function<real_t(real_t)> f, int order)
{
  ...
  constexpr const real_t abs6[6]={-0.9324695142,-0.6612093865,-0.2386191861,0.2386191861,0.6612093865,0.9324695142};
  constexpr const real_t wei6[6]={0.1713244924,0.3607615730,0.4679139346,0.4679139346,0.3607615730,0.1713244924};

  constexpr const real_t abs7[7]={-0.9491079123,-0.7415311856,-0.4058451514,0.,0.4058451514,0.7415311856,0.9491079123};
  constexpr const real_t wei7[7]={0.1294849662,0.2797053915,0.3818300505,0.4179591837,0.3818300505,0.2797053915,0.1294849662};

  constexpr const real_t abs8[8]={-0.9602898565,-0.7966664774,-0.5255324099,-0.1834346425,0.1834346425,0.5255324099,0.7966664774,0.9602898565};
  constexpr const real_t wei8[8]={0.1012285363,0.2223810345,0.3137066459,0.3626837834,0.3626837834,0.3137066459,0.2223810345,0.1012285363};

  constexpr const real_t wei9[9]={0.3302393550012598, 0.1806481606948574, 0.1806481606948574, 0.0812743883615744, 0.0812743883615744, 0.3123470770400029, 0.3123470770400029, 0.2606106964029354, 0.2606106964029354};
  constexpr const real_t abs9[9]={ 0., -0.8360311073266358, 0.8360311073266358, -0.9681602395076261, 0.9681602395076261, -0.3242534234038089, 0.3242534234038089, -0.6133714327005904, 0.6133714327005904};
  ...
  for(int i = 0 ; i < order; i++) integral+=wei[i]*fut[i].get();
  
  return integral*m; 
}

real_t integrate_gauss_comparative(real_t xmin, real_t xmax, std::function<real_t(real_t)> f, real_t maxdiscrepancy, real_t *err)
{
  ...
  if(err != nullptr) *err = discrepancy;
  return res2;
}
...
//       printf("Entering cycle with npoints = %d, prev_npoints = %d, prev_start = %d\n", npoints, prev_npoints, prev_start);
      ...
      for( i = 0; i < prev_npoints-1; i++)
      {
        ...
        {
            printf("i = %d, error on the old stepsize: %.4e %4.e\n", i, abscissas_sorted[i+1]-abscissas_sorted[i], deltax  );
#ifdef DEBUG      
            for(k = 0 ; k < newpoint-1; k++) printf("abscissas_sorted[%d] = %.4e\n", k, abscissas_sorted[k]);
#endif
#ifdef DEBUG2
            for(k = 0 ; k < newpoint-1; k++) printf("abscissas_unsorted[%d] = %.4e\n", k, abscissas_unsorted[k]);
#endif
            exit(2);
        }
...
#ifdef DEBUG2      
      printf("result is %.4e +- %.4e (%.4e). Bad comparison, resetting things\n", kinfac_onlysqrtS*res2, kinfac_onlysqrtS*discrepancy*res2, discrepancy);
#endif
      res1=res2;
      res2/=deltax;
#ifdef DEBUG      
...
#ifdef DEBUG      
      for(k = 0 ; k < npoints; k++) printf("abscissas_sorted[%d] = %.4e\n", k, abscissas_sorted[k]);
#ifdef DEBUG2
      for(k = 0 ; k < npoints; k++) printf("abscissas_unsorted[%d] = %.4e\n", k, abscissas_unsorted[k]);
#endif
#endif
...
// Source 
// Paul Godfrey (2022). Special Functions math library (https://www.mathworks.com/matlabcentral/fileexchange/978-special-functions-math-library), MATLAB Central File Exchange.
  if( round(z.real()) == z.real() && round(z.imag()) == z.imag()   && z.real() <= 0. && z.imag() == 0.) return INFINITY;
...
/* K0exp(x,z)=K0(x)*exp(z)*sqrt(z) with x=num_x/denom, z =num_z/denom*/
{
  ...
  const real_t z=num_z/denom;
	...
	{
		y=2./num_x*denom;
				return (std::exp((num_z-num_x)/denom)*sqrt(num_z/num_x))*(1.25331414+y*(-0.7832358e-1+y*(0.2189568e-1+y*(-0.1062446e-1+y*(0.587872e-2+y*(-0.251540e-2+y*0.53208e-3))))));
	}
}
...
/* K1exp(x,z)=K1(x)*exp(z)*sqrt(z) with x=num_x/denom, z =num_z/denom*/
{
  ...
  const real_t z=num_z/denom;
	...
	{
		y=2./num_x*denom;
		return (std::exp((num_z-num_x)/denom)*sqrt(num_z/num_x))*(1.25331414+y*(0.23498619+y*(-0.3655620e-1+y*(0.1504268e-1+y*(-0.780353e-2+y*(0.325614e-2+y*(-0.68245e-3)))))));
	}
}
