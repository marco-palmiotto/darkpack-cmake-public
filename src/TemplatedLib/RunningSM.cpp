...
void RunningSM::init(const Param_t &input)
{
  ...
  for( i = STRANGE; i>=UP; i--) QuarkMassQ[i] = 2.;
  ...
  }else
  {
    ...
    for( i = TOP ; i >=UP ; i--) LastQuarkMassQ[i]=input.Running_scale;    
  }
#ifdef DEBUG
...
{
    if(this->alphas_MZ!=other.alphas_MZ)return false;
    if(this->mass_Z!=other.mass_Z)return false;
    if(this->m_d2GeV!=other.m_d2GeV)return false;
    if(this->m_u2GeV!=other.m_u2GeV)return false;
    if(this->m_s2GeV!=other.m_s2GeV)return false;
    if(this->mc_mc!=other.mc_mc)return false;
    if(this->mb_mb!=other.mb_mb)return false;
    if(this->mass_top_pole!=other.mass_top_pole)return false;
    if(this->mass_b_pole!=other.mass_b_pole)return false;
    if(this->mass_c_pole!=other.mass_c_pole)return false;
    for (int i = 0; i < 6; i++) 
    {
        if(this->QuarkMass[i]!=other.QuarkMass[i])return false;
        if(this->QuarkMassQ[i]!=other.QuarkMassQ[i])return false;
        if(this->LastQuarkMass[i]!=other.LastQuarkMass[i])return false;
        if(this->LastQuarkMassQ[i]!=other.LastQuarkMassQ[i])return false;
    }
    if(this->alphas!=other.alphas)return false;
    if(this->Q_alphas!=other.Q_alphas)return false;
    if(this->Lambda3!=other.Lambda3)return false;
    if(this->alphasMZ_Lambda3!=other.alphasMZ_Lambda3)return false;
    if(this->Lambda4!=other.Lambda4)return false;
    if(this->alphasMZ_Lambda4!=other.alphasMZ_Lambda4)return false;
    if(this->Lambda5!=other.Lambda5)return false;
    if(this->alphasMZ_Lambda5!=other.alphasMZ_Lambda5)return false;
    if(this->Lambda6!=other.Lambda6)return false;
    if(this->alphasMZ_Lambda6!=other.alphasMZ_Lambda6)return false;
    return true;
}
...
real_t RunningSM::AlphaStrong(real_t Q , real_t mtop)
{
...
#endif
   if(std::fabs(Q- Q_alphas) < NEGLIGIBLE_ENERGY_DIFFERENCE) return alphas;
  ...
  if(mtop<0.0) mtop=mass_top_pole;
  ...
  if(Lambda5==0.||std::fabs(1.-alphasMZ_Lambda5/alphas_MZ)>1.e-5)
  {
     ...
     while((std::fabs(1.-alphas_min/alphas_MZ)>=1.e-4)&&(std::fabs(1.-Lambda_min/Lambda_max)>1.e-5))
     {
       ...
       alphas_min=4.*M_PI/beta0/log_MZ_Lambda_min*(1.-2.*beta1/beta0_sq*std::log(log_MZ_Lambda_min)/log_MZ_Lambda_min+4.*beta1_sq/std::pow(beta0_sq*log_MZ_Lambda_min,2.)*(std::pow(std::log(log_MZ_Lambda_min)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

       Lambda_avg=(Lambda_min+Lambda_max)/2.;
       real_t log_MZ_Lambda_avg = std::log(std::pow(mass_Z/Lambda_avg,2.));

       alphas_avg=4.*M_PI/beta0/log_MZ_Lambda_avg*(1.-2.*beta1/beta0_sq*std::log(log_MZ_Lambda_avg)/log_MZ_Lambda_avg+4.*beta1_sq/std::pow(beta0_sq*log_MZ_Lambda_avg,2.)*(std::pow(std::log(log_MZ_Lambda_avg)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

       if((alphas_MZ>=alphas_min)&&(alphas_MZ<=alphas_avg))
       Lambda_max=Lambda_avg;
       else Lambda_min=Lambda_avg;
     }//End of the while
  ...
  *executed, the two values coincide*/
  ...
  /* 5 active flavours */
  {
     alphas_running=4.*M_PI/beta0/std::log(std::pow(Q/Lambda5,2.))*(1.-2.*beta1/beta0_sq*std::log(std::log(std::pow(Q/Lambda5,2.)))/std::log(std::pow(Q/Lambda5,2.))+4.*beta1_sq/std::pow(beta0_sq*std::log(std::pow(Q/Lambda5,2.)),2.)*(std::pow(std::log(std::log(std::pow(Q/Lambda5,2.)))-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
     alphas = alphas_running; Q_alphas  = Q; return alphas_running;
  }
  else if((Q>mtop))
  /* 6 active flavours */
  {
     alphas_running=4.*M_PI/beta0/std::log(std::pow(mtop/Lambda5,2.))*(1.-2.*beta1/beta0_sq*std::log(std::log(std::pow(mtop/Lambda5,2.)))/std::log(std::pow(mtop/Lambda5,2.))+4.*beta1_sq/std::pow(beta0_sq*std::log(std::pow(mtop/Lambda5,2.)),2.)*(std::pow(std::log(std::log(std::pow(mtop/Lambda5,2.)))-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
     ...
     if(Lambda6==0.||std::fabs(1.-alphasMZ_Lambda6/alphas_MZ)>1.e-5)
     {
       ...
       while((std::fabs(1.-alphas_min/alphas_running)>=1.e-4)&&(std::fabs(1.-Lambda_min/Lambda_max)>1.e-5))
       {
          real_t log_mtop_Lambda_min = std::log(std::pow(mtop/Lambda_min,2.));
                  
          alphas_min=4.*M_PI/beta0/log_mtop_Lambda_min*(1.-2.*beta1/beta0_sq*std::log(log_mtop_Lambda_min)/log_mtop_Lambda_min+4.*beta1_sq/std::pow(beta0_sq*log_mtop_Lambda_min,2.)*(std::pow(std::log(log_mtop_Lambda_min)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

          Lambda_avg=(Lambda_min+Lambda_max)/2.;
          real_t log_mtop_Lambda_avg = std::log(std::pow(mtop/Lambda_avg,2.));
                  
          alphas_avg=4.*M_PI/beta0/log_mtop_Lambda_avg*(1.-2.*beta1/beta0_sq*std::log(log_mtop_Lambda_avg)/log_mtop_Lambda_avg+4.*beta1_sq/std::pow(beta0_sq*log_mtop_Lambda_avg,2.)*(std::pow(std::log(log_mtop_Lambda_avg)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
     ...
     *executed, the two values coincide*/
     ...
     alphas_running=4.*M_PI/beta0/std::log(std::pow(Q/Lambda6,2.))*(1.-2.*beta1/beta0_sq*std::log(std::log(std::pow(Q/Lambda6,2.)))/std::log(std::pow(Q/Lambda6,2.))+4.*beta1_sq/std::pow(beta0_sq*std::log(std::pow(Q/Lambda6,2.)),2.)*(std::pow(std::log(std::log(std::pow(Q/Lambda6,2.)))-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
  ...
  /* 4 active flavours */
  {
     alphas_running=4.*M_PI/beta0/std::log(std::pow(mbot/Lambda5,2.))*(1.-2.*beta1/beta0_sq*std::log(std::log(std::pow(mbot/Lambda5,2.)))/std::log(std::pow(mbot/Lambda5,2.))+4.*beta1_sq/std::pow(beta0_sq*std::log(std::pow(mbot/Lambda5,2.)),2.)*(std::pow(std::log(std::log(std::pow(mbot/Lambda5,2.)))-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
     ...
     if(Lambda4==0.||std::fabs(1.-alphasMZ_Lambda4/alphas_MZ)>1.e-5)
     {
       ...
       while((std::fabs(1.-alphas_min/alphas_running)>=1.e-4)&&(std::fabs(1.-Lambda_min/Lambda_max)>1.e-5))
       {
          ...
          alphas_min=4.*M_PI/beta0/log_mbot_Lambda_min*(1.-2.*beta1/beta0_sq*std::log(log_mbot_Lambda_min)/log_mbot_Lambda_min+4.*beta1_sq/std::pow(beta0_sq*log_mbot_Lambda_min,2.)*(std::pow(std::log(log_mbot_Lambda_min)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

          Lambda_avg=(Lambda_min+Lambda_max)/2.;
          real_t log_mbot_Lambda_moy = std::log(std::pow(mbot/Lambda_avg,2.));
                  
          alphas_avg=4.*M_PI/beta0/log_mbot_Lambda_moy*(1.-2.*beta1/beta0_sq*std::log(log_mbot_Lambda_moy)/log_mbot_Lambda_moy+4.*beta1_sq/std::pow(beta0_sq*log_mbot_Lambda_moy,2.)*(std::pow(std::log(log_mbot_Lambda_moy)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
     ...
     if(Q>QuarkMass[CHARM])
     {
       ...
       alphas_running=4.*M_PI/beta0/log_Q_lambda4*(1.-2.*beta1/beta0_sq*std::log(log_Q_lambda4)/log_Q_lambda4+4.*beta1_sq/std::pow(beta0_sq*log_Q_lambda4,2.)*(std::pow(std::log(log_Q_lambda4)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

       alphas = alphas_running; Q_alphas  = Q; return alphas_running;
     }
     else
     {       
       alphas_running=4.*M_PI/beta0/std::log(std::pow(QuarkMass[CHARM]/Lambda4,2.))*(1.-2.*beta1/beta0_sq*std::log(std::log(std::pow(QuarkMass[CHARM]/Lambda4,2.)))/std::log(std::pow(QuarkMass[CHARM]/Lambda4,2.))+4.*beta1_sq/std::pow(beta0_sq*std::log(std::pow(QuarkMass[CHARM]/Lambda4,2.)),2.)*(std::pow(std::log(std::log(std::pow(QuarkMass[CHARM]/Lambda4,2.)))-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
       ...
       if(Lambda3==0.||std::fabs(1.-alphasMZ_Lambda3/alphas_MZ)>1.e-5)
       {
          ...
          while((std::fabs(1.-alphas_min/alphas_running)>=1.e-4)&&(std::fabs(1.-Lambda_min/Lambda_max)>1.e-5))
          {
             ...
             alphas_min=4.*M_PI/beta0/log_mcha_Lambda_min*(1.-2.*beta1/beta0_sq*std::log(log_mcha_Lambda_min)/log_mcha_Lambda_min+4.*beta1_sq/std::pow(beta0_sq*log_mcha_Lambda_min,2.)*(std::pow(std::log(log_mcha_Lambda_min)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));

             Lambda_avg=(Lambda_min+Lambda_max)/2.;
             real_t log_mcha_Lambda_moy = std::log(std::pow(QuarkMass[CHARM]/Lambda_avg,2.));
                  
             alphas_avg=4.*M_PI/beta0/log_mcha_Lambda_moy*(1.-2.*beta1/beta0_sq*std::log(log_mcha_Lambda_moy)/log_mcha_Lambda_moy+4.*beta1_sq/std::pow(beta0_sq*log_mcha_Lambda_moy,2.)*(std::pow(std::log(log_mcha_Lambda_moy)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
       ...
       alphas_running=4.*M_PI/beta0/log_Q_lambda3*(1.-2.*beta1/beta0_sq*std::log(log_Q_lambda3)/log_Q_lambda3+4.*beta1_sq/std::pow(beta0_sq*log_Q_lambda3,2.)*(std::pow(std::log(log_Q_lambda3)-1./2.,2.)+beta2*beta0/8./beta1_sq-5./4.));
...
{  
  ...
  real_t mtop=mass_top_pole/(1.+alphas_mtop/M_PI*(4./3.+alphas_mtop/M_PI*(307./32.+M_PI*M_PI/3.+M_PI*M_PI/9.*log(2.)-1./6.*ZETA3-71./144.*5.)));

  alphas_mtop=AlphaStrong(mtop, mtop);
  
  return mass_top_pole/(1.+alphas_mtop/M_PI*(4./3.+alphas_mtop/M_PI*(307./32.+M_PI*M_PI/3.+M_PI*M_PI/9.*std::log(2.)-1./6.*ZETA3-71./144.*5.))); /* 0906.5273 */
}
...
/* computes the running quark mass at the energy Qfin, from a given running quark mass quark_mass at energy Qinit */
{
  ...
  /* 4 active flavours at Qinit */
  {
     ...
     R_Qinit = std::pow(beta0/2./M_PI*alphas_Qinit,2.*gamma0/beta0)*(1.+(2.*gamma1/beta0-beta1*gamma0/beta0/beta0)*alphas_Qinit/M_PI+1./2.*(std::pow(2.*gamma1/beta0-beta1*gamma0/beta0/beta0,2.)+2.*gamma2/beta0-beta1*gamma1/beta0/beta0-beta2*gamma0/16./beta0/beta0+beta1*beta1*gamma0/2./beta0/beta0/beta0)*std::pow(alphas_Qinit/M_PI,2.));
     ...
     if(Qfin <= mbot) 
     {
       ...
       R_Qfin = std::pow(beta0/2./M_PI*alphas_Qfin,2.*gamma0/beta0)*(1.+(2.*gamma1/beta0-beta1*gamma0/beta0/beta0)*alphas_Qfin/M_PI+1./2.*(std::pow(2.*gamma1/beta0-beta1*gamma0/beta0/beta0,2.)+2.*gamma2/beta0-beta1*gamma1/beta0/beta0-beta2*gamma0/16./beta0/beta0+beta1*beta1*gamma0/2./beta0/beta0/beta0)*std::pow(alphas_Qfin/M_PI,2.));
...
{  
  ...
  return mc_mc*(1+alphas_mc/M_PI*(4./3.+alphas_mc/M_PI*((13.4434-1.0414*3.+1.0414*4./3.*(
     (m_u2GeV + m_d2GeV + m_s2GeV)/mc_mc)))));
}
...
{  
  ...
  return mb_mb*(1.+alphas_mb/M_PI*(4./3.+alphas_mb/M_PI*((13.4434-1.0414*4.+1.0414*4./3.*(
    (m_u2GeV + m_d2GeV + m_s2GeV + mc_mc)/mb_mb))
...
{  
  ...
  return mc_mc*(1+alphas_mc/M_PI*(4./3.+alphas_mc/M_PI*((13.4434-1.0414*3.+1.0414*4./3.*((m_u2GeV + m_d2GeV + m_s2GeV)/mc_mc))
...
/* computes the running c mass mc(mc) */
{
  ...
  do
  {
     ...
     if(loop==2) mcpole_med=GetMcPole(); 
     else if(loop==1) mcpole_med=GetMcPole1Loop(); 
     else mcpole_med=GetMcPole3Loops();
...
real_t RunningSM::GetQuarkMass(enum ParticlesList part, const real_t &Qf)
{
  if( part > RunningSM::TOP || part < RunningSM::UP)
  {
   ...
   if(part == BEAUTY)
   {
       ...
       if(mass_b_pole < mb) return mass_b_pole;
       else return mb;
   }
   ...
   if(part == TOP)
   {
       ...
       if(mass_top_pole < mt) return mass_top_pole;
       else return mt;
   }
...
#ifdef MODEL_MSSM
#define FACABREF std::abs(input.m_d*input.V_td*std::conj(input.V_td.get()) + input.m_s*input.V_ts*std::conj(input.V_ts.get()) + input.m_b*input.V_tb*std::conj(input.V_tb.get()))
  const real_t Atref= input.A_t; //m_t is the factor
  const real_t Abref = input.A_b; // std::abs(input.m_d*input.V_td*std::conj(input.V_td.get()) + input.m_s*input.V_ts*std::conj(input.V_ts.get()) + input.m_b*input.V_tb*std::conj(input.V_tb.get()))
...
void TestRunningMasses( RunningSM * const run, Param_t &input)
{
  ...
  csl::InitSanitizer<real_t>* values[7]=
  {
   &input.g_s,  &input.m_u, &input.m_d, &input.m_s,
