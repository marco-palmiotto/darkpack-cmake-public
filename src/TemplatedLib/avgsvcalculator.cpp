...
template <typename T> static inline T sq(const T x) { return x * x; }

static inline unsigned int kdelta(const int i, const int j) { return static_cast<unsigned int>(i == j); }

namespace __SPEC_LIB_NAME__
{

  inline real_t getContributionSingle(Process2to2 p, Param_t input, const real_t Ecm, const real_t ctheta)
  {
    return p.get_process_dependent_contrib_dweff_dcos(input, Ecm, ctheta);
  }

  inline real_t getContribution(const std::shared_ptr<std::vector<Process2to2>>& vp, Param_t input, const real_t Ecm,
                                const real_t ctheta, const bool cut = false)
  {
  ...
  void AvgSvCalculator::sort_bsm_spectrum()
  {
    ...
    std::sort(bsm_particles_sorted.begin(), bsm_particles_sorted.end(),
              [&](const int a, const int b) { return this->input.masses_vector[a] < this->input.masses_vector[b]; });
  ...
  void AvgSvCalculator::changeInput(const Param_t& input_ext, const bool hasMassSpetrumChanged)
  {
    ...
    if (g2_Wefftab.empty())
    {
      ...
      tabulateValues(std::max(10. * getMassLBSM(), maxenergy), 3000);
    }
  ...
  void AvgSvCalculator::setProcessList(std::shared_ptr<std::vector<Process2to2>> p_ext) { p_ptr = p_ext; }
  ...
  AvgSvCalculator::AvgSvCalculator(const AvgSvCalculator& other)
          : input(other.input), run(other.run), p_ptr(other.p_ptr), Weffcuts(other.Weffcuts), sqrtStab(other.sqrtStab),
            pefftab(other.pefftab), g2_Wefftab(other.g2_Wefftab), T_lim_sigmav(other.T_lim_sigmav),
            orderT(other.orderT), wastherenosplitting(false), werealldeltaxinull(false)
            /*
...
// This function calculates [g^2_LSP * dWeff/dcostheta]
  ...
  real_t AvgSvCalculator::get_g2_Weff(const real_t& sqrtS)
  {
    ...
    if (discr > 5.0e-3)
      weff = advmath::integrate_trap(-1., 1., lambda, 5.0e-3, &discr);
...
// no optimisations
  ...
  void AvgSvCalculator::setWeffcuts(bool x) { Weffcuts = x; }
...
//    Timer timer;
    ...
    auto no_duplicates =
        [](const real_t x, const std::vector<real_t>::iterator itbegin, const std::vector<real_t>::iterator itend)
    {
      for (std::vector<real_t>::iterator it = itbegin; it != itend; it++)
        if (advmath::getdiscrepancy(x, *it) < 1.0e-3)
          return false;
      return true;
    };
    ...
    for (auto i = 1; i <= corr::TOTAL_PARTICLES; i++)
    {
      ...
      if (temp < sqrtSmax && temp > sqrtSmin && no_duplicates(temp, sqrtStab.begin(), sqrtStab.end()))
...
#endif
      if (input.widths_vector[i] < 1.0e-6)
        continue;
#ifdef DEBUG
...
#ifdef DEBUG
    for (size_t i = 0; i < pefftab.size(); i++)
      std::cout << "i = " << i << " peff = " << pefftab[i] << std::endl;
    ...
    if (temp_size < Nmax)
    {
      ...
      for (size_t i = 0; i < pefftab.size(); i++)
        list.push_back({pefftab[i], g2_Wefftab[i]});
      ...
      while (temp_size < Nmax && next != list.end())
      {
        ...
        auto cond2 = [&]()
        {
          return std::abs(1. - (weffnext() - weff()) / (peffnext() - peff()) * (peff() - peffprev) /
                                   (weff() - weffprev)) > 0.05;
        };
        ...
        auto cond4 = [&]()
        {
          return std::abs((weffnext() - weff()) / (peffnext() - peff())) >
        ...
        while ((temp_size < Nmax && advmath::getdiscrepancy(peff(), peffprev) > 1.0e-3 &&
               ...
               std::abs((weffprev - weff()) / (weffprev + weff())) > 0.8)
        {
          ...
          if (advmath::getdiscrepancy(peffprev, newpeff) < 1.0e-12 || newpeff < peffprev)
            break;
      ...
      {
#ifdef DEBUG
        std::cout << "Processing entry: entry.first = " << entry.first << ", entry.second = " << entry.second
                  << std::endl;
#endif
...
#ifdef DEBUG
          std::cout << "Added: pefftab.back() = " << pefftab.back() << ", g2_Wefftab.back() = " << g2_Wefftab.back()
        ...
        {
#ifdef DEBUG
          std::cout << "Skipped: entry.first = " << entry.first << ", entry.second = " << entry.second << std::endl;
#endif
        }
      }
    }
  ...
  real_t AvgSvCalculator::getAverageSigmav_coan_noopti(const real_t& T)
  {
    ...
    {
      denominator +=
          corr::part_hel_dof[part] * SQUARE(input.masses_vector[part]) * advmath::K2(input.masses_vector[part] / T);
    }
    ...
    if (T < T1)
    {
      ...
      if (ftabptr != nullptr)
      {
        ...
        fprintf(ftabptr, "-1\t%.5e\t%.5e\t%.5e\n", T, T1,
                g2_Wefftab.front() / (4. * SQUARE(getMassLBSM()) * input.getLightestBSMdof()));
        fclose(ftabptr);
      }
#endif
...
//     if(C/C0 < 1.0e-6) break;
    ...
    for (auto part : corr::bsm_particles)
    {
      C = corr::part_hel_dof[part] * SQUARE(input.masses_vector[part]) *
          advmath::K2exp(input.masses_vector[part] / T, getMassLBSM() / T);
#ifdef DEBUG
...
// if(C/C0 < 1.0e-6) break;
  ...
  void AvgSvCalculator::setLimitingValues()
  {
    ...
    for (int count = 1; count <= maxcount && std::abs(limit - oldlimit) / std::abs(limit + oldlimit) >= maxdiscrepancy;
         count++)
    {
#ifdef DEBUG
    ...
    for (int count = 1; count < orderT; count++)
    {
      ...
      {
        orderT = count + 1;
        std::cout << "Error too large for order " << count + 1 << ": stopping at the order " << orderT
                  << " for the derivatives of Weff\n";
        break;
      }
    }
  }
...
#ifdef DEBUG
        std::cout << "i = " << i << " j = " << j << " Cij = " << CoefflowTsv[i][j] << " Wj = " << Weffderiv[j]
                  << " res = " << tempresult << std::endl;
  ...
  void AvgSvCalculator::setTaylorCoeffSVx_splitting(const real_t deltax[], const int M_rho, const int M_eta)
  {
    ...
    ci.emplace_back(1.);
    for (size_t i = 1; i < ci.capacity(); i++)
      ci.emplace_back(1. * (corr::part_hel_dof[bsm_particles_sorted[i]] / input.getLightestBSMdof() *
                            SQUARE(input.masses_vector[bsm_particles_sorted[i]] / getMassLBSM())));
    //  I define rho_tilde
    real_t rho_tilde = 0.;
    for (int i = 0; i < M_rho; i++)
      for (int j = i; j < M_rho; j++)
        rho_tilde += (2 - kdelta(i, j)) * ci[i] * ci[j];


    // I define eta_tilde
    real_t eta_tilde = 0.;
    for (int i = 0; i < M_eta; i++)
      for (int j = i; j < M_eta; j++)
        eta_tilde -= (1. - 0.5 * kdelta(i, j)) * ci[i] * ci[j] * (deltax[i] + deltax[j]);
      ...
      for (int M_eta_temp = 0; M_eta_temp < corr::SIZEPHYSICALBSM; M_eta_temp++)
      {
        ...
        for (int i = 0; i < M_eta_temp; i++)
          for (int j = i; j < M_eta_temp; j++)
            eta_o1_tab[M_rho_temp][M_eta_temp] -= (1. - 0.5 * kdelta(i, j)) * ci[i] * ci[j] * (deltax[i] + deltax[j]);
        eta_o1_tab[M_rho_temp][M_eta_temp] /= rho_tilde_tab[M_rho_temp][M_eta_temp];

        zerothcoeff[M_rho_temp][M_eta_temp] =
            std::pow(4. * rho_tilde_tab[M_rho_temp][M_eta_temp] * SQUARE(getMassLBSM()), -1) * Weffderiv[0] *
            (sv_beta[0] + sv_lambda[0] * eta_o1_tab[M_rho_temp][M_eta_temp]);
      }
    }
    ...
    for (int j = 0; j <= orderT; j++)
      phitilde[j] = sv_beta[j] + eta_o1 * sv_lambda[j];
  ...
  {
    if (T_lim_sigmav > 0. && T < T_lim_sigmav)
      return getAverageSigmav_coan_lowtemp(T);
