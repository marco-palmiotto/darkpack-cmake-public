...
{    
  ...
  while(Yeq(Tmin)==0.&&Tmin<Tmax_local) Tmin*=2.;
  if(Tmin>=Tmax_local)
  {
    this->Tfo=-1.;    
    return;
  ...
  if(1.-Sigmatildestar_init(Tmin,delta)>0.) signSigmatildestar=1; else signSigmatildestar=-1;
  ...
  {  
    ...
    if(std::fabs(1.-testTfo/testTfo_prev)>1.e-2) {if(testTfo*signSigmatildestar<0.) Tmax_local0=T; else Tmin0=T;}
  ...
  if(compteur<100)
  {
    ...
    if((1.-Sigmatildestar_init(Tmax_local,delta))*signSigmatildestar<=0.) Tmax_local-=Tmax_local0-Tmin0;
  }
  ...
  {  
    ...
    if(testTfo<0.) Tmax_local=T; else Tmin=T;
        
    compteur++;
  }
  while(std::fabs(1.-testTfo/testTfo_prev) > 1.e-2 && compteur<100);
      
  if(compteur==100||T<3.e-13||T>9.e3) this->Tfo=-3.; else this->Tfo=this->Tmax=T;
...
//   if(this->phi_model&&Yphi!=0.) darkdensitytilde=rho_phi/rhorad; else darkdensitytilde=dark_density(T)/rhorad;
  const real_t darkdensitytilde = ( phi_model.get()==1 && Yphi!=0. ?
                                    rho_phi/rhorad : 
                                    dark_density(T)/rhorad );
  
  const real_t Htilde=std::sqrt(1.+darkdensitytilde+dmdensitytilde); /*Htilde = H / sqrt(8 pi / 3 M_P^2) / rho_rad */

//   real_t Sigmarad;
//   if(this->phi_model&&Yphi!=0.) Sigmarad=this->Gamma_phi*rho_phi/T; else Sigmarad=entropy_Sigmarad(T);
  const real_t Sigmarad = ( phi_model.get()==1 && Yphi!=0. ?
                              this->Gamma_phi*rho_phi/T : 
                              entropy_Sigmarad(T) );
...
//   if(this->phi_model&&Yphi!=0.) nontherm=this->eta_phi*this->Gamma_phi*rho_phi; else nontherm=nonthermal(T);
  const real_t nontherm = ( phi_model.get()==1 && Yphi!=0. ?
                            this->eta_phi*this->Gamma_phi*rho_phi : 
                            nonthermal(T) );

  return std::sqrt(pi/45.)*Mplanck*T*getsqrtgstar(T)*(1.+darkentropytilde)/Htilde*(sv*(Y*Y-YeqT*YeqT)+(Y*(Sigmarad+dark_entropy_Sigmad(T))-nontherm)/std::pow(srad*(1.+darkentropytilde),2.))/(1.-Sigmatildestar_local);
}
...
{
  if(phi_model.get()==0||Yphi==0.) return 0.;
  ...
  rhot_phi0= (T_prev_boltzphi > Tinit*K_to_eV ) && ( T < Tinit*K_to_eV) ?
                    (  rhophi_prev_boltzphi + 
                      (std::log(this->Tinit*K_to_eV)-std::log(T_prev_boltzphi))*(Yphi*srad-rhophi_prev_boltzphi)/(std::log(T)-std::log(T_prev_boltzphi)) ) 
...
//   rho_phi=rhophi_prev_boltzphi=Yphi*srad;
  ...
  const real_t Htilde=std::sqrt(1.+darkdensitytilde+dmdensitytilde); /*Htilde = H / std::sqrt(8 pi / 3 M_P^2) / rho_rad */

  const real_t Sigmarad=Gamma_phi*rho_phi/T;
  
  const real_t Sigmatildestar_local=45.*std::sqrt(5.)/4./std::pow(pi,3.5)*Mplanck/heffT/std::sqrt(geffT)/std::pow(T,5.)/Htilde*Sigmarad;
    
  return std::sqrt(pi/45.)*Mplanck*T*getsqrtgstar(T)/Htilde*Gamma_phi*Yphi*(1.+Yphi/T)/srad/(1.-Sigmatildestar_local);
}
...
/* Computes the right-handed term of the decaying scalar_local field Klein-Gordon equation, with logarithmic derivative */
{
  if(phi_model.get()==0||Yphi==0.) return 0.;
  ...
  rhot_phi0= (T_prev_boltzphi > Tinit*K_to_eV) && (T < Tinit*K_to_eV) ? 
                    ( rhophi_prev_boltzphi+(std::log(Tinit*K_to_eV)-std::log(T_prev_boltzphi))*(Yphi*srad-rhophi_prev_boltzphi)/(std::log(T)-std::log(T_prev_boltzphi)) ) 
                    /(pi*pi/15.*std::pow(T,4.))
                    : 
                    rhot_phi0.get(); 
...
//   rho_phi=rhophi_prev_boltzphi=Yphi*srad;
  ...
  return std::sqrt(pi/45.)*Mplanck*T*getsqrtgstar(T)/Htilde*Gamma_phi*(1.+Yphi/T)/srad/(1.-Sigmatildestar_local);
}
/*--------------------------------------------------------------*/
...
//   testY=testYphi=testdx=1;    
  ...
  if(phi_model.get()==0) this->rhot_phi_Tmax=this->eta_phi=0.;
  ...
  if(this->Tmax<Tfin) return -1.;
  
  rho_phi=this->rhot_phi_Tmax*pi*pi/15.*std::pow(this->Tmax,4.);
  
#ifdef DEBUG
  if(this->full_comput==0) std::cout << this->Tfo << '\n';
//   FILE *sig,*phi0;
...
//   if(this->full_comput==0) adapt_error=adapt_error_phi=1.e-3; else adapt_error=adapt_error_phi=1.e-2;
  real_t adapt_error_phi= (this->full_comput==0 ? 
                                1.e-3 :
                                1.e-2);
...
//   else dx=(std::log(Tfin)-std::log(this->Tmax))*std::min(adapt_error,adapt_error_phi)*10.;
  ...
  {  
    ...
    if(Y<1.e-30) return 0.;

#ifdef DEBUG
    if(counter%10000==0) 
//       printf("T=%.5e    Y=%.5e  Yeq=%.5e  Yphi=%.5e     dx=%.3e\n",std::exp(x),Y,Yeq(std::exp(x)),Yphi,dx);
         std::cout << "T=" << std::scientific << std::setprecision(5) << std::exp(x) 
                   << "  Y=" << Y 
                   << "  Yeq=" << Yeq(std::exp(x)) 
                   << "  Yphi=" << Yphi 
                   << "  dx=" << std::scientific << std::setprecision(3) << dx << std::endl;
#endif    
    
    if(this->full_comput==0) if(testdx==0) 
    {
      if((adapt_error>0.25)||(adapt_error>0.5)) 
      {
#ifdef DEBUG
//         printf("Warning: adaptative step too small and accepted error too large, abandoning...\n");
      ...
      {        
        ...
        {
#ifdef DEBUG
//           printf("Warning: adaptative step too small (%.3e), increasing the accepted error (%.3e).\n",std::fabs(dx),adapt_error);
          std::cout << "Warning: adaptive step too small (" 
                    << std::scientific << std::setprecision(3) << std::fabs(dx) 
                    << "), increasing the accepted error (" 
                    << std::scientific << std::setprecision(3) << adapt_error << ")." 
                    << std::endl;
#endif    
          adapt_error*=2.;          
        }
        ...
        {
#ifdef DEBUG
//           printf("Warning: adaptative step too small (%.3e) for phi, increasing the accepted error (%.3e).\n",std::fabs(dx),adapt_error_phi);
          std::cerr << "Warning: adaptive step too small (" 
                    << std::scientific << std::setprecision(3) << std::fabs(dx) 
                    << ") for phi, increasing the accepted error (" 
                    << std::scientific << std::setprecision(3) << adapt_error_phi << ")." 
                    << std::endl;
#endif    
          adapt_error_phi*=2.;
        }
...
//               Yphi*(2.*pi*pi/45.*getheff(T)*std::pow(T,3.)),Y*(2.*pi*pi/45.*heff(T)*std::pow(T,3.)*this->relicmass),(pi*pi/30.*getgeff(T)*std::pow(T,4.)),2.*pi*pi/45.*heff(T)*std::pow(T,3.));
        fphi0 << std::scientific << std::setprecision(5) 
              << T << "\t"
              << relicmass/T << "\t"
    ...
    */
    ...
    {        
      ...
      if(k1<-1.e5*Y) return 0.;

      if(Y+k1*dx/2.<0.) dx*=std::fabs(Y/k1)/2.2;
            
      testdiv=0;
      do
      {  
#ifdef DEBUG
        if(testdiv) std::cout << "Avoiding negative densities...\n";
#endif
        testdiv=0;
        T=std::exp(x+dx/2.);
        sv=getAverageSigmav(T);
    ...
    /*-------------*/
      ...
      testY=(std::isnan(Ytemp1)||(Ytemp1<0.)||std::isnan(Ytemp2)||(Ytemp2<0.)||(std::fabs((Ytemp2-Ytemp1)/(Ytemp2))>adapt_error));
      
      testYphi=(phi_model.get())*(std::isnan(Yphitemp1)||(Yphitemp1<0.)||std::isnan(Yphitemp2)||(Yphitemp2<0.)||(std::fabs((Yphitemp2-Yphitemp1)/(Yphitemp2))>adapt_error_phi));

      if(this->full_comput) 
      {
        Y_eq=Yeq(std::exp(x+2.*dx));
        if(Y_eq>0.) if(
        ((!std::isnan(Ytemp2)&&!std::isinf(Ytemp2))&&((Ytemp2<Y_eq)||(std::fabs(1.-Ytemp2/Y_eq)<0.01)))
        ||((std::isnan(Ytemp2)||std::isinf(Ytemp2))&&((Y+2.*dx*k1<Y_eq)||(std::fabs(1.-(Y+2.*dx*k1)/Y_eq)<0.001)))    
        )
        {
          Ytemp2=Ytemp1=Y_eq;
          testY=0;
      ...
      if(testY||testYphi) 
      {
        ...
        dx*=2.*0.9*std::min(std::max(std::min(std::fabs(Ytemp2*adapt_error/(Ytemp2-Ytemp1))*!std::isnan(Ytemp2/(Ytemp2-Ytemp1)),std::fabs(Yphitemp2*adapt_error_phi/(Yphitemp2-Yphitemp1))*!std::isnan(Yphitemp2/(Yphitemp2-Yphitemp1))),0.3),1.);
        else dx*=2.*0.9*std::min(std::max(std::fabs(Ytemp2*adapt_error/(Ytemp2-Ytemp1))*!std::isnan(Ytemp2/(Ytemp2-Ytemp1)),0.3),1.);
      }      
    }
    ...
    if(Yphi<1e-10*Y) Yphi=0.;
    
    if((phi_model.get())&&(Yphitemp2-Yphitemp1!=0.)) dx*=2.*0.9*std::max(1.,std::min(std::min(std::fabs(Ytemp2*adapt_error/(Ytemp2-Ytemp1))*!std::isnan(Ytemp2/(Ytemp2-Ytemp1)),std::fabs(Yphitemp2*adapt_error_phi/(Yphitemp2-Yphitemp1))*!std::isnan(Yphitemp2/(Yphitemp2-Yphitemp1))),2.));    
    else 
    dx*=2.*0.9*std::max(1.,std::min(std::fabs(Ytemp2*adapt_error/(Ytemp2-Ytemp1))*!std::isnan(Ytemp2/(Ytemp2-Ytemp1)),2.));

    if(dx>x-std::log(Tfin)) dx=x-std::log(Tfin);    
  }
  while(T>Tfin);
...
//   printf("%d iterations where needed for the integration of the Boltzmann equation.\n\n",counter);
  ...
  fsig.close();
  if(fphi0) fphi0.close();
  std::cout << counter << " iterations were needed for the integration of the Boltzmann equation.\n\n";
#endif

  if((this->mgravitino!=0.)&&(this->mgravitino<this->relicmass)) return 2.755e8*this->mgravitino*Y;
...
{  
  ...
  real_t rho_phi=( phi_model.get()==1 ? 
                   this->rhot_phi_Tmax*pi*pi/15.*std::pow(this->Tmax,4.) : 
                   0.);
#ifdef DEBUG
...
//   else dx=(std::log(Tfin)-std::log(this->Tmax))*std::min(adapt_error,adapt_error_phi)*10.;
  ...
  real_t Yphi = ( phi_model.get()==1 && rho_phi!=0.) ? 
                  rho_phi/(2.*pi*pi/45.*getheff(T)*std::pow(T,3.)) : 
                  0.;
...
//   if(Yphi==0.) logYphi=-1.e100; else logYphi=std::log(Yphi);
  ...
  {  
    ...
    std::cout << "counter="<< counter << ", T="<< std::exp(x) << ", Yeq=" << Yeq(std::exp(x)) << 
                 ", Tfo=" << (this->Tfo).get() << ", Yphi="<< Yphi << ", dx="<< dx << 
                 ", s="<< rho_phi/Yphi << '\n';
//     fprintf(fptr, "%.4e\t%.4e\n", 1./T, Y);
    fYt << 1./T << '\t' << Y << '\n';
#endif
    ...
    if(this->full_comput==0) if(testdx==0) 
    {
      ...
      {        
        ...
        {
#ifdef DEBUG
//           printf("Warning: adaptive step too small (%.3e), increasing the accepted error (%.3e).\n",std::fabs(dx),adapt_error);
          std::cout << "Warning: adaptive step too small (" 
                    << std::scientific << std::setprecision(3) << std::fabs(dx) 
                    << "), increasing the accepted error (" 
                    << std::scientific << std::setprecision(3) << adapt_error << ")." 
        ...
        {
#ifdef DEBUG
//           printf("Warning: adaptive step too small (%.3e) for phi, increasing the accepted error (%.3e).\n",std::fabs(dx),adapt_error_phi);
          std::cout << "Warning: adaptive step too small (" 
          << std::scientific << std::setprecision(3) << std::fabs(dx) 
          << ") for phi, increasing the accepted error (" 
          << std::scientific << std::setprecision(3) << adapt_error_phi << ")." 
...
//         if(this->phi_model) rho_phi=this->rhot_phi_Tmax*pi*pi/15.*std::pow(this->Tmax,4.); else rho_phi=0.;
        rho_phi = (phi_model.get()==1) ?
                  this->rhot_phi_Tmax*pi*pi/15.*std::pow(this->Tmax,4.) :
                  0.;
...
//         fsig.open("sigmav.out");
        ...
        Y=Yeq(T)*(1.+delta);
//         if(phi_model.get() !=0 && rho_phi!=0.) Yphi=rho_phi/(2.*pi*pi/45.*getheff(T)*std::pow(T,3.)); else Yphi=0.;
         Yphi = (phi_model.get()==1 && rho_phi!=0.) ? 
                rho_phi/(2.*pi*pi/45.*getheff(T)*std::pow(T,3.)) : 
                0.;
      }
      else return -1.;
...
//     if(Yphi==0.) logYphi=-1.e100; else logYphi=std::log(Yphi);
    ...
    */
    ...
    /*-------------*/
      ...
      testY=(std::isinf(logYtemp1)||std::isnan(logYtemp1)||std::isinf(logYtemp2)||std::isnan(logYtemp2)||(logYtemp2<-1.e5&&std::fabs(1.+logYtemp2/1.e100)>1.e-5)||((logYtemp2!=0.)&&(std::fabs((logYtemp2-logYtemp1)/(logYtemp2))>adapt_error)));
            
      testYphi=(phi_model.get())*(std::isnan(logYphitemp1)||std::isnan(logYphitemp1)||std::isnan(logYphitemp2)||std::isnan(logYphitemp2)||(logYphitemp2<-1.e5&&std::fabs(1.+logYphitemp2/1.e100)>1.e-5)||((logYphitemp2!=0.)&&(std::fabs((logYphitemp2-logYphitemp1)/(logYphitemp2))>adapt_error_phi)));
      ...
      if(this->full_comput) 
      {
        ...
        if(Y_eq>0.) if(
        ((!std::isnan(logYtemp2)&&!std::isinf(logYtemp2))&&((logYtemp2<std::log(Y_eq))||(std::fabs(1.-logYtemp2/std::log(Y_eq))<0.01)))
        ||((std::isnan(logYtemp2)||std::isinf(logYtemp2))&&((logY+2.*dx*k1<std::log(Y_eq))||(std::fabs(1.-(logY+2.*dx*k1)/std::log(Y_eq))<0.001)))    
        )
        {
      ...
      if(!std::isinf(logYtemp1)&&std::exp(logYtemp1)==0.) logYtemp1=-1.e100;
      if(!std::isinf(logYphitemp1)&&std::exp(logYphitemp1)==0.) logYphitemp1=-1.e100;
      if(!std::isinf(logYtemp2)&&std::exp(logYtemp2)==0.) logYtemp2=-1.e100;
      if(!std::isinf(logYphitemp2)&&std::exp(logYphitemp2)==0.) logYphitemp2=-1.e100;
      
      if(testY||testYphi) 
      {
        if((phi_model.get() !=0 && rho_phi!=0.)&&(logYphitemp2-logYphitemp1!=0.)&&logYphitemp2>-1.e-100)
        dx*=2.*0.9*std::min(std::max(std::min(std::fabs(logYtemp2*adapt_error/(logYtemp2-logYtemp1))*!std::isnan(logYtemp2/(logYtemp2-logYtemp1)),std::fabs(logYphitemp2*adapt_error_phi/(logYphitemp2-logYphitemp1))*!std::isnan(logYphitemp2/(logYphitemp2-logYphitemp1))),0.3),1.);
        else dx*=2.*0.9*std::min(std::max(std::fabs(logYtemp2*adapt_error/(logYtemp2-logYtemp1))*!std::isnan(logYtemp2/(logYtemp2-logYtemp1)),0.3),1.);
      }    
    }      
  
    if(this->full_comput) 
    {
      if((std::exp(logY)<(1.+delta)*Yeq(std::exp(x)))&&((std::exp(logYb)>(1.+delta)*Yeq(std::exp(x+dx)))))
      {
      ...
      else if((std::exp(logYb)<(1.+delta)*Yeq(std::exp(x+dx)))&&((std::exp(logYtemp2)>(1.+delta)*Yeq(std::exp(x+2.*dx)))))
      {
...
//     fprintf(fptr, "%.4e\t%.4e\n", std::exp(x), Y);
    ...
    if(phi_model.get() !=0 && rho_phi!=0.&&(logYphitemp2-logYphitemp1!=0.)) dx*=2.*0.9*std::max(1.,std::min(std::min(std::fabs(logYtemp2*adapt_error/(logYtemp2-logYtemp1))*!std::isnan(logYtemp2/(logYtemp2-logYtemp1)),std::fabs(Yphitemp2*adapt_error_phi/(Yphitemp2-Yphitemp1))*!std::isnan(Yphitemp2/(Yphitemp2-Yphitemp1))),2.));    
    else 
    dx*=2.*0.9*std::max(1.,std::min(std::fabs(logYtemp2*adapt_error/(logYtemp2-logYtemp1))*!std::isnan(logYtemp2/(logYtemp2-logYtemp1)),2.));
