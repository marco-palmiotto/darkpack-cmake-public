...
namespace scalar2to2
{
  ...
  {    
    return( this==&other || 
        (
           (Running_scale== other.Running_scale)        
        && (alpha_str_Mz== other.alpha_str_Mz)        
        && (m_Z_pole== other.m_Z_pole)        
        && (mbmb== other.mbmb)        
        && (mtop_pole== other.mtop_pole)        
        && (alpha_em_MZ_inv== other.alpha_em_MZ_inv)        
        && (Gfermi== other.Gfermi)        
        && (m_tau_pole== other.m_tau_pole)        
        && (m_el_pole== other.m_el_pole)        
        && (m_mu_pole== other.m_mu_pole)        
        && (m_d2GeV== other.m_d2GeV)        
        && (m_u2GeV== other.m_u2GeV)        
        && (m_s2GeV== other.m_s2GeV)        
        && (m_c_m_c== other.m_c_m_c)        
        && (ckm_lambda== other.ckm_lambda)        
        && (ckm_A== other.ckm_A)        
        && (ckm_rhobar== other.ckm_rhobar)        
        && (ckm_etabar== other.ckm_etabar)        
        && (part_lbsm== other.part_lbsm)        
        && (m_lbsm== other.m_lbsm)        
        && (em_charge_lbsm== other.em_charge_lbsm)        
        && (g_lbsm== other.g_lbsm)));
  }
  
  bool darkpackparam_t::operator==(const darkpackparam_t &other) const
  {
    return compare_with_darkpackparam(other);
  }
  
  unsigned int Param_t::change_values(const Param_t &other)
  {
    if( !compare_with_darkpackparam(other) ) return 1;
    
    if( part_lbsm != other.part_lbsm ) return 2;
  ...
  void Param_t::setLightestBSMparticle()
  {
    ...
    if(!aremassesassigned) AssignMassesVector();
#ifdef DEBUG
