...
// #endif
  Key.get().clear();
  if( haveToFreerunptr ) delete runptr;
}
...
std::string Process2to2::findKey()
{
   ...
   for(auto i = 0; i <4 ; i++)
   {
     if(!ap[i])key.push_back(ANTICHAR);
...
std::string Process2to2::getKey()const
{ 
   return Key;
}
...
bool Process2to2::EP13calculation(const Param_t &input, const real_t &sqrts, real_t& p1, real_t& p3, real_t& E1, real_t& E3)const
{
  // In this function we suppose that the running is already taken care of
  
  if( (sqrts< getMass(3, input)+getMass(4, input)) || (sqrts< getMass(1, input)+getMass(2, input))) return false;
...
bool Process2to2::setKinematics(Param_t &input, const real_t &sqrts, const real_t &ctheta, real_t& p1, real_t& p3, real_t sij[5][5])const
{
  // In this function we suppose that the running is already taken care of
  ...
  if(!EP13calculation(input, sqrts, p1, p3, E1, E3)) return false;
  real_t  s = SQUARE(sqrts);
...
real_t Process2to2::getSumSquaredAmpl(Param_t &input, const real_t &sqrts, const real_t &ctheta)
{
  ...
  if(!setKinematics(input, sqrts, ctheta, p1, p3, sij)) return 0.;
...
real_t Process2to2::get_process_dependent_contrib_dweff_dcos(Param_t &input, const real_t &Ecm, const real_t &ctheta)const
{
  ...
  real_t p1, p3, sij[5][5];
  if(!setKinematics(input, Ecm, ctheta, p1, p3, sij)) return 0.;
  update_kinematics(input, sij);
  ...
  const real_t p12= (Ecm > getMass(1, input) + getMass(2, input)) ? 
              std::sqrt( ( SQUARE(Ecm) - std::pow(getMass(1, input)+getMass(2, input),2))*
                          ( SQUARE(Ecm) - std::pow(getMass(1, input)-getMass(2, input),2)) )/2.0/Ecm : 0.0;
  const real_t p34= (Ecm > getMass(3, input) + getMass(4, input)) ? 
              std::sqrt( (SQUARE(Ecm) - std::pow(getMass(3, input)+getMass(4, input),2))*
                          (SQUARE(Ecm) - std::pow(getMass(3, input)-getMass(4, input),2))  )/2.0/Ecm : 0.0;
...
real_t Process2to2::get_g2_dweff_dcos(Param_t &input, const real_t &Ecm, const real_t &ctheta)
{
  ...
  if(!isRunningExternal ) handleRunning(input, Ecm);
  real_t p1, p3, sij[5][5];
  if(!setKinematics(input, Ecm, ctheta, p1, p3, sij)) return 0.;
  update_kinematics(input, sij);
...
real_t Process2to2::getTotalCrossSection(Param_t &input, const real_t &Ecm, real_t *discr)
{
  ...
  if(!setKinematics(input, Ecm, 0., p1, p3, sij)) return 0.;
  ...
  auto lambda_opti = [=](real_t cosine) mutable
  {
    ...
    if(!setKinematics(input, Ecm, cosine, p1_local, p3_local, sij_local)) return 0.;
  ...
  if(discrepancy > maxdiscrepancy) integral = advmath::integrate_trap(-1., 1., lambda_opti, maxdiscrepancy, &discrepancy);
...
real_t Process2to2::getDiffW12Contrib(Param_t &input, const real_t &Ecm, const real_t &ctheta, const bool wantWeffPrefac)
{
...
real_t Process2to2::getTotalW12Contrib(Param_t &input, const real_t &sqrts, real_t *discr)
{
  real_t p1, p3, sij[5][5];
  // Returning 0 if the process is kinematically forbidden
  if(!setKinematics(input, sqrts, 0., p1, p3, sij)) return 0.;
  update_kinematics(input, sij);
...
bool Process2to2::operator==(const Process2to2 &other) const
{
  return this->Key.get()==other.Key.get();
}
