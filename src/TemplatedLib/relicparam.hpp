...
*/
     ...
     * @param EM_coupled_local Flag indicating if the WIMP couples to the electromagnetic sector (1 for yes, 0 for no).
     * @param neut_coupled_local Flag indicating if the WIMP couples to neutrinos (1 for yes, 0 for no).
     * @param neuteq_coupled_local Flag indicating if the WIMP couples to neutrino-equivalent particles (1 for yes, 0 for no).
    ...
    */
    ...
    /**
      ...
      \f[ \rho_D(T) = \rho_{rad}(T) \cdot \frac{g_{eff}(T_{ddeq})}{g_{eff}(T)} \cdot \left(\frac{h_{eff}(T)}{h_{eff}(T_{ddeq})}\right)^{n_{dd}/3} \cdot \left(\frac{T}{T_{ddeq}}\right)^{n_{dd}} \f]
    ...
    */
    ...
    /**
      ...
      @param Sigmad0_local \f$\kappa_\Sigma\f$: ratio of dark entropy injection to radiation entropy injection at \f$T_{BBN}\f$
      @param nSigmad_local \f$n_\Sigma\f$: the decreasing exponent of dark entropy injection
      @param T_end Reheating temperature
      @note This equation is referenced as Eq. (35) in the SuperIso Relic 3.1 Manual and Eq. (A8) in the SuperIso Relic 4 Manual.
    ...
    */
    ...
    /**
      ...
      \f[ \Sigma_{rad} = \kappa_{\Sigma_{rad}} \cdot \Sigma_{rad}^{eff}(T_{BBN}) \cdot \left( \frac{T}{T_{BBN}} \right)^{n_{\Sigma_{rad}}} \f]
      This modifies the radiation entropy density \f$s_{rad}\f$ following:
      \f[ \dot{s}_{rad} = -3 H s_{rad} + \Sigma_{rad}(T) \f]
      or, equivalently:
      \f[ \dot{s}_{rad} = -3 H ( 1 - \tilde\Sigma_{rad}^\ast) s_{rad}\f]
      @param Sigmarad0_local \f$\kappa_{\Sigma_{rad}}\f$: ratio of standard entropy injection to effective standard entropy injection at \f$T_{BBN}\f$
