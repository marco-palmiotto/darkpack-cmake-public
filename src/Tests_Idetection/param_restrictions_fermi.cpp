#pragma once
#ifndef __cplusplus
  #define __cplusplus
#endif

#include <gtest/gtest.h>

#include "dp_mssm2to2/RunningSM.hpp"
#include "dp_mssm2to2/indirect.hpp"
#include "dp_mssm2to2/process.hpp"
#include "dp_mssm2to2/process_1to2.hpp"
#include "dp_mssm2to2/propagation.hpp"
#include "dp_mssm2to2/relicparam.hpp"

using namespace mssm2to2;
using namespace advmath;

int main(int argc, char** argv)
{
  if (argc != 3)
  {
    std::cerr << "The program requires two arguments:" << std::endl
              << "1. The path to the model SLHA file" << std::endl
              << "2. the output .txt file" << std::endl;
    return 1;
  }

  struct Param_t input(argv[1]);

  // Propagation_param_t prop_param(input, Propagation_param_t::propagation_models::MED,
  //                              Propagation_param_t::halo_profiles::EINASTO);

  Propagation_param_t prop_param(input, Propagation_param_t::propagation_models::MAX,
                                 Propagation_param_t::halo_profiles::EINASTO);

  // Propagation_param_t prop_param(input, Propagation_param_t::propagation_models::MED,
  //                              Propagation_param_t::halo_profiles::BURKERT);

  std::ofstream chi2_file(argv[2]);
  real_t mass_chi_min = input.masses_vector.at(corr::W);
  real_t mass_chi_max = 10000.;

  int powell_min, powell_max;
  real_t chi2_min, chi2_max;
  chi2_file << "# Mass   Total sigma_v (cm^3/s) (95 C.L.)     Delta chi2 Fermi-LAT\n";
  chi2_file.flush();
  if (chi2_file)
  {

    for (int j = 0; j < 151; j++)
    {

      real_t new_DM_mass = mass_chi_min * pow((mass_chi_max / mass_chi_min), ((real_t)(j - 1) / 150.));
      std::cout << "Testing mass: " << new_DM_mass << " GeV" << std::endl;
      std::cout << j << std::endl;


      if (j != 0)
      {
        prop_param.change_ind_param(0., new_DM_mass);
        prop_param.single_out_annihilation_channel(corr::W);
      }
      else
        continue;
      real_t total_sigma_v = prop_param.get_total_sigma_v();

      std::array<std::vector<real_t>, 14> sigma_v_table = prop_param.get_sigma_v_table();
      std::vector<real_t> energy_table = prop_param.get_energy_table();
      std::cout << "Total sigma_v = " << total_sigma_v << std::endl;
      real_t sigma_v_min = total_sigma_v;



      int dof = prop_param.get_dof();
      std::cout << "Degrees of freedom: " << dof << std::endl;
      std::cout << chi2_table.at(4).at(dof) << std::endl;
      chi2_min = prop_param.deltalikelihood_fermi(powell_min) + chi2_table.at(4).at(dof);
      int test2 = 0;
      while (powell_min == 0)
      {
        std::cout << "Recalculating chi2_min" << std::endl;
        sigma_v_min *= .9;
        prop_param.change_ind_param(sigma_v_min, 0.);
        chi2_min = prop_param.deltalikelihood_fermi(powell_min) + chi2_table.at(4).at(dof);
        std::cout << "New chi_2 min: " << std::endl;
        test2++;
        if (test2 == 5)
          break;
      }
      if (test2 == 5)
      {
        std::cerr << "Unable to find a converging chi_2 after 10 attempts. Skipping this mass point." << std::endl;
        continue;
      }
      real_t sigma_v_max = sigma_v_min * pow(10., 2.);
      prop_param.change_ind_param(sigma_v_max, 0.);
      std::cout << "New sigma_v = " << prop_param.get_total_sigma_v() << std::endl;

      chi2_max = prop_param.deltalikelihood_fermi(powell_max) + chi2_table.at(4).at(dof);

      std::cout << "Chi2 min: " << chi2_min << ", Chi2 max: " << chi2_max << std::endl;
      int test = 0;

      while (powell_max == 0)
      {
        std::cout << "Recalculating chi2_max" << std::endl;
        sigma_v_max *= .9;
        prop_param.change_ind_param(sigma_v_max, 0.);
        chi2_max = prop_param.deltalikelihood_fermi(powell_max) + chi2_table.at(4).at(dof);
        std::cout << "New chi_2 max: " << chi2_max << std::endl;
        test2++;
        if (test2 == 5)
          break;
      }
      if (test2 == 5)
      {
        std::cerr << "Unable to find a converging chi_2 after 10 attempts. Skipping this mass point." << std::endl;
        continue;
      }
      while (chi2_min * chi2_max > 0. && test < 30)
      {
        test2 = 0;
        std::cout << "Expanding the range of sigma_v. Attempt: " << test << std::endl;
        chi2_min = chi2_max;
        sigma_v_min = sigma_v_max;
        sigma_v_max *= 2.;
        prop_param.change_ind_param(sigma_v_max, 0.);
        chi2_max = prop_param.deltalikelihood_fermi(powell_max) + chi2_table.at(4).at(dof);
        while (powell_max == 0)
        {
          sigma_v_max *= .9;
          prop_param.change_ind_param(sigma_v_max, 0.);
          chi2_max = prop_param.deltalikelihood_fermi(powell_max) + chi2_table.at(4).at(dof);
          test2++;
          if (test2 == 5)
            break;
        }
        test++;
        if (test2 == 5)
          break;
      }
      if (test2 == 5)
      {
        std::cerr << "Unable to find a converging chi_2 after 10 attempts. Skipping this mass point." << std::endl;
        continue;
      }
      if (test == 30)
      {
        std::cerr << "Unable to find a valid range for sigma_v after 30 attempts. Skipping this mass point."
                  << std::endl;
        continue;
      }


      real_t xnew, chi2_new;
      const int MAXITER = 20;
      int iter = 0;

      while (iter < MAXITER)
      {
        std::cout << iter << std::endl;
        xnew = sigma_v_max - chi2_max * (sigma_v_max - sigma_v_min) / (chi2_max - chi2_min);
        real_t x_init_iter = xnew;
        test2 = 0;
        prop_param.change_ind_param(xnew, 0.);
        chi2_new = prop_param.deltalikelihood_fermi(powell_min) + chi2_table.at(4).at(dof);
        real_t chi2_init_iter = chi2_new;
        while (powell_min == 0)
        {
          xnew *= .9;
          if (xnew <= sigma_v_min)
            break;
          prop_param.change_ind_param(xnew, 0.);
          chi2_new = prop_param.deltalikelihood_fermi(powell_min) + chi2_table.at(4).at(dof);
          test2++;
        }

        if (xnew <= sigma_v_min)
        {
          xnew = x_init_iter;
          chi2_new = chi2_init_iter;
        }
        printf("Sigma_v: %e, Chi2: %e\n", xnew, chi2_new);

        if (std::abs(chi2_new) < 5.e-5) // Found a root
          break;

        if (chi2_min * chi2_new < 0.)
        {
          sigma_v_max = xnew;
          chi2_max = chi2_new;
        }
        else
        {
          sigma_v_min = xnew;
          chi2_min = chi2_new;
        }

        iter++;
      }
      if (test2 == 5)
      {
        std::cerr << "Unable to find a converging chi_2 after 10 attempts. Skipping this mass point." << std::endl;
        continue;
      }
      if (iter == MAXITER)
      {
        continue;
      }
      chi2_file << new_DM_mass << " " << xnew << " " << chi2_new << "\n";
      chi2_file.flush();
    }
  }
  else
  {
    std::cerr << "Unable to open output file." << std::endl;
    return 1;
  }
  chi2_file.close();
  return 0;
}
