...
namespace mssm2to2::input_manip
{
  ...
  void FixTrilinears(param_t &input)
  {
      ...
      input.A_t = input.e_em/(std::sqrt(2)*std::sin(input.beta)*std::sin(input.theta_W)*input.m_W)*input.m_t*input.A_t;
      input.A_b = prefactor*std::abs(input.m_d*input.V_td*std::conj(input.V_td.get()) + input.m_s*input.V_ts*std::conj(input.V_ts.get()) + input.m_b*input.V_tb*std::conj(input.V_tb.get()))*input.A_b;
  }
  ...
  bool CheckUnitarity(param_t &input)
  {
      ...
      for( auto j = 0 ; j < 3 ; j++ )
      {
          for( auto k = 0 ; k < 3 ; k++)
          {
              ...
              if(k == j && (std::abs(sum1 -1.0) > 1.0e-12 ||std::abs(sum2 -1.0) > 1.0e-12)  ) unitary=false;
              else if (std::abs(sum1) > 1.0e-12 ||std::abs(sum2) > 1.0e-12) unitary=false;
  ...
  bool ImposeUnitarity(param_t &input)
  {
      ...
      if( !CheckUnitarity(input)) exit(2);
      else return true;
