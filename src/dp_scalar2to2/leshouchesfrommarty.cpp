...
//     const complex_t I{0.0,1.0};
    ...
    s13= std::abs(param.ckm_A*std::pow(param.ckm_lambda,3.)*(rhopIeta)*std::sqrt(1.-std::pow(param.ckm_A*param.ckm_lambda*param.ckm_lambda,2.))/std::sqrt(1.-param.ckm_lambda*param.ckm_lambda)/(1.-std::pow(param.ckm_A*param.ckm_lambda*param.ckm_lambda,2.)*(rhopIeta)));

    
    expid=
       ((complex_t)(param.ckm_A)*std::pow(param.ckm_lambda,3.)*
       (rhopIeta)*
       std::sqrt(1.-std::pow(param.ckm_A*param.ckm_lambda*param.ckm_lambda,2.))
...
//     IMCKM[3][2]=(-c12*s23-s12*c23*s13*expid).imag();
  ...
  void ReadBSMparams( Param_t &param, const std::string namefile)
  {
    ...
    param.g_d = get_value(data, "SCALARMODEL", 1);
    param.g_u = get_value(data, "SCALARMODEL", 2);
    param.g_l = get_value(data, "SCALARMODEL", 3);
    param.g_chi = get_value(data, "SCALARMODEL", 4);
    
    param.m_chi = get_value(data, "SCALARMODEL", 5);
    param.m_phi = get_value(data, "SCALARMODEL", 6);
  }
  ...
  void AssignBSMparams( Param_t &param)
  {
    ...
    // ----  Assigning widths ----
    param.Gamma_phi = width_phi(param).real();
    param.widths_vector[corr::phi] = param.Gamma_phi;
...
#endif
    for( auto i = 1 ; i <= corr::TOTAL_PARTICLES ; i++) param.widths_vector[i] = 0.;
#ifdef DEBUG
...
}//End of namespace scalar2to2::readmodule
