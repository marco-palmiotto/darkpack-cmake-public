...
void test_getSingle_ext(Process2to2 p, mssm2to2::Param_t input, bool wantrun = false)
{
  ...
  char pathplots[1000];
  if (wantrun)
    sprintf(pathplots, "%s", PATHPLOTSRUN.c_str());
  else
    sprintf(pathplots, "%s", PATHPLOTS.c_str());
...
int main(int argc, char** argv)
{
  ...
  {
    std::cout << "Computing runned total xsec for " << allprocsptr->getProcess(j).getName() << " n = " << j << " / "
              << allprocsptr->getN() << std::endl;
