...
int main(int argc, char** argv)
{
  ...
  for (input.g_chi = 0.1; input.g_chi < 0.9; input.g_chi = input.g_chi + step_gchi)
    for (input.m_chi = ref_mchi - halfrange_mchi; input.m_chi < ref_mchi + halfrange_mchi;
         input.m_chi = input.m_chi + step_mchi)
    {
      ...
      tempfile << static_cast<double>(input.g_chi) << '\t' << static_cast<double>(input.m_chi) << '\t' << relicdensity
               << '\t' << xfo << '\n';
    }
