...
int main(int argc, char* argv[])
{
  ...
  std::cout << "FIXED: " << input.m_chi << "SEED: g_chi=" << x0[0] << " , m_phi/m_chi=" << x0[1] << '\n';
  ...
  // const double* x_dptr = bcand.get_x_ptr(); // vector of objective function parameters at minimum, as C-style
  // double array Eigen::VectorXd x_ev = bcand.get_x_dvec(); // vector of objective function parameters at minimum, as
  // Eigen vector double edm = cmasols.edm(); // expected distance to the minimum.

  Candidate bcand = cmasols.best_candidate();
  double fmin = bcand.get_fvalue();           // min objective function value the optimizer converged to
  std::vector<double> x_stdv = bcand.get_x(); // vector of objective function parameters at minimum.
  const double* x_dptr =
      bcand.get_x_ptr(); // vector of objective function parameters at minimum, as C-style double array
  ...
  while (input.m_chi < end_m_chi)
  {
    ...
    CMAParameters<GenoPheno<pwqBoundStrategy>> cmaparams1(
        x0, sigma, -1, 0,
        gp); // -1 for automaticallydecided lambda, 0 is for random	seeding	of the internal generator.
