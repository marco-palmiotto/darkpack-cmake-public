...
int main(int argc, char* argv[])
{
  ...
  const double mchi_over_mv = std::strtod(argv[read_args++], &end), init_m_V = std::strtod(argv[read_args++], &end),
  ...
  for (int i = 1; i < npoints; i++)
  {
    ...
    std::cout << "RESULT=" << m_phi_over_mv << '\t' << mchi_over_mv << '\t' << input.m_V.get() << '\t'
              << bestparameters_dvec[0] << '\t' << fmin << '\t' << edm << '\t' << return_val << '\n';
