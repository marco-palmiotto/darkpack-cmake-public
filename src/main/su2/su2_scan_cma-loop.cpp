...
int main(int argc, char* argv[])
{
  ...
  constexpr const char* par_names[npar] = {"g_chi", "m_chi/m_V"}; // Name of the parameters to be optimised

  [[maybe_unused]] constexpr const double init_seed_g_chi = 0.3;

  double lbounds[npar] = {1.e-6, 0.25},
         ubounds[npar] = {1.0, 3.}; // arrays for lower and upper parameter bounds, respectively

  std::cout << "This function requires " << n_required_args - 1 << " arguments:\n"
            << " - The name of the input file\n"
            << " - The name of the output file\n"
            << " - The seed for the parameter g_f\n"
            << " - The seed for the parameter m_chi/m_V\n"
  ...
  const double init_m_V = std::strtod(argv[read_args++], &end), end_m_V = std::strtod(argv[read_args++], &end);
  ...
  for (int i = 0; i < npoints; i++)
    abscissae[i] = init_m_V * std::pow(stepsize, i);

  std::cout << "Abscissas are:";
  for (int i = 0; i < npoints; i++)
    std::cout << ' ' << abscissae[i];
  std::cout << '\n';
  ...
  input.m_chi_dm_1 = x0[1] * abscissae[0];
  input.m_chi_dm_2 = x0[1] * abscissae[0];
  input.refresh();
  ...
  auto function_to_minimize = [&](const double* parameters, [[maybe_unused]] const int size_array = 2)
  {
    ...
    input.m_chi_dm_1 = parameters[1] * input.m_V;
    input.m_chi_dm_2 = parameters[1] * input.m_V;
    input.refresh();
  ...
  std::cout << "# m_V   m_chi/m_V    g_f   pull-Oh2   EDM  exit_code  \nRESULT=";
  std::cout << input.m_V.get() << '\t' << bestparameters_dvec[1] << '\t' << bestparameters_dvec[0] << '\t' << fmin
            << '\t' << edm << '\t' << return_val << '\n';

  outfile << "# m_V   m_chi/m_V    g_f   pull-Oh2   EDM  exit_code \n";
  outfile << input.m_V.get() << '\t' << bestparameters_dvec[1] << '\t' << bestparameters_dvec[0] << '\t' << fmin << '\t'
          << edm << '\t' << return_val << '\n';

  for (int i = 1; i < npoints; i++)
  {
    input.m_V = abscissae[i];
    input.m_phi = 2 * input.m_V;
    x0[0] = (fmin < 1.0e-3) ? bestparameters_dvec[0] : 0.3;
    x0[1] = (fmin < 1.0e-3) ? bestparameters_dvec[1] : 2.2;
    CMAParameters<GenoPheno<pwqBoundStrategy>> cmaparams1(
        x0, sigma, -1, 0,
        gp); // -1 for automatically decided lambda, 0 is for random	seeding	of the internal generator.
    ...
    std::cout << "RESULT=" << input.m_V.get() << '\t' << bestparameters_dvec[1] << '\t' << bestparameters_dvec[0]
              << '\t' << fmin << '\t' << edm << '\t' << return_val << '\n';
