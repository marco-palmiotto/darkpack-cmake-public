...
int main(int argc, char* argv[])
{
  ...
  const real_t m_V = std::strtod(argv[read_args++], &end);
  ...
  std::cout << m_phi_over_mv << '\t' << mchi_over_mv << '\t' << input.m_V_3.get() << '\t' << bestparameters_dvec[0]
            << '\t' << fmin << '\t' << edm << '\t' << return_val << '\n';

  if (outfile)
  {
    outfile << m_phi_over_mv << '\t' << mchi_over_mv << '\t' << input.m_V_3.get() << '\t' << bestparameters_dvec[0]
            << '\t' << fmin << '\t' << edm << '\t' << return_val << '\n';
  }
