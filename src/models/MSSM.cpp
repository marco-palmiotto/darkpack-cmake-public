...
#define UNITARY_PROCESSES "data/processes_unitary.psiso" // File that contains only the processes better suited in the unitary gauge
#define NAMES_CLEAN "data/names_clean.dat"     // File containing the names in both MARTY and SuperIso convention
#define INPUT_NOSISO ""                        // File containing the processes you want to compute which are not included
                                               // in SuperIso
...
// Output files and paths 
#define DIMCHEPSTRING 30      // String dimension for calcHep names' output
#define NAMECHEPFILE "data/processes_chep.txt" // File that will contain a correspondance among the names of the 
...
int main()
{
  ...
  for ( size_t i = 0 ; i != part.size() ; i++ )
  {
    if(    part[i]->getName() ==  "h" 
        ...
        || part[i]->getName() ==  "tau" 
        || part[i]->getName() ==  "nu_e" 
        || part[i]->getName() ==  "nu_mu" 
        || part[i]->getName() ==  "nu_tau"
        || part[i]->getName() ==  "H0" 
        || part[i]->getName() ==  "Hp" 
        || part[i]->getName() ==  "A0" )
    {
  ...
  const std::array<Expr,7> expr_to_expand={width_h, 
    width_W, width_Z, width_t, width_H0, width_Hp, width_A0
  };
