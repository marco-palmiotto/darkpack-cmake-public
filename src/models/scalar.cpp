...
// #endif
  ...
  for( auto field : smfermions )
  {
    ...
    proc_c.process={Incoming(AntiPart(scalar.getParticle("chi"))), 
                    Incoming(scalar.getParticle("chi")), 
                    Outgoing(AntiPart(field)),
                    Outgoing(field) };
  ...
  std::vector<Expr> expr_to_expand={width_h, 
    width_W, width_Z, width_t, width_phi
  };
