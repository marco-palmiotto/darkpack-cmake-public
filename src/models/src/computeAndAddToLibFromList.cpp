                           ...
                           )
{
  ...
  for (const auto &particle : part) {
    ...
    std::cout << particle->getName() << (isSelfConjugate.back() ? "is self-conjugate" : "is not self-conjugate") << std::endl;
  }
...
//     sprintf(tempstring_c, "[ -d \"%s/include\" ] || mkdir -p \"%s/include\" ; [ -d \"%s/src\" ] || mkdir -p \"%s/src\"", 
//             lib.getName().c_str(), lib.getName().c_str(),lib.getName().c_str(),lib.getName().c_str());
//     std::system( tempstring_c );
    
    std::cout << "Building the directory if needed\n";
    sprintf(tempstring_c, "[ -d \"auxiliary_library/%s\" ] || mkdir -p \"auxiliary_library/%s\"",
    ...
    correspondancefileh << "// We are using char 32 (space) as possible separator, char 126 (tilde) as antiParticle tag\n";
    ...
    {  
        ...
        auto lambda_ampl = [&]()
        {
          if(!listofprocs[i].leading_order) return model.computeAmplitude( listofprocs[i].order, listofprocs[i].process);
          ...
          if( !ampl_tree.empty() ) return ampl_tree;
        ...
        if( listofprocs[i].process[0].getField()->getName() != prevname1 ||  listofprocs[i].process[1].getField()->getName() != prevname2)
        {
        ...
        Sf34 = ( ( ( listofprocs[i].process[2].isOutgoingParticle() && listofprocs[i].process[3].isOutgoingParticle()        ) || 
        ...
        for(int j = 0 ; j <= 2 ; j++) 
        {
          if(!listofprocs[i].process[j].isParticle()) correspondancefileh << "ANTICHAR," ;
          correspondancefileh << "EMPTYCHAR+corr::" << listofprocs[i].process[j].getField()->getName() << ",";
        }
          if(!listofprocs[i].process[3].isParticle()) correspondancefileh << "ANTICHAR," ;
          ...
          correspondancefileh << "} }";
        if(i != lastproc-1) correspondancefileh << ",\n";
#endif// End of ifdef CORRESPONDANCE
