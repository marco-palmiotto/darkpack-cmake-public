               ...
               )                     
{
    ...
    const std::string listSisoSusy[SIZEPYSICALSUSY]={
        "o1", "o2", "o3", "o4",
        "c1", "c2", 
        "t1", "t2", 
        "b1", "b2",
    ...
    const std::string listSisoSM[SIZEPYSICALSM]={
        "h", "hh", "hc", "h3",
        "w", "z", "g", "a", 
        "d", "u", "s", "c", "b", "t",
        "e", "m", "l",
    ...
    const std::string listChepSM[SIZEPYSICALSM]={
        "h", "H", "H+", "H3",
        "W+", "Z", "G", "A", 
        "d", "u", "s", "c", "b", "t",
        "e", "m", "l",
    ...
    const std::string listChepSMAnti[SIZEPYSICALSM]={
        "h", "H", "H-", "H3",
        "W-", "Z", "G", "A", 
        "D", "U", "S", "C", "B", "T",
        "E", "M", "L",
    ...
    const std::string listMartySM[SIZEPYSICALSM]={
        "h", "H0", "Hp", "A0",
        "W", "Z", "G", "A",
        "d", "u", "s", "c", "b", "t",
        "e", "mu", "tau",
    ...
    while(!feof(source))
    {
        ...
        do
        {
            if(line.at(position) != '\0') buffer.push_back(line.at(position));
            else break;
            for(int i = 0; i < SIZEPYSICALSUSY; i++)
            {
                ...
                {
                    indexes[particle] = i;
                    if( line.size() >= position+3 && line[position+1] == 'b' && line[position+2] == 'a' && line[position+3] == 'r')
                    {
                ...
                if( buffer==listSisoSM[i] )
                {
                    ...
                    {   
                        ...
                        if(particle == 2)
                        {
                            ...
                            for( size_t newpos = position+1 ; newpos<line.size() ; newpos++) termination_buf.push_back(line.at(newpos));
                        ...
                        if(!perfectmatch) switch (line.at(position+1))
                        {
                            case 'h':
                                i = 1;
                                position++;
                    ...
                    indexes[particle] = i;
                    if( line.size() >= position+3 && line.at(position+1) == 'b' && line.at(position+2) == 'a' && line.at(position+3) == 'r')
                    {
                        antiPart[particle]=true;
                        position+=3;
                    }else
