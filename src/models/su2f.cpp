...
void SU2f_Model::initYukawas() {
    ...
    addLagrangianTerm(Yu({I, J}) * csl::GetComplexConjugate(H(i)) * eps({i, j})
                      * GetComplexConjugate(Q({I, a, j, al})) * U({J, a, al}), true);
    addLagrangianTerm(-Yd({I, J}) * H(i)
                      * GetComplexConjugate(Q({I, a, i, al})) * D({J, a, al}), true);
    ...
    addLagrangianTerm(-Ye({II, JJ}) * H(i)
                    * GetComplexConjugate(L({II, i, al})) * E({JJ, al}), true);
...
csl::Expr axisRotation(char axis, const csl::Expr &angle) {
    csl::Expr rotMatrix {};
    switch(axis) {
        case 'x': rotMatrix = csl::matrix_s({{CSL_1, CSL_0            , CSL_0             },
                                             {CSL_0, csl::cos_s(angle), -csl::sin_s(angle)},
                                             {CSL_0, csl::sin_s(angle), csl::cos_s(angle)}});
            break;
        case 'y': rotMatrix = csl::matrix_s({{csl::cos_s(angle), CSL_0, -csl::sin_s(angle)},
                                             {CSL_0            , CSL_1, CSL_0             },
                                             {csl::sin_s(angle), CSL_0, csl::cos_s(angle)}});
            break;
        case 'z': rotMatrix = csl::matrix_s({{csl::cos_s(angle), -csl::sin_s(angle), CSL_0},
...
csl::Tensor Q(const std::string& name, const csl::Expr& T, const csl::Space* space,
              const csl::Expr &t_12, const csl::Expr &t_23, const csl::Expr &t_13) {
    csl::Expr V_mtx = rotation(t_23, t_13, t_12);
    csl::Tensor Q = csl::tensor_s(name, {space, space},
                                  mtxProd({hermitian(V_mtx), T, V_mtx}));
    return Q;
}
...
void SU2f_Model::initFlavorCouplings() {
    ...
    switch (this->m_model) {
        case 12:
            T_p = csl::matrix_s({{CSL_0, CSL_1, CSL_0},
                                   {CSL_0, CSL_0, CSL_0},
                                   {CSL_0, CSL_0, CSL_0}}) / csl::sqrt_s(2);

            T_3 = csl::matrix_s({{CSL_HALF, CSL_0   , CSL_0},
                                   {CSL_0, CSL_M_HALF, CSL_0},
                                   {CSL_0, CSL_0   , CSL_0}});
            break;
        case 13:
            T_p = csl::matrix_s({{CSL_0, CSL_0, CSL_1},
                                   {CSL_0, CSL_0, CSL_0},
                                   {CSL_0, CSL_0, CSL_0}}) / csl::sqrt_s(2);

            T_3 = csl::matrix_s({{CSL_HALF, CSL_0   , CSL_0},
                                   {CSL_0, CSL_0, CSL_0},
                                   {CSL_0, CSL_0   , CSL_M_HALF}});
            break;
        case 23:
            T_p = csl::matrix_s({{CSL_0, CSL_0, CSL_0},
                                   {CSL_0, CSL_0, CSL_1},
                                   {CSL_0, CSL_0, CSL_0}}) / csl::sqrt_s(2);

            T_3 = csl::matrix_s({{CSL_0, CSL_0   , CSL_0},
                                   {CSL_0, CSL_HALF, CSL_0},
                                   {CSL_0, CSL_0   , CSL_M_HALF}});
            break;
    }
    ...
    addLagrangianTerm(
            -g * V(+mu) * csl::GetComplexConjugate(L({I, i, al})) * Q_L({I, J})
            * gamma({mu, al, be}) * L({J, i, be}), true
    );
    
    // Coupling between V3 and L
    addLagrangianTerm(
            -g * V_3(+mu) * csl::GetComplexConjugate(L({I, i, al})) * Q_3_L({I, J})
            * gamma({mu, al, be}) * L({J, i, be})
    );
    
    // Coupling between Vpm and E
    addLagrangianTerm(
            -g * V(+mu) * csl::GetComplexConjugate(E({I, al})) * Q_R({I, J})
            * gamma({mu, al, be}) * E({J, be}), true
    );
    
    // Coupling between V3 and E
    addLagrangianTerm(
            -g * V_3(+mu) * csl::GetComplexConjugate(E({I, al})) * Q_3_R({I, J})
            * gamma({mu, al, be}) * E({J, be})
    );
...
template <typename T>
inline void printdebug(const T &s1)
{
#ifdef DEBUG
...
int main()
{
  ...
  for ( size_t i = 0 ; i != part_0.size() ; i++ )
  {
      ...
      if(!(IsOfType<GhostBoson>(part_0[i]) || IsOfType<GoldstoneBoson>(part_0[i]))) part.push_back(part_0[i]);
  }
  
  std::cout << "Printing Physical particles after filtering\n";  
  for( auto elem : part ) std::cout << elem->getName() << '\n';
  ...
  for ( size_t i = 0 ; i != part.size() ; i++ )
  {
     ...
     if(istoadd) psm.push_back(part[i]); 
  }
  
  if(pbsm.size() > 0) generateCorrespondance(psm, pbsm);
  ...
  std::vector<Expr> expr_to_expand={width_h, 
    width_W, width_Z, width_t, width_V, width_V3
  };
  
  for( Expr expression : expr_to_expand)
  {
      auto temp = csl::Evaluated(expression, csl::eval::abbreviation);
      csl::ForEachLeaf( temp, [&](csl::Expr &sub) {if (csl::IsConstant(sub) || csl::IsVariable(sub)) { sub->setValue(CSL_UNDEF);}});
  }
