#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_1_C_2_to_Hp_Hp.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_1_C_2_to_Hp_Hp(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_Hp = param->m_Hp;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_24 = param->s_24;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t IT_0000 = pow(m_Hp, 2);
    const ccomplex_t IT_0001 = pow(m_C_1, 2);
    const ccomplex_t IT_0002 = cpow((-2)*s_13 + IT_0000 + IT_0001 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0003 = sin(beta);
    const ccomplex_t IT_0004 = sin(theta_W);
    const ccomplex_t IT_0005 = cpow(IT_0004, -1);
    const ccomplex_t IT_0006 = IT_0003*IT_0005;
    const ccomplex_t IT_0007 = conj(N_d2)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = (-2)*IT_0008;
    const ccomplex_t IT_0010 = cos(theta_W);
    const ccomplex_t IT_0011 = cpow(IT_0010, -1);
    const ccomplex_t IT_0012 = IT_0003*IT_0011;
    const ccomplex_t IT_0013 = conj(N_B2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0014 = IT_0012*IT_0013;
    const ccomplex_t IT_0015 = 1.4142135623731*IT_0014;
    const ccomplex_t IT_0016 = conj(N_W2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0017 = IT_0006*IT_0016;
    const ccomplex_t IT_0018 = 1.4142135623731*IT_0017;
    const ccomplex_t IT_0019 = IT_0015 + IT_0018;
    const ccomplex_t IT_0020 = IT_0009 + IT_0019;
    const ccomplex_t IT_0021 = (0 + _Complex_I*1)*IT_0020;
    const ccomplex_t IT_0022 = 0.5*IT_0021;
    const ccomplex_t IT_0023 = cos(beta);
    const ccomplex_t IT_0024 = IT_0011*IT_0023;
    const ccomplex_t IT_0025 = N_B2*V_u1*e_em;
    const ccomplex_t IT_0026 = IT_0024*IT_0025;
    const ccomplex_t IT_0027 = 1.4142135623731*IT_0026;
    const ccomplex_t IT_0028 = IT_0005*IT_0023;
    const ccomplex_t IT_0029 = N_W2*V_u1*e_em;
    const ccomplex_t IT_0030 = IT_0028*IT_0029;
    const ccomplex_t IT_0031 = 1.4142135623731*IT_0030;
    const ccomplex_t IT_0032 = IT_0027 + IT_0031;
    const ccomplex_t IT_0033 = N_u2*e_em*V_Wp1;
    const ccomplex_t IT_0034 = IT_0028*IT_0033;
    const ccomplex_t IT_0035 = 2*IT_0034;
    const ccomplex_t IT_0036 = IT_0032 + IT_0035;
    const ccomplex_t IT_0037 = (0 + _Complex_I*1)*IT_0036;
    const ccomplex_t IT_0038 = 0.5*IT_0037;
    const ccomplex_t IT_0039 = IT_0022*IT_0038;
    const ccomplex_t IT_0040 = IT_0002*IT_0039;
    const ccomplex_t IT_0041 = (0 + _Complex_I*1)*IT_0040;
    const ccomplex_t IT_0042 = conj(N_d4)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0043 = IT_0006*IT_0042;
    const ccomplex_t IT_0044 = (-2)*IT_0043;
    const ccomplex_t IT_0045 = conj(N_B4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0046 = IT_0012*IT_0045;
    const ccomplex_t IT_0047 = 1.4142135623731*IT_0046;
    const ccomplex_t IT_0048 = conj(N_W4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0049 = IT_0006*IT_0048;
    const ccomplex_t IT_0050 = 1.4142135623731*IT_0049;
    const ccomplex_t IT_0051 = IT_0047 + IT_0050;
    const ccomplex_t IT_0052 = IT_0044 + IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*IT_0052;
    const ccomplex_t IT_0054 = 0.5*IT_0053;
    const ccomplex_t IT_0055 = N_u4*e_em*V_Wp1;
    const ccomplex_t IT_0056 = IT_0028*IT_0055;
    const ccomplex_t IT_0057 = 2*IT_0056;
    const ccomplex_t IT_0058 = N_B4*V_u1*e_em;
    const ccomplex_t IT_0059 = IT_0024*IT_0058;
    const ccomplex_t IT_0060 = 1.4142135623731*IT_0059;
    const ccomplex_t IT_0061 = N_W4*V_u1*e_em;
    const ccomplex_t IT_0062 = IT_0028*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = IT_0060 + IT_0063;
    const ccomplex_t IT_0065 = IT_0057 + IT_0064;
    const ccomplex_t IT_0066 = (0 + _Complex_I*1)*IT_0065;
    const ccomplex_t IT_0067 = 0.5*IT_0066;
    const ccomplex_t IT_0068 = IT_0054*IT_0067;
    const ccomplex_t IT_0069 = cpow((-2)*s_13 + IT_0000 + IT_0001 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0070 = IT_0068*IT_0069;
    const ccomplex_t IT_0071 = (0 + _Complex_I*1)*IT_0070;
    const ccomplex_t IT_0072 = pow(m_C_2, 2);
    const ccomplex_t IT_0073 = cpow((-2)*s_23 + IT_0000 + IT_0072 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0074 = IT_0039*IT_0073;
    const ccomplex_t IT_0075 = (0 + _Complex_I*1)*IT_0074;
    const ccomplex_t IT_0076 = cpow((-2)*s_13 + IT_0000 + IT_0001 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0077 = N_u1*e_em*V_Wp1;
    const ccomplex_t IT_0078 = IT_0028*IT_0077;
    const ccomplex_t IT_0079 = 2*IT_0078;
    const ccomplex_t IT_0080 = N_B1*V_u1*e_em;
    const ccomplex_t IT_0081 = IT_0024*IT_0080;
    const ccomplex_t IT_0082 = 1.4142135623731*IT_0081;
    const ccomplex_t IT_0083 = N_W1*V_u1*e_em;
    const ccomplex_t IT_0084 = IT_0028*IT_0083;
    const ccomplex_t IT_0085 = 1.4142135623731*IT_0084;
    const ccomplex_t IT_0086 = IT_0082 + IT_0085;
    const ccomplex_t IT_0087 = IT_0079 + IT_0086;
    const ccomplex_t IT_0088 = (0 + _Complex_I*1)*IT_0087;
    const ccomplex_t IT_0089 = 0.5*IT_0088;
    const ccomplex_t IT_0090 = conj(N_W1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0091 = IT_0006*IT_0090;
    const ccomplex_t IT_0092 = 1.4142135623731*IT_0091;
    const ccomplex_t IT_0093 = conj(N_B1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0094 = IT_0012*IT_0093;
    const ccomplex_t IT_0095 = 1.4142135623731*IT_0094;
    const ccomplex_t IT_0096 = IT_0092 + IT_0095;
    const ccomplex_t IT_0097 = conj(N_d1)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0098 = IT_0006*IT_0097;
    const ccomplex_t IT_0099 = (-2)*IT_0098;
    const ccomplex_t IT_0100 = IT_0096 + IT_0099;
    const ccomplex_t IT_0101 = (0 + _Complex_I*1)*IT_0100;
    const ccomplex_t IT_0102 = 0.5*IT_0101;
    const ccomplex_t IT_0103 = IT_0089*IT_0102;
    const ccomplex_t IT_0104 = IT_0076*IT_0103;
    const ccomplex_t IT_0105 = (0 + _Complex_I*1)*IT_0104;
    const ccomplex_t IT_0106 = cpow((-2)*s_23 + IT_0000 + IT_0072 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0107 = IT_0068*IT_0106;
    const ccomplex_t IT_0108 = (0 + _Complex_I*1)*IT_0107;
    const ccomplex_t IT_0109 = cpow((-2)*s_23 + IT_0000 + IT_0072 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0110 = IT_0103*IT_0109;
    const ccomplex_t IT_0111 = (0 + _Complex_I*1)*IT_0110;
    const ccomplex_t IT_0112 = conj(N_d3)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0113 = IT_0006*IT_0112;
    const ccomplex_t IT_0114 = (-2)*IT_0113;
    const ccomplex_t IT_0115 = conj(N_B3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0116 = IT_0012*IT_0115;
    const ccomplex_t IT_0117 = 1.4142135623731*IT_0116;
    const ccomplex_t IT_0118 = conj(N_W3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0119 = IT_0006*IT_0118;
    const ccomplex_t IT_0120 = 1.4142135623731*IT_0119;
    const ccomplex_t IT_0121 = IT_0117 + IT_0120;
    const ccomplex_t IT_0122 = IT_0114 + IT_0121;
    const ccomplex_t IT_0123 = (0 + _Complex_I*1)*IT_0122;
    const ccomplex_t IT_0124 = 0.5*IT_0123;
    const ccomplex_t IT_0125 = N_u3*e_em*V_Wp1;
    const ccomplex_t IT_0126 = IT_0028*IT_0125;
    const ccomplex_t IT_0127 = 2*IT_0126;
    const ccomplex_t IT_0128 = N_B3*V_u1*e_em;
    const ccomplex_t IT_0129 = IT_0024*IT_0128;
    const ccomplex_t IT_0130 = 1.4142135623731*IT_0129;
    const ccomplex_t IT_0131 = N_W3*V_u1*e_em;
    const ccomplex_t IT_0132 = IT_0028*IT_0131;
    const ccomplex_t IT_0133 = 1.4142135623731*IT_0132;
    const ccomplex_t IT_0134 = IT_0130 + IT_0133;
    const ccomplex_t IT_0135 = IT_0127 + IT_0134;
    const ccomplex_t IT_0136 = (0 + _Complex_I*1)*IT_0135;
    const ccomplex_t IT_0137 = 0.5*IT_0136;
    const ccomplex_t IT_0138 = IT_0124*IT_0137;
    const ccomplex_t IT_0139 = cpow((-2)*s_23 + IT_0000 + IT_0072 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0140 = IT_0138*IT_0139;
    const ccomplex_t IT_0141 = (0 + _Complex_I*1)*IT_0140;
    const ccomplex_t IT_0142 = cpow((-2)*s_13 + IT_0000 + IT_0001 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0143 = IT_0138*IT_0142;
    const ccomplex_t IT_0144 = (0 + _Complex_I*1)*IT_0143;
    const ccomplex_t IT_0145 = IT_0041 + IT_0071 + -IT_0075 + IT_0105 + 
      -IT_0108 + -IT_0111 + -IT_0141 + IT_0144;
    const ccomplex_t IT_0146 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0147 = N_u2*e_em*V_Wp2;
    const ccomplex_t IT_0148 = IT_0028*IT_0147;
    const ccomplex_t IT_0149 = 2*IT_0148;
    const ccomplex_t IT_0150 = N_B2*V_u2*e_em;
    const ccomplex_t IT_0151 = IT_0024*IT_0150;
    const ccomplex_t IT_0152 = 1.4142135623731*IT_0151;
    const ccomplex_t IT_0153 = N_W2*V_u2*e_em;
    const ccomplex_t IT_0154 = IT_0028*IT_0153;
    const ccomplex_t IT_0155 = 1.4142135623731*IT_0154;
    const ccomplex_t IT_0156 = IT_0152 + IT_0155;
    const ccomplex_t IT_0157 = IT_0149 + IT_0156;
    const ccomplex_t IT_0158 = (0 + _Complex_I*1)*IT_0157;
    const ccomplex_t IT_0159 = 0.5*IT_0158;
    const ccomplex_t IT_0160 = IT_0038*IT_0159;
    const ccomplex_t IT_0161 = IT_0073*IT_0146*IT_0160;
    const ccomplex_t IT_0162 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0163 = N_u3*e_em*V_Wp2;
    const ccomplex_t IT_0164 = IT_0028*IT_0163;
    const ccomplex_t IT_0165 = 2*IT_0164;
    const ccomplex_t IT_0166 = N_B3*V_u2*e_em;
    const ccomplex_t IT_0167 = IT_0024*IT_0166;
    const ccomplex_t IT_0168 = 1.4142135623731*IT_0167;
    const ccomplex_t IT_0169 = N_W3*V_u2*e_em;
    const ccomplex_t IT_0170 = IT_0028*IT_0169;
    const ccomplex_t IT_0171 = 1.4142135623731*IT_0170;
    const ccomplex_t IT_0172 = IT_0168 + IT_0171;
    const ccomplex_t IT_0173 = IT_0165 + IT_0172;
    const ccomplex_t IT_0174 = (0 + _Complex_I*1)*IT_0173;
    const ccomplex_t IT_0175 = 0.5*IT_0174;
    const ccomplex_t IT_0176 = IT_0137*IT_0175;
    const ccomplex_t IT_0177 = IT_0139*IT_0162*IT_0176;
    const ccomplex_t IT_0178 = IT_0142*IT_0162*IT_0176;
    const ccomplex_t IT_0179 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0180 = N_u4*e_em*V_Wp2;
    const ccomplex_t IT_0181 = IT_0028*IT_0180;
    const ccomplex_t IT_0182 = 2*IT_0181;
    const ccomplex_t IT_0183 = N_B4*V_u2*e_em;
    const ccomplex_t IT_0184 = IT_0024*IT_0183;
    const ccomplex_t IT_0185 = 1.4142135623731*IT_0184;
    const ccomplex_t IT_0186 = N_W4*V_u2*e_em;
    const ccomplex_t IT_0187 = IT_0028*IT_0186;
    const ccomplex_t IT_0188 = 1.4142135623731*IT_0187;
    const ccomplex_t IT_0189 = IT_0185 + IT_0188;
    const ccomplex_t IT_0190 = IT_0182 + IT_0189;
    const ccomplex_t IT_0191 = (0 + _Complex_I*1)*IT_0190;
    const ccomplex_t IT_0192 = 0.5*IT_0191;
    const ccomplex_t IT_0193 = IT_0067*IT_0192;
    const ccomplex_t IT_0194 = IT_0106*IT_0179*IT_0193;
    const ccomplex_t IT_0195 = IT_0069*IT_0179*IT_0193;
    const ccomplex_t IT_0196 = m_C_2*IT_0041;
    const ccomplex_t IT_0197 = m_C_2*IT_0144;
    const ccomplex_t IT_0198 = m_C_2*IT_0071;
    const ccomplex_t IT_0199 = conj(N_d4)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0200 = IT_0006*IT_0199;
    const ccomplex_t IT_0201 = (-2)*IT_0200;
    const ccomplex_t IT_0202 = conj(N_B4)*conj(U_d1)*e_em;
    const ccomplex_t IT_0203 = IT_0012*IT_0202;
    const ccomplex_t IT_0204 = 1.4142135623731*IT_0203;
    const ccomplex_t IT_0205 = conj(N_W4)*conj(U_d1)*e_em;
    const ccomplex_t IT_0206 = IT_0006*IT_0205;
    const ccomplex_t IT_0207 = 1.4142135623731*IT_0206;
    const ccomplex_t IT_0208 = IT_0204 + IT_0207;
    const ccomplex_t IT_0209 = IT_0201 + IT_0208;
    const ccomplex_t IT_0210 = (0 + _Complex_I*1)*IT_0209;
    const ccomplex_t IT_0211 = 0.5*IT_0210;
    const ccomplex_t IT_0212 = IT_0192*IT_0211;
    const ccomplex_t IT_0213 = IT_0106*IT_0212;
    const ccomplex_t IT_0214 = (0 + _Complex_I*1)*IT_0213;
    const ccomplex_t IT_0215 = m_C_1*IT_0214;
    const ccomplex_t IT_0216 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0217 = N_u1*e_em*V_Wp2;
    const ccomplex_t IT_0218 = IT_0028*IT_0217;
    const ccomplex_t IT_0219 = 2*IT_0218;
    const ccomplex_t IT_0220 = N_B1*V_u2*e_em;
    const ccomplex_t IT_0221 = IT_0024*IT_0220;
    const ccomplex_t IT_0222 = 1.4142135623731*IT_0221;
    const ccomplex_t IT_0223 = N_W1*V_u2*e_em;
    const ccomplex_t IT_0224 = IT_0028*IT_0223;
    const ccomplex_t IT_0225 = 1.4142135623731*IT_0224;
    const ccomplex_t IT_0226 = IT_0222 + IT_0225;
    const ccomplex_t IT_0227 = IT_0219 + IT_0226;
    const ccomplex_t IT_0228 = (0 + _Complex_I*1)*IT_0227;
    const ccomplex_t IT_0229 = 0.5*IT_0228;
    const ccomplex_t IT_0230 = IT_0089*IT_0229;
    const ccomplex_t IT_0231 = IT_0109*IT_0216*IT_0230;
    const ccomplex_t IT_0232 = IT_0076*IT_0216*IT_0230;
    const ccomplex_t IT_0233 = conj(N_B1)*conj(U_d1)*e_em;
    const ccomplex_t IT_0234 = IT_0012*IT_0233;
    const ccomplex_t IT_0235 = 1.4142135623731*IT_0234;
    const ccomplex_t IT_0236 = conj(N_W1)*conj(U_d1)*e_em;
    const ccomplex_t IT_0237 = IT_0006*IT_0236;
    const ccomplex_t IT_0238 = 1.4142135623731*IT_0237;
    const ccomplex_t IT_0239 = IT_0235 + IT_0238;
    const ccomplex_t IT_0240 = conj(N_d1)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0241 = IT_0006*IT_0240;
    const ccomplex_t IT_0242 = (-2)*IT_0241;
    const ccomplex_t IT_0243 = IT_0239 + IT_0242;
    const ccomplex_t IT_0244 = (0 + _Complex_I*1)*IT_0243;
    const ccomplex_t IT_0245 = 0.5*IT_0244;
    const ccomplex_t IT_0246 = IT_0229*IT_0245;
    const ccomplex_t IT_0247 = IT_0109*IT_0246;
    const ccomplex_t IT_0248 = (0 + _Complex_I*1)*IT_0247;
    const ccomplex_t IT_0249 = m_C_1*IT_0248;
    const ccomplex_t IT_0250 = IT_0002*IT_0146*IT_0160;
    const ccomplex_t IT_0251 = conj(N_d2)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0252 = IT_0006*IT_0251;
    const ccomplex_t IT_0253 = (-2)*IT_0252;
    const ccomplex_t IT_0254 = conj(N_B2)*conj(U_d1)*e_em;
    const ccomplex_t IT_0255 = IT_0012*IT_0254;
    const ccomplex_t IT_0256 = 1.4142135623731*IT_0255;
    const ccomplex_t IT_0257 = conj(N_W2)*conj(U_d1)*e_em;
    const ccomplex_t IT_0258 = IT_0006*IT_0257;
    const ccomplex_t IT_0259 = 1.4142135623731*IT_0258;
    const ccomplex_t IT_0260 = IT_0256 + IT_0259;
    const ccomplex_t IT_0261 = IT_0253 + IT_0260;
    const ccomplex_t IT_0262 = (0 + _Complex_I*1)*IT_0261;
    const ccomplex_t IT_0263 = 0.5*IT_0262;
    const ccomplex_t IT_0264 = IT_0159*IT_0263;
    const ccomplex_t IT_0265 = IT_0073*IT_0264;
    const ccomplex_t IT_0266 = (0 + _Complex_I*1)*IT_0265;
    const ccomplex_t IT_0267 = m_C_1*IT_0266;
    const ccomplex_t IT_0268 = m_C_2*IT_0105;
    const ccomplex_t IT_0269 = conj(N_W3)*conj(U_d1)*e_em;
    const ccomplex_t IT_0270 = IT_0006*IT_0269;
    const ccomplex_t IT_0271 = 1.4142135623731*IT_0270;
    const ccomplex_t IT_0272 = conj(N_B3)*conj(U_d1)*e_em;
    const ccomplex_t IT_0273 = IT_0012*IT_0272;
    const ccomplex_t IT_0274 = 1.4142135623731*IT_0273;
    const ccomplex_t IT_0275 = IT_0271 + IT_0274;
    const ccomplex_t IT_0276 = conj(N_d3)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0277 = IT_0006*IT_0276;
    const ccomplex_t IT_0278 = (-2)*IT_0277;
    const ccomplex_t IT_0279 = IT_0275 + IT_0278;
    const ccomplex_t IT_0280 = (0 + _Complex_I*1)*IT_0279;
    const ccomplex_t IT_0281 = 0.5*IT_0280;
    const ccomplex_t IT_0282 = IT_0175*IT_0281;
    const ccomplex_t IT_0283 = IT_0139*IT_0282;
    const ccomplex_t IT_0284 = (0 + _Complex_I*1)*IT_0283;
    const ccomplex_t IT_0285 = m_C_1*IT_0284;
    const ccomplex_t IT_0286 = IT_0161 + IT_0177 + IT_0178 + IT_0194 + IT_0195
       + -IT_0196 + -IT_0197 + -IT_0198 + -IT_0215 + IT_0231 + IT_0232 + 
      -IT_0249 + IT_0250 + -IT_0267 + -IT_0268 + -IT_0285;
    const ccomplex_t IT_0287 = s_14*m_C_2;
    const ccomplex_t IT_0288 = 2*IT_0287;
    const ccomplex_t IT_0289 = IT_0054*IT_0211;
    const ccomplex_t IT_0290 = IT_0069*IT_0179*IT_0289;
    const ccomplex_t IT_0291 = IT_0076*IT_0246;
    const ccomplex_t IT_0292 = (0 + _Complex_I*1)*IT_0291;
    const ccomplex_t IT_0293 = m_C_2*IT_0292;
    const ccomplex_t IT_0294 = m_C_1*IT_0075;
    const ccomplex_t IT_0295 = IT_0069*IT_0212;
    const ccomplex_t IT_0296 = (0 + _Complex_I*1)*IT_0295;
    const ccomplex_t IT_0297 = m_C_2*IT_0296;
    const ccomplex_t IT_0298 = IT_0106*IT_0179*IT_0289;
    const ccomplex_t IT_0299 = IT_0102*IT_0245;
    const ccomplex_t IT_0300 = IT_0076*IT_0216*IT_0299;
    const ccomplex_t IT_0301 = IT_0109*IT_0216*IT_0299;
    const ccomplex_t IT_0302 = IT_0124*IT_0281;
    const ccomplex_t IT_0303 = IT_0139*IT_0162*IT_0302;
    const ccomplex_t IT_0304 = IT_0142*IT_0162*IT_0302;
    const ccomplex_t IT_0305 = IT_0002*IT_0264;
    const ccomplex_t IT_0306 = (0 + _Complex_I*1)*IT_0305;
    const ccomplex_t IT_0307 = m_C_2*IT_0306;
    const ccomplex_t IT_0308 = m_C_1*IT_0141;
    const ccomplex_t IT_0309 = IT_0142*IT_0282;
    const ccomplex_t IT_0310 = (0 + _Complex_I*1)*IT_0309;
    const ccomplex_t IT_0311 = m_C_2*IT_0310;
    const ccomplex_t IT_0312 = m_C_1*IT_0108;
    const ccomplex_t IT_0313 = IT_0022*IT_0263;
    const ccomplex_t IT_0314 = IT_0002*IT_0146*IT_0313;
    const ccomplex_t IT_0315 = IT_0073*IT_0146*IT_0313;
    const ccomplex_t IT_0316 = m_C_1*IT_0111;
    const ccomplex_t IT_0317 = IT_0290 + -IT_0293 + -IT_0294 + -IT_0297 +
       IT_0298 + IT_0300 + IT_0301 + IT_0303 + IT_0304 + -IT_0307 + -IT_0308 + 
      -IT_0311 + -IT_0312 + IT_0314 + IT_0315 + -IT_0316;
    const ccomplex_t IT_0318 = s_24*m_C_1;
    const ccomplex_t IT_0319 = (-2)*IT_0318;
    const ccomplex_t IT_0320 = s_12*IT_0000;
    const ccomplex_t IT_0321 = s_14*s_24;
    const ccomplex_t IT_0322 = (-2)*IT_0321;
    const ccomplex_t IT_0323 = IT_0320 + IT_0322;
    const ccomplex_t IT_0324 = (-2)*IT_0323;
    const ccomplex_t IT_0325 = -IT_0214 + -IT_0248 + -IT_0266 + -IT_0284 +
       IT_0292 + IT_0296 + IT_0306 + IT_0310;
    const ccomplex_t IT_0326 = m_C_1*m_C_2*IT_0000;
    const ccomplex_t IT_0327 = (-2)*IT_0326;
    const ccomplex_t IT_0328 = m_C_1*m_C_2;
    const ccomplex_t IT_0329 = (-2)*IT_0328;
    const ccomplex_t IT_0330 = 0.5*conj(IT_0325);
    const ccomplex_t IT_0331 = IT_0325*(IT_0288*conj(IT_0317) + conj(IT_0286)
      *IT_0319 + IT_0324*conj(IT_0325) + conj(IT_0145)*IT_0327) + IT_0145*(conj
      (IT_0286)*IT_0288 + conj(IT_0317)*IT_0319 + conj(IT_0145)*IT_0324 + conj
      (IT_0325)*IT_0327) + 2*IT_0317*(s_12*conj(IT_0317) + 0.5*conj(IT_0145)
      *IT_0319 + 0.5*conj(IT_0286)*IT_0329 + IT_0288*IT_0330) + 2*IT_0286*(s_12
      *conj(IT_0286) + 0.5*conj(IT_0145)*IT_0288 + 0.5*conj(IT_0317)*IT_0329 +
       IT_0319*IT_0330);
    return create_ccomplex_return(IT_0331);
}

