#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_1_anti_se_L_to_anti_e_W.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_1_anti_se_L_to_anti_e_W(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_e = param->m_e;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_se_L = param->m_se_L;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_snu_mu = param->m_snu_mu;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_nuel = param->Gamma_nuel;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t IT_0000 = pow(m_W, -1);
    const ccomplex_t IT_0001 = cos(beta);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = sin(theta_W);
    const ccomplex_t IT_0004 = cpow(IT_0003, -1);
    const ccomplex_t IT_0005 = (0 + _Complex_I*1.4142135623731)*m_e*conj(N_d1)
      *e_em*IT_0000*IT_0002*IT_0004;
    const ccomplex_t IT_0006 = (-0.5)*IT_0005;
    const ccomplex_t IT_0007 = N_W1*e_em*conj(U_Wm1);
    const ccomplex_t IT_0008 = IT_0004*IT_0007;
    const ccomplex_t IT_0009 = N_d1*conj(U_d1)*e_em;
    const ccomplex_t IT_0010 = IT_0004*IT_0009;
    const ccomplex_t IT_0011 = 1.4142135623731*IT_0010;
    const ccomplex_t IT_0012 = (0 + _Complex_I*1)*(IT_0008 + 0.5*IT_0011);
    const ccomplex_t IT_0013 = -IT_0012;
    const ccomplex_t IT_0014 = IT_0006*IT_0013;
    const ccomplex_t IT_0015 = pow(m_e, 2);
    const ccomplex_t IT_0016 = pow(m_se_L, 2);
    const ccomplex_t IT_0017 = cpow((-2)*s_23 + IT_0015 + IT_0016 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0018 = IT_0014*IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1)*IT_0018;
    const ccomplex_t IT_0020 = (0 + _Complex_I*1.4142135623731)*m_e*conj(N_d2)
      *e_em*IT_0000*IT_0002*IT_0004;
    const ccomplex_t IT_0021 = (-0.5)*IT_0020;
    const ccomplex_t IT_0022 = N_W2*e_em*conj(U_Wm1);
    const ccomplex_t IT_0023 = IT_0004*IT_0022;
    const ccomplex_t IT_0024 = N_d2*conj(U_d1)*e_em;
    const ccomplex_t IT_0025 = IT_0004*IT_0024;
    const ccomplex_t IT_0026 = 1.4142135623731*IT_0025;
    const ccomplex_t IT_0027 = (0 + _Complex_I*1)*(IT_0023 + 0.5*IT_0026);
    const ccomplex_t IT_0028 = -IT_0027;
    const ccomplex_t IT_0029 = IT_0021*IT_0028;
    const ccomplex_t IT_0030 = cpow((-2)*s_23 + IT_0015 + IT_0016 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0031 = IT_0029*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1)*IT_0031;
    const ccomplex_t IT_0033 = (0 + _Complex_I*1.4142135623731)*m_e*conj(N_d3)
      *e_em*IT_0000*IT_0002*IT_0004;
    const ccomplex_t IT_0034 = (-0.5)*IT_0033;
    const ccomplex_t IT_0035 = N_W3*e_em*conj(U_Wm1);
    const ccomplex_t IT_0036 = IT_0004*IT_0035;
    const ccomplex_t IT_0037 = N_d3*conj(U_d1)*e_em;
    const ccomplex_t IT_0038 = IT_0004*IT_0037;
    const ccomplex_t IT_0039 = 1.4142135623731*IT_0038;
    const ccomplex_t IT_0040 = (0 + _Complex_I*1)*(IT_0036 + 0.5*IT_0039);
    const ccomplex_t IT_0041 = -IT_0040;
    const ccomplex_t IT_0042 = IT_0034*IT_0041;
    const ccomplex_t IT_0043 = cpow((-2)*s_23 + IT_0015 + IT_0016 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0044 = IT_0042*IT_0043;
    const ccomplex_t IT_0045 = (0 + _Complex_I*1)*IT_0044;
    const ccomplex_t IT_0046 = (0 + _Complex_I*1.4142135623731)*m_e*conj(N_d4)
      *e_em*IT_0000*IT_0002*IT_0004;
    const ccomplex_t IT_0047 = (-0.5)*IT_0046;
    const ccomplex_t IT_0048 = N_W4*e_em*conj(U_Wm1);
    const ccomplex_t IT_0049 = IT_0004*IT_0048;
    const ccomplex_t IT_0050 = N_d4*conj(U_d1)*e_em;
    const ccomplex_t IT_0051 = IT_0004*IT_0050;
    const ccomplex_t IT_0052 = 1.4142135623731*IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*(IT_0049 + 0.5*IT_0052);
    const ccomplex_t IT_0054 = -IT_0053;
    const ccomplex_t IT_0055 = IT_0047*IT_0054;
    const ccomplex_t IT_0056 = cpow((-2)*s_23 + IT_0015 + IT_0016 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0057 = IT_0055*IT_0056;
    const ccomplex_t IT_0058 = (0 + _Complex_I*1)*IT_0057;
    const ccomplex_t IT_0059 = IT_0019 + IT_0032 + IT_0045 + IT_0058;
    const ccomplex_t IT_0060 = m_C_1*IT_0059;
    const ccomplex_t IT_0061 = m_e*m_C_1;
    const ccomplex_t IT_0062 = cos(theta_W);
    const ccomplex_t IT_0063 = cpow(IT_0062, -1);
    const ccomplex_t IT_0064 = N_B1*e_em;
    const ccomplex_t IT_0065 = IT_0063*IT_0064;
    const ccomplex_t IT_0066 = 1.4142135623731*IT_0065;
    const ccomplex_t IT_0067 = N_W1*e_em;
    const ccomplex_t IT_0068 = IT_0004*IT_0067;
    const ccomplex_t IT_0069 = 1.4142135623731*IT_0068;
    const ccomplex_t IT_0070 = (0 + _Complex_I*1)*(IT_0066 + IT_0069);
    const ccomplex_t IT_0071 = (-0.5)*IT_0070;
    const ccomplex_t IT_0072 = IT_0013*IT_0071;
    const ccomplex_t IT_0073 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0074 = IT_0017*IT_0072*IT_0073;
    const ccomplex_t IT_0075 = N_B2*e_em;
    const ccomplex_t IT_0076 = IT_0063*IT_0075;
    const ccomplex_t IT_0077 = 1.4142135623731*IT_0076;
    const ccomplex_t IT_0078 = N_W2*e_em;
    const ccomplex_t IT_0079 = IT_0004*IT_0078;
    const ccomplex_t IT_0080 = 1.4142135623731*IT_0079;
    const ccomplex_t IT_0081 = (0 + _Complex_I*1)*(IT_0077 + IT_0080);
    const ccomplex_t IT_0082 = (-0.5)*IT_0081;
    const ccomplex_t IT_0083 = IT_0028*IT_0082;
    const ccomplex_t IT_0084 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0085 = IT_0030*IT_0083*IT_0084;
    const ccomplex_t IT_0086 = N_B3*e_em;
    const ccomplex_t IT_0087 = IT_0063*IT_0086;
    const ccomplex_t IT_0088 = 1.4142135623731*IT_0087;
    const ccomplex_t IT_0089 = N_W3*e_em;
    const ccomplex_t IT_0090 = IT_0004*IT_0089;
    const ccomplex_t IT_0091 = 1.4142135623731*IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1)*(IT_0088 + IT_0091);
    const ccomplex_t IT_0093 = (-0.5)*IT_0092;
    const ccomplex_t IT_0094 = IT_0041*IT_0093;
    const ccomplex_t IT_0095 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0096 = IT_0043*IT_0094*IT_0095;
    const ccomplex_t IT_0097 = N_B4*e_em;
    const ccomplex_t IT_0098 = IT_0063*IT_0097;
    const ccomplex_t IT_0099 = 1.4142135623731*IT_0098;
    const ccomplex_t IT_0100 = N_W4*e_em;
    const ccomplex_t IT_0101 = IT_0004*IT_0100;
    const ccomplex_t IT_0102 = 1.4142135623731*IT_0101;
    const ccomplex_t IT_0103 = (0 + _Complex_I*1)*(IT_0099 + IT_0102);
    const ccomplex_t IT_0104 = (-0.5)*IT_0103;
    const ccomplex_t IT_0105 = IT_0054*IT_0104;
    const ccomplex_t IT_0106 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0107 = IT_0056*IT_0105*IT_0106;
    const ccomplex_t IT_0108 = conj(N_W1)*e_em*V_Wp1;
    const ccomplex_t IT_0109 = IT_0004*IT_0108;
    const ccomplex_t IT_0110 = conj(N_u1)*V_u1*e_em;
    const ccomplex_t IT_0111 = IT_0004*IT_0110;
    const ccomplex_t IT_0112 = 1.4142135623731*IT_0111;
    const ccomplex_t IT_0113 = (0 + _Complex_I*1)*(IT_0109 + (-0.5)*IT_0112);
    const ccomplex_t IT_0114 = -IT_0113;
    const ccomplex_t IT_0115 = IT_0071*IT_0114;
    const ccomplex_t IT_0116 = IT_0017*IT_0115;
    const ccomplex_t IT_0117 = (0 + _Complex_I*1)*IT_0116;
    const ccomplex_t IT_0118 = m_C_1*IT_0117;
    const ccomplex_t IT_0119 = conj(N_W2)*e_em*V_Wp1;
    const ccomplex_t IT_0120 = IT_0004*IT_0119;
    const ccomplex_t IT_0121 = conj(N_u2)*V_u1*e_em;
    const ccomplex_t IT_0122 = IT_0004*IT_0121;
    const ccomplex_t IT_0123 = 1.4142135623731*IT_0122;
    const ccomplex_t IT_0124 = (0 + _Complex_I*1)*(IT_0120 + (-0.5)*IT_0123);
    const ccomplex_t IT_0125 = -IT_0124;
    const ccomplex_t IT_0126 = IT_0082*IT_0125;
    const ccomplex_t IT_0127 = IT_0030*IT_0126;
    const ccomplex_t IT_0128 = (0 + _Complex_I*1)*IT_0127;
    const ccomplex_t IT_0129 = m_C_1*IT_0128;
    const ccomplex_t IT_0130 = conj(N_W3)*e_em*V_Wp1;
    const ccomplex_t IT_0131 = IT_0004*IT_0130;
    const ccomplex_t IT_0132 = conj(N_u3)*V_u1*e_em;
    const ccomplex_t IT_0133 = IT_0004*IT_0132;
    const ccomplex_t IT_0134 = 1.4142135623731*IT_0133;
    const ccomplex_t IT_0135 = (0 + _Complex_I*1)*(IT_0131 + (-0.5)*IT_0134);
    const ccomplex_t IT_0136 = -IT_0135;
    const ccomplex_t IT_0137 = IT_0093*IT_0136;
    const ccomplex_t IT_0138 = IT_0043*IT_0137;
    const ccomplex_t IT_0139 = (0 + _Complex_I*1)*IT_0138;
    const ccomplex_t IT_0140 = m_C_1*IT_0139;
    const ccomplex_t IT_0141 = conj(N_W4)*e_em*V_Wp1;
    const ccomplex_t IT_0142 = IT_0004*IT_0141;
    const ccomplex_t IT_0143 = conj(N_u4)*V_u1*e_em;
    const ccomplex_t IT_0144 = IT_0004*IT_0143;
    const ccomplex_t IT_0145 = 1.4142135623731*IT_0144;
    const ccomplex_t IT_0146 = (0 + _Complex_I*1)*(IT_0142 + (-0.5)*IT_0145);
    const ccomplex_t IT_0147 = -IT_0146;
    const ccomplex_t IT_0148 = IT_0104*IT_0147;
    const ccomplex_t IT_0149 = IT_0056*IT_0148;
    const ccomplex_t IT_0150 = (0 + _Complex_I*1)*IT_0149;
    const ccomplex_t IT_0151 = m_C_1*IT_0150;
    const ccomplex_t IT_0152 = IT_0074 + IT_0085 + IT_0096 + IT_0107 + 
      -IT_0118 + -IT_0129 + -IT_0140 + -IT_0151;
    const ccomplex_t IT_0153 = IT_0006*IT_0114;
    const ccomplex_t IT_0154 = IT_0017*IT_0073*IT_0153;
    const ccomplex_t IT_0155 = IT_0021*IT_0125;
    const ccomplex_t IT_0156 = IT_0030*IT_0084*IT_0155;
    const ccomplex_t IT_0157 = IT_0034*IT_0136;
    const ccomplex_t IT_0158 = IT_0043*IT_0095*IT_0157;
    const ccomplex_t IT_0159 = IT_0047*IT_0147;
    const ccomplex_t IT_0160 = IT_0056*IT_0106*IT_0159;
    const ccomplex_t IT_0161 = -IT_0154 + -IT_0156 + -IT_0158 + -IT_0160;
    const ccomplex_t IT_0162 = pow(m_W, -2);
    const ccomplex_t IT_0163 = pow(m_W, 2);
    const ccomplex_t IT_0164 = s_13*IT_0163;
    const ccomplex_t IT_0165 = s_14*s_34;
    const ccomplex_t IT_0166 = (-2)*IT_0165;
    const ccomplex_t IT_0167 = IT_0164 + IT_0166;
    const ccomplex_t IT_0168 = IT_0162*IT_0167;
    const ccomplex_t IT_0169 = (-0.5)*IT_0168;
    const ccomplex_t IT_0170 = s_13 + IT_0169;
    const ccomplex_t IT_0171 = s_14*s_34*IT_0162;
    const ccomplex_t IT_0172 = -IT_0171;
    const ccomplex_t IT_0173 = s_13 + IT_0172;
    const ccomplex_t IT_0174 = m_e*IT_0173;
    const ccomplex_t IT_0175 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0004;
    const ccomplex_t IT_0176 = -IT_0175;
    const ccomplex_t IT_0177 = (0 + _Complex_I*1)*e_em*V_Wp1*IT_0004;
    const ccomplex_t IT_0178 = -IT_0177;
    const ccomplex_t IT_0179 = pow(m_C_1, 2);
    const ccomplex_t IT_0180 = cpow((-2)*s_13 + IT_0015 + IT_0179 + reg_prop +
       -m_snu_mu*(m_snu_mu + (0 + _Complex_I*-1)*Gamma_nuel), -1);
    const ccomplex_t IT_0181 = IT_0178*IT_0180;
    const ccomplex_t IT_0182 = (0 + _Complex_I*1)*IT_0181;
    const ccomplex_t IT_0183 = IT_0176*IT_0182;
    const ccomplex_t IT_0184 = m_C_1*IT_0173;
    const ccomplex_t IT_0185 = (0 + _Complex_I*1.4142135623731)*m_e*conj(U_d1)
      *e_em*IT_0000*IT_0002*IT_0004;
    const ccomplex_t IT_0186 = (-0.5)*IT_0185;
    const ccomplex_t IT_0187 = IT_0180*IT_0186;
    const ccomplex_t IT_0188 = (0 + _Complex_I*1)*IT_0187;
    const ccomplex_t IT_0189 = IT_0176*IT_0188;
    const ccomplex_t IT_0190 = 2*IT_0019 + 2*IT_0032 + 2*IT_0045 + 2*IT_0058 +
       IT_0189;
    const ccomplex_t IT_0191 = pow(s_34, 2);
    const ccomplex_t IT_0192 = IT_0162*IT_0191;
    const ccomplex_t IT_0193 = -IT_0192;
    const ccomplex_t IT_0194 = IT_0015 + IT_0193;
    const ccomplex_t IT_0195 = m_C_1*IT_0194;
    const ccomplex_t IT_0196 = -IT_0189;
    const ccomplex_t IT_0197 = 2*IT_0060;
    const ccomplex_t IT_0198 = pow(s_14, 2);
    const ccomplex_t IT_0199 = IT_0162*IT_0198;
    const ccomplex_t IT_0200 = -IT_0179;
    const ccomplex_t IT_0201 = IT_0199 + IT_0200;
    const ccomplex_t IT_0202 = m_e*IT_0201;
    const ccomplex_t IT_0203 = (-2)*IT_0117 + (-2)*IT_0128 + (-2)*IT_0139 + (
      -2)*IT_0150 + -IT_0183;
    const ccomplex_t IT_0204 = s_14*IT_0163;
    const ccomplex_t IT_0205 = IT_0162*IT_0204;
    const ccomplex_t IT_0206 = 0.5*IT_0205;
    const ccomplex_t IT_0207 = s_14 + IT_0206;
    const ccomplex_t IT_0208 = m_e*IT_0207;
    const ccomplex_t IT_0209 = 2*IT_0208;
    const ccomplex_t IT_0210 = IT_0117 + IT_0128 + IT_0139 + IT_0150;
    const ccomplex_t IT_0211 = s_34*IT_0163;
    const ccomplex_t IT_0212 = IT_0162*IT_0211;
    const ccomplex_t IT_0213 = (-0.25)*IT_0212;
    const ccomplex_t IT_0214 = s_34 + IT_0213;
    const ccomplex_t IT_0215 = m_C_1*IT_0214;
    const ccomplex_t IT_0216 = (-4)*IT_0215;
    const ccomplex_t IT_0217 = -IT_0019 + -IT_0032 + -IT_0045 + -IT_0058;
    const ccomplex_t IT_0218 = (-2)*IT_0060;
    const ccomplex_t IT_0219 = 2*conj(IT_0060);
    const ccomplex_t IT_0220 = s_14*IT_0015;
    const ccomplex_t IT_0221 = s_13*s_34;
    const ccomplex_t IT_0222 = -IT_0221;
    const ccomplex_t IT_0223 = IT_0220 + IT_0222;
    const ccomplex_t IT_0224 = 2*IT_0223;
    const ccomplex_t IT_0225 = 2*IT_0174;
    const ccomplex_t IT_0226 = 2*IT_0195;
    const ccomplex_t IT_0227 = s_13*IT_0173;
    const ccomplex_t IT_0228 = (-2)*IT_0227;
    const ccomplex_t IT_0229 = s_13*IT_0194;
    const ccomplex_t IT_0230 = (-2)*IT_0229;
    const ccomplex_t IT_0231 = IT_0061*IT_0173;
    const ccomplex_t IT_0232 = (-2)*IT_0231;
    const ccomplex_t IT_0233 = IT_0061*IT_0194;
    const ccomplex_t IT_0234 = (-2)*IT_0233;
    const ccomplex_t IT_0235 = s_13*s_14;
    const ccomplex_t IT_0236 = s_34*IT_0179;
    const ccomplex_t IT_0237 = -IT_0236;
    const ccomplex_t IT_0238 = IT_0235 + IT_0237;
    const ccomplex_t IT_0239 = 2*IT_0238;
    const ccomplex_t IT_0240 = IT_0061*IT_0163;
    const ccomplex_t IT_0241 = (-3)*IT_0240;
    const ccomplex_t IT_0242 = 2*IT_0241;
    const ccomplex_t IT_0243 = pow(m_W, 4);
    const ccomplex_t IT_0244 = s_13*IT_0243;
    const ccomplex_t IT_0245 = IT_0162*IT_0244;
    const ccomplex_t IT_0246 = IT_0164 + IT_0166 + (-0.5)*IT_0245;
    const ccomplex_t IT_0247 = (-4)*IT_0246;
    const ccomplex_t IT_0248 = (-2)*conj(IT_0060);
    const ccomplex_t IT_0249 = (-2)*IT_0209;
    const ccomplex_t IT_0250 = (-2)*IT_0216;
    const ccomplex_t IT_0251 = 2*IT_0184;
    const ccomplex_t IT_0252 = 4*IT_0170;
    const ccomplex_t IT_0253 = (-6)*IT_0061;
    const ccomplex_t IT_0254 = (-2)*IT_0202;
    const ccomplex_t IT_0255 = conj(IT_0060)*IT_0061 + (-0.166666666666667)
      *conj(IT_0196)*IT_0225 + (-0.166666666666667)*conj(IT_0183)*IT_0226 + (
      -0.166666666666667)*conj(IT_0217)*IT_0249 + (-0.166666666666667)*conj
      (IT_0210)*IT_0250 + (-0.166666666666667)*conj(IT_0203)*IT_0251 + (
      -0.166666666666667)*conj(IT_0152)*IT_0252 + (-0.166666666666667)*conj
      (IT_0161)*IT_0253 + (-0.166666666666667)*conj(IT_0190)*IT_0254;
    const ccomplex_t IT_0256 = conj(IT_0060)*IT_0170 + 0.25*conj(IT_0183)
      *IT_0225 + 0.25*conj(IT_0196)*IT_0226 + 0.25*conj(IT_0210)*IT_0249 + 0.25
      *conj(IT_0217)*IT_0250 + 0.25*conj(IT_0190)*IT_0251 + 0.25*conj(IT_0161)
      *IT_0252 + 0.25*conj(IT_0152)*IT_0253 + 0.25*conj(IT_0203)*IT_0254;
    const ccomplex_t IT_0257 = s_13*IT_0201;
    const ccomplex_t IT_0258 = IT_0061*IT_0201;
    const ccomplex_t IT_0259 = 2*IT_0258;
    const ccomplex_t IT_0260 = 0.5*conj(IT_0183);
    const ccomplex_t IT_0261 = 0.5*conj(IT_0161);
    const ccomplex_t IT_0262 = 0.5*conj(IT_0196);
    const ccomplex_t IT_0263 = 0.5*conj(IT_0152);
    const ccomplex_t IT_0264 = 2*IT_0203*(0.5*conj(IT_0210)*IT_0239 + 0.5
      *IT_0202*IT_0248 + conj(IT_0203)*IT_0257 + 0.5*conj(IT_0190)*IT_0259 +
       IT_0228*IT_0260 + IT_0254*IT_0261 + IT_0232*IT_0262 + IT_0251*IT_0263);
    const ccomplex_t IT_0265 = 2*IT_0190*(0.5*IT_0184*IT_0219 + 0.5*conj
      (IT_0217)*IT_0239 + conj(IT_0190)*IT_0257 + 0.5*conj(IT_0203)*IT_0259 +
       IT_0232*IT_0260 + IT_0251*IT_0261 + IT_0228*IT_0262 + IT_0254*IT_0263);
    const ccomplex_t IT_0266 = (-6)*IT_0060*(IT_0061*conj(IT_0152) + (
      -0.666666666666667)*(conj(IT_0060) + conj(IT_0161))*IT_0170) + (IT_0174
      *conj(IT_0183) + IT_0184*conj(IT_0190) + IT_0195*conj(IT_0196))*IT_0197 + 
      (IT_0202*conj(IT_0203) + IT_0209*conj(IT_0210) + IT_0216*conj(IT_0217))
      *IT_0218 + IT_0196*(IT_0195*IT_0219 + conj(IT_0217)*IT_0224 + conj(IT_0152
      )*IT_0225 + conj(IT_0161)*IT_0226 + conj(IT_0190)*IT_0228 + conj(IT_0196)
      *IT_0230 + conj(IT_0203)*IT_0232 + conj(IT_0183)*IT_0234) + IT_0183*
      (IT_0174*IT_0219 + conj(IT_0210)*IT_0224 + conj(IT_0161)*IT_0225 + conj
      (IT_0152)*IT_0226 + conj(IT_0203)*IT_0228 + conj(IT_0183)*IT_0230 + conj
      (IT_0190)*IT_0232 + conj(IT_0196)*IT_0234) + IT_0210*(conj(IT_0183)
      *IT_0224 + conj(IT_0203)*IT_0239 + conj(IT_0217)*IT_0242 + conj(IT_0210)
      *IT_0247 + IT_0209*IT_0248 + conj(IT_0161)*IT_0249 + conj(IT_0152)*IT_0250
      ) + IT_0217*(conj(IT_0196)*IT_0224 + conj(IT_0190)*IT_0239 + conj(IT_0210)
      *IT_0242 + conj(IT_0217)*IT_0247 + IT_0216*IT_0248 + conj(IT_0152)*IT_0249
       + conj(IT_0161)*IT_0250) + (-6)*IT_0152*IT_0255 + 4*IT_0161*IT_0256 +
       IT_0264 + IT_0265;
    return create_ccomplex_return(IT_0266);
}

