#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_1_anti_stau_1_to_anti_tau_Hp.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_1_anti_stau_1_to_anti_tau_Hp(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t mu_h = param->mu_h;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t A_tau = param->A_tau;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_stau_1 = param->m_stau_1;
    const creal_t reg_prop = param->reg_prop;
    const creal_t m_snu_tau = param->m_snu_tau;
    const creal_t Gamma_nutaul = param->Gamma_nutaul;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t U_stau_00 = param->U_stau_00;
    const ccomplex_t U_stau_10 = param->U_stau_10;
    const ccomplex_t IT_0000 = pow(m_tau, 2);
    const ccomplex_t IT_0001 = pow(m_stau_1, 2);
    const ccomplex_t IT_0002 = cpow(s_23 + (-0.5)*IT_0000 + (-0.5)*IT_0001 +
       0.5*m_N_1*(m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0003 = cos(theta_W);
    const ccomplex_t IT_0004 = cpow(IT_0003, -1);
    const ccomplex_t IT_0005 = sin(beta);
    const ccomplex_t IT_0006 = IT_0004*IT_0005;
    const ccomplex_t IT_0007 = conj(N_B1)*conj(U_d1)*e_em;
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = 1.4142135623731*IT_0008;
    const ccomplex_t IT_0010 = sin(theta_W);
    const ccomplex_t IT_0011 = cpow(IT_0010, -1);
    const ccomplex_t IT_0012 = IT_0005*IT_0011;
    const ccomplex_t IT_0013 = conj(N_W1)*conj(U_d1)*e_em;
    const ccomplex_t IT_0014 = IT_0012*IT_0013;
    const ccomplex_t IT_0015 = 1.4142135623731*IT_0014;
    const ccomplex_t IT_0016 = IT_0009 + IT_0015;
    const ccomplex_t IT_0017 = conj(N_d1)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0018 = IT_0012*IT_0017;
    const ccomplex_t IT_0019 = (-2)*IT_0018;
    const ccomplex_t IT_0020 = IT_0016 + IT_0019;
    const ccomplex_t IT_0021 = (0 + _Complex_I*1)*IT_0020;
    const ccomplex_t IT_0022 = 0.5*IT_0021;
    const ccomplex_t IT_0023 = N_B1*e_em*conj(U_stau_00);
    const ccomplex_t IT_0024 = IT_0004*IT_0023;
    const ccomplex_t IT_0025 = 1.4142135623731*IT_0024;
    const ccomplex_t IT_0026 = N_W1*e_em*conj(U_stau_00);
    const ccomplex_t IT_0027 = IT_0011*IT_0026;
    const ccomplex_t IT_0028 = 1.4142135623731*IT_0027;
    const ccomplex_t IT_0029 = cos(beta);
    const ccomplex_t IT_0030 = cpow(IT_0029, -1);
    const ccomplex_t IT_0031 = IT_0011*IT_0030;
    const ccomplex_t IT_0032 = pow(m_W, -1);
    const ccomplex_t IT_0033 = N_d1*e_em*m_tau*IT_0032*conj(U_stau_10);
    const ccomplex_t IT_0034 = IT_0031*IT_0033;
    const ccomplex_t IT_0035 = 1.4142135623731*IT_0034;
    const ccomplex_t IT_0036 = (0 + _Complex_I*1)*(IT_0025 + IT_0028 + 
      -IT_0035);
    const ccomplex_t IT_0037 = (-0.5)*IT_0036;
    const ccomplex_t IT_0038 = IT_0022*IT_0037;
    const ccomplex_t IT_0039 = IT_0002*IT_0038;
    const ccomplex_t IT_0040 = (0 + _Complex_I*1)*IT_0039;
    const ccomplex_t IT_0041 = cpow(s_23 + (-0.5)*IT_0000 + (-0.5)*IT_0001 +
       0.5*m_N_4*(m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0042 = conj(N_d4)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0043 = IT_0012*IT_0042;
    const ccomplex_t IT_0044 = (-2)*IT_0043;
    const ccomplex_t IT_0045 = conj(N_B4)*conj(U_d1)*e_em;
    const ccomplex_t IT_0046 = IT_0006*IT_0045;
    const ccomplex_t IT_0047 = 1.4142135623731*IT_0046;
    const ccomplex_t IT_0048 = conj(N_W4)*conj(U_d1)*e_em;
    const ccomplex_t IT_0049 = IT_0012*IT_0048;
    const ccomplex_t IT_0050 = 1.4142135623731*IT_0049;
    const ccomplex_t IT_0051 = IT_0047 + IT_0050;
    const ccomplex_t IT_0052 = IT_0044 + IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*IT_0052;
    const ccomplex_t IT_0054 = 0.5*IT_0053;
    const ccomplex_t IT_0055 = N_B4*e_em*conj(U_stau_00);
    const ccomplex_t IT_0056 = IT_0004*IT_0055;
    const ccomplex_t IT_0057 = 1.4142135623731*IT_0056;
    const ccomplex_t IT_0058 = N_W4*e_em*conj(U_stau_00);
    const ccomplex_t IT_0059 = IT_0011*IT_0058;
    const ccomplex_t IT_0060 = 1.4142135623731*IT_0059;
    const ccomplex_t IT_0061 = N_d4*e_em*m_tau*IT_0032*conj(U_stau_10);
    const ccomplex_t IT_0062 = IT_0031*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = (0 + _Complex_I*1)*(IT_0057 + IT_0060 + 
      -IT_0063);
    const ccomplex_t IT_0065 = (-0.5)*IT_0064;
    const ccomplex_t IT_0066 = IT_0054*IT_0065;
    const ccomplex_t IT_0067 = IT_0041*IT_0066;
    const ccomplex_t IT_0068 = (0 + _Complex_I*1)*IT_0067;
    const ccomplex_t IT_0069 = cpow(s_23 + (-0.5)*IT_0000 + (-0.5)*IT_0001 +
       0.5*m_N_2*(m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0070 = conj(N_d2)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0071 = IT_0012*IT_0070;
    const ccomplex_t IT_0072 = (-2)*IT_0071;
    const ccomplex_t IT_0073 = conj(N_B2)*conj(U_d1)*e_em;
    const ccomplex_t IT_0074 = IT_0006*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = conj(N_W2)*conj(U_d1)*e_em;
    const ccomplex_t IT_0077 = IT_0012*IT_0076;
    const ccomplex_t IT_0078 = 1.4142135623731*IT_0077;
    const ccomplex_t IT_0079 = IT_0075 + IT_0078;
    const ccomplex_t IT_0080 = IT_0072 + IT_0079;
    const ccomplex_t IT_0081 = (0 + _Complex_I*1)*IT_0080;
    const ccomplex_t IT_0082 = 0.5*IT_0081;
    const ccomplex_t IT_0083 = N_B2*e_em*conj(U_stau_00);
    const ccomplex_t IT_0084 = IT_0004*IT_0083;
    const ccomplex_t IT_0085 = 1.4142135623731*IT_0084;
    const ccomplex_t IT_0086 = N_W2*e_em*conj(U_stau_00);
    const ccomplex_t IT_0087 = IT_0011*IT_0086;
    const ccomplex_t IT_0088 = 1.4142135623731*IT_0087;
    const ccomplex_t IT_0089 = N_d2*e_em*m_tau*IT_0032*conj(U_stau_10);
    const ccomplex_t IT_0090 = IT_0031*IT_0089;
    const ccomplex_t IT_0091 = 1.4142135623731*IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1)*(IT_0085 + IT_0088 + 
      -IT_0091);
    const ccomplex_t IT_0093 = (-0.5)*IT_0092;
    const ccomplex_t IT_0094 = IT_0082*IT_0093;
    const ccomplex_t IT_0095 = IT_0069*IT_0094;
    const ccomplex_t IT_0096 = (0 + _Complex_I*1)*IT_0095;
    const ccomplex_t IT_0097 = cpow(s_23 + (-0.5)*IT_0000 + (-0.5)*IT_0001 +
       0.5*m_N_3*(m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0098 = conj(N_W3)*conj(U_d1)*e_em;
    const ccomplex_t IT_0099 = IT_0012*IT_0098;
    const ccomplex_t IT_0100 = 1.4142135623731*IT_0099;
    const ccomplex_t IT_0101 = conj(N_B3)*conj(U_d1)*e_em;
    const ccomplex_t IT_0102 = IT_0006*IT_0101;
    const ccomplex_t IT_0103 = 1.4142135623731*IT_0102;
    const ccomplex_t IT_0104 = IT_0100 + IT_0103;
    const ccomplex_t IT_0105 = conj(N_d3)*e_em*conj(U_Wm1);
    const ccomplex_t IT_0106 = IT_0012*IT_0105;
    const ccomplex_t IT_0107 = (-2)*IT_0106;
    const ccomplex_t IT_0108 = IT_0104 + IT_0107;
    const ccomplex_t IT_0109 = (0 + _Complex_I*1)*IT_0108;
    const ccomplex_t IT_0110 = 0.5*IT_0109;
    const ccomplex_t IT_0111 = N_B3*e_em*conj(U_stau_00);
    const ccomplex_t IT_0112 = IT_0004*IT_0111;
    const ccomplex_t IT_0113 = 1.4142135623731*IT_0112;
    const ccomplex_t IT_0114 = N_W3*e_em*conj(U_stau_00);
    const ccomplex_t IT_0115 = IT_0011*IT_0114;
    const ccomplex_t IT_0116 = 1.4142135623731*IT_0115;
    const ccomplex_t IT_0117 = N_d3*e_em*m_tau*IT_0032*conj(U_stau_10);
    const ccomplex_t IT_0118 = IT_0031*IT_0117;
    const ccomplex_t IT_0119 = 1.4142135623731*IT_0118;
    const ccomplex_t IT_0120 = (0 + _Complex_I*1)*(IT_0113 + IT_0116 + 
      -IT_0119);
    const ccomplex_t IT_0121 = (-0.5)*IT_0120;
    const ccomplex_t IT_0122 = IT_0110*IT_0121;
    const ccomplex_t IT_0123 = IT_0097*IT_0122;
    const ccomplex_t IT_0124 = (0 + _Complex_I*1)*IT_0123;
    const ccomplex_t IT_0125 = 0.5*IT_0040 + 0.5*IT_0068 + 0.5*IT_0096 + 0.5
      *IT_0124;
    const ccomplex_t IT_0126 = m_C_1*m_tau*IT_0001;
    const ccomplex_t IT_0127 = IT_0011*IT_0029;
    const ccomplex_t IT_0128 = N_u3*e_em*V_Wp1;
    const ccomplex_t IT_0129 = IT_0127*IT_0128;
    const ccomplex_t IT_0130 = 2*IT_0129;
    const ccomplex_t IT_0131 = IT_0004*IT_0029;
    const ccomplex_t IT_0132 = N_B3*V_u1*e_em;
    const ccomplex_t IT_0133 = IT_0131*IT_0132;
    const ccomplex_t IT_0134 = 1.4142135623731*IT_0133;
    const ccomplex_t IT_0135 = N_W3*V_u1*e_em;
    const ccomplex_t IT_0136 = IT_0127*IT_0135;
    const ccomplex_t IT_0137 = 1.4142135623731*IT_0136;
    const ccomplex_t IT_0138 = IT_0134 + IT_0137;
    const ccomplex_t IT_0139 = IT_0130 + IT_0138;
    const ccomplex_t IT_0140 = (0 + _Complex_I*1)*IT_0139;
    const ccomplex_t IT_0141 = 0.5*IT_0140;
    const ccomplex_t IT_0142 = conj(N_B3)*e_em*conj(U_stau_10);
    const ccomplex_t IT_0143 = IT_0004*IT_0142;
    const ccomplex_t IT_0144 = 1.4142135623731*IT_0143;
    const ccomplex_t IT_0145 = conj(N_d3)*e_em*m_tau*IT_0032*conj(U_stau_00);
    const ccomplex_t IT_0146 = IT_0031*IT_0145;
    const ccomplex_t IT_0147 = 1.4142135623731*IT_0146;
    const ccomplex_t IT_0148 = (0 + _Complex_I*1)*(IT_0144 + 0.5*IT_0147);
    const ccomplex_t IT_0149 = -IT_0148;
    const ccomplex_t IT_0150 = IT_0141*IT_0149;
    const ccomplex_t IT_0151 = IT_0097*IT_0150;
    const ccomplex_t IT_0152 = (0 + _Complex_I*1)*IT_0151;
    const ccomplex_t IT_0153 = N_u4*e_em*V_Wp1;
    const ccomplex_t IT_0154 = IT_0127*IT_0153;
    const ccomplex_t IT_0155 = 2*IT_0154;
    const ccomplex_t IT_0156 = N_B4*V_u1*e_em;
    const ccomplex_t IT_0157 = IT_0131*IT_0156;
    const ccomplex_t IT_0158 = 1.4142135623731*IT_0157;
    const ccomplex_t IT_0159 = N_W4*V_u1*e_em;
    const ccomplex_t IT_0160 = IT_0127*IT_0159;
    const ccomplex_t IT_0161 = 1.4142135623731*IT_0160;
    const ccomplex_t IT_0162 = IT_0158 + IT_0161;
    const ccomplex_t IT_0163 = IT_0155 + IT_0162;
    const ccomplex_t IT_0164 = (0 + _Complex_I*1)*IT_0163;
    const ccomplex_t IT_0165 = 0.5*IT_0164;
    const ccomplex_t IT_0166 = conj(N_B4)*e_em*conj(U_stau_10);
    const ccomplex_t IT_0167 = IT_0004*IT_0166;
    const ccomplex_t IT_0168 = 1.4142135623731*IT_0167;
    const ccomplex_t IT_0169 = conj(N_d4)*e_em*m_tau*IT_0032*conj(U_stau_00);
    const ccomplex_t IT_0170 = IT_0031*IT_0169;
    const ccomplex_t IT_0171 = 1.4142135623731*IT_0170;
    const ccomplex_t IT_0172 = (0 + _Complex_I*1)*(IT_0168 + 0.5*IT_0171);
    const ccomplex_t IT_0173 = -IT_0172;
    const ccomplex_t IT_0174 = IT_0165*IT_0173;
    const ccomplex_t IT_0175 = IT_0041*IT_0174;
    const ccomplex_t IT_0176 = (0 + _Complex_I*1)*IT_0175;
    const ccomplex_t IT_0177 = N_u1*e_em*V_Wp1;
    const ccomplex_t IT_0178 = IT_0127*IT_0177;
    const ccomplex_t IT_0179 = 2*IT_0178;
    const ccomplex_t IT_0180 = N_B1*V_u1*e_em;
    const ccomplex_t IT_0181 = IT_0131*IT_0180;
    const ccomplex_t IT_0182 = 1.4142135623731*IT_0181;
    const ccomplex_t IT_0183 = N_W1*V_u1*e_em;
    const ccomplex_t IT_0184 = IT_0127*IT_0183;
    const ccomplex_t IT_0185 = 1.4142135623731*IT_0184;
    const ccomplex_t IT_0186 = IT_0182 + IT_0185;
    const ccomplex_t IT_0187 = IT_0179 + IT_0186;
    const ccomplex_t IT_0188 = (0 + _Complex_I*1)*IT_0187;
    const ccomplex_t IT_0189 = 0.5*IT_0188;
    const ccomplex_t IT_0190 = conj(N_B1)*e_em*conj(U_stau_10);
    const ccomplex_t IT_0191 = IT_0004*IT_0190;
    const ccomplex_t IT_0192 = 1.4142135623731*IT_0191;
    const ccomplex_t IT_0193 = conj(N_d1)*e_em*m_tau*IT_0032*conj(U_stau_00);
    const ccomplex_t IT_0194 = IT_0031*IT_0193;
    const ccomplex_t IT_0195 = 1.4142135623731*IT_0194;
    const ccomplex_t IT_0196 = (0 + _Complex_I*1)*(IT_0192 + 0.5*IT_0195);
    const ccomplex_t IT_0197 = -IT_0196;
    const ccomplex_t IT_0198 = IT_0189*IT_0197;
    const ccomplex_t IT_0199 = IT_0002*IT_0198;
    const ccomplex_t IT_0200 = (0 + _Complex_I*1)*IT_0199;
    const ccomplex_t IT_0201 = N_B2*V_u1*e_em;
    const ccomplex_t IT_0202 = IT_0131*IT_0201;
    const ccomplex_t IT_0203 = 1.4142135623731*IT_0202;
    const ccomplex_t IT_0204 = N_W2*V_u1*e_em;
    const ccomplex_t IT_0205 = IT_0127*IT_0204;
    const ccomplex_t IT_0206 = 1.4142135623731*IT_0205;
    const ccomplex_t IT_0207 = IT_0203 + IT_0206;
    const ccomplex_t IT_0208 = N_u2*e_em*V_Wp1;
    const ccomplex_t IT_0209 = IT_0127*IT_0208;
    const ccomplex_t IT_0210 = 2*IT_0209;
    const ccomplex_t IT_0211 = IT_0207 + IT_0210;
    const ccomplex_t IT_0212 = (0 + _Complex_I*1)*IT_0211;
    const ccomplex_t IT_0213 = 0.5*IT_0212;
    const ccomplex_t IT_0214 = conj(N_B2)*e_em*conj(U_stau_10);
    const ccomplex_t IT_0215 = IT_0004*IT_0214;
    const ccomplex_t IT_0216 = 1.4142135623731*IT_0215;
    const ccomplex_t IT_0217 = conj(N_d2)*e_em*m_tau*IT_0032*conj(U_stau_00);
    const ccomplex_t IT_0218 = IT_0031*IT_0217;
    const ccomplex_t IT_0219 = 1.4142135623731*IT_0218;
    const ccomplex_t IT_0220 = (0 + _Complex_I*1)*(IT_0216 + 0.5*IT_0219);
    const ccomplex_t IT_0221 = -IT_0220;
    const ccomplex_t IT_0222 = IT_0213*IT_0221;
    const ccomplex_t IT_0223 = IT_0069*IT_0222;
    const ccomplex_t IT_0224 = (0 + _Complex_I*1)*IT_0223;
    const ccomplex_t IT_0225 = 0.5*IT_0152 + 0.5*IT_0176 + 0.5*IT_0200 + 0.5
      *IT_0224;
    const ccomplex_t IT_0226 = s_23*m_C_1;
    const ccomplex_t IT_0227 = (-2)*IT_0226;
    const ccomplex_t IT_0228 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0229 = IT_0093*IT_0213;
    const ccomplex_t IT_0230 = IT_0069*IT_0228*IT_0229;
    const ccomplex_t IT_0231 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0232 = IT_0121*IT_0141;
    const ccomplex_t IT_0233 = IT_0097*IT_0231*IT_0232;
    const ccomplex_t IT_0234 = pow(m_C_1, 2);
    const ccomplex_t IT_0235 = cpow(s_13 + (-0.5)*IT_0000 + (-0.5)*IT_0234 + (
      -0.5)*reg_prop + 0.5*m_snu_tau*(m_snu_tau + (0 + _Complex_I*-1)
      *Gamma_nutaul), -1);
    const ccomplex_t IT_0236 = (0 + _Complex_I*1)*(A_tau*IT_0005*conj
      (U_stau_10) + (-1.4142135623731)*e_em*IT_0011*(m_W*IT_0005*IT_0029*conj
      (U_stau_00) + (-0.5)*m_tau*IT_0032*(m_tau*IT_0005*IT_0030*conj(U_stau_00) 
      + mu_h*conj(U_stau_10))));
    const ccomplex_t IT_0237 = (0 + _Complex_I*1)*e_em*V_Wp1*IT_0011;
    const ccomplex_t IT_0238 = -IT_0237;
    const ccomplex_t IT_0239 = IT_0236*IT_0238;
    const ccomplex_t IT_0240 = IT_0235*IT_0239;
    const ccomplex_t IT_0241 = (0 + _Complex_I*1)*IT_0240;
    const ccomplex_t IT_0242 = m_tau*IT_0200;
    const ccomplex_t IT_0243 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0244 = IT_0037*IT_0189;
    const ccomplex_t IT_0245 = IT_0002*IT_0243*IT_0244;
    const ccomplex_t IT_0246 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0247 = IT_0065*IT_0165;
    const ccomplex_t IT_0248 = IT_0041*IT_0246*IT_0247;
    const ccomplex_t IT_0249 = m_tau*IT_0224;
    const ccomplex_t IT_0250 = m_tau*IT_0152;
    const ccomplex_t IT_0251 = m_tau*IT_0176;
    const ccomplex_t IT_0252 = (-0.5)*IT_0230 + (-0.5)*IT_0233 + (-0.5)
      *IT_0241 + 0.5*IT_0242 + (-0.5)*IT_0245 + (-0.5)*IT_0248 + 0.5*IT_0249 +
       0.5*IT_0250 + 0.5*IT_0251;
    const ccomplex_t IT_0253 = 0.5*conj(IT_0252);
    const ccomplex_t IT_0254 = s_12*m_tau;
    const ccomplex_t IT_0255 = (-2)*IT_0254;
    const ccomplex_t IT_0256 = (0 + _Complex_I*1.4142135623731)*conj(U_d1)
      *e_em*m_tau*IT_0011*IT_0030*IT_0032;
    const ccomplex_t IT_0257 = (-0.5)*IT_0256;
    const ccomplex_t IT_0258 = IT_0236*IT_0257;
    const ccomplex_t IT_0259 = IT_0235*IT_0258;
    const ccomplex_t IT_0260 = (0 + _Complex_I*1)*IT_0259;
    const ccomplex_t IT_0261 = IT_0054*IT_0173;
    const ccomplex_t IT_0262 = IT_0041*IT_0246*IT_0261;
    const ccomplex_t IT_0263 = IT_0022*IT_0197;
    const ccomplex_t IT_0264 = IT_0002*IT_0243*IT_0263;
    const ccomplex_t IT_0265 = IT_0082*IT_0221;
    const ccomplex_t IT_0266 = IT_0069*IT_0228*IT_0265;
    const ccomplex_t IT_0267 = m_tau*IT_0040;
    const ccomplex_t IT_0268 = m_tau*IT_0096;
    const ccomplex_t IT_0269 = m_tau*IT_0124;
    const ccomplex_t IT_0270 = IT_0110*IT_0149;
    const ccomplex_t IT_0271 = IT_0097*IT_0231*IT_0270;
    const ccomplex_t IT_0272 = m_tau*IT_0068;
    const ccomplex_t IT_0273 = 0.5*IT_0260 + (-0.5)*IT_0262 + (-0.5)*IT_0264 +
       (-0.5)*IT_0266 + 0.5*IT_0267 + 0.5*IT_0268 + 0.5*IT_0269 + (-0.5)*IT_0271
       + 0.5*IT_0272;
    const ccomplex_t IT_0274 = 0.5*conj(IT_0273);
    const ccomplex_t IT_0275 = s_12*s_23;
    const ccomplex_t IT_0276 = s_13*IT_0001;
    const ccomplex_t IT_0277 = (-0.5)*IT_0276;
    const ccomplex_t IT_0278 = IT_0275 + IT_0277;
    const ccomplex_t IT_0279 = 4*IT_0278;
    const ccomplex_t IT_0280 = 0.5*conj(IT_0125);
    const ccomplex_t IT_0281 = 0.5*conj(IT_0225);
    const ccomplex_t IT_0282 = m_C_1*m_tau;
    const ccomplex_t IT_0283 = 2*IT_0282;
    const ccomplex_t IT_0284 = 2*IT_0125*(IT_0126*conj(IT_0225) + IT_0227
      *IT_0253 + IT_0255*IT_0274 + IT_0279*IT_0280) + 2*IT_0225*(conj(IT_0125)
      *IT_0126 + IT_0253*IT_0255 + IT_0227*IT_0274 + IT_0279*IT_0281) + 2
      *IT_0273*(s_13*conj(IT_0273) + IT_0255*IT_0280 + IT_0227*IT_0281 + IT_0253
      *IT_0283) + 2*IT_0252*(s_13*conj(IT_0252) + IT_0227*IT_0280 + IT_0255
      *IT_0281 + IT_0274*IT_0283);
    return create_ccomplex_return(IT_0284);
}

