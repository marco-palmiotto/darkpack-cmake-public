#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_1_stau_2_to_nu_tau_A.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_1_stau_2_to_nu_tau_A(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_c1 = param->Gamma_c1;
    const creal_t m_stau_2 = param->m_stau_2;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_tau2 = param->Gamma_tau2;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t U_stau_01 = param->U_stau_01;
    const ccomplex_t U_stau_11 = param->U_stau_11;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*e_em;
    const ccomplex_t IT_0001 = sin(theta_W);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = e_em*conj(U_Wm1)*U_stau_01;
    const ccomplex_t IT_0004 = IT_0002*IT_0003;
    const ccomplex_t IT_0005 = cos(beta);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = IT_0002*IT_0006;
    const ccomplex_t IT_0008 = pow(m_W, -1);
    const ccomplex_t IT_0009 = conj(U_d1)*e_em*m_tau*IT_0008*U_stau_11;
    const ccomplex_t IT_0010 = IT_0007*IT_0009;
    const ccomplex_t IT_0011 = 1.4142135623731*IT_0010;
    const ccomplex_t IT_0012 = (0 + _Complex_I*1)*(IT_0004 + (-0.5)*IT_0011);
    const ccomplex_t IT_0013 = pow(m_C_1, 2);
    const ccomplex_t IT_0014 = cpow(s_13 + (-0.5)*IT_0013 + (-0.5)*reg_prop +
       0.5*m_stau_2*(m_stau_2 + (0 + _Complex_I*-1)*Gamma_tau2), -1);
    const ccomplex_t IT_0015 = IT_0012*IT_0014;
    const ccomplex_t IT_0016 = (0 + _Complex_I*1)*IT_0015;
    const ccomplex_t IT_0017 = IT_0000*IT_0016;
    const ccomplex_t IT_0018 = (-0.5)*IT_0017;
    const ccomplex_t IT_0019 = 2*IT_0000;
    const ccomplex_t IT_0020 = IT_0016*IT_0019;
    const ccomplex_t IT_0021 = IT_0000*IT_0012;
    const ccomplex_t IT_0022 = pow(m_stau_2, 2);
    const ccomplex_t IT_0023 = cpow(s_23 + (-0.5)*IT_0022 + 0.5*m_C_1*(m_C_1 +
       (0 + _Complex_I*-1)*Gamma_c1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0024 = IT_0021*IT_0023;
    const ccomplex_t IT_0025 = (0 + _Complex_I*1)*IT_0024;
    const ccomplex_t IT_0026 = 0.5*IT_0020 + IT_0025;
    const ccomplex_t IT_0027 = (-2)*s_13;
    const ccomplex_t IT_0028 = (-0.5)*IT_0020;
    const ccomplex_t IT_0029 = s_34*IT_0027;
    const ccomplex_t IT_0030 = s_13*s_34;
    const ccomplex_t IT_0031 = (-0.5)*IT_0025;
    const ccomplex_t IT_0032 = pow(s_13, 2);
    const ccomplex_t IT_0033 = (-2)*IT_0032;
    const ccomplex_t IT_0034 = s_14*s_34;
    const ccomplex_t IT_0035 = s_34*IT_0013;
    const ccomplex_t IT_0036 = (-2)*IT_0035;
    const ccomplex_t IT_0037 = IT_0018*(s_14*conj(IT_0026)*IT_0027 + conj
      (IT_0028)*IT_0029) + (-4)*IT_0028*((-0.25)*conj(IT_0018)*IT_0029 + IT_0030
      *conj(IT_0031) + (-0.25)*conj(IT_0026)*IT_0033) + (-4)*IT_0031*(conj
      (IT_0028)*IT_0030 + (-2)*conj(IT_0031)*IT_0034 + (-0.25)*conj(IT_0026)
      *IT_0036) + IT_0026*((s_14*conj(IT_0018) + IT_0013*conj(IT_0026))*IT_0027 
      + conj(IT_0028)*IT_0033 + conj(IT_0031)*IT_0036);
    return create_ccomplex_return(IT_0037);
}

