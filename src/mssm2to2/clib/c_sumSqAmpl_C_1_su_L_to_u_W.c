#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_1_su_L_to_u_W.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_1_su_L_to_u_W(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_b = param->m_b;
    const creal_t m_u = param->m_u;
    const creal_t V_ud = param->V_ud;
    const creal_t V_us = param->V_us;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_sb_1 = param->m_sb_1;
    const creal_t m_sb_2 = param->m_sb_2;
    const creal_t m_sd_L = param->m_sd_L;
    const creal_t m_ss_L = param->m_ss_L;
    const creal_t m_su_L = param->m_su_L;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_b1 = param->Gamma_b1;
    const creal_t Gamma_b2 = param->Gamma_b2;
    const creal_t Gamma_dl = param->Gamma_dl;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t Gamma_sl = param->Gamma_sl;
    const creal_t V_ub_mod = param->V_ub_mod;
    const creal_t reg_prop = param->reg_prop;
    const creal_t delta_wolf = param->delta_wolf;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t U_sb_00 = param->U_sb_00;
    const ccomplex_t U_sb_01 = param->U_sb_01;
    const ccomplex_t U_sb_10 = param->U_sb_10;
    const ccomplex_t U_sb_11 = param->U_sb_11;
    const ccomplex_t IT_0000 = cos(theta_W);
    const ccomplex_t IT_0001 = cpow(IT_0000, -1);
    const ccomplex_t IT_0002 = conj(N_B4)*e_em;
    const ccomplex_t IT_0003 = IT_0001*IT_0002;
    const ccomplex_t IT_0004 = 1.4142135623731*IT_0003;
    const ccomplex_t IT_0005 = sin(theta_W);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = conj(N_W4)*e_em;
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = 1.4142135623731*IT_0008;
    const ccomplex_t IT_0010 = (0 + _Complex_I*1)*(IT_0004 + 3*IT_0009);
    const ccomplex_t IT_0011 = 0.166666666666667*IT_0010;
    const ccomplex_t IT_0012 = N_W4*e_em*conj(U_Wm1);
    const ccomplex_t IT_0013 = IT_0006*IT_0012;
    const ccomplex_t IT_0014 = N_d4*conj(U_d1)*e_em;
    const ccomplex_t IT_0015 = IT_0006*IT_0014;
    const ccomplex_t IT_0016 = 1.4142135623731*IT_0015;
    const ccomplex_t IT_0017 = (0 + _Complex_I*1)*(IT_0013 + 0.5*IT_0016);
    const ccomplex_t IT_0018 = -IT_0017;
    const ccomplex_t IT_0019 = IT_0011*IT_0018;
    const ccomplex_t IT_0020 = pow(m_u, 2);
    const ccomplex_t IT_0021 = pow(m_su_L, 2);
    const ccomplex_t IT_0022 = cpow((-2)*s_23 + IT_0020 + IT_0021 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0023 = IT_0019*IT_0022;
    const ccomplex_t IT_0024 = (0 + _Complex_I*1)*IT_0023;
    const ccomplex_t IT_0025 = cpow((-2)*s_23 + IT_0020 + IT_0021 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0026 = conj(N_B1)*e_em;
    const ccomplex_t IT_0027 = IT_0001*IT_0026;
    const ccomplex_t IT_0028 = 1.4142135623731*IT_0027;
    const ccomplex_t IT_0029 = conj(N_W1)*e_em;
    const ccomplex_t IT_0030 = IT_0006*IT_0029;
    const ccomplex_t IT_0031 = 1.4142135623731*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1)*(IT_0028 + 3*IT_0031);
    const ccomplex_t IT_0033 = 0.166666666666667*IT_0032;
    const ccomplex_t IT_0034 = N_W1*e_em*conj(U_Wm1);
    const ccomplex_t IT_0035 = IT_0006*IT_0034;
    const ccomplex_t IT_0036 = N_d1*conj(U_d1)*e_em;
    const ccomplex_t IT_0037 = IT_0006*IT_0036;
    const ccomplex_t IT_0038 = 1.4142135623731*IT_0037;
    const ccomplex_t IT_0039 = (0 + _Complex_I*1)*(IT_0035 + 0.5*IT_0038);
    const ccomplex_t IT_0040 = -IT_0039;
    const ccomplex_t IT_0041 = IT_0033*IT_0040;
    const ccomplex_t IT_0042 = IT_0025*IT_0041;
    const ccomplex_t IT_0043 = (0 + _Complex_I*1)*IT_0042;
    const ccomplex_t IT_0044 = conj(N_B2)*e_em;
    const ccomplex_t IT_0045 = IT_0001*IT_0044;
    const ccomplex_t IT_0046 = 1.4142135623731*IT_0045;
    const ccomplex_t IT_0047 = conj(N_W2)*e_em;
    const ccomplex_t IT_0048 = IT_0006*IT_0047;
    const ccomplex_t IT_0049 = 1.4142135623731*IT_0048;
    const ccomplex_t IT_0050 = (0 + _Complex_I*1)*(IT_0046 + 3*IT_0049);
    const ccomplex_t IT_0051 = 0.166666666666667*IT_0050;
    const ccomplex_t IT_0052 = N_W2*e_em*conj(U_Wm1);
    const ccomplex_t IT_0053 = IT_0006*IT_0052;
    const ccomplex_t IT_0054 = N_d2*conj(U_d1)*e_em;
    const ccomplex_t IT_0055 = IT_0006*IT_0054;
    const ccomplex_t IT_0056 = 1.4142135623731*IT_0055;
    const ccomplex_t IT_0057 = (0 + _Complex_I*1)*(IT_0053 + 0.5*IT_0056);
    const ccomplex_t IT_0058 = -IT_0057;
    const ccomplex_t IT_0059 = IT_0051*IT_0058;
    const ccomplex_t IT_0060 = cpow((-2)*s_23 + IT_0020 + IT_0021 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0061 = IT_0059*IT_0060;
    const ccomplex_t IT_0062 = (0 + _Complex_I*1)*IT_0061;
    const ccomplex_t IT_0063 = conj(N_B3)*e_em;
    const ccomplex_t IT_0064 = IT_0001*IT_0063;
    const ccomplex_t IT_0065 = 1.4142135623731*IT_0064;
    const ccomplex_t IT_0066 = conj(N_W3)*e_em;
    const ccomplex_t IT_0067 = IT_0006*IT_0066;
    const ccomplex_t IT_0068 = 1.4142135623731*IT_0067;
    const ccomplex_t IT_0069 = (0 + _Complex_I*1)*(IT_0065 + 3*IT_0068);
    const ccomplex_t IT_0070 = 0.166666666666667*IT_0069;
    const ccomplex_t IT_0071 = N_W3*e_em*conj(U_Wm1);
    const ccomplex_t IT_0072 = IT_0006*IT_0071;
    const ccomplex_t IT_0073 = N_d3*conj(U_d1)*e_em;
    const ccomplex_t IT_0074 = IT_0006*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = (0 + _Complex_I*1)*(IT_0072 + 0.5*IT_0075);
    const ccomplex_t IT_0077 = -IT_0076;
    const ccomplex_t IT_0078 = IT_0070*IT_0077;
    const ccomplex_t IT_0079 = cpow((-2)*s_23 + IT_0020 + IT_0021 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0080 = IT_0078*IT_0079;
    const ccomplex_t IT_0081 = (0 + _Complex_I*1)*IT_0080;
    const ccomplex_t IT_0082 = IT_0024 + IT_0043 + IT_0062 + IT_0081;
    const ccomplex_t IT_0083 = pow(m_W, -2);
    const ccomplex_t IT_0084 = pow(m_W, 4);
    const ccomplex_t IT_0085 = s_13*IT_0084;
    const ccomplex_t IT_0086 = IT_0083*IT_0085;
    const ccomplex_t IT_0087 = 6*IT_0086;
    const ccomplex_t IT_0088 = s_14*s_34;
    const ccomplex_t IT_0089 = 24*IT_0088;
    const ccomplex_t IT_0090 = pow(m_W, 2);
    const ccomplex_t IT_0091 = s_13*IT_0090;
    const ccomplex_t IT_0092 = (-12)*IT_0091;
    const ccomplex_t IT_0093 = IT_0087 + IT_0089 + IT_0092;
    const ccomplex_t IT_0094 = (0 + _Complex_I*1.4142135623731)*V_us*e_em
      *IT_0006;
    const ccomplex_t IT_0095 = -IT_0094;
    const ccomplex_t IT_0096 = (0 + _Complex_I*1)*V_us*e_em*conj(U_Wm1)*IT_0006;
    const ccomplex_t IT_0097 = pow(m_C_1, 2);
    const ccomplex_t IT_0098 = cpow((-2)*s_13 + IT_0020 + IT_0097 + -m_ss_L*
      (m_ss_L + (0 + _Complex_I*-1)*Gamma_sl) + reg_prop, -1);
    const ccomplex_t IT_0099 = IT_0096*IT_0098;
    const ccomplex_t IT_0100 = (0 + _Complex_I*1)*IT_0099;
    const ccomplex_t IT_0101 = IT_0095*IT_0100;
    const ccomplex_t IT_0102 = cexp((0 + _Complex_I*1)*delta_wolf);
    const ccomplex_t IT_0103 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0006
      *IT_0102*conj(U_sb_00)*V_ub_mod;
    const ccomplex_t IT_0104 = -IT_0103;
    const ccomplex_t IT_0105 = cexp((0 + _Complex_I*-1)*delta_wolf);
    const ccomplex_t IT_0106 = IT_0006*IT_0105;
    const ccomplex_t IT_0107 = e_em*conj(U_Wm1)*U_sb_00*V_ub_mod;
    const ccomplex_t IT_0108 = IT_0106*IT_0107;
    const ccomplex_t IT_0109 = cos(beta);
    const ccomplex_t IT_0110 = cpow(IT_0109, -1);
    const ccomplex_t IT_0111 = IT_0006*IT_0105*IT_0110;
    const ccomplex_t IT_0112 = pow(m_W, -1);
    const ccomplex_t IT_0113 = m_b*conj(U_d1)*e_em*IT_0112*U_sb_10*V_ub_mod;
    const ccomplex_t IT_0114 = IT_0111*IT_0113;
    const ccomplex_t IT_0115 = 1.4142135623731*IT_0114;
    const ccomplex_t IT_0116 = (0 + _Complex_I*1)*(IT_0108 + (-0.5)*IT_0115);
    const ccomplex_t IT_0117 = cpow((-2)*s_13 + IT_0020 + IT_0097 + -m_sb_1*
      (m_sb_1 + (0 + _Complex_I*-1)*Gamma_b1) + reg_prop, -1);
    const ccomplex_t IT_0118 = IT_0116*IT_0117;
    const ccomplex_t IT_0119 = (0 + _Complex_I*1)*IT_0118;
    const ccomplex_t IT_0120 = IT_0104*IT_0119;
    const ccomplex_t IT_0121 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0006
      *IT_0102*conj(U_sb_01)*V_ub_mod;
    const ccomplex_t IT_0122 = -IT_0121;
    const ccomplex_t IT_0123 = e_em*conj(U_Wm1)*U_sb_01*V_ub_mod;
    const ccomplex_t IT_0124 = IT_0106*IT_0123;
    const ccomplex_t IT_0125 = m_b*conj(U_d1)*e_em*IT_0112*U_sb_11*V_ub_mod;
    const ccomplex_t IT_0126 = IT_0111*IT_0125;
    const ccomplex_t IT_0127 = 1.4142135623731*IT_0126;
    const ccomplex_t IT_0128 = (0 + _Complex_I*1)*(IT_0124 + (-0.5)*IT_0127);
    const ccomplex_t IT_0129 = cpow((-2)*s_13 + IT_0020 + IT_0097 + -m_sb_2*
      (m_sb_2 + (0 + _Complex_I*-1)*Gamma_b2) + reg_prop, -1);
    const ccomplex_t IT_0130 = IT_0128*IT_0129;
    const ccomplex_t IT_0131 = (0 + _Complex_I*1)*IT_0130;
    const ccomplex_t IT_0132 = IT_0122*IT_0131;
    const ccomplex_t IT_0133 = (0 + _Complex_I*1.4142135623731)*V_ud*e_em
      *IT_0006;
    const ccomplex_t IT_0134 = -IT_0133;
    const ccomplex_t IT_0135 = (0 + _Complex_I*1)*V_ud*e_em*conj(U_Wm1)*IT_0006;
    const ccomplex_t IT_0136 = cpow((-2)*s_13 + IT_0020 + IT_0097 + -m_sd_L*
      (m_sd_L + (0 + _Complex_I*-1)*Gamma_dl) + reg_prop, -1);
    const ccomplex_t IT_0137 = IT_0135*IT_0136;
    const ccomplex_t IT_0138 = (0 + _Complex_I*1)*IT_0137;
    const ccomplex_t IT_0139 = IT_0134*IT_0138;
    const ccomplex_t IT_0140 = (-2)*IT_0024 + (-2)*IT_0043 + (-2)*IT_0062 + (
      -2)*IT_0081 + -IT_0101 + -IT_0120 + -IT_0132 + -IT_0139;
    const ccomplex_t IT_0141 = s_13*s_14;
    const ccomplex_t IT_0142 = s_34*IT_0097;
    const ccomplex_t IT_0143 = -IT_0142;
    const ccomplex_t IT_0144 = IT_0141 + IT_0143;
    const ccomplex_t IT_0145 = 6*IT_0144;
    const ccomplex_t IT_0146 = IT_0101 + IT_0120 + IT_0132 + IT_0139;
    const ccomplex_t IT_0147 = s_14*IT_0020;
    const ccomplex_t IT_0148 = s_13*s_34;
    const ccomplex_t IT_0149 = -IT_0148;
    const ccomplex_t IT_0150 = IT_0147 + IT_0149;
    const ccomplex_t IT_0151 = 6*IT_0150;
    const ccomplex_t IT_0152 = sin(beta);
    const ccomplex_t IT_0153 = cpow(IT_0152, -1);
    const ccomplex_t IT_0154 = (0 + _Complex_I*1.4142135623731)*m_u*N_u1*e_em
      *IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0155 = (-0.5)*IT_0154;
    const ccomplex_t IT_0156 = IT_0040*IT_0155;
    const ccomplex_t IT_0157 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0158 = IT_0025*IT_0156*IT_0157;
    const ccomplex_t IT_0159 = (0 + _Complex_I*1.4142135623731)*m_u*N_u2*e_em
      *IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0160 = (-0.5)*IT_0159;
    const ccomplex_t IT_0161 = IT_0058*IT_0160;
    const ccomplex_t IT_0162 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0163 = IT_0060*IT_0161*IT_0162;
    const ccomplex_t IT_0164 = (0 + _Complex_I*1.4142135623731)*m_u*N_u3*e_em
      *IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0165 = (-0.5)*IT_0164;
    const ccomplex_t IT_0166 = IT_0077*IT_0165;
    const ccomplex_t IT_0167 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0168 = IT_0079*IT_0166*IT_0167;
    const ccomplex_t IT_0169 = (0 + _Complex_I*1.4142135623731)*m_u*N_u4*e_em
      *IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0170 = (-0.5)*IT_0169;
    const ccomplex_t IT_0171 = IT_0018*IT_0170;
    const ccomplex_t IT_0172 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0173 = IT_0022*IT_0171*IT_0172;
    const ccomplex_t IT_0174 = conj(N_W1)*e_em*V_Wp1;
    const ccomplex_t IT_0175 = IT_0006*IT_0174;
    const ccomplex_t IT_0176 = conj(N_u1)*V_u1*e_em;
    const ccomplex_t IT_0177 = IT_0006*IT_0176;
    const ccomplex_t IT_0178 = 1.4142135623731*IT_0177;
    const ccomplex_t IT_0179 = (0 + _Complex_I*1)*(IT_0175 + (-0.5)*IT_0178);
    const ccomplex_t IT_0180 = -IT_0179;
    const ccomplex_t IT_0181 = IT_0155*IT_0180;
    const ccomplex_t IT_0182 = IT_0025*IT_0181;
    const ccomplex_t IT_0183 = (0 + _Complex_I*1)*IT_0182;
    const ccomplex_t IT_0184 = m_C_1*IT_0183;
    const ccomplex_t IT_0185 = conj(N_W2)*e_em*V_Wp1;
    const ccomplex_t IT_0186 = IT_0006*IT_0185;
    const ccomplex_t IT_0187 = conj(N_u2)*V_u1*e_em;
    const ccomplex_t IT_0188 = IT_0006*IT_0187;
    const ccomplex_t IT_0189 = 1.4142135623731*IT_0188;
    const ccomplex_t IT_0190 = (0 + _Complex_I*1)*(IT_0186 + (-0.5)*IT_0189);
    const ccomplex_t IT_0191 = -IT_0190;
    const ccomplex_t IT_0192 = IT_0160*IT_0191;
    const ccomplex_t IT_0193 = IT_0060*IT_0192;
    const ccomplex_t IT_0194 = (0 + _Complex_I*1)*IT_0193;
    const ccomplex_t IT_0195 = m_C_1*IT_0194;
    const ccomplex_t IT_0196 = conj(N_W3)*e_em*V_Wp1;
    const ccomplex_t IT_0197 = IT_0006*IT_0196;
    const ccomplex_t IT_0198 = conj(N_u3)*V_u1*e_em;
    const ccomplex_t IT_0199 = IT_0006*IT_0198;
    const ccomplex_t IT_0200 = 1.4142135623731*IT_0199;
    const ccomplex_t IT_0201 = (0 + _Complex_I*1)*(IT_0197 + (-0.5)*IT_0200);
    const ccomplex_t IT_0202 = -IT_0201;
    const ccomplex_t IT_0203 = IT_0165*IT_0202;
    const ccomplex_t IT_0204 = IT_0079*IT_0203;
    const ccomplex_t IT_0205 = (0 + _Complex_I*1)*IT_0204;
    const ccomplex_t IT_0206 = m_C_1*IT_0205;
    const ccomplex_t IT_0207 = conj(N_W4)*e_em*V_Wp1;
    const ccomplex_t IT_0208 = IT_0006*IT_0207;
    const ccomplex_t IT_0209 = conj(N_u4)*V_u1*e_em;
    const ccomplex_t IT_0210 = IT_0006*IT_0209;
    const ccomplex_t IT_0211 = 1.4142135623731*IT_0210;
    const ccomplex_t IT_0212 = (0 + _Complex_I*1)*(IT_0208 + (-0.5)*IT_0211);
    const ccomplex_t IT_0213 = -IT_0212;
    const ccomplex_t IT_0214 = IT_0170*IT_0213;
    const ccomplex_t IT_0215 = IT_0022*IT_0214;
    const ccomplex_t IT_0216 = (0 + _Complex_I*1)*IT_0215;
    const ccomplex_t IT_0217 = m_C_1*IT_0216;
    const ccomplex_t IT_0218 = IT_0158 + IT_0163 + IT_0168 + IT_0173 + 
      -IT_0184 + -IT_0195 + -IT_0206 + -IT_0217;
    const ccomplex_t IT_0219 = s_14*IT_0090;
    const ccomplex_t IT_0220 = IT_0083*IT_0219;
    const ccomplex_t IT_0221 = 0.5*IT_0220;
    const ccomplex_t IT_0222 = s_14 + IT_0221;
    const ccomplex_t IT_0223 = m_u*IT_0222;
    const ccomplex_t IT_0224 = 2*IT_0223;
    const ccomplex_t IT_0225 = 6*IT_0224;
    const ccomplex_t IT_0226 = IT_0051*IT_0191;
    const ccomplex_t IT_0227 = IT_0060*IT_0162*IT_0226;
    const ccomplex_t IT_0228 = IT_0070*IT_0202;
    const ccomplex_t IT_0229 = IT_0079*IT_0167*IT_0228;
    const ccomplex_t IT_0230 = IT_0011*IT_0213;
    const ccomplex_t IT_0231 = IT_0022*IT_0172*IT_0230;
    const ccomplex_t IT_0232 = m_C_1*IT_0062;
    const ccomplex_t IT_0233 = IT_0033*IT_0180;
    const ccomplex_t IT_0234 = IT_0025*IT_0157*IT_0233;
    const ccomplex_t IT_0235 = m_C_1*IT_0043;
    const ccomplex_t IT_0236 = m_C_1*IT_0081;
    const ccomplex_t IT_0237 = m_C_1*IT_0024;
    const ccomplex_t IT_0238 = -IT_0227 + -IT_0229 + -IT_0231 + IT_0232 + 
      -IT_0234 + IT_0235 + IT_0236 + IT_0237;
    const ccomplex_t IT_0239 = s_34*IT_0090;
    const ccomplex_t IT_0240 = IT_0083*IT_0239;
    const ccomplex_t IT_0241 = (-0.25)*IT_0240;
    const ccomplex_t IT_0242 = s_34 + IT_0241;
    const ccomplex_t IT_0243 = m_C_1*IT_0242;
    const ccomplex_t IT_0244 = (-4)*IT_0243;
    const ccomplex_t IT_0245 = 6*IT_0244;
    const ccomplex_t IT_0246 = -IT_0183 + -IT_0194 + -IT_0205 + -IT_0216;
    const ccomplex_t IT_0247 = m_u*m_C_1;
    const ccomplex_t IT_0248 = IT_0090*IT_0247;
    const ccomplex_t IT_0249 = (-3)*IT_0248;
    const ccomplex_t IT_0250 = 6*IT_0249;
    const ccomplex_t IT_0251 = (0 + _Complex_I*1.4142135623731)*m_u*V_u1*V_us
      *e_em*IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0252 = 0.5*IT_0251;
    const ccomplex_t IT_0253 = IT_0098*IT_0252;
    const ccomplex_t IT_0254 = (0 + _Complex_I*1)*IT_0253;
    const ccomplex_t IT_0255 = IT_0095*IT_0254;
    const ccomplex_t IT_0256 = (0 + _Complex_I*1.4142135623731)*m_u*V_u1*V_ud
      *e_em*IT_0006*IT_0112*IT_0153;
    const ccomplex_t IT_0257 = 0.5*IT_0256;
    const ccomplex_t IT_0258 = IT_0136*IT_0257;
    const ccomplex_t IT_0259 = (0 + _Complex_I*1)*IT_0258;
    const ccomplex_t IT_0260 = IT_0134*IT_0259;
    const ccomplex_t IT_0261 = (0 + _Complex_I*1.4142135623731)*m_u*V_u1*e_em
      *IT_0006*IT_0105*IT_0112*IT_0153*U_sb_00*V_ub_mod;
    const ccomplex_t IT_0262 = 0.5*IT_0261;
    const ccomplex_t IT_0263 = IT_0117*IT_0262;
    const ccomplex_t IT_0264 = (0 + _Complex_I*1)*IT_0263;
    const ccomplex_t IT_0265 = IT_0104*IT_0264;
    const ccomplex_t IT_0266 = (0 + _Complex_I*1.4142135623731)*m_u*V_u1*e_em
      *IT_0006*IT_0105*IT_0112*IT_0153*U_sb_01*V_ub_mod;
    const ccomplex_t IT_0267 = 0.5*IT_0266;
    const ccomplex_t IT_0268 = IT_0129*IT_0267;
    const ccomplex_t IT_0269 = (0 + _Complex_I*1)*IT_0268;
    const ccomplex_t IT_0270 = IT_0122*IT_0269;
    const ccomplex_t IT_0271 = 2*IT_0183 + 2*IT_0194 + 2*IT_0205 + 2*IT_0216 +
       IT_0255 + IT_0260 + IT_0265 + IT_0270;
    const ccomplex_t IT_0272 = -IT_0255 + -IT_0260 + -IT_0265 + -IT_0270;
    const ccomplex_t IT_0273 = (-2)*IT_0088;
    const ccomplex_t IT_0274 = IT_0091 + IT_0273;
    const ccomplex_t IT_0275 = IT_0083*IT_0274;
    const ccomplex_t IT_0276 = (-6)*IT_0275;
    const ccomplex_t IT_0277 = 12*s_13;
    const ccomplex_t IT_0278 = IT_0276 + IT_0277;
    const ccomplex_t IT_0279 = pow(s_14, 2);
    const ccomplex_t IT_0280 = IT_0083*IT_0279;
    const ccomplex_t IT_0281 = -IT_0097;
    const ccomplex_t IT_0282 = IT_0280 + IT_0281;
    const ccomplex_t IT_0283 = m_u*IT_0282;
    const ccomplex_t IT_0284 = 6*IT_0283;
    const ccomplex_t IT_0285 = (-18)*IT_0247;
    const ccomplex_t IT_0286 = pow(s_34, 2);
    const ccomplex_t IT_0287 = IT_0083*IT_0286;
    const ccomplex_t IT_0288 = -IT_0287;
    const ccomplex_t IT_0289 = IT_0020 + IT_0288;
    const ccomplex_t IT_0290 = m_C_1*IT_0289;
    const ccomplex_t IT_0291 = (-6)*IT_0290;
    const ccomplex_t IT_0292 = s_14*s_34*IT_0083;
    const ccomplex_t IT_0293 = -IT_0292;
    const ccomplex_t IT_0294 = s_13 + IT_0293;
    const ccomplex_t IT_0295 = m_u*IT_0294;
    const ccomplex_t IT_0296 = (-6)*IT_0295;
    const ccomplex_t IT_0297 = m_C_1*IT_0294;
    const ccomplex_t IT_0298 = (-6)*IT_0297;
    const ccomplex_t IT_0299 = s_13*IT_0294;
    const ccomplex_t IT_0300 = (-6)*IT_0299;
    const ccomplex_t IT_0301 = s_13*IT_0289;
    const ccomplex_t IT_0302 = (-6)*IT_0301;
    const ccomplex_t IT_0303 = IT_0247*IT_0294;
    const ccomplex_t IT_0304 = (-6)*IT_0303;
    const ccomplex_t IT_0305 = IT_0247*IT_0289;
    const ccomplex_t IT_0306 = (-6)*IT_0305;
    const ccomplex_t IT_0307 = s_13*IT_0282;
    const ccomplex_t IT_0308 = IT_0247*IT_0282;
    const ccomplex_t IT_0309 = 6*IT_0308;
    const ccomplex_t IT_0310 = 0.166666666666667*conj(IT_0272);
    const ccomplex_t IT_0311 = 0.166666666666667*conj(IT_0238);
    const ccomplex_t IT_0312 = 0.166666666666667*conj(IT_0146);
    const ccomplex_t IT_0313 = 0.166666666666667*conj(IT_0218);
    const ccomplex_t IT_0314 = IT_0082*(conj(IT_0082)*IT_0093 + conj(IT_0140)
      *IT_0145 + conj(IT_0146)*IT_0151 + conj(IT_0218)*IT_0225 + conj(IT_0238)
      *IT_0245 + conj(IT_0246)*IT_0250) + IT_0246*(IT_0225*conj(IT_0238) + conj
      (IT_0218)*IT_0245 + IT_0093*conj(IT_0246) + conj(IT_0082)*IT_0250 +
       IT_0145*conj(IT_0271) + IT_0151*conj(IT_0272)) + IT_0238*(conj(IT_0082)
      *IT_0245 + IT_0225*conj(IT_0246) + conj(IT_0238)*IT_0278 + conj(IT_0271)
      *IT_0284 + conj(IT_0218)*IT_0285 + conj(IT_0146)*IT_0291 + conj(IT_0272)
      *IT_0296 + conj(IT_0140)*IT_0298) + IT_0218*(conj(IT_0082)*IT_0225 +
       IT_0245*conj(IT_0246) + conj(IT_0218)*IT_0278 + conj(IT_0140)*IT_0284 +
       conj(IT_0238)*IT_0285 + conj(IT_0272)*IT_0291 + conj(IT_0146)*IT_0296 +
       conj(IT_0271)*IT_0298) + IT_0272*(IT_0151*conj(IT_0246) + conj(IT_0218)
      *IT_0291 + conj(IT_0238)*IT_0296 + conj(IT_0271)*IT_0300 + conj(IT_0272)
      *IT_0302 + conj(IT_0140)*IT_0304 + conj(IT_0146)*IT_0306) + IT_0146*(conj
      (IT_0082)*IT_0151 + conj(IT_0238)*IT_0291 + conj(IT_0218)*IT_0296 + conj
      (IT_0140)*IT_0300 + conj(IT_0146)*IT_0302 + conj(IT_0271)*IT_0304 + conj
      (IT_0272)*IT_0306) + 6*IT_0140*(0.166666666666667*conj(IT_0082)*IT_0145 +
       conj(IT_0140)*IT_0307 + 0.166666666666667*conj(IT_0271)*IT_0309 + IT_0304
      *IT_0310 + IT_0298*IT_0311 + IT_0300*IT_0312 + IT_0284*IT_0313) + 6
      *IT_0271*(0.166666666666667*IT_0145*conj(IT_0246) + conj(IT_0271)*IT_0307 
      + 0.166666666666667*conj(IT_0140)*IT_0309 + IT_0300*IT_0310 + IT_0284
      *IT_0311 + IT_0304*IT_0312 + IT_0298*IT_0313);
    return create_ccomplex_return(IT_0314);
}

