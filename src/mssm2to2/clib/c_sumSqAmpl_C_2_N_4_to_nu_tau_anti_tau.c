#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_N_4_to_nu_tau_anti_tau.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_N_4_to_nu_tau_anti_tau(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_Hp = param->m_Hp;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_24 = param->s_24;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_tau = param->m_tau;
    const creal_t Gamma_W = param->Gamma_W;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_Hp = param->Gamma_Hp;
    const creal_t m_stau_1 = param->m_stau_1;
    const creal_t m_stau_2 = param->m_stau_2;
    const creal_t reg_prop = param->reg_prop;
    const creal_t m_snu_tau = param->m_snu_tau;
    const creal_t Gamma_tau1 = param->Gamma_tau1;
    const creal_t Gamma_tau2 = param->Gamma_tau2;
    const creal_t Gamma_nutaul = param->Gamma_nutaul;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t U_stau_00 = param->U_stau_00;
    const ccomplex_t U_stau_01 = param->U_stau_01;
    const ccomplex_t U_stau_10 = param->U_stau_10;
    const ccomplex_t U_stau_11 = param->U_stau_11;
    const ccomplex_t IT_0000 = s_12*s_34;
    const ccomplex_t IT_0001 = pow(m_W, 2);
    const ccomplex_t IT_0002 = pow(m_C_2, 2);
    const ccomplex_t IT_0003 = pow(m_N_4, 2);
    const ccomplex_t IT_0004 = cpow((-2)*s_12 + IT_0001 + -IT_0002 + -IT_0003 
      + -reg_prop, -1);
    const ccomplex_t IT_0005 = pow(m_W, -1);
    const ccomplex_t IT_0006 = sin(theta_W);
    const ccomplex_t IT_0007 = cpow(IT_0006, -1);
    const ccomplex_t IT_0008 = (0 + _Complex_I*1.4142135623731)*e_em*m_tau
      *IT_0005*IT_0007;
    const ccomplex_t IT_0009 = (-0.5)*IT_0008;
    const ccomplex_t IT_0010 = sin(beta);
    const ccomplex_t IT_0011 = IT_0007*IT_0010;
    const ccomplex_t IT_0012 = N_u4*e_em*V_Wp2;
    const ccomplex_t IT_0013 = IT_0011*IT_0012;
    const ccomplex_t IT_0014 = 2*IT_0013;
    const ccomplex_t IT_0015 = cos(theta_W);
    const ccomplex_t IT_0016 = cpow(IT_0015, -1);
    const ccomplex_t IT_0017 = IT_0010*IT_0016;
    const ccomplex_t IT_0018 = N_B4*V_u2*e_em;
    const ccomplex_t IT_0019 = IT_0017*IT_0018;
    const ccomplex_t IT_0020 = 1.4142135623731*IT_0019;
    const ccomplex_t IT_0021 = N_W4*V_u2*e_em;
    const ccomplex_t IT_0022 = IT_0011*IT_0021;
    const ccomplex_t IT_0023 = 1.4142135623731*IT_0022;
    const ccomplex_t IT_0024 = IT_0020 + IT_0023;
    const ccomplex_t IT_0025 = IT_0014 + IT_0024;
    const ccomplex_t IT_0026 = (0 + _Complex_I*1)*IT_0025;
    const ccomplex_t IT_0027 = 0.5*IT_0026;
    const ccomplex_t IT_0028 = IT_0009*IT_0027;
    const ccomplex_t IT_0029 = IT_0004*IT_0028;
    const ccomplex_t IT_0030 = (0 + _Complex_I*1)*IT_0029;
    const ccomplex_t IT_0031 = cpow((-2)*s_12 + -IT_0002 + -IT_0003 + m_Hp*
      (m_Hp + (0 + _Complex_I*-1)*Gamma_Hp) + -reg_prop, -1);
    const ccomplex_t IT_0032 = cos(beta);
    const ccomplex_t IT_0033 = cpow(IT_0032, -1);
    const ccomplex_t IT_0034 = (0 + _Complex_I*1.4142135623731)*e_em*m_tau
      *IT_0005*IT_0007*IT_0010*IT_0033;
    const ccomplex_t IT_0035 = 0.5*IT_0034;
    const ccomplex_t IT_0036 = IT_0007*IT_0032;
    const ccomplex_t IT_0037 = IT_0012*IT_0036;
    const ccomplex_t IT_0038 = 2*IT_0037;
    const ccomplex_t IT_0039 = IT_0016*IT_0032;
    const ccomplex_t IT_0040 = IT_0018*IT_0039;
    const ccomplex_t IT_0041 = 1.4142135623731*IT_0040;
    const ccomplex_t IT_0042 = IT_0021*IT_0036;
    const ccomplex_t IT_0043 = 1.4142135623731*IT_0042;
    const ccomplex_t IT_0044 = IT_0041 + IT_0043;
    const ccomplex_t IT_0045 = IT_0038 + IT_0044;
    const ccomplex_t IT_0046 = (0 + _Complex_I*1)*IT_0045;
    const ccomplex_t IT_0047 = 0.5*IT_0046;
    const ccomplex_t IT_0048 = IT_0035*IT_0047;
    const ccomplex_t IT_0049 = IT_0031*IT_0048;
    const ccomplex_t IT_0050 = (0 + _Complex_I*1)*IT_0049;
    const ccomplex_t IT_0051 = IT_0030 + IT_0050;
    const ccomplex_t IT_0052 = conj(N_d4)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0053 = IT_0036*IT_0052;
    const ccomplex_t IT_0054 = (-2)*IT_0053;
    const ccomplex_t IT_0055 = conj(N_B4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0056 = IT_0039*IT_0055;
    const ccomplex_t IT_0057 = 1.4142135623731*IT_0056;
    const ccomplex_t IT_0058 = conj(N_W4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0059 = IT_0036*IT_0058;
    const ccomplex_t IT_0060 = 1.4142135623731*IT_0059;
    const ccomplex_t IT_0061 = IT_0057 + IT_0060;
    const ccomplex_t IT_0062 = IT_0054 + IT_0061;
    const ccomplex_t IT_0063 = (0 + _Complex_I*1)*IT_0062;
    const ccomplex_t IT_0064 = (-0.5)*IT_0063;
    const ccomplex_t IT_0065 = IT_0009*IT_0064;
    const ccomplex_t IT_0066 = IT_0004*IT_0065;
    const ccomplex_t IT_0067 = (0 + _Complex_I*1)*IT_0066;
    const ccomplex_t IT_0068 = IT_0011*IT_0052;
    const ccomplex_t IT_0069 = (-2)*IT_0068;
    const ccomplex_t IT_0070 = IT_0017*IT_0055;
    const ccomplex_t IT_0071 = 1.4142135623731*IT_0070;
    const ccomplex_t IT_0072 = IT_0011*IT_0058;
    const ccomplex_t IT_0073 = 1.4142135623731*IT_0072;
    const ccomplex_t IT_0074 = IT_0071 + IT_0073;
    const ccomplex_t IT_0075 = IT_0069 + IT_0074;
    const ccomplex_t IT_0076 = (0 + _Complex_I*1)*IT_0075;
    const ccomplex_t IT_0077 = 0.5*IT_0076;
    const ccomplex_t IT_0078 = IT_0035*IT_0077;
    const ccomplex_t IT_0079 = IT_0031*IT_0078;
    const ccomplex_t IT_0080 = (0 + _Complex_I*1)*IT_0079;
    const ccomplex_t IT_0081 = -IT_0067 + -IT_0080;
    const ccomplex_t IT_0082 = 4*IT_0081;
    const ccomplex_t IT_0083 = s_13*s_24;
    const ccomplex_t IT_0084 = N_B4*e_em*conj(U_stau_00);
    const ccomplex_t IT_0085 = IT_0016*IT_0084;
    const ccomplex_t IT_0086 = 1.4142135623731*IT_0085;
    const ccomplex_t IT_0087 = N_W4*e_em*conj(U_stau_00);
    const ccomplex_t IT_0088 = IT_0007*IT_0087;
    const ccomplex_t IT_0089 = 1.4142135623731*IT_0088;
    const ccomplex_t IT_0090 = IT_0007*IT_0033;
    const ccomplex_t IT_0091 = N_d4*e_em*m_tau*IT_0005*conj(U_stau_10);
    const ccomplex_t IT_0092 = IT_0090*IT_0091;
    const ccomplex_t IT_0093 = 1.4142135623731*IT_0092;
    const ccomplex_t IT_0094 = (0 + _Complex_I*1)*(IT_0086 + IT_0089 + 
      -IT_0093);
    const ccomplex_t IT_0095 = (-0.5)*IT_0094;
    const ccomplex_t IT_0096 = e_em*conj(U_Wm2)*U_stau_00;
    const ccomplex_t IT_0097 = IT_0007*IT_0096;
    const ccomplex_t IT_0098 = conj(U_d2)*e_em*m_tau*IT_0005*U_stau_10;
    const ccomplex_t IT_0099 = IT_0090*IT_0098;
    const ccomplex_t IT_0100 = 1.4142135623731*IT_0099;
    const ccomplex_t IT_0101 = (0 + _Complex_I*1)*(IT_0097 + (-0.5)*IT_0100);
    const ccomplex_t IT_0102 = IT_0095*IT_0101;
    const ccomplex_t IT_0103 = cpow(s_13 + (-0.5)*IT_0002 + (-0.5)*reg_prop +
       0.5*m_stau_1*(m_stau_1 + (0 + _Complex_I*-1)*Gamma_tau1), -1);
    const ccomplex_t IT_0104 = IT_0102*IT_0103;
    const ccomplex_t IT_0105 = (0 + _Complex_I*1)*IT_0104;
    const ccomplex_t IT_0106 = N_B4*e_em*conj(U_stau_01);
    const ccomplex_t IT_0107 = IT_0016*IT_0106;
    const ccomplex_t IT_0108 = 1.4142135623731*IT_0107;
    const ccomplex_t IT_0109 = N_W4*e_em*conj(U_stau_01);
    const ccomplex_t IT_0110 = IT_0007*IT_0109;
    const ccomplex_t IT_0111 = 1.4142135623731*IT_0110;
    const ccomplex_t IT_0112 = N_d4*e_em*m_tau*IT_0005*conj(U_stau_11);
    const ccomplex_t IT_0113 = IT_0090*IT_0112;
    const ccomplex_t IT_0114 = 1.4142135623731*IT_0113;
    const ccomplex_t IT_0115 = (0 + _Complex_I*1)*(IT_0108 + IT_0111 + 
      -IT_0114);
    const ccomplex_t IT_0116 = (-0.5)*IT_0115;
    const ccomplex_t IT_0117 = e_em*conj(U_Wm2)*U_stau_01;
    const ccomplex_t IT_0118 = IT_0007*IT_0117;
    const ccomplex_t IT_0119 = conj(U_d2)*e_em*m_tau*IT_0005*U_stau_11;
    const ccomplex_t IT_0120 = IT_0090*IT_0119;
    const ccomplex_t IT_0121 = 1.4142135623731*IT_0120;
    const ccomplex_t IT_0122 = (0 + _Complex_I*1)*(IT_0118 + (-0.5)*IT_0121);
    const ccomplex_t IT_0123 = IT_0116*IT_0122;
    const ccomplex_t IT_0124 = cpow(s_13 + (-0.5)*IT_0002 + (-0.5)*reg_prop +
       0.5*m_stau_2*(m_stau_2 + (0 + _Complex_I*-1)*Gamma_tau2), -1);
    const ccomplex_t IT_0125 = IT_0123*IT_0124;
    const ccomplex_t IT_0126 = (0 + _Complex_I*1)*IT_0125;
    const ccomplex_t IT_0127 = 0.5*IT_0105 + 0.5*IT_0126;
    const ccomplex_t IT_0128 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0007;
    const ccomplex_t IT_0129 = 0.5*IT_0128;
    const ccomplex_t IT_0130 = N_W4*e_em*conj(U_Wm2);
    const ccomplex_t IT_0131 = IT_0007*IT_0130;
    const ccomplex_t IT_0132 = N_d4*conj(U_d2)*e_em;
    const ccomplex_t IT_0133 = IT_0007*IT_0132;
    const ccomplex_t IT_0134 = 1.4142135623731*IT_0133;
    const ccomplex_t IT_0135 = (0 + _Complex_I*1)*(IT_0131 + 0.5*IT_0134);
    const ccomplex_t IT_0136 = -IT_0135;
    const ccomplex_t IT_0137 = IT_0129*IT_0136;
    const ccomplex_t IT_0138 = cpow((-2)*s_12 + m_W*(m_W + (0 + _Complex_I*-1)
      *Gamma_W) + -IT_0002 + -IT_0003 + -reg_prop, -1);
    const ccomplex_t IT_0139 = IT_0137*IT_0138;
    const ccomplex_t IT_0140 = (0 + _Complex_I*1)*IT_0139;
    const ccomplex_t IT_0141 = 4*IT_0127;
    const ccomplex_t IT_0142 = conj(N_B4)*e_em*conj(U_stau_10);
    const ccomplex_t IT_0143 = IT_0016*IT_0142;
    const ccomplex_t IT_0144 = 1.4142135623731*IT_0143;
    const ccomplex_t IT_0145 = conj(N_d4)*e_em*m_tau*IT_0005*conj(U_stau_00);
    const ccomplex_t IT_0146 = IT_0090*IT_0145;
    const ccomplex_t IT_0147 = 1.4142135623731*IT_0146;
    const ccomplex_t IT_0148 = (0 + _Complex_I*1)*(IT_0144 + 0.5*IT_0147);
    const ccomplex_t IT_0149 = -IT_0148;
    const ccomplex_t IT_0150 = IT_0101*IT_0149;
    const ccomplex_t IT_0151 = IT_0103*IT_0150;
    const ccomplex_t IT_0152 = (0 + _Complex_I*1)*IT_0151;
    const ccomplex_t IT_0153 = conj(N_B4)*e_em*conj(U_stau_11);
    const ccomplex_t IT_0154 = IT_0016*IT_0153;
    const ccomplex_t IT_0155 = 1.4142135623731*IT_0154;
    const ccomplex_t IT_0156 = conj(N_d4)*e_em*m_tau*IT_0005*conj(U_stau_01);
    const ccomplex_t IT_0157 = IT_0090*IT_0156;
    const ccomplex_t IT_0158 = 1.4142135623731*IT_0157;
    const ccomplex_t IT_0159 = (0 + _Complex_I*1)*(IT_0155 + 0.5*IT_0158);
    const ccomplex_t IT_0160 = -IT_0159;
    const ccomplex_t IT_0161 = IT_0122*IT_0160;
    const ccomplex_t IT_0162 = IT_0124*IT_0161;
    const ccomplex_t IT_0163 = (0 + _Complex_I*1)*IT_0162;
    const ccomplex_t IT_0164 = (-0.5)*IT_0152 + (-0.5)*IT_0163;
    const ccomplex_t IT_0165 = 4*IT_0164;
    const ccomplex_t IT_0166 = s_14*s_23;
    const ccomplex_t IT_0167 = cpow(s_23 + (-0.5)*IT_0003 + (-0.5)*reg_prop +
       0.5*m_snu_tau*(m_snu_tau + (0 + _Complex_I*-1)*Gamma_nutaul), -1);
    const ccomplex_t IT_0168 = conj(N_B4)*e_em;
    const ccomplex_t IT_0169 = IT_0016*IT_0168;
    const ccomplex_t IT_0170 = 1.4142135623731*IT_0169;
    const ccomplex_t IT_0171 = conj(N_W4)*e_em;
    const ccomplex_t IT_0172 = IT_0007*IT_0171;
    const ccomplex_t IT_0173 = 1.4142135623731*IT_0172;
    const ccomplex_t IT_0174 = (0 + _Complex_I*1)*(IT_0170 + -IT_0173);
    const ccomplex_t IT_0175 = (-0.5)*IT_0174;
    const ccomplex_t IT_0176 = (0 + _Complex_I*1)*e_em*V_Wp2*IT_0007;
    const ccomplex_t IT_0177 = -IT_0176;
    const ccomplex_t IT_0178 = IT_0175*IT_0177;
    const ccomplex_t IT_0179 = IT_0167*IT_0178;
    const ccomplex_t IT_0180 = (0 + _Complex_I*1)*IT_0179;
    const ccomplex_t IT_0181 = 0.5*IT_0180;
    const ccomplex_t IT_0182 = conj(N_W4)*e_em*V_Wp2;
    const ccomplex_t IT_0183 = IT_0007*IT_0182;
    const ccomplex_t IT_0184 = conj(N_u4)*V_u2*e_em;
    const ccomplex_t IT_0185 = IT_0007*IT_0184;
    const ccomplex_t IT_0186 = 1.4142135623731*IT_0185;
    const ccomplex_t IT_0187 = (0 + _Complex_I*1)*(IT_0183 + (-0.5)*IT_0186);
    const ccomplex_t IT_0188 = -IT_0187;
    const ccomplex_t IT_0189 = IT_0129*IT_0188;
    const ccomplex_t IT_0190 = IT_0138*IT_0189;
    const ccomplex_t IT_0191 = (0 + _Complex_I*1)*IT_0190;
    const ccomplex_t IT_0192 = 4*IT_0181;
    const ccomplex_t IT_0193 = 0.125*IT_0192;
    const ccomplex_t IT_0194 = IT_0191 + IT_0193;
    const ccomplex_t IT_0195 = (0 + _Complex_I*1.4142135623731)*conj(U_d2)
      *e_em*m_tau*IT_0005*IT_0007*IT_0033;
    const ccomplex_t IT_0196 = (-0.5)*IT_0195;
    const ccomplex_t IT_0197 = IT_0175*IT_0196;
    const ccomplex_t IT_0198 = IT_0167*IT_0197;
    const ccomplex_t IT_0199 = (0 + _Complex_I*1)*IT_0198;
    const ccomplex_t IT_0200 = (-0.5)*IT_0199;
    const ccomplex_t IT_0201 = 4*IT_0200;
    const ccomplex_t IT_0202 = -IT_0083;
    const ccomplex_t IT_0203 = (-2)*IT_0081;
    const ccomplex_t IT_0204 = (-2)*conj(IT_0081);
    const ccomplex_t IT_0205 = s_13*m_N_4*m_tau;
    const ccomplex_t IT_0206 = 2*conj(IT_0051);
    const ccomplex_t IT_0207 = 2*IT_0081;
    const ccomplex_t IT_0208 = 2*conj(IT_0081);
    const ccomplex_t IT_0209 = 2*conj(IT_0191);
    const ccomplex_t IT_0210 = 4*conj(IT_0127);
    const ccomplex_t IT_0211 = 0.5*IT_0164;
    const ccomplex_t IT_0212 = 0.5*conj(IT_0164);
    const ccomplex_t IT_0213 = conj(IT_0051)*IT_0191 + IT_0140*(conj(IT_0081) 
      + 2*conj(IT_0164) + conj(IT_0200)) + 2*conj(IT_0140)*(0.125*IT_0082 +
       IT_0164 + 0.125*IT_0201) + 0.25*IT_0181*IT_0206 + 0.5*conj(IT_0127)*
      (IT_0200 + 0.5*IT_0207) + 0.5*IT_0127*(conj(IT_0200) + 0.5*IT_0208) + 0.5
      *IT_0051*(conj(IT_0181) + IT_0209) + 0.5*IT_0210*IT_0211 + 0.5*IT_0141
      *IT_0212;
    const ccomplex_t IT_0214 = s_34*m_C_2*m_N_4;
    const ccomplex_t IT_0215 = 2*IT_0181;
    const ccomplex_t IT_0216 = 4*conj(IT_0181);
    const ccomplex_t IT_0217 = (-4)*conj(IT_0051);
    const ccomplex_t IT_0218 = (-2)*conj(IT_0051);
    const ccomplex_t IT_0219 = (-0.5)*conj(IT_0200);
    const ccomplex_t IT_0220 = IT_0127*conj(IT_0181) + 0.5*IT_0141*conj
      (IT_0191) + 4*conj(IT_0140)*IT_0194 + 0.5*IT_0200*IT_0206 + 0.5*IT_0191
      *IT_0210 + 0.5*conj(IT_0127)*IT_0215 + 4*IT_0140*(conj(IT_0191) + 0.125
      *IT_0216) + 0.5*IT_0081*IT_0217 + IT_0211*IT_0218 + (-2)*IT_0051*(conj
      (IT_0081) + IT_0212 + IT_0219);
    const ccomplex_t IT_0221 = s_23*m_C_2*m_tau;
    const ccomplex_t IT_0222 = IT_0051*(conj(IT_0127) + 2*conj(IT_0140)) + 2
      *IT_0191*(conj(IT_0081) + -conj(IT_0164) + (-2)*conj(IT_0200)) + -conj
      (IT_0181)*(IT_0164 + 0.5*IT_0203) + (-0.5)*IT_0181*IT_0204 + (IT_0081 + (
      -0.25)*IT_0165 + (-2)*IT_0200)*IT_0209 + (-0.5)*conj(IT_0164)*IT_0215 + (
      -0.5)*IT_0200*IT_0216 + (-0.5)*IT_0140*IT_0217 + (-0.5)*IT_0127*IT_0218 +
       IT_0192*IT_0219;
    const ccomplex_t IT_0223 = -IT_0166;
    const ccomplex_t IT_0224 = IT_0000 + IT_0223;
    const ccomplex_t IT_0225 = IT_0202 + IT_0224;
    const ccomplex_t IT_0226 = IT_0083 + IT_0224;
    const ccomplex_t IT_0227 = (-0.5)*IT_0226;
    const ccomplex_t IT_0228 = 4*IT_0000*(IT_0051*conj(IT_0051) + 0.25*conj
      (IT_0081)*IT_0082) + 8*IT_0083*((IT_0127 + 2*IT_0140)*conj(IT_0140) + conj
      (IT_0127)*(IT_0140 + 0.125*IT_0141) + 0.125*conj(IT_0164)*IT_0165) + 8
      *IT_0166*((IT_0181 + 2*IT_0191)*conj(IT_0191) + conj(IT_0181)*IT_0194 +
       0.125*conj(IT_0200)*IT_0201) + (IT_0000 + IT_0166 + IT_0202)*(conj
      (IT_0200)*IT_0203 + IT_0200*IT_0204) + 4*IT_0205*IT_0213 + 2*IT_0214
      *IT_0220 + (-2)*IT_0221*IT_0222 + (-2)*conj(IT_0164)*(IT_0200*IT_0225 +
       IT_0207*IT_0227) + (-2)*IT_0164*(conj(IT_0200)*IT_0225 + IT_0208*IT_0227);
    return create_ccomplex_return(IT_0228);
}

