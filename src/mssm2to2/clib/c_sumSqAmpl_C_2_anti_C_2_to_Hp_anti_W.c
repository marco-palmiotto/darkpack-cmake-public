#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_anti_C_2_to_Hp_anti_W.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_anti_C_2_to_Hp_anti_W(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_h = param->m_h;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_A0 = param->m_A0;
    const creal_t m_H0 = param->m_H0;
    const creal_t m_Hp = param->m_Hp;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_24 = param->s_24;
    const creal_t alpha = param->alpha;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t Gamma_h = param->Gamma_h;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_A0 = param->Gamma_A0;
    const creal_t Gamma_H0 = param->Gamma_H0;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0001 = cos(beta);
    const ccomplex_t IT_0002 = sin(theta_W);
    const ccomplex_t IT_0003 = cpow(IT_0002, -1);
    const ccomplex_t IT_0004 = IT_0001*IT_0003;
    const ccomplex_t IT_0005 = N_u1*e_em*V_Wp2;
    const ccomplex_t IT_0006 = IT_0004*IT_0005;
    const ccomplex_t IT_0007 = 2*IT_0006;
    const ccomplex_t IT_0008 = cos(theta_W);
    const ccomplex_t IT_0009 = cpow(IT_0008, -1);
    const ccomplex_t IT_0010 = IT_0001*IT_0009;
    const ccomplex_t IT_0011 = N_B1*V_u2*e_em;
    const ccomplex_t IT_0012 = IT_0010*IT_0011;
    const ccomplex_t IT_0013 = 1.4142135623731*IT_0012;
    const ccomplex_t IT_0014 = N_W1*V_u2*e_em;
    const ccomplex_t IT_0015 = IT_0004*IT_0014;
    const ccomplex_t IT_0016 = 1.4142135623731*IT_0015;
    const ccomplex_t IT_0017 = IT_0013 + IT_0016;
    const ccomplex_t IT_0018 = IT_0007 + IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1)*IT_0018;
    const ccomplex_t IT_0020 = 0.5*IT_0019;
    const ccomplex_t IT_0021 = N_W1*e_em*conj(V_Wp2);
    const ccomplex_t IT_0022 = IT_0003*IT_0021;
    const ccomplex_t IT_0023 = N_u1*conj(V_u2)*e_em;
    const ccomplex_t IT_0024 = IT_0003*IT_0023;
    const ccomplex_t IT_0025 = 1.4142135623731*IT_0024;
    const ccomplex_t IT_0026 = (0 + _Complex_I*1)*(IT_0022 + (-0.5)*IT_0025);
    const ccomplex_t IT_0027 = -IT_0026;
    const ccomplex_t IT_0028 = IT_0020*IT_0027;
    const ccomplex_t IT_0029 = pow(m_Hp, 2);
    const ccomplex_t IT_0030 = pow(m_C_2, 2);
    const ccomplex_t IT_0031 = cpow((-2)*s_13 + IT_0029 + IT_0030 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0032 = IT_0000*IT_0028*IT_0031;
    const ccomplex_t IT_0033 = conj(N_W1)*e_em*U_Wm2;
    const ccomplex_t IT_0034 = IT_0003*IT_0033;
    const ccomplex_t IT_0035 = conj(N_d1)*U_d2*e_em;
    const ccomplex_t IT_0036 = IT_0003*IT_0035;
    const ccomplex_t IT_0037 = 1.4142135623731*IT_0036;
    const ccomplex_t IT_0038 = (0 + _Complex_I*1)*(IT_0034 + 0.5*IT_0037);
    const ccomplex_t IT_0039 = -IT_0038;
    const ccomplex_t IT_0040 = IT_0020*IT_0039;
    const ccomplex_t IT_0041 = IT_0031*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*IT_0041;
    const ccomplex_t IT_0043 = m_C_2*IT_0042;
    const ccomplex_t IT_0044 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0045 = cpow((-2)*s_13 + IT_0029 + IT_0030 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0046 = N_u2*e_em*V_Wp2;
    const ccomplex_t IT_0047 = IT_0004*IT_0046;
    const ccomplex_t IT_0048 = 2*IT_0047;
    const ccomplex_t IT_0049 = N_B2*V_u2*e_em;
    const ccomplex_t IT_0050 = IT_0010*IT_0049;
    const ccomplex_t IT_0051 = 1.4142135623731*IT_0050;
    const ccomplex_t IT_0052 = N_W2*V_u2*e_em;
    const ccomplex_t IT_0053 = IT_0004*IT_0052;
    const ccomplex_t IT_0054 = 1.4142135623731*IT_0053;
    const ccomplex_t IT_0055 = IT_0051 + IT_0054;
    const ccomplex_t IT_0056 = IT_0048 + IT_0055;
    const ccomplex_t IT_0057 = (0 + _Complex_I*1)*IT_0056;
    const ccomplex_t IT_0058 = 0.5*IT_0057;
    const ccomplex_t IT_0059 = N_W2*e_em*conj(V_Wp2);
    const ccomplex_t IT_0060 = IT_0003*IT_0059;
    const ccomplex_t IT_0061 = N_u2*conj(V_u2)*e_em;
    const ccomplex_t IT_0062 = IT_0003*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = (0 + _Complex_I*1)*(IT_0060 + (-0.5)*IT_0063);
    const ccomplex_t IT_0065 = -IT_0064;
    const ccomplex_t IT_0066 = IT_0058*IT_0065;
    const ccomplex_t IT_0067 = IT_0044*IT_0045*IT_0066;
    const ccomplex_t IT_0068 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0069 = cpow((-2)*s_13 + IT_0029 + IT_0030 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0070 = N_u3*e_em*V_Wp2;
    const ccomplex_t IT_0071 = IT_0004*IT_0070;
    const ccomplex_t IT_0072 = 2*IT_0071;
    const ccomplex_t IT_0073 = N_B3*V_u2*e_em;
    const ccomplex_t IT_0074 = IT_0010*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = N_W3*V_u2*e_em;
    const ccomplex_t IT_0077 = IT_0004*IT_0076;
    const ccomplex_t IT_0078 = 1.4142135623731*IT_0077;
    const ccomplex_t IT_0079 = IT_0075 + IT_0078;
    const ccomplex_t IT_0080 = IT_0072 + IT_0079;
    const ccomplex_t IT_0081 = (0 + _Complex_I*1)*IT_0080;
    const ccomplex_t IT_0082 = 0.5*IT_0081;
    const ccomplex_t IT_0083 = N_W3*e_em*conj(V_Wp2);
    const ccomplex_t IT_0084 = IT_0003*IT_0083;
    const ccomplex_t IT_0085 = N_u3*conj(V_u2)*e_em;
    const ccomplex_t IT_0086 = IT_0003*IT_0085;
    const ccomplex_t IT_0087 = 1.4142135623731*IT_0086;
    const ccomplex_t IT_0088 = (0 + _Complex_I*1)*(IT_0084 + (-0.5)*IT_0087);
    const ccomplex_t IT_0089 = -IT_0088;
    const ccomplex_t IT_0090 = IT_0082*IT_0089;
    const ccomplex_t IT_0091 = IT_0068*IT_0069*IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0093 = cpow((-2)*s_13 + IT_0029 + IT_0030 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0094 = N_W4*e_em*conj(V_Wp2);
    const ccomplex_t IT_0095 = IT_0003*IT_0094;
    const ccomplex_t IT_0096 = N_u4*conj(V_u2)*e_em;
    const ccomplex_t IT_0097 = IT_0003*IT_0096;
    const ccomplex_t IT_0098 = 1.4142135623731*IT_0097;
    const ccomplex_t IT_0099 = (0 + _Complex_I*1)*(IT_0095 + (-0.5)*IT_0098);
    const ccomplex_t IT_0100 = -IT_0099;
    const ccomplex_t IT_0101 = N_u4*e_em*V_Wp2;
    const ccomplex_t IT_0102 = IT_0004*IT_0101;
    const ccomplex_t IT_0103 = 2*IT_0102;
    const ccomplex_t IT_0104 = N_B4*V_u2*e_em;
    const ccomplex_t IT_0105 = IT_0010*IT_0104;
    const ccomplex_t IT_0106 = 1.4142135623731*IT_0105;
    const ccomplex_t IT_0107 = N_W4*V_u2*e_em;
    const ccomplex_t IT_0108 = IT_0004*IT_0107;
    const ccomplex_t IT_0109 = 1.4142135623731*IT_0108;
    const ccomplex_t IT_0110 = IT_0106 + IT_0109;
    const ccomplex_t IT_0111 = IT_0103 + IT_0110;
    const ccomplex_t IT_0112 = (0 + _Complex_I*1)*IT_0111;
    const ccomplex_t IT_0113 = 0.5*IT_0112;
    const ccomplex_t IT_0114 = IT_0100*IT_0113;
    const ccomplex_t IT_0115 = IT_0092*IT_0093*IT_0114;
    const ccomplex_t IT_0116 = conj(N_W2)*e_em*U_Wm2;
    const ccomplex_t IT_0117 = IT_0003*IT_0116;
    const ccomplex_t IT_0118 = conj(N_d2)*U_d2*e_em;
    const ccomplex_t IT_0119 = IT_0003*IT_0118;
    const ccomplex_t IT_0120 = 1.4142135623731*IT_0119;
    const ccomplex_t IT_0121 = (0 + _Complex_I*1)*(IT_0117 + 0.5*IT_0120);
    const ccomplex_t IT_0122 = -IT_0121;
    const ccomplex_t IT_0123 = IT_0058*IT_0122;
    const ccomplex_t IT_0124 = IT_0045*IT_0123;
    const ccomplex_t IT_0125 = (0 + _Complex_I*1)*IT_0124;
    const ccomplex_t IT_0126 = m_C_2*IT_0125;
    const ccomplex_t IT_0127 = conj(N_W3)*e_em*U_Wm2;
    const ccomplex_t IT_0128 = IT_0003*IT_0127;
    const ccomplex_t IT_0129 = conj(N_d3)*U_d2*e_em;
    const ccomplex_t IT_0130 = IT_0003*IT_0129;
    const ccomplex_t IT_0131 = 1.4142135623731*IT_0130;
    const ccomplex_t IT_0132 = (0 + _Complex_I*1)*(IT_0128 + 0.5*IT_0131);
    const ccomplex_t IT_0133 = -IT_0132;
    const ccomplex_t IT_0134 = IT_0082*IT_0133;
    const ccomplex_t IT_0135 = IT_0069*IT_0134;
    const ccomplex_t IT_0136 = (0 + _Complex_I*1)*IT_0135;
    const ccomplex_t IT_0137 = m_C_2*IT_0136;
    const ccomplex_t IT_0138 = conj(N_W4)*e_em*U_Wm2;
    const ccomplex_t IT_0139 = IT_0003*IT_0138;
    const ccomplex_t IT_0140 = conj(N_d4)*U_d2*e_em;
    const ccomplex_t IT_0141 = IT_0003*IT_0140;
    const ccomplex_t IT_0142 = 1.4142135623731*IT_0141;
    const ccomplex_t IT_0143 = (0 + _Complex_I*1)*(IT_0139 + 0.5*IT_0142);
    const ccomplex_t IT_0144 = -IT_0143;
    const ccomplex_t IT_0145 = IT_0113*IT_0144;
    const ccomplex_t IT_0146 = IT_0093*IT_0145;
    const ccomplex_t IT_0147 = (0 + _Complex_I*1)*IT_0146;
    const ccomplex_t IT_0148 = m_C_2*IT_0147;
    const ccomplex_t IT_0149 = -IT_0032 + IT_0043 + -IT_0067 + -IT_0091 + 
      -IT_0115 + IT_0126 + IT_0137 + IT_0148;
    const ccomplex_t IT_0150 = e_em*IT_0003;
    const ccomplex_t IT_0151 = V_u2*U_Wm2;
    const ccomplex_t IT_0152 = IT_0001*IT_0151;
    const ccomplex_t IT_0153 = sin(beta);
    const ccomplex_t IT_0154 = U_d2*V_Wp2;
    const ccomplex_t IT_0155 = IT_0153*IT_0154;
    const ccomplex_t IT_0156 = IT_0003*(IT_0152 + IT_0155);
    const ccomplex_t IT_0157 = 1.4142135623731*e_em*IT_0156;
    const ccomplex_t IT_0158 = 0.5*IT_0157;
    const ccomplex_t IT_0159 = cpow((-2)*s_12 + (-2)*IT_0030 + m_A0*(m_A0 + (0
       + _Complex_I*-1)*Gamma_A0) + -reg_prop, -1);
    const ccomplex_t IT_0160 = IT_0158*IT_0159;
    const ccomplex_t IT_0161 = (0 + _Complex_I*1)*IT_0160;
    const ccomplex_t IT_0162 = IT_0150*IT_0161;
    const ccomplex_t IT_0163 = cos(alpha);
    const ccomplex_t IT_0164 = sin(alpha);
    const ccomplex_t IT_0165 = (0 + _Complex_I*1)*e_em*IT_0003*(IT_0153
      *IT_0163 + -IT_0001*IT_0164);
    const ccomplex_t IT_0166 = IT_0154*IT_0163;
    const ccomplex_t IT_0167 = IT_0151*IT_0164;
    const ccomplex_t IT_0168 = IT_0003*(IT_0166 + IT_0167);
    const ccomplex_t IT_0169 = 1.4142135623731*e_em*IT_0168;
    const ccomplex_t IT_0170 = (0 + _Complex_I*1)*IT_0169;
    const ccomplex_t IT_0171 = 0.5*IT_0170;
    const ccomplex_t IT_0172 = cpow((-2)*s_12 + (-2)*IT_0030 + m_H0*(m_H0 + (0
       + _Complex_I*-1)*Gamma_H0) + -reg_prop, -1);
    const ccomplex_t IT_0173 = IT_0171*IT_0172;
    const ccomplex_t IT_0174 = (0 + _Complex_I*1)*IT_0173;
    const ccomplex_t IT_0175 = IT_0165*IT_0174;
    const ccomplex_t IT_0176 = (0 + _Complex_I*1)*e_em*IT_0003*(IT_0001
      *IT_0163 + IT_0153*IT_0164);
    const ccomplex_t IT_0177 = -IT_0176;
    const ccomplex_t IT_0178 = IT_0151*IT_0163;
    const ccomplex_t IT_0179 = IT_0154*IT_0164;
    const ccomplex_t IT_0180 = IT_0003*(IT_0178 + -IT_0179);
    const ccomplex_t IT_0181 = 1.4142135623731*e_em*IT_0180;
    const ccomplex_t IT_0182 = 0.5*IT_0181;
    const ccomplex_t IT_0183 = (0 + _Complex_I*1)*IT_0182;
    const ccomplex_t IT_0184 = cpow((-2)*s_12 + m_h*(m_h + (0 + _Complex_I*-1)
      *Gamma_h) + (-2)*IT_0030 + -reg_prop, -1);
    const ccomplex_t IT_0185 = IT_0183*IT_0184;
    const ccomplex_t IT_0186 = (0 + _Complex_I*1)*IT_0185;
    const ccomplex_t IT_0187 = IT_0177*IT_0186;
    const ccomplex_t IT_0188 = 2*IT_0042 + 2*IT_0125 + 2*IT_0136 + 2*IT_0147 +
       -IT_0162 + -IT_0175 + -IT_0187;
    const ccomplex_t IT_0189 = pow(m_W, -2);
    const ccomplex_t IT_0190 = s_14*s_24*IT_0189;
    const ccomplex_t IT_0191 = -IT_0190;
    const ccomplex_t IT_0192 = s_12 + IT_0191;
    const ccomplex_t IT_0193 = m_C_2*IT_0192;
    const ccomplex_t IT_0194 = 2*IT_0193;
    const ccomplex_t IT_0195 = conj(V_u2)*conj(U_Wm2);
    const ccomplex_t IT_0196 = IT_0163*IT_0195;
    const ccomplex_t IT_0197 = conj(U_d2)*conj(V_Wp2);
    const ccomplex_t IT_0198 = IT_0164*IT_0197;
    const ccomplex_t IT_0199 = IT_0003*(IT_0196 + -IT_0198);
    const ccomplex_t IT_0200 = 1.4142135623731*e_em*IT_0199;
    const ccomplex_t IT_0201 = 0.5*IT_0200;
    const ccomplex_t IT_0202 = (0 + _Complex_I*1)*IT_0201;
    const ccomplex_t IT_0203 = IT_0184*IT_0202;
    const ccomplex_t IT_0204 = (0 + _Complex_I*1)*IT_0203;
    const ccomplex_t IT_0205 = IT_0177*IT_0204;
    const ccomplex_t IT_0206 = IT_0003*IT_0153;
    const ccomplex_t IT_0207 = conj(N_W1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0208 = IT_0206*IT_0207;
    const ccomplex_t IT_0209 = 1.4142135623731*IT_0208;
    const ccomplex_t IT_0210 = IT_0009*IT_0153;
    const ccomplex_t IT_0211 = conj(N_B1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0212 = IT_0210*IT_0211;
    const ccomplex_t IT_0213 = 1.4142135623731*IT_0212;
    const ccomplex_t IT_0214 = IT_0209 + IT_0213;
    const ccomplex_t IT_0215 = conj(N_d1)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0216 = IT_0206*IT_0215;
    const ccomplex_t IT_0217 = (-2)*IT_0216;
    const ccomplex_t IT_0218 = IT_0214 + IT_0217;
    const ccomplex_t IT_0219 = (0 + _Complex_I*1)*IT_0218;
    const ccomplex_t IT_0220 = 0.5*IT_0219;
    const ccomplex_t IT_0221 = IT_0027*IT_0220;
    const ccomplex_t IT_0222 = IT_0031*IT_0221;
    const ccomplex_t IT_0223 = (0 + _Complex_I*1)*IT_0222;
    const ccomplex_t IT_0224 = IT_0163*IT_0197;
    const ccomplex_t IT_0225 = IT_0164*IT_0195;
    const ccomplex_t IT_0226 = IT_0003*(IT_0224 + IT_0225);
    const ccomplex_t IT_0227 = 1.4142135623731*e_em*IT_0226;
    const ccomplex_t IT_0228 = (0 + _Complex_I*1)*IT_0227;
    const ccomplex_t IT_0229 = 0.5*IT_0228;
    const ccomplex_t IT_0230 = IT_0172*IT_0229;
    const ccomplex_t IT_0231 = (0 + _Complex_I*1)*IT_0230;
    const ccomplex_t IT_0232 = IT_0165*IT_0231;
    const ccomplex_t IT_0233 = conj(N_d2)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0234 = IT_0206*IT_0233;
    const ccomplex_t IT_0235 = (-2)*IT_0234;
    const ccomplex_t IT_0236 = conj(N_B2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0237 = IT_0210*IT_0236;
    const ccomplex_t IT_0238 = 1.4142135623731*IT_0237;
    const ccomplex_t IT_0239 = conj(N_W2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0240 = IT_0206*IT_0239;
    const ccomplex_t IT_0241 = 1.4142135623731*IT_0240;
    const ccomplex_t IT_0242 = IT_0238 + IT_0241;
    const ccomplex_t IT_0243 = IT_0235 + IT_0242;
    const ccomplex_t IT_0244 = (0 + _Complex_I*1)*IT_0243;
    const ccomplex_t IT_0245 = 0.5*IT_0244;
    const ccomplex_t IT_0246 = IT_0065*IT_0245;
    const ccomplex_t IT_0247 = IT_0045*IT_0246;
    const ccomplex_t IT_0248 = (0 + _Complex_I*1)*IT_0247;
    const ccomplex_t IT_0249 = conj(N_d3)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0250 = IT_0206*IT_0249;
    const ccomplex_t IT_0251 = (-2)*IT_0250;
    const ccomplex_t IT_0252 = conj(N_B3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0253 = IT_0210*IT_0252;
    const ccomplex_t IT_0254 = 1.4142135623731*IT_0253;
    const ccomplex_t IT_0255 = conj(N_W3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0256 = IT_0206*IT_0255;
    const ccomplex_t IT_0257 = 1.4142135623731*IT_0256;
    const ccomplex_t IT_0258 = IT_0254 + IT_0257;
    const ccomplex_t IT_0259 = IT_0251 + IT_0258;
    const ccomplex_t IT_0260 = (0 + _Complex_I*1)*IT_0259;
    const ccomplex_t IT_0261 = 0.5*IT_0260;
    const ccomplex_t IT_0262 = IT_0089*IT_0261;
    const ccomplex_t IT_0263 = IT_0069*IT_0262;
    const ccomplex_t IT_0264 = (0 + _Complex_I*1)*IT_0263;
    const ccomplex_t IT_0265 = conj(N_d4)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0266 = IT_0206*IT_0265;
    const ccomplex_t IT_0267 = (-2)*IT_0266;
    const ccomplex_t IT_0268 = conj(N_B4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0269 = IT_0210*IT_0268;
    const ccomplex_t IT_0270 = 1.4142135623731*IT_0269;
    const ccomplex_t IT_0271 = conj(N_W4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0272 = IT_0206*IT_0271;
    const ccomplex_t IT_0273 = 1.4142135623731*IT_0272;
    const ccomplex_t IT_0274 = IT_0270 + IT_0273;
    const ccomplex_t IT_0275 = IT_0267 + IT_0274;
    const ccomplex_t IT_0276 = (0 + _Complex_I*1)*IT_0275;
    const ccomplex_t IT_0277 = 0.5*IT_0276;
    const ccomplex_t IT_0278 = IT_0100*IT_0277;
    const ccomplex_t IT_0279 = IT_0093*IT_0278;
    const ccomplex_t IT_0280 = (0 + _Complex_I*1)*IT_0279;
    const ccomplex_t IT_0281 = IT_0001*IT_0195;
    const ccomplex_t IT_0282 = IT_0153*IT_0197;
    const ccomplex_t IT_0283 = IT_0003*(IT_0281 + IT_0282);
    const ccomplex_t IT_0284 = 1.4142135623731*e_em*IT_0283;
    const ccomplex_t IT_0285 = (-0.5)*IT_0284;
    const ccomplex_t IT_0286 = IT_0159*IT_0285;
    const ccomplex_t IT_0287 = (0 + _Complex_I*1)*IT_0286;
    const ccomplex_t IT_0288 = IT_0150*IT_0287;
    const ccomplex_t IT_0289 = -IT_0205 + (-2)*IT_0223 + -IT_0232 + (-2)
      *IT_0248 + (-2)*IT_0264 + (-2)*IT_0280 + -IT_0288;
    const ccomplex_t IT_0290 = pow(s_24, 2);
    const ccomplex_t IT_0291 = IT_0189*IT_0290;
    const ccomplex_t IT_0292 = -IT_0291;
    const ccomplex_t IT_0293 = IT_0030 + IT_0292;
    const ccomplex_t IT_0294 = m_C_2*IT_0293;
    const ccomplex_t IT_0295 = -IT_0294;
    const ccomplex_t IT_0296 = 2*IT_0295;
    const ccomplex_t IT_0297 = IT_0223 + IT_0248 + IT_0264 + IT_0280;
    const ccomplex_t IT_0298 = pow(m_W, 2);
    const ccomplex_t IT_0299 = s_24*IT_0298;
    const ccomplex_t IT_0300 = IT_0189*IT_0299;
    const ccomplex_t IT_0301 = 0.5*IT_0300;
    const ccomplex_t IT_0302 = s_24 + IT_0301;
    const ccomplex_t IT_0303 = m_C_2*IT_0302;
    const ccomplex_t IT_0304 = 2*IT_0303;
    const ccomplex_t IT_0305 = 2*IT_0304;
    const ccomplex_t IT_0306 = s_12*IT_0298;
    const ccomplex_t IT_0307 = s_14*s_24;
    const ccomplex_t IT_0308 = (-2)*IT_0307;
    const ccomplex_t IT_0309 = IT_0306 + IT_0308;
    const ccomplex_t IT_0310 = IT_0189*IT_0309;
    const ccomplex_t IT_0311 = (-0.5)*IT_0310;
    const ccomplex_t IT_0312 = s_12 + IT_0311;
    const ccomplex_t IT_0313 = 4*IT_0312;
    const ccomplex_t IT_0314 = IT_0122*IT_0245;
    const ccomplex_t IT_0315 = IT_0044*IT_0045*IT_0314;
    const ccomplex_t IT_0316 = IT_0144*IT_0277;
    const ccomplex_t IT_0317 = IT_0092*IT_0093*IT_0316;
    const ccomplex_t IT_0318 = IT_0039*IT_0220;
    const ccomplex_t IT_0319 = IT_0000*IT_0031*IT_0318;
    const ccomplex_t IT_0320 = IT_0133*IT_0261;
    const ccomplex_t IT_0321 = IT_0068*IT_0069*IT_0320;
    const ccomplex_t IT_0322 = m_C_2*IT_0223;
    const ccomplex_t IT_0323 = m_C_2*IT_0248;
    const ccomplex_t IT_0324 = m_C_2*IT_0264;
    const ccomplex_t IT_0325 = m_C_2*IT_0280;
    const ccomplex_t IT_0326 = IT_0315 + IT_0317 + IT_0319 + IT_0321 + 
      -IT_0322 + -IT_0323 + -IT_0324 + -IT_0325;
    const ccomplex_t IT_0327 = 6*IT_0030;
    const ccomplex_t IT_0328 = -IT_0162 + -IT_0175 + -IT_0187;
    const ccomplex_t IT_0329 = pow(s_14, 2);
    const ccomplex_t IT_0330 = IT_0189*IT_0329;
    const ccomplex_t IT_0331 = -IT_0030;
    const ccomplex_t IT_0332 = IT_0330 + IT_0331;
    const ccomplex_t IT_0333 = m_C_2*IT_0332;
    const ccomplex_t IT_0334 = (-2)*IT_0333;
    const ccomplex_t IT_0335 = -IT_0205 + -IT_0232 + -IT_0288;
    const ccomplex_t IT_0336 = (-2)*IT_0193;
    const ccomplex_t IT_0337 = -IT_0042 + -IT_0125 + -IT_0136 + -IT_0147;
    const ccomplex_t IT_0338 = s_14*IT_0298;
    const ccomplex_t IT_0339 = IT_0189*IT_0338;
    const ccomplex_t IT_0340 = (-0.25)*IT_0339;
    const ccomplex_t IT_0341 = s_14 + IT_0340;
    const ccomplex_t IT_0342 = m_C_2*IT_0341;
    const ccomplex_t IT_0343 = (-4)*IT_0342;
    const ccomplex_t IT_0344 = (-2)*IT_0343;
    const ccomplex_t IT_0345 = s_12*IT_0293;
    const ccomplex_t IT_0346 = -IT_0345;
    const ccomplex_t IT_0347 = s_12*IT_0192;
    const ccomplex_t IT_0348 = (-2)*IT_0347;
    const ccomplex_t IT_0349 = 0.5*conj(IT_0328);
    const ccomplex_t IT_0350 = IT_0030*IT_0192;
    const ccomplex_t IT_0351 = 2*IT_0350;
    const ccomplex_t IT_0352 = 0.5*conj(IT_0335);
    const ccomplex_t IT_0353 = IT_0030*IT_0293;
    const ccomplex_t IT_0354 = -IT_0353;
    const ccomplex_t IT_0355 = (-2)*IT_0354;
    const ccomplex_t IT_0356 = 0.5*conj(IT_0289);
    const ccomplex_t IT_0357 = 0.5*conj(IT_0149);
    const ccomplex_t IT_0358 = 0.5*conj(IT_0326);
    const ccomplex_t IT_0359 = s_12*s_24;
    const ccomplex_t IT_0360 = s_14*IT_0030;
    const ccomplex_t IT_0361 = -IT_0360;
    const ccomplex_t IT_0362 = IT_0359 + IT_0361;
    const ccomplex_t IT_0363 = 2*IT_0362;
    const ccomplex_t IT_0364 = 0.5*conj(IT_0337);
    const ccomplex_t IT_0365 = s_12*IT_0332;
    const ccomplex_t IT_0366 = 0.5*conj(IT_0188);
    const ccomplex_t IT_0367 = IT_0030*IT_0332;
    const ccomplex_t IT_0368 = (-2)*IT_0367;
    const ccomplex_t IT_0369 = s_12*s_14;
    const ccomplex_t IT_0370 = s_24*IT_0030;
    const ccomplex_t IT_0371 = -IT_0370;
    const ccomplex_t IT_0372 = IT_0369 + IT_0371;
    const ccomplex_t IT_0373 = (-2)*IT_0372;
    const ccomplex_t IT_0374 = pow(m_W, 4);
    const ccomplex_t IT_0375 = s_12*IT_0374;
    const ccomplex_t IT_0376 = IT_0189*IT_0375;
    const ccomplex_t IT_0377 = (-2)*IT_0309;
    const ccomplex_t IT_0378 = IT_0030*IT_0298;
    const ccomplex_t IT_0379 = (-3)*IT_0378;
    const ccomplex_t IT_0380 = (-2)*IT_0379;
    const ccomplex_t IT_0381 = 0.5*conj(IT_0297);
    const ccomplex_t IT_0382 = IT_0326*(IT_0194*conj(IT_0289) + conj(IT_0188)
      *IT_0296 + IT_0313*conj(IT_0326) + conj(IT_0149)*IT_0327 + IT_0334*conj
      (IT_0335) + conj(IT_0328)*IT_0336 + IT_0305*conj(IT_0337) + conj(IT_0297)
      *IT_0344) + IT_0149*(conj(IT_0188)*IT_0194 + conj(IT_0289)*IT_0296 + conj
      (IT_0297)*IT_0305 + conj(IT_0149)*IT_0313 + conj(IT_0326)*IT_0327 + conj
      (IT_0328)*IT_0334 + conj(IT_0335)*IT_0336 + conj(IT_0337)*IT_0344) + 2
      *IT_0188*(conj(IT_0188)*IT_0346 + IT_0348*IT_0349 + IT_0351*IT_0352 +
       IT_0355*IT_0356 + IT_0194*IT_0357 + IT_0296*IT_0358 + IT_0363*IT_0364) +
       2*IT_0328*(IT_0351*IT_0356 + IT_0334*IT_0357 + IT_0336*IT_0358 + conj
      (IT_0328)*IT_0365 + IT_0348*IT_0366 + IT_0352*IT_0368 + IT_0364*IT_0373) +
       2*IT_0297*(IT_0305*IT_0357 + IT_0344*IT_0358 + IT_0356*IT_0363 + IT_0352
      *IT_0373 + conj(IT_0297)*(IT_0376 + IT_0377) + IT_0364*IT_0380) + 2
      *IT_0289*(conj(IT_0289)*IT_0346 + IT_0349*IT_0351 + IT_0348*IT_0352 +
       IT_0296*IT_0357 + IT_0194*IT_0358 + IT_0355*IT_0366 + IT_0363*IT_0381) +
       2*IT_0335*(IT_0348*IT_0356 + IT_0336*IT_0357 + IT_0334*IT_0358 + conj
      (IT_0335)*IT_0365 + IT_0351*IT_0366 + IT_0349*IT_0368 + IT_0373*IT_0381) +
       2*IT_0337*(IT_0344*IT_0357 + IT_0305*IT_0358 + IT_0363*IT_0366 + IT_0349
      *IT_0373 + conj(IT_0337)*(IT_0376 + IT_0377) + IT_0380*IT_0381);
    return create_ccomplex_return(IT_0382);
}

