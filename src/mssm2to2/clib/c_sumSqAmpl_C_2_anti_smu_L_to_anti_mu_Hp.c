#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_anti_smu_L_to_anti_mu_Hp.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_anti_smu_L_to_anti_mu_Hp(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_mu = param->m_mu;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_smu_L = param->m_smu_L;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_snu_mu = param->m_snu_mu;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_numl = param->Gamma_numl;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0001 = cos(theta_W);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = N_B1*e_em;
    const ccomplex_t IT_0004 = IT_0002*IT_0003;
    const ccomplex_t IT_0005 = 1.4142135623731*IT_0004;
    const ccomplex_t IT_0006 = sin(theta_W);
    const ccomplex_t IT_0007 = cpow(IT_0006, -1);
    const ccomplex_t IT_0008 = N_W1*e_em;
    const ccomplex_t IT_0009 = IT_0007*IT_0008;
    const ccomplex_t IT_0010 = 1.4142135623731*IT_0009;
    const ccomplex_t IT_0011 = (0 + _Complex_I*1)*(IT_0005 + IT_0010);
    const ccomplex_t IT_0012 = (-0.5)*IT_0011;
    const ccomplex_t IT_0013 = cos(beta);
    const ccomplex_t IT_0014 = IT_0007*IT_0013;
    const ccomplex_t IT_0015 = N_u1*e_em*V_Wp2;
    const ccomplex_t IT_0016 = IT_0014*IT_0015;
    const ccomplex_t IT_0017 = 2*IT_0016;
    const ccomplex_t IT_0018 = IT_0002*IT_0013;
    const ccomplex_t IT_0019 = N_B1*V_u2*e_em;
    const ccomplex_t IT_0020 = IT_0018*IT_0019;
    const ccomplex_t IT_0021 = 1.4142135623731*IT_0020;
    const ccomplex_t IT_0022 = N_W1*V_u2*e_em;
    const ccomplex_t IT_0023 = IT_0014*IT_0022;
    const ccomplex_t IT_0024 = 1.4142135623731*IT_0023;
    const ccomplex_t IT_0025 = IT_0021 + IT_0024;
    const ccomplex_t IT_0026 = IT_0017 + IT_0025;
    const ccomplex_t IT_0027 = (0 + _Complex_I*1)*IT_0026;
    const ccomplex_t IT_0028 = 0.5*IT_0027;
    const ccomplex_t IT_0029 = IT_0012*IT_0028;
    const ccomplex_t IT_0030 = pow(m_mu, 2);
    const ccomplex_t IT_0031 = pow(m_smu_L, 2);
    const ccomplex_t IT_0032 = cpow((-2)*s_23 + IT_0030 + IT_0031 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0033 = IT_0000*IT_0029*IT_0032;
    const ccomplex_t IT_0034 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0035 = N_B3*e_em;
    const ccomplex_t IT_0036 = IT_0002*IT_0035;
    const ccomplex_t IT_0037 = 1.4142135623731*IT_0036;
    const ccomplex_t IT_0038 = N_W3*e_em;
    const ccomplex_t IT_0039 = IT_0007*IT_0038;
    const ccomplex_t IT_0040 = 1.4142135623731*IT_0039;
    const ccomplex_t IT_0041 = (0 + _Complex_I*1)*(IT_0037 + IT_0040);
    const ccomplex_t IT_0042 = (-0.5)*IT_0041;
    const ccomplex_t IT_0043 = N_u3*e_em*V_Wp2;
    const ccomplex_t IT_0044 = IT_0014*IT_0043;
    const ccomplex_t IT_0045 = 2*IT_0044;
    const ccomplex_t IT_0046 = N_B3*V_u2*e_em;
    const ccomplex_t IT_0047 = IT_0018*IT_0046;
    const ccomplex_t IT_0048 = 1.4142135623731*IT_0047;
    const ccomplex_t IT_0049 = N_W3*V_u2*e_em;
    const ccomplex_t IT_0050 = IT_0014*IT_0049;
    const ccomplex_t IT_0051 = 1.4142135623731*IT_0050;
    const ccomplex_t IT_0052 = IT_0048 + IT_0051;
    const ccomplex_t IT_0053 = IT_0045 + IT_0052;
    const ccomplex_t IT_0054 = (0 + _Complex_I*1)*IT_0053;
    const ccomplex_t IT_0055 = 0.5*IT_0054;
    const ccomplex_t IT_0056 = IT_0042*IT_0055;
    const ccomplex_t IT_0057 = cpow((-2)*s_23 + IT_0030 + IT_0031 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0058 = IT_0034*IT_0056*IT_0057;
    const ccomplex_t IT_0059 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0060 = cpow((-2)*s_23 + IT_0030 + IT_0031 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0061 = N_B4*e_em;
    const ccomplex_t IT_0062 = IT_0002*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = N_W4*e_em;
    const ccomplex_t IT_0065 = IT_0007*IT_0064;
    const ccomplex_t IT_0066 = 1.4142135623731*IT_0065;
    const ccomplex_t IT_0067 = (0 + _Complex_I*1)*(IT_0063 + IT_0066);
    const ccomplex_t IT_0068 = (-0.5)*IT_0067;
    const ccomplex_t IT_0069 = N_u4*e_em*V_Wp2;
    const ccomplex_t IT_0070 = IT_0014*IT_0069;
    const ccomplex_t IT_0071 = 2*IT_0070;
    const ccomplex_t IT_0072 = N_B4*V_u2*e_em;
    const ccomplex_t IT_0073 = IT_0018*IT_0072;
    const ccomplex_t IT_0074 = 1.4142135623731*IT_0073;
    const ccomplex_t IT_0075 = N_W4*V_u2*e_em;
    const ccomplex_t IT_0076 = IT_0014*IT_0075;
    const ccomplex_t IT_0077 = 1.4142135623731*IT_0076;
    const ccomplex_t IT_0078 = IT_0074 + IT_0077;
    const ccomplex_t IT_0079 = IT_0071 + IT_0078;
    const ccomplex_t IT_0080 = (0 + _Complex_I*1)*IT_0079;
    const ccomplex_t IT_0081 = 0.5*IT_0080;
    const ccomplex_t IT_0082 = IT_0068*IT_0081;
    const ccomplex_t IT_0083 = IT_0059*IT_0060*IT_0082;
    const ccomplex_t IT_0084 = pow(m_W, -1);
    const ccomplex_t IT_0085 = cpow(IT_0013, -1);
    const ccomplex_t IT_0086 = (0 + _Complex_I*1.4142135623731)*conj(N_d1)
      *e_em*m_mu*IT_0007*IT_0084*IT_0085;
    const ccomplex_t IT_0087 = (-0.5)*IT_0086;
    const ccomplex_t IT_0088 = IT_0028*IT_0087;
    const ccomplex_t IT_0089 = IT_0032*IT_0088;
    const ccomplex_t IT_0090 = (0 + _Complex_I*1)*IT_0089;
    const ccomplex_t IT_0091 = m_mu*IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1.4142135623731)*conj(N_d4)
      *e_em*m_mu*IT_0007*IT_0084*IT_0085;
    const ccomplex_t IT_0093 = (-0.5)*IT_0092;
    const ccomplex_t IT_0094 = IT_0081*IT_0093;
    const ccomplex_t IT_0095 = IT_0060*IT_0094;
    const ccomplex_t IT_0096 = (0 + _Complex_I*1)*IT_0095;
    const ccomplex_t IT_0097 = m_mu*IT_0096;
    const ccomplex_t IT_0098 = sin(beta);
    const ccomplex_t IT_0099 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0007*
      (m_W*IT_0013 + (-0.5)*IT_0030*IT_0084*IT_0085)*IT_0098;
    const ccomplex_t IT_0100 = -IT_0099;
    const ccomplex_t IT_0101 = (0 + _Complex_I*1)*e_em*V_Wp2*IT_0007;
    const ccomplex_t IT_0102 = -IT_0101;
    const ccomplex_t IT_0103 = IT_0100*IT_0102;
    const ccomplex_t IT_0104 = pow(m_C_2, 2);
    const ccomplex_t IT_0105 = cpow((-2)*s_13 + IT_0030 + IT_0104 + reg_prop +
       -m_snu_mu*(m_snu_mu + (0 + _Complex_I*-1)*Gamma_numl), -1);
    const ccomplex_t IT_0106 = IT_0103*IT_0105;
    const ccomplex_t IT_0107 = (0 + _Complex_I*1)*IT_0106;
    const ccomplex_t IT_0108 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0109 = cpow((-2)*s_23 + IT_0030 + IT_0031 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0110 = N_B2*e_em;
    const ccomplex_t IT_0111 = IT_0002*IT_0110;
    const ccomplex_t IT_0112 = 1.4142135623731*IT_0111;
    const ccomplex_t IT_0113 = N_W2*e_em;
    const ccomplex_t IT_0114 = IT_0007*IT_0113;
    const ccomplex_t IT_0115 = 1.4142135623731*IT_0114;
    const ccomplex_t IT_0116 = (0 + _Complex_I*1)*(IT_0112 + IT_0115);
    const ccomplex_t IT_0117 = (-0.5)*IT_0116;
    const ccomplex_t IT_0118 = N_u2*e_em*V_Wp2;
    const ccomplex_t IT_0119 = IT_0014*IT_0118;
    const ccomplex_t IT_0120 = 2*IT_0119;
    const ccomplex_t IT_0121 = N_B2*V_u2*e_em;
    const ccomplex_t IT_0122 = IT_0018*IT_0121;
    const ccomplex_t IT_0123 = 1.4142135623731*IT_0122;
    const ccomplex_t IT_0124 = N_W2*V_u2*e_em;
    const ccomplex_t IT_0125 = IT_0014*IT_0124;
    const ccomplex_t IT_0126 = 1.4142135623731*IT_0125;
    const ccomplex_t IT_0127 = IT_0123 + IT_0126;
    const ccomplex_t IT_0128 = IT_0120 + IT_0127;
    const ccomplex_t IT_0129 = (0 + _Complex_I*1)*IT_0128;
    const ccomplex_t IT_0130 = 0.5*IT_0129;
    const ccomplex_t IT_0131 = IT_0117*IT_0130;
    const ccomplex_t IT_0132 = IT_0108*IT_0109*IT_0131;
    const ccomplex_t IT_0133 = (0 + _Complex_I*1.4142135623731)*conj(N_d2)
      *e_em*m_mu*IT_0007*IT_0084*IT_0085;
    const ccomplex_t IT_0134 = (-0.5)*IT_0133;
    const ccomplex_t IT_0135 = IT_0130*IT_0134;
    const ccomplex_t IT_0136 = IT_0109*IT_0135;
    const ccomplex_t IT_0137 = (0 + _Complex_I*1)*IT_0136;
    const ccomplex_t IT_0138 = m_mu*IT_0137;
    const ccomplex_t IT_0139 = (0 + _Complex_I*1.4142135623731)*conj(N_d3)
      *e_em*m_mu*IT_0007*IT_0084*IT_0085;
    const ccomplex_t IT_0140 = (-0.5)*IT_0139;
    const ccomplex_t IT_0141 = IT_0055*IT_0140;
    const ccomplex_t IT_0142 = IT_0057*IT_0141;
    const ccomplex_t IT_0143 = (0 + _Complex_I*1)*IT_0142;
    const ccomplex_t IT_0144 = m_mu*IT_0143;
    const ccomplex_t IT_0145 = IT_0033 + IT_0058 + IT_0083 + -IT_0091 + 
      -IT_0097 + IT_0107 + IT_0132 + -IT_0138 + -IT_0144;
    const ccomplex_t IT_0146 = (0 + _Complex_I*1.4142135623731)*conj(U_d2)
      *e_em*m_mu*IT_0007*IT_0084*IT_0085;
    const ccomplex_t IT_0147 = (-0.5)*IT_0146;
    const ccomplex_t IT_0148 = IT_0100*IT_0147;
    const ccomplex_t IT_0149 = IT_0105*IT_0148;
    const ccomplex_t IT_0150 = (0 + _Complex_I*1)*IT_0149;
    const ccomplex_t IT_0151 = IT_0007*IT_0098;
    const ccomplex_t IT_0152 = conj(N_d2)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0153 = IT_0151*IT_0152;
    const ccomplex_t IT_0154 = (-2)*IT_0153;
    const ccomplex_t IT_0155 = IT_0002*IT_0098;
    const ccomplex_t IT_0156 = conj(N_B2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0157 = IT_0155*IT_0156;
    const ccomplex_t IT_0158 = 1.4142135623731*IT_0157;
    const ccomplex_t IT_0159 = conj(N_W2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0160 = IT_0151*IT_0159;
    const ccomplex_t IT_0161 = 1.4142135623731*IT_0160;
    const ccomplex_t IT_0162 = IT_0158 + IT_0161;
    const ccomplex_t IT_0163 = IT_0154 + IT_0162;
    const ccomplex_t IT_0164 = (0 + _Complex_I*1)*IT_0163;
    const ccomplex_t IT_0165 = 0.5*IT_0164;
    const ccomplex_t IT_0166 = IT_0134*IT_0165;
    const ccomplex_t IT_0167 = IT_0108*IT_0109*IT_0166;
    const ccomplex_t IT_0168 = IT_0117*IT_0165;
    const ccomplex_t IT_0169 = IT_0109*IT_0168;
    const ccomplex_t IT_0170 = (0 + _Complex_I*1)*IT_0169;
    const ccomplex_t IT_0171 = m_mu*IT_0170;
    const ccomplex_t IT_0172 = conj(N_W1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0173 = IT_0151*IT_0172;
    const ccomplex_t IT_0174 = 1.4142135623731*IT_0173;
    const ccomplex_t IT_0175 = conj(N_B1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0176 = IT_0155*IT_0175;
    const ccomplex_t IT_0177 = 1.4142135623731*IT_0176;
    const ccomplex_t IT_0178 = IT_0174 + IT_0177;
    const ccomplex_t IT_0179 = conj(N_d1)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0180 = IT_0151*IT_0179;
    const ccomplex_t IT_0181 = (-2)*IT_0180;
    const ccomplex_t IT_0182 = IT_0178 + IT_0181;
    const ccomplex_t IT_0183 = (0 + _Complex_I*1)*IT_0182;
    const ccomplex_t IT_0184 = 0.5*IT_0183;
    const ccomplex_t IT_0185 = IT_0087*IT_0184;
    const ccomplex_t IT_0186 = IT_0000*IT_0032*IT_0185;
    const ccomplex_t IT_0187 = conj(N_d3)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0188 = IT_0151*IT_0187;
    const ccomplex_t IT_0189 = (-2)*IT_0188;
    const ccomplex_t IT_0190 = conj(N_B3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0191 = IT_0155*IT_0190;
    const ccomplex_t IT_0192 = 1.4142135623731*IT_0191;
    const ccomplex_t IT_0193 = conj(N_W3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0194 = IT_0151*IT_0193;
    const ccomplex_t IT_0195 = 1.4142135623731*IT_0194;
    const ccomplex_t IT_0196 = IT_0192 + IT_0195;
    const ccomplex_t IT_0197 = IT_0189 + IT_0196;
    const ccomplex_t IT_0198 = (0 + _Complex_I*1)*IT_0197;
    const ccomplex_t IT_0199 = 0.5*IT_0198;
    const ccomplex_t IT_0200 = IT_0140*IT_0199;
    const ccomplex_t IT_0201 = IT_0034*IT_0057*IT_0200;
    const ccomplex_t IT_0202 = conj(N_d4)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0203 = IT_0151*IT_0202;
    const ccomplex_t IT_0204 = (-2)*IT_0203;
    const ccomplex_t IT_0205 = conj(N_B4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0206 = IT_0155*IT_0205;
    const ccomplex_t IT_0207 = 1.4142135623731*IT_0206;
    const ccomplex_t IT_0208 = conj(N_W4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0209 = IT_0151*IT_0208;
    const ccomplex_t IT_0210 = 1.4142135623731*IT_0209;
    const ccomplex_t IT_0211 = IT_0207 + IT_0210;
    const ccomplex_t IT_0212 = IT_0204 + IT_0211;
    const ccomplex_t IT_0213 = (0 + _Complex_I*1)*IT_0212;
    const ccomplex_t IT_0214 = 0.5*IT_0213;
    const ccomplex_t IT_0215 = IT_0093*IT_0214;
    const ccomplex_t IT_0216 = IT_0059*IT_0060*IT_0215;
    const ccomplex_t IT_0217 = IT_0012*IT_0184;
    const ccomplex_t IT_0218 = IT_0032*IT_0217;
    const ccomplex_t IT_0219 = (0 + _Complex_I*1)*IT_0218;
    const ccomplex_t IT_0220 = m_mu*IT_0219;
    const ccomplex_t IT_0221 = IT_0042*IT_0199;
    const ccomplex_t IT_0222 = IT_0057*IT_0221;
    const ccomplex_t IT_0223 = (0 + _Complex_I*1)*IT_0222;
    const ccomplex_t IT_0224 = m_mu*IT_0223;
    const ccomplex_t IT_0225 = IT_0068*IT_0214;
    const ccomplex_t IT_0226 = IT_0060*IT_0225;
    const ccomplex_t IT_0227 = (0 + _Complex_I*1)*IT_0226;
    const ccomplex_t IT_0228 = m_mu*IT_0227;
    const ccomplex_t IT_0229 = -IT_0150 + IT_0167 + -IT_0171 + IT_0186 +
       IT_0201 + IT_0216 + -IT_0220 + -IT_0224 + -IT_0228;
    const ccomplex_t IT_0230 = m_mu*m_C_2;
    const ccomplex_t IT_0231 = -IT_0170 + -IT_0219 + -IT_0223 + -IT_0227;
    const ccomplex_t IT_0232 = s_23*m_C_2;
    const ccomplex_t IT_0233 = -IT_0090 + -IT_0096 + -IT_0137 + -IT_0143;
    const ccomplex_t IT_0234 = m_mu*s_12;
    const ccomplex_t IT_0235 = 2*s_13;
    const ccomplex_t IT_0236 = (-2)*IT_0229;
    const ccomplex_t IT_0237 = m_mu*m_C_2*IT_0031;
    const ccomplex_t IT_0238 = s_12*s_23;
    const ccomplex_t IT_0239 = s_13*IT_0031;
    const ccomplex_t IT_0240 = (-0.5)*IT_0239;
    const ccomplex_t IT_0241 = IT_0238 + IT_0240;
    const ccomplex_t IT_0242 = (-2)*conj(IT_0229);
    const ccomplex_t IT_0243 = 0.5*IT_0232;
    const ccomplex_t IT_0244 = (-2)*conj(IT_0145);
    const ccomplex_t IT_0245 = 0.5*IT_0234;
    const ccomplex_t IT_0246 = 2*IT_0145*(conj(IT_0229)*IT_0230 + -conj
      (IT_0231)*IT_0232 + -conj(IT_0233)*IT_0234 + 0.5*conj(IT_0145)*IT_0235) +
       2*IT_0229*(conj(IT_0145)*IT_0230 + 0.5*conj(IT_0229)*IT_0235) + (IT_0232
      *conj(IT_0233) + conj(IT_0231)*IT_0234)*IT_0236 + 2*IT_0231*(conj(IT_0233)
      *IT_0237 + 2*conj(IT_0231)*IT_0241 + IT_0243*IT_0244 + IT_0242*IT_0245) +
       2*IT_0233*(conj(IT_0231)*IT_0237 + 2*conj(IT_0233)*IT_0241 + IT_0242
      *IT_0243 + IT_0244*IT_0245);
    return create_ccomplex_return(IT_0246);
}

