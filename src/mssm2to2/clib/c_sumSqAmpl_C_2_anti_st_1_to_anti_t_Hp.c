#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_anti_st_1_to_anti_t_Hp.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_anti_st_1_to_anti_t_Hp(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_b = param->m_b;
    const creal_t m_d = param->m_d;
    const creal_t m_s = param->m_s;
    const creal_t m_t = param->m_t;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_st_1 = param->m_st_1;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_tb = param->V_tb;
    const ccomplex_t V_td = param->V_td;
    const ccomplex_t V_ts = param->V_ts;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t U_st_00 = param->U_st_00;
    const ccomplex_t U_st_10 = param->U_st_10;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0001 = cos(beta);
    const ccomplex_t IT_0002 = sin(theta_W);
    const ccomplex_t IT_0003 = cpow(IT_0002, -1);
    const ccomplex_t IT_0004 = IT_0001*IT_0003;
    const ccomplex_t IT_0005 = N_u1*e_em*V_Wp2;
    const ccomplex_t IT_0006 = IT_0004*IT_0005;
    const ccomplex_t IT_0007 = 2*IT_0006;
    const ccomplex_t IT_0008 = cos(theta_W);
    const ccomplex_t IT_0009 = cpow(IT_0008, -1);
    const ccomplex_t IT_0010 = IT_0001*IT_0009;
    const ccomplex_t IT_0011 = N_B1*V_u2*e_em;
    const ccomplex_t IT_0012 = IT_0010*IT_0011;
    const ccomplex_t IT_0013 = 1.4142135623731*IT_0012;
    const ccomplex_t IT_0014 = N_W1*V_u2*e_em;
    const ccomplex_t IT_0015 = IT_0004*IT_0014;
    const ccomplex_t IT_0016 = 1.4142135623731*IT_0015;
    const ccomplex_t IT_0017 = IT_0013 + IT_0016;
    const ccomplex_t IT_0018 = IT_0007 + IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1)*IT_0018;
    const ccomplex_t IT_0020 = 0.5*IT_0019;
    const ccomplex_t IT_0021 = N_B1*e_em*conj(U_st_00);
    const ccomplex_t IT_0022 = IT_0009*IT_0021;
    const ccomplex_t IT_0023 = 1.4142135623731*IT_0022;
    const ccomplex_t IT_0024 = N_W1*e_em*conj(U_st_00);
    const ccomplex_t IT_0025 = IT_0003*IT_0024;
    const ccomplex_t IT_0026 = 1.4142135623731*IT_0025;
    const ccomplex_t IT_0027 = sin(beta);
    const ccomplex_t IT_0028 = cpow(IT_0027, -1);
    const ccomplex_t IT_0029 = IT_0003*IT_0028;
    const ccomplex_t IT_0030 = pow(m_W, -1);
    const ccomplex_t IT_0031 = m_t*N_u1*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0032 = IT_0029*IT_0031;
    const ccomplex_t IT_0033 = 1.4142135623731*IT_0032;
    const ccomplex_t IT_0034 = (0 + _Complex_I*1)*(IT_0023 + 3*IT_0026 + 3
      *IT_0033);
    const ccomplex_t IT_0035 = 0.166666666666667*IT_0034;
    const ccomplex_t IT_0036 = IT_0020*IT_0035;
    const ccomplex_t IT_0037 = pow(m_t, 2);
    const ccomplex_t IT_0038 = pow(m_st_1, 2);
    const ccomplex_t IT_0039 = cpow((-2)*s_23 + IT_0037 + IT_0038 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0040 = IT_0000*IT_0036*IT_0039;
    const ccomplex_t IT_0041 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0042 = N_u2*e_em*V_Wp2;
    const ccomplex_t IT_0043 = IT_0004*IT_0042;
    const ccomplex_t IT_0044 = 2*IT_0043;
    const ccomplex_t IT_0045 = N_B2*V_u2*e_em;
    const ccomplex_t IT_0046 = IT_0010*IT_0045;
    const ccomplex_t IT_0047 = 1.4142135623731*IT_0046;
    const ccomplex_t IT_0048 = N_W2*V_u2*e_em;
    const ccomplex_t IT_0049 = IT_0004*IT_0048;
    const ccomplex_t IT_0050 = 1.4142135623731*IT_0049;
    const ccomplex_t IT_0051 = IT_0047 + IT_0050;
    const ccomplex_t IT_0052 = IT_0044 + IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*IT_0052;
    const ccomplex_t IT_0054 = 0.5*IT_0053;
    const ccomplex_t IT_0055 = N_B2*e_em*conj(U_st_00);
    const ccomplex_t IT_0056 = IT_0009*IT_0055;
    const ccomplex_t IT_0057 = 1.4142135623731*IT_0056;
    const ccomplex_t IT_0058 = N_W2*e_em*conj(U_st_00);
    const ccomplex_t IT_0059 = IT_0003*IT_0058;
    const ccomplex_t IT_0060 = 1.4142135623731*IT_0059;
    const ccomplex_t IT_0061 = m_t*N_u2*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0062 = IT_0029*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = (0 + _Complex_I*1)*(IT_0057 + 3*IT_0060 + 3
      *IT_0063);
    const ccomplex_t IT_0065 = 0.166666666666667*IT_0064;
    const ccomplex_t IT_0066 = IT_0054*IT_0065;
    const ccomplex_t IT_0067 = cpow((-2)*s_23 + IT_0037 + IT_0038 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0068 = IT_0041*IT_0066*IT_0067;
    const ccomplex_t IT_0069 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0070 = N_u3*e_em*V_Wp2;
    const ccomplex_t IT_0071 = IT_0004*IT_0070;
    const ccomplex_t IT_0072 = 2*IT_0071;
    const ccomplex_t IT_0073 = N_B3*V_u2*e_em;
    const ccomplex_t IT_0074 = IT_0010*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = N_W3*V_u2*e_em;
    const ccomplex_t IT_0077 = IT_0004*IT_0076;
    const ccomplex_t IT_0078 = 1.4142135623731*IT_0077;
    const ccomplex_t IT_0079 = IT_0075 + IT_0078;
    const ccomplex_t IT_0080 = IT_0072 + IT_0079;
    const ccomplex_t IT_0081 = (0 + _Complex_I*1)*IT_0080;
    const ccomplex_t IT_0082 = 0.5*IT_0081;
    const ccomplex_t IT_0083 = N_B3*e_em*conj(U_st_00);
    const ccomplex_t IT_0084 = IT_0009*IT_0083;
    const ccomplex_t IT_0085 = 1.4142135623731*IT_0084;
    const ccomplex_t IT_0086 = N_W3*e_em*conj(U_st_00);
    const ccomplex_t IT_0087 = IT_0003*IT_0086;
    const ccomplex_t IT_0088 = 1.4142135623731*IT_0087;
    const ccomplex_t IT_0089 = m_t*N_u3*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0090 = IT_0029*IT_0089;
    const ccomplex_t IT_0091 = 1.4142135623731*IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1)*(IT_0085 + 3*IT_0088 + 3
      *IT_0091);
    const ccomplex_t IT_0093 = 0.166666666666667*IT_0092;
    const ccomplex_t IT_0094 = IT_0082*IT_0093;
    const ccomplex_t IT_0095 = cpow((-2)*s_23 + IT_0037 + IT_0038 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0096 = IT_0069*IT_0094*IT_0095;
    const ccomplex_t IT_0097 = conj(N_B1)*e_em*conj(U_st_10);
    const ccomplex_t IT_0098 = IT_0009*IT_0097;
    const ccomplex_t IT_0099 = 1.4142135623731*IT_0098;
    const ccomplex_t IT_0100 = m_t*conj(N_u1)*e_em*IT_0030*conj(U_st_00);
    const ccomplex_t IT_0101 = IT_0029*IT_0100;
    const ccomplex_t IT_0102 = 1.4142135623731*IT_0101;
    const ccomplex_t IT_0103 = (0 + _Complex_I*1)*(IT_0099 + (-0.75)*IT_0102);
    const ccomplex_t IT_0104 = 0.666666666666667*IT_0103;
    const ccomplex_t IT_0105 = IT_0020*IT_0104;
    const ccomplex_t IT_0106 = IT_0039*IT_0105;
    const ccomplex_t IT_0107 = (0 + _Complex_I*1)*IT_0106;
    const ccomplex_t IT_0108 = m_t*IT_0107;
    const ccomplex_t IT_0109 = conj(N_B2)*e_em*conj(U_st_10);
    const ccomplex_t IT_0110 = IT_0009*IT_0109;
    const ccomplex_t IT_0111 = 1.4142135623731*IT_0110;
    const ccomplex_t IT_0112 = m_t*conj(N_u2)*e_em*IT_0030*conj(U_st_00);
    const ccomplex_t IT_0113 = IT_0029*IT_0112;
    const ccomplex_t IT_0114 = 1.4142135623731*IT_0113;
    const ccomplex_t IT_0115 = (0 + _Complex_I*1)*(IT_0111 + (-0.75)*IT_0114);
    const ccomplex_t IT_0116 = 0.666666666666667*IT_0115;
    const ccomplex_t IT_0117 = IT_0054*IT_0116;
    const ccomplex_t IT_0118 = IT_0067*IT_0117;
    const ccomplex_t IT_0119 = (0 + _Complex_I*1)*IT_0118;
    const ccomplex_t IT_0120 = m_t*IT_0119;
    const ccomplex_t IT_0121 = conj(N_B3)*e_em*conj(U_st_10);
    const ccomplex_t IT_0122 = IT_0009*IT_0121;
    const ccomplex_t IT_0123 = 1.4142135623731*IT_0122;
    const ccomplex_t IT_0124 = m_t*conj(N_u3)*e_em*IT_0030*conj(U_st_00);
    const ccomplex_t IT_0125 = IT_0029*IT_0124;
    const ccomplex_t IT_0126 = 1.4142135623731*IT_0125;
    const ccomplex_t IT_0127 = (0 + _Complex_I*1)*(IT_0123 + (-0.75)*IT_0126);
    const ccomplex_t IT_0128 = 0.666666666666667*IT_0127;
    const ccomplex_t IT_0129 = IT_0082*IT_0128;
    const ccomplex_t IT_0130 = IT_0095*IT_0129;
    const ccomplex_t IT_0131 = (0 + _Complex_I*1)*IT_0130;
    const ccomplex_t IT_0132 = m_t*IT_0131;
    const ccomplex_t IT_0133 = pow(m_b, 2);
    const ccomplex_t IT_0134 = pow(m_C_2, 2);
    const ccomplex_t IT_0135 = cpow((-2)*s_12 + -IT_0038 + IT_0133 + -IT_0134 
      + -reg_prop, -1);
    const ccomplex_t IT_0136 = cpow(IT_0001, -1);
    const ccomplex_t IT_0137 = (0 + _Complex_I*1.4142135623731)*m_b*conj(U_d2)
      *V_tb*e_em*IT_0003*IT_0030*IT_0136*conj(U_st_00);
    const ccomplex_t IT_0138 = (-0.5)*IT_0137;
    const ccomplex_t IT_0139 = (0 + _Complex_I*1.4142135623731)*m_b*V_tb*e_em
      *IT_0003*IT_0027*IT_0030*IT_0136;
    const ccomplex_t IT_0140 = 0.5*IT_0139;
    const ccomplex_t IT_0141 = IT_0138*IT_0140;
    const ccomplex_t IT_0142 = IT_0135*IT_0141;
    const ccomplex_t IT_0143 = (0 + _Complex_I*1)*IT_0142;
    const ccomplex_t IT_0144 = m_C_2*IT_0143;
    const ccomplex_t IT_0145 = pow(m_d, 2);
    const ccomplex_t IT_0146 = cpow((-2)*s_12 + -IT_0038 + -IT_0134 + IT_0145 
      + -reg_prop, -1);
    const ccomplex_t IT_0147 = (0 + _Complex_I*1.4142135623731)*m_d*conj(U_d2)
      *V_td*e_em*IT_0003*IT_0030*IT_0136*conj(U_st_00);
    const ccomplex_t IT_0148 = (-0.5)*IT_0147;
    const ccomplex_t IT_0149 = (0 + _Complex_I*1.4142135623731)*m_d*conj(V_td)
      *e_em*IT_0003*IT_0027*IT_0030*IT_0136;
    const ccomplex_t IT_0150 = 0.5*IT_0149;
    const ccomplex_t IT_0151 = IT_0148*IT_0150;
    const ccomplex_t IT_0152 = IT_0146*IT_0151;
    const ccomplex_t IT_0153 = (0 + _Complex_I*1)*IT_0152;
    const ccomplex_t IT_0154 = m_C_2*IT_0153;
    const ccomplex_t IT_0155 = pow(m_s, 2);
    const ccomplex_t IT_0156 = cpow((-2)*s_12 + -IT_0038 + -IT_0134 + IT_0155 
      + -reg_prop, -1);
    const ccomplex_t IT_0157 = (0 + _Complex_I*1.4142135623731)*m_s*conj(U_d2)
      *V_ts*e_em*IT_0003*IT_0030*IT_0136*conj(U_st_00);
    const ccomplex_t IT_0158 = (-0.5)*IT_0157;
    const ccomplex_t IT_0159 = (0 + _Complex_I*1.4142135623731)*m_s*conj(V_ts)
      *e_em*IT_0003*IT_0027*IT_0030*IT_0136;
    const ccomplex_t IT_0160 = 0.5*IT_0159;
    const ccomplex_t IT_0161 = IT_0158*IT_0160;
    const ccomplex_t IT_0162 = IT_0156*IT_0161;
    const ccomplex_t IT_0163 = (0 + _Complex_I*1)*IT_0162;
    const ccomplex_t IT_0164 = m_C_2*IT_0163;
    const ccomplex_t IT_0165 = (0 + _Complex_I*1)*m_b;
    const ccomplex_t IT_0166 = m_t*V_tb*V_u2*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0167 = IT_0029*IT_0166;
    const ccomplex_t IT_0168 = 1.4142135623731*IT_0167;
    const ccomplex_t IT_0169 = (0 + _Complex_I*1)*IT_0168;
    const ccomplex_t IT_0170 = (-0.5)*IT_0169;
    const ccomplex_t IT_0171 = V_tb*e_em*V_Wp2*conj(U_st_00);
    const ccomplex_t IT_0172 = IT_0003*IT_0171;
    const ccomplex_t IT_0173 = (0 + _Complex_I*1)*IT_0172;
    const ccomplex_t IT_0174 = -IT_0173;
    const ccomplex_t IT_0175 = -IT_0174;
    const ccomplex_t IT_0176 = IT_0170 + IT_0175;
    const ccomplex_t IT_0177 = IT_0140*IT_0176;
    const ccomplex_t IT_0178 = IT_0135*IT_0165*IT_0177;
    const ccomplex_t IT_0179 = (0 + _Complex_I*1)*m_d;
    const ccomplex_t IT_0180 = m_t*V_td*V_u2*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0181 = IT_0029*IT_0180;
    const ccomplex_t IT_0182 = 1.4142135623731*IT_0181;
    const ccomplex_t IT_0183 = (0 + _Complex_I*1)*IT_0182;
    const ccomplex_t IT_0184 = (-0.5)*IT_0183;
    const ccomplex_t IT_0185 = V_td*e_em*V_Wp2*conj(U_st_00);
    const ccomplex_t IT_0186 = IT_0003*IT_0185;
    const ccomplex_t IT_0187 = (0 + _Complex_I*1)*IT_0186;
    const ccomplex_t IT_0188 = -IT_0187;
    const ccomplex_t IT_0189 = -IT_0188;
    const ccomplex_t IT_0190 = IT_0184 + IT_0189;
    const ccomplex_t IT_0191 = IT_0150*IT_0190;
    const ccomplex_t IT_0192 = IT_0146*IT_0179*IT_0191;
    const ccomplex_t IT_0193 = (0 + _Complex_I*1)*m_s;
    const ccomplex_t IT_0194 = m_t*V_ts*V_u2*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0195 = IT_0029*IT_0194;
    const ccomplex_t IT_0196 = 1.4142135623731*IT_0195;
    const ccomplex_t IT_0197 = (0 + _Complex_I*1)*IT_0196;
    const ccomplex_t IT_0198 = (-0.5)*IT_0197;
    const ccomplex_t IT_0199 = V_ts*e_em*V_Wp2*conj(U_st_00);
    const ccomplex_t IT_0200 = IT_0003*IT_0199;
    const ccomplex_t IT_0201 = (0 + _Complex_I*1)*IT_0200;
    const ccomplex_t IT_0202 = -IT_0201;
    const ccomplex_t IT_0203 = -IT_0202;
    const ccomplex_t IT_0204 = IT_0198 + IT_0203;
    const ccomplex_t IT_0205 = IT_0160*IT_0204;
    const ccomplex_t IT_0206 = IT_0156*IT_0193*IT_0205;
    const ccomplex_t IT_0207 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0208 = cpow((-2)*s_23 + IT_0037 + IT_0038 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0209 = N_u4*e_em*V_Wp2;
    const ccomplex_t IT_0210 = IT_0004*IT_0209;
    const ccomplex_t IT_0211 = 2*IT_0210;
    const ccomplex_t IT_0212 = N_B4*V_u2*e_em;
    const ccomplex_t IT_0213 = IT_0010*IT_0212;
    const ccomplex_t IT_0214 = 1.4142135623731*IT_0213;
    const ccomplex_t IT_0215 = N_W4*V_u2*e_em;
    const ccomplex_t IT_0216 = IT_0004*IT_0215;
    const ccomplex_t IT_0217 = 1.4142135623731*IT_0216;
    const ccomplex_t IT_0218 = IT_0214 + IT_0217;
    const ccomplex_t IT_0219 = IT_0211 + IT_0218;
    const ccomplex_t IT_0220 = (0 + _Complex_I*1)*IT_0219;
    const ccomplex_t IT_0221 = 0.5*IT_0220;
    const ccomplex_t IT_0222 = N_B4*e_em*conj(U_st_00);
    const ccomplex_t IT_0223 = IT_0009*IT_0222;
    const ccomplex_t IT_0224 = 1.4142135623731*IT_0223;
    const ccomplex_t IT_0225 = N_W4*e_em*conj(U_st_00);
    const ccomplex_t IT_0226 = IT_0003*IT_0225;
    const ccomplex_t IT_0227 = 1.4142135623731*IT_0226;
    const ccomplex_t IT_0228 = m_t*N_u4*e_em*IT_0030*conj(U_st_10);
    const ccomplex_t IT_0229 = IT_0029*IT_0228;
    const ccomplex_t IT_0230 = 1.4142135623731*IT_0229;
    const ccomplex_t IT_0231 = (0 + _Complex_I*1)*(IT_0224 + 3*IT_0227 + 3
      *IT_0230);
    const ccomplex_t IT_0232 = 0.166666666666667*IT_0231;
    const ccomplex_t IT_0233 = IT_0221*IT_0232;
    const ccomplex_t IT_0234 = IT_0207*IT_0208*IT_0233;
    const ccomplex_t IT_0235 = conj(N_B4)*e_em*conj(U_st_10);
    const ccomplex_t IT_0236 = IT_0009*IT_0235;
    const ccomplex_t IT_0237 = 1.4142135623731*IT_0236;
    const ccomplex_t IT_0238 = m_t*conj(N_u4)*e_em*IT_0030*conj(U_st_00);
    const ccomplex_t IT_0239 = IT_0029*IT_0238;
    const ccomplex_t IT_0240 = 1.4142135623731*IT_0239;
    const ccomplex_t IT_0241 = (0 + _Complex_I*1)*(IT_0237 + (-0.75)*IT_0240);
    const ccomplex_t IT_0242 = 0.666666666666667*IT_0241;
    const ccomplex_t IT_0243 = IT_0221*IT_0242;
    const ccomplex_t IT_0244 = IT_0208*IT_0243;
    const ccomplex_t IT_0245 = (0 + _Complex_I*1)*IT_0244;
    const ccomplex_t IT_0246 = m_t*IT_0245;
    const ccomplex_t IT_0247 = IT_0040 + IT_0068 + IT_0096 + -IT_0108 + 
      -IT_0120 + -IT_0132 + IT_0144 + IT_0154 + IT_0164 + IT_0178 + IT_0192 +
       IT_0206 + IT_0234 + -IT_0246;
    const ccomplex_t IT_0248 = (0 + _Complex_I*1.4142135623731)*m_t*conj(V_ts)
      *e_em*IT_0001*IT_0003*IT_0028*IT_0030;
    const ccomplex_t IT_0249 = 0.5*IT_0248;
    const ccomplex_t IT_0250 = IT_0158*IT_0249;
    const ccomplex_t IT_0251 = IT_0156*IT_0193*IT_0250;
    const ccomplex_t IT_0252 = IT_0003*IT_0027;
    const ccomplex_t IT_0253 = conj(N_W1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0254 = IT_0252*IT_0253;
    const ccomplex_t IT_0255 = 1.4142135623731*IT_0254;
    const ccomplex_t IT_0256 = IT_0009*IT_0027;
    const ccomplex_t IT_0257 = conj(N_B1)*conj(U_d2)*e_em;
    const ccomplex_t IT_0258 = IT_0256*IT_0257;
    const ccomplex_t IT_0259 = 1.4142135623731*IT_0258;
    const ccomplex_t IT_0260 = IT_0255 + IT_0259;
    const ccomplex_t IT_0261 = conj(N_d1)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0262 = IT_0252*IT_0261;
    const ccomplex_t IT_0263 = (-2)*IT_0262;
    const ccomplex_t IT_0264 = IT_0260 + IT_0263;
    const ccomplex_t IT_0265 = (0 + _Complex_I*1)*IT_0264;
    const ccomplex_t IT_0266 = 0.5*IT_0265;
    const ccomplex_t IT_0267 = IT_0104*IT_0266;
    const ccomplex_t IT_0268 = IT_0000*IT_0039*IT_0267;
    const ccomplex_t IT_0269 = conj(N_d2)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0270 = IT_0252*IT_0269;
    const ccomplex_t IT_0271 = (-2)*IT_0270;
    const ccomplex_t IT_0272 = conj(N_B2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0273 = IT_0256*IT_0272;
    const ccomplex_t IT_0274 = 1.4142135623731*IT_0273;
    const ccomplex_t IT_0275 = conj(N_W2)*conj(U_d2)*e_em;
    const ccomplex_t IT_0276 = IT_0252*IT_0275;
    const ccomplex_t IT_0277 = 1.4142135623731*IT_0276;
    const ccomplex_t IT_0278 = IT_0274 + IT_0277;
    const ccomplex_t IT_0279 = IT_0271 + IT_0278;
    const ccomplex_t IT_0280 = (0 + _Complex_I*1)*IT_0279;
    const ccomplex_t IT_0281 = 0.5*IT_0280;
    const ccomplex_t IT_0282 = IT_0116*IT_0281;
    const ccomplex_t IT_0283 = IT_0041*IT_0067*IT_0282;
    const ccomplex_t IT_0284 = conj(N_d3)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0285 = IT_0252*IT_0284;
    const ccomplex_t IT_0286 = (-2)*IT_0285;
    const ccomplex_t IT_0287 = conj(N_B3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0288 = IT_0256*IT_0287;
    const ccomplex_t IT_0289 = 1.4142135623731*IT_0288;
    const ccomplex_t IT_0290 = conj(N_W3)*conj(U_d2)*e_em;
    const ccomplex_t IT_0291 = IT_0252*IT_0290;
    const ccomplex_t IT_0292 = 1.4142135623731*IT_0291;
    const ccomplex_t IT_0293 = IT_0289 + IT_0292;
    const ccomplex_t IT_0294 = IT_0286 + IT_0293;
    const ccomplex_t IT_0295 = (0 + _Complex_I*1)*IT_0294;
    const ccomplex_t IT_0296 = 0.5*IT_0295;
    const ccomplex_t IT_0297 = IT_0128*IT_0296;
    const ccomplex_t IT_0298 = IT_0069*IT_0095*IT_0297;
    const ccomplex_t IT_0299 = conj(N_d4)*e_em*conj(U_Wm2);
    const ccomplex_t IT_0300 = IT_0252*IT_0299;
    const ccomplex_t IT_0301 = (-2)*IT_0300;
    const ccomplex_t IT_0302 = conj(N_B4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0303 = IT_0256*IT_0302;
    const ccomplex_t IT_0304 = 1.4142135623731*IT_0303;
    const ccomplex_t IT_0305 = conj(N_W4)*conj(U_d2)*e_em;
    const ccomplex_t IT_0306 = IT_0252*IT_0305;
    const ccomplex_t IT_0307 = 1.4142135623731*IT_0306;
    const ccomplex_t IT_0308 = IT_0304 + IT_0307;
    const ccomplex_t IT_0309 = IT_0301 + IT_0308;
    const ccomplex_t IT_0310 = (0 + _Complex_I*1)*IT_0309;
    const ccomplex_t IT_0311 = 0.5*IT_0310;
    const ccomplex_t IT_0312 = IT_0242*IT_0311;
    const ccomplex_t IT_0313 = IT_0207*IT_0208*IT_0312;
    const ccomplex_t IT_0314 = IT_0035*IT_0266;
    const ccomplex_t IT_0315 = IT_0039*IT_0314;
    const ccomplex_t IT_0316 = (0 + _Complex_I*1)*IT_0315;
    const ccomplex_t IT_0317 = m_t*IT_0316;
    const ccomplex_t IT_0318 = IT_0065*IT_0281;
    const ccomplex_t IT_0319 = IT_0067*IT_0318;
    const ccomplex_t IT_0320 = (0 + _Complex_I*1)*IT_0319;
    const ccomplex_t IT_0321 = m_t*IT_0320;
    const ccomplex_t IT_0322 = IT_0093*IT_0296;
    const ccomplex_t IT_0323 = IT_0095*IT_0322;
    const ccomplex_t IT_0324 = (0 + _Complex_I*1)*IT_0323;
    const ccomplex_t IT_0325 = m_t*IT_0324;
    const ccomplex_t IT_0326 = IT_0232*IT_0311;
    const ccomplex_t IT_0327 = IT_0208*IT_0326;
    const ccomplex_t IT_0328 = (0 + _Complex_I*1)*IT_0327;
    const ccomplex_t IT_0329 = m_t*IT_0328;
    const ccomplex_t IT_0330 = (0 + _Complex_I*1.4142135623731)*m_t*V_tb*e_em
      *IT_0001*IT_0003*IT_0028*IT_0030;
    const ccomplex_t IT_0331 = 0.5*IT_0330;
    const ccomplex_t IT_0332 = IT_0138*IT_0331;
    const ccomplex_t IT_0333 = IT_0135*IT_0165*IT_0332;
    const ccomplex_t IT_0334 = (0 + _Complex_I*1.4142135623731)*m_t*conj(V_td)
      *e_em*IT_0001*IT_0003*IT_0028*IT_0030;
    const ccomplex_t IT_0335 = 0.5*IT_0334;
    const ccomplex_t IT_0336 = IT_0148*IT_0335;
    const ccomplex_t IT_0337 = IT_0146*IT_0179*IT_0336;
    const ccomplex_t IT_0338 = m_C_2*IT_0170;
    const ccomplex_t IT_0339 = m_C_2*IT_0174;
    const ccomplex_t IT_0340 = -IT_0339;
    const ccomplex_t IT_0341 = IT_0338 + IT_0340;
    const ccomplex_t IT_0342 = IT_0135*IT_0331;
    const ccomplex_t IT_0343 = (0 + _Complex_I*1)*IT_0342;
    const ccomplex_t IT_0344 = IT_0341*IT_0343;
    const ccomplex_t IT_0345 = IT_0146*IT_0335;
    const ccomplex_t IT_0346 = (0 + _Complex_I*1)*IT_0345;
    const ccomplex_t IT_0347 = m_C_2*IT_0184;
    const ccomplex_t IT_0348 = m_C_2*IT_0188;
    const ccomplex_t IT_0349 = -IT_0348;
    const ccomplex_t IT_0350 = IT_0347 + IT_0349;
    const ccomplex_t IT_0351 = IT_0346*IT_0350;
    const ccomplex_t IT_0352 = m_C_2*IT_0198;
    const ccomplex_t IT_0353 = m_C_2*IT_0202;
    const ccomplex_t IT_0354 = -IT_0353;
    const ccomplex_t IT_0355 = IT_0352 + IT_0354;
    const ccomplex_t IT_0356 = IT_0156*IT_0249;
    const ccomplex_t IT_0357 = (0 + _Complex_I*1)*IT_0356;
    const ccomplex_t IT_0358 = IT_0355*IT_0357;
    const ccomplex_t IT_0359 = IT_0251 + IT_0268 + IT_0283 + IT_0298 + IT_0313
       + -IT_0317 + -IT_0321 + -IT_0325 + -IT_0329 + IT_0333 + IT_0337 + IT_0344
       + IT_0351 + IT_0358;
    const ccomplex_t IT_0360 = m_t*m_C_2;
    const ccomplex_t IT_0361 = -IT_0143 + -IT_0153 + -IT_0163 + -IT_0316 + 
      -IT_0320 + -IT_0324 + -IT_0328;
    const ccomplex_t IT_0362 = s_23*m_C_2;
    const ccomplex_t IT_0363 = IT_0176*IT_0343;
    const ccomplex_t IT_0364 = IT_0190*IT_0346;
    const ccomplex_t IT_0365 = IT_0204*IT_0357;
    const ccomplex_t IT_0366 = -IT_0107 + -IT_0119 + -IT_0131 + -IT_0245 + 
      -IT_0363 + -IT_0364 + -IT_0365;
    const ccomplex_t IT_0367 = m_t*s_12;
    const ccomplex_t IT_0368 = 6*s_13;
    const ccomplex_t IT_0369 = (-6)*IT_0359;
    const ccomplex_t IT_0370 = m_t*m_C_2*IT_0038;
    const ccomplex_t IT_0371 = s_13*IT_0038;
    const ccomplex_t IT_0372 = (-6)*IT_0371;
    const ccomplex_t IT_0373 = s_12*s_23;
    const ccomplex_t IT_0374 = 12*IT_0373;
    const ccomplex_t IT_0375 = IT_0372 + IT_0374;
    const ccomplex_t IT_0376 = (-6)*conj(IT_0359);
    const ccomplex_t IT_0377 = 0.166666666666667*IT_0362;
    const ccomplex_t IT_0378 = (-6)*conj(IT_0247);
    const ccomplex_t IT_0379 = 0.166666666666667*IT_0367;
    const ccomplex_t IT_0380 = 6*IT_0247*(conj(IT_0359)*IT_0360 + -conj
      (IT_0361)*IT_0362 + -conj(IT_0366)*IT_0367 + 0.166666666666667*conj
      (IT_0247)*IT_0368) + 6*IT_0359*(conj(IT_0247)*IT_0360 + 0.166666666666667
      *conj(IT_0359)*IT_0368) + (IT_0362*conj(IT_0366) + conj(IT_0361)*IT_0367)
      *IT_0369 + 6*IT_0361*(conj(IT_0366)*IT_0370 + 0.166666666666667*conj
      (IT_0361)*IT_0375 + IT_0377*IT_0378 + IT_0376*IT_0379) + 6*IT_0366*(conj
      (IT_0361)*IT_0370 + 0.166666666666667*conj(IT_0366)*IT_0375 + IT_0376
      *IT_0377 + IT_0378*IT_0379);
    return create_ccomplex_return(IT_0380);
}

