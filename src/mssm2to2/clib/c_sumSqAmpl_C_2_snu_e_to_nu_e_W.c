#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_snu_e_to_nu_e_W.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_snu_e_to_nu_e_W(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_se_L = param->m_se_L;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_el = param->Gamma_el;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_snu_mu = param->m_snu_mu;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t IT_0000 = cos(theta_W);
    const ccomplex_t IT_0001 = cpow(IT_0000, -1);
    const ccomplex_t IT_0002 = conj(N_B3)*e_em;
    const ccomplex_t IT_0003 = IT_0001*IT_0002;
    const ccomplex_t IT_0004 = 1.4142135623731*IT_0003;
    const ccomplex_t IT_0005 = sin(theta_W);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = conj(N_W3)*e_em;
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = 1.4142135623731*IT_0008;
    const ccomplex_t IT_0010 = (0 + _Complex_I*1)*(IT_0004 + -IT_0009);
    const ccomplex_t IT_0011 = (-0.5)*IT_0010;
    const ccomplex_t IT_0012 = N_W3*e_em*conj(U_Wm2);
    const ccomplex_t IT_0013 = IT_0006*IT_0012;
    const ccomplex_t IT_0014 = N_d3*conj(U_d2)*e_em;
    const ccomplex_t IT_0015 = IT_0006*IT_0014;
    const ccomplex_t IT_0016 = 1.4142135623731*IT_0015;
    const ccomplex_t IT_0017 = (0 + _Complex_I*1)*(IT_0013 + 0.5*IT_0016);
    const ccomplex_t IT_0018 = -IT_0017;
    const ccomplex_t IT_0019 = IT_0011*IT_0018;
    const ccomplex_t IT_0020 = pow(m_snu_mu, 2);
    const ccomplex_t IT_0021 = cpow(s_23 + (-0.5)*IT_0020 + 0.5*m_N_3*(m_N_3 +
       (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0022 = IT_0019*IT_0021;
    const ccomplex_t IT_0023 = (0 + _Complex_I*1)*IT_0022;
    const ccomplex_t IT_0024 = conj(N_B4)*e_em;
    const ccomplex_t IT_0025 = IT_0001*IT_0024;
    const ccomplex_t IT_0026 = 1.4142135623731*IT_0025;
    const ccomplex_t IT_0027 = conj(N_W4)*e_em;
    const ccomplex_t IT_0028 = IT_0006*IT_0027;
    const ccomplex_t IT_0029 = 1.4142135623731*IT_0028;
    const ccomplex_t IT_0030 = (0 + _Complex_I*1)*(IT_0026 + -IT_0029);
    const ccomplex_t IT_0031 = (-0.5)*IT_0030;
    const ccomplex_t IT_0032 = N_W4*e_em*conj(U_Wm2);
    const ccomplex_t IT_0033 = IT_0006*IT_0032;
    const ccomplex_t IT_0034 = N_d4*conj(U_d2)*e_em;
    const ccomplex_t IT_0035 = IT_0006*IT_0034;
    const ccomplex_t IT_0036 = 1.4142135623731*IT_0035;
    const ccomplex_t IT_0037 = (0 + _Complex_I*1)*(IT_0033 + 0.5*IT_0036);
    const ccomplex_t IT_0038 = -IT_0037;
    const ccomplex_t IT_0039 = IT_0031*IT_0038;
    const ccomplex_t IT_0040 = cpow(s_23 + (-0.5)*IT_0020 + 0.5*m_N_4*(m_N_4 +
       (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0041 = IT_0039*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*IT_0041;
    const ccomplex_t IT_0043 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0006;
    const ccomplex_t IT_0044 = -IT_0043;
    const ccomplex_t IT_0045 = (0 + _Complex_I*1)*e_em*conj(U_Wm2)*IT_0006;
    const ccomplex_t IT_0046 = pow(m_C_2, 2);
    const ccomplex_t IT_0047 = cpow(s_13 + (-0.5)*IT_0046 + 0.5*m_se_L*(m_se_L
       + (0 + _Complex_I*-1)*Gamma_el) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0048 = IT_0045*IT_0047;
    const ccomplex_t IT_0049 = (0 + _Complex_I*1)*IT_0048;
    const ccomplex_t IT_0050 = IT_0044*IT_0049;
    const ccomplex_t IT_0051 = cpow(s_23 + (-0.5)*IT_0020 + 0.5*m_N_1*(m_N_1 +
       (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0052 = conj(N_B1)*e_em;
    const ccomplex_t IT_0053 = IT_0001*IT_0052;
    const ccomplex_t IT_0054 = 1.4142135623731*IT_0053;
    const ccomplex_t IT_0055 = conj(N_W1)*e_em;
    const ccomplex_t IT_0056 = IT_0006*IT_0055;
    const ccomplex_t IT_0057 = 1.4142135623731*IT_0056;
    const ccomplex_t IT_0058 = (0 + _Complex_I*1)*(IT_0054 + -IT_0057);
    const ccomplex_t IT_0059 = (-0.5)*IT_0058;
    const ccomplex_t IT_0060 = N_W1*e_em*conj(U_Wm2);
    const ccomplex_t IT_0061 = IT_0006*IT_0060;
    const ccomplex_t IT_0062 = N_d1*conj(U_d2)*e_em;
    const ccomplex_t IT_0063 = IT_0006*IT_0062;
    const ccomplex_t IT_0064 = 1.4142135623731*IT_0063;
    const ccomplex_t IT_0065 = (0 + _Complex_I*1)*(IT_0061 + 0.5*IT_0064);
    const ccomplex_t IT_0066 = -IT_0065;
    const ccomplex_t IT_0067 = IT_0059*IT_0066;
    const ccomplex_t IT_0068 = IT_0051*IT_0067;
    const ccomplex_t IT_0069 = (0 + _Complex_I*1)*IT_0068;
    const ccomplex_t IT_0070 = conj(N_B2)*e_em;
    const ccomplex_t IT_0071 = IT_0001*IT_0070;
    const ccomplex_t IT_0072 = 1.4142135623731*IT_0071;
    const ccomplex_t IT_0073 = conj(N_W2)*e_em;
    const ccomplex_t IT_0074 = IT_0006*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = (0 + _Complex_I*1)*(IT_0072 + -IT_0075);
    const ccomplex_t IT_0077 = (-0.5)*IT_0076;
    const ccomplex_t IT_0078 = N_W2*e_em*conj(U_Wm2);
    const ccomplex_t IT_0079 = IT_0006*IT_0078;
    const ccomplex_t IT_0080 = N_d2*conj(U_d2)*e_em;
    const ccomplex_t IT_0081 = IT_0006*IT_0080;
    const ccomplex_t IT_0082 = 1.4142135623731*IT_0081;
    const ccomplex_t IT_0083 = (0 + _Complex_I*1)*(IT_0079 + 0.5*IT_0082);
    const ccomplex_t IT_0084 = -IT_0083;
    const ccomplex_t IT_0085 = IT_0077*IT_0084;
    const ccomplex_t IT_0086 = cpow(s_23 + (-0.5)*IT_0020 + 0.5*m_N_2*(m_N_2 +
       (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0087 = IT_0085*IT_0086;
    const ccomplex_t IT_0088 = (0 + _Complex_I*1)*IT_0087;
    const ccomplex_t IT_0089 = IT_0023 + IT_0042 + 0.5*IT_0050 + IT_0069 +
       IT_0088;
    const ccomplex_t IT_0090 = pow(m_W, -2);
    const ccomplex_t IT_0091 = pow(s_14, 2);
    const ccomplex_t IT_0092 = IT_0090*IT_0091;
    const ccomplex_t IT_0093 = -IT_0046;
    const ccomplex_t IT_0094 = IT_0092 + IT_0093;
    const ccomplex_t IT_0095 = s_13*IT_0094;
    const ccomplex_t IT_0096 = (-0.5)*IT_0050;
    const ccomplex_t IT_0097 = s_14*s_34*IT_0090;
    const ccomplex_t IT_0098 = -IT_0097;
    const ccomplex_t IT_0099 = s_13 + IT_0098;
    const ccomplex_t IT_0100 = s_13*IT_0099;
    const ccomplex_t IT_0101 = conj(N_W1)*e_em*V_Wp2;
    const ccomplex_t IT_0102 = IT_0006*IT_0101;
    const ccomplex_t IT_0103 = conj(N_u1)*V_u2*e_em;
    const ccomplex_t IT_0104 = IT_0006*IT_0103;
    const ccomplex_t IT_0105 = 1.4142135623731*IT_0104;
    const ccomplex_t IT_0106 = (0 + _Complex_I*1)*(IT_0102 + (-0.5)*IT_0105);
    const ccomplex_t IT_0107 = -IT_0106;
    const ccomplex_t IT_0108 = IT_0059*IT_0107;
    const ccomplex_t IT_0109 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0110 = IT_0051*IT_0108*IT_0109;
    const ccomplex_t IT_0111 = conj(N_W4)*e_em*V_Wp2;
    const ccomplex_t IT_0112 = IT_0006*IT_0111;
    const ccomplex_t IT_0113 = conj(N_u4)*V_u2*e_em;
    const ccomplex_t IT_0114 = IT_0006*IT_0113;
    const ccomplex_t IT_0115 = 1.4142135623731*IT_0114;
    const ccomplex_t IT_0116 = (0 + _Complex_I*1)*(IT_0112 + (-0.5)*IT_0115);
    const ccomplex_t IT_0117 = -IT_0116;
    const ccomplex_t IT_0118 = IT_0031*IT_0117;
    const ccomplex_t IT_0119 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0120 = IT_0040*IT_0118*IT_0119;
    const ccomplex_t IT_0121 = conj(N_W3)*e_em*V_Wp2;
    const ccomplex_t IT_0122 = IT_0006*IT_0121;
    const ccomplex_t IT_0123 = conj(N_u3)*V_u2*e_em;
    const ccomplex_t IT_0124 = IT_0006*IT_0123;
    const ccomplex_t IT_0125 = 1.4142135623731*IT_0124;
    const ccomplex_t IT_0126 = (0 + _Complex_I*1)*(IT_0122 + (-0.5)*IT_0125);
    const ccomplex_t IT_0127 = -IT_0126;
    const ccomplex_t IT_0128 = IT_0011*IT_0127;
    const ccomplex_t IT_0129 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0130 = IT_0021*IT_0128*IT_0129;
    const ccomplex_t IT_0131 = m_C_2*IT_0023;
    const ccomplex_t IT_0132 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0133 = conj(N_W2)*e_em*V_Wp2;
    const ccomplex_t IT_0134 = IT_0006*IT_0133;
    const ccomplex_t IT_0135 = conj(N_u2)*V_u2*e_em;
    const ccomplex_t IT_0136 = IT_0006*IT_0135;
    const ccomplex_t IT_0137 = 1.4142135623731*IT_0136;
    const ccomplex_t IT_0138 = (0 + _Complex_I*1)*(IT_0134 + (-0.5)*IT_0137);
    const ccomplex_t IT_0139 = -IT_0138;
    const ccomplex_t IT_0140 = IT_0077*IT_0139;
    const ccomplex_t IT_0141 = IT_0086*IT_0132*IT_0140;
    const ccomplex_t IT_0142 = m_C_2*IT_0069;
    const ccomplex_t IT_0143 = m_C_2*IT_0088;
    const ccomplex_t IT_0144 = m_C_2*IT_0042;
    const ccomplex_t IT_0145 = 0.5*IT_0110 + 0.5*IT_0120 + 0.5*IT_0130 + (-0.5
      )*IT_0131 + 0.5*IT_0141 + (-0.5)*IT_0142 + (-0.5)*IT_0143 + (-0.5)*IT_0144;
    const ccomplex_t IT_0146 = m_C_2*IT_0099;
    const ccomplex_t IT_0147 = (-0.5)*IT_0023 + (-0.5)*IT_0042 + (-0.5)
      *IT_0069 + (-0.5)*IT_0088;
    const ccomplex_t IT_0148 = s_13*s_14;
    const ccomplex_t IT_0149 = s_34*IT_0046;
    const ccomplex_t IT_0150 = -IT_0149;
    const ccomplex_t IT_0151 = IT_0148 + IT_0150;
    const ccomplex_t IT_0152 = s_13*s_34;
    const ccomplex_t IT_0153 = pow(m_W, 2);
    const ccomplex_t IT_0154 = s_34*IT_0153;
    const ccomplex_t IT_0155 = IT_0090*IT_0154;
    const ccomplex_t IT_0156 = (-0.25)*IT_0155;
    const ccomplex_t IT_0157 = s_34 + IT_0156;
    const ccomplex_t IT_0158 = m_C_2*IT_0157;
    const ccomplex_t IT_0159 = (-4)*IT_0158;
    const ccomplex_t IT_0160 = s_13*IT_0153;
    const ccomplex_t IT_0161 = s_14*s_34;
    const ccomplex_t IT_0162 = pow(m_W, 4);
    const ccomplex_t IT_0163 = s_13*IT_0162;
    const ccomplex_t IT_0164 = IT_0090*IT_0163;
    const ccomplex_t IT_0165 = pow(s_34, 2);
    const ccomplex_t IT_0166 = s_13*IT_0090*IT_0165;
    const ccomplex_t IT_0167 = m_C_2*IT_0090*IT_0165;
    const ccomplex_t IT_0168 = (-2)*conj(IT_0089);
    const ccomplex_t IT_0169 = (-2)*IT_0161;
    const ccomplex_t IT_0170 = IT_0160 + IT_0169;
    const ccomplex_t IT_0171 = IT_0090*IT_0170;
    const ccomplex_t IT_0172 = 2*IT_0089*(conj(IT_0089)*IT_0095 + -conj
      (IT_0096)*IT_0100 + -conj(IT_0145)*IT_0146 + conj(IT_0147)*IT_0151) + 2
      *IT_0147*(conj(IT_0089)*IT_0151 + -conj(IT_0096)*IT_0152 + conj(IT_0145)
      *IT_0159 + (-2)*conj(IT_0147)*(IT_0160 + (-2)*IT_0161 + (-0.5)*IT_0164)) +
       (-2)*IT_0096*(conj(IT_0147)*IT_0152 + -conj(IT_0096)*IT_0166 + -conj
      (IT_0145)*IT_0167 + (-0.5)*IT_0100*IT_0168) + 2*IT_0145*(conj(IT_0147)
      *IT_0159 + conj(IT_0096)*IT_0167 + 0.5*IT_0146*IT_0168 + 2*conj(IT_0145)*
      (s_13 + (-0.5)*IT_0171));
    return create_ccomplex_return(IT_0172);
}

