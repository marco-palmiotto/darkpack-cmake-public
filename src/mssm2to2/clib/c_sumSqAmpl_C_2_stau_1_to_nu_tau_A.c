#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_C_2_stau_1_to_nu_tau_A.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_C_2_stau_1_to_nu_tau_A(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_c2 = param->Gamma_c2;
    const creal_t m_stau_1 = param->m_stau_1;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_tau1 = param->Gamma_tau1;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t U_stau_00 = param->U_stau_00;
    const ccomplex_t U_stau_10 = param->U_stau_10;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*e_em;
    const ccomplex_t IT_0001 = sin(theta_W);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = e_em*conj(U_Wm2)*U_stau_00;
    const ccomplex_t IT_0004 = IT_0002*IT_0003;
    const ccomplex_t IT_0005 = cos(beta);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = IT_0002*IT_0006;
    const ccomplex_t IT_0008 = pow(m_W, -1);
    const ccomplex_t IT_0009 = conj(U_d2)*e_em*m_tau*IT_0008*U_stau_10;
    const ccomplex_t IT_0010 = IT_0007*IT_0009;
    const ccomplex_t IT_0011 = 1.4142135623731*IT_0010;
    const ccomplex_t IT_0012 = (0 + _Complex_I*1)*(IT_0004 + (-0.5)*IT_0011);
    const ccomplex_t IT_0013 = IT_0000*IT_0012;
    const ccomplex_t IT_0014 = pow(m_stau_1, 2);
    const ccomplex_t IT_0015 = cpow(s_23 + (-0.5)*IT_0014 + 0.5*m_C_2*(m_C_2 +
       (0 + _Complex_I*-1)*Gamma_c2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0016 = IT_0013*IT_0015;
    const ccomplex_t IT_0017 = (0 + _Complex_I*1)*IT_0016;
    const ccomplex_t IT_0018 = (-0.5)*IT_0017;
    const ccomplex_t IT_0019 = s_14*s_34;
    const ccomplex_t IT_0020 = 2*IT_0000;
    const ccomplex_t IT_0021 = pow(m_C_2, 2);
    const ccomplex_t IT_0022 = cpow(s_13 + (-0.5)*IT_0021 + (-0.5)*reg_prop +
       0.5*m_stau_1*(m_stau_1 + (0 + _Complex_I*-1)*Gamma_tau1), -1);
    const ccomplex_t IT_0023 = IT_0012*IT_0022;
    const ccomplex_t IT_0024 = (0 + _Complex_I*1)*IT_0023;
    const ccomplex_t IT_0025 = IT_0020*IT_0024;
    const ccomplex_t IT_0026 = (-0.5)*IT_0025;
    const ccomplex_t IT_0027 = s_13*s_34;
    const ccomplex_t IT_0028 = IT_0000*IT_0024;
    const ccomplex_t IT_0029 = (-0.5)*IT_0028;
    const ccomplex_t IT_0030 = (-2)*s_13;
    const ccomplex_t IT_0031 = s_34*IT_0030;
    const ccomplex_t IT_0032 = IT_0017 + 0.5*IT_0025;
    const ccomplex_t IT_0033 = s_34*IT_0021;
    const ccomplex_t IT_0034 = pow(s_13, 2);
    const ccomplex_t IT_0035 = (-2)*IT_0034;
    const ccomplex_t IT_0036 = 8*IT_0018*conj(IT_0018)*IT_0019 + (-4)*conj
      (IT_0026)*(IT_0018*IT_0027 + (-0.25)*IT_0029*IT_0031) + (-4)*IT_0026*(conj
      (IT_0018)*IT_0027 + (-0.25)*conj(IT_0029)*IT_0031) + (-2)*conj(IT_0032)*((
      -0.5)*IT_0030*(s_14*IT_0029 + IT_0021*IT_0032) + IT_0018*IT_0033 + (-0.5)
      *IT_0026*IT_0035) + (-2)*IT_0032*((-0.5)*s_14*conj(IT_0029)*IT_0030 + conj
      (IT_0018)*IT_0033 + (-0.5)*conj(IT_0026)*IT_0035);
    return create_ccomplex_return(IT_0036);
}

