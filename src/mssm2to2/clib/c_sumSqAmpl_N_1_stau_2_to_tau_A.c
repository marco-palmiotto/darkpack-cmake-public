#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_1_stau_2_to_tau_A.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_1_stau_2_to_tau_A(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t m_stau_2 = param->m_stau_2;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_tau2 = param->Gamma_tau2;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t U_stau_01 = param->U_stau_01;
    const ccomplex_t U_stau_11 = param->U_stau_11;
    const ccomplex_t IT_0000 = pow(m_N_1, 2);
    const ccomplex_t IT_0001 = pow(m_tau, 2);
    const ccomplex_t IT_0002 = pow(m_stau_2, 2);
    const ccomplex_t IT_0003 = cpow(s_12 + 0.5*IT_0000 + (-0.5)*IT_0001 + 0.5
      *IT_0002 + 0.5*reg_prop, -1);
    const ccomplex_t IT_0004 = (0 + _Complex_I*1)*e_em;
    const ccomplex_t IT_0005 = -IT_0004;
    const ccomplex_t IT_0006 = cos(theta_W);
    const ccomplex_t IT_0007 = cpow(IT_0006, -1);
    const ccomplex_t IT_0008 = conj(N_B1)*e_em*U_stau_01;
    const ccomplex_t IT_0009 = IT_0007*IT_0008;
    const ccomplex_t IT_0010 = 1.4142135623731*IT_0009;
    const ccomplex_t IT_0011 = sin(theta_W);
    const ccomplex_t IT_0012 = cpow(IT_0011, -1);
    const ccomplex_t IT_0013 = conj(N_W1)*e_em*U_stau_01;
    const ccomplex_t IT_0014 = IT_0012*IT_0013;
    const ccomplex_t IT_0015 = 1.4142135623731*IT_0014;
    const ccomplex_t IT_0016 = cos(beta);
    const ccomplex_t IT_0017 = cpow(IT_0016, -1);
    const ccomplex_t IT_0018 = IT_0012*IT_0017;
    const ccomplex_t IT_0019 = pow(m_W, -1);
    const ccomplex_t IT_0020 = conj(N_d1)*e_em*m_tau*IT_0019*U_stau_11;
    const ccomplex_t IT_0021 = IT_0018*IT_0020;
    const ccomplex_t IT_0022 = 1.4142135623731*IT_0021;
    const ccomplex_t IT_0023 = (0 + _Complex_I*1)*(IT_0010 + IT_0015 + 
      -IT_0022);
    const ccomplex_t IT_0024 = (-0.5)*IT_0023;
    const ccomplex_t IT_0025 = IT_0005*IT_0024;
    const ccomplex_t IT_0026 = IT_0003*IT_0025;
    const ccomplex_t IT_0027 = (0 + _Complex_I*1)*IT_0026;
    const ccomplex_t IT_0028 = 2*IT_0004;
    const ccomplex_t IT_0029 = cpow(s_13 + (-0.5)*IT_0000 + (-0.5)*IT_0001 + (
      -0.5)*reg_prop + 0.5*m_stau_2*(m_stau_2 + (0 + _Complex_I*-1)*Gamma_tau2),
       -1);
    const ccomplex_t IT_0030 = IT_0024*IT_0029;
    const ccomplex_t IT_0031 = (0 + _Complex_I*1)*IT_0030;
    const ccomplex_t IT_0032 = IT_0028*IT_0031;
    const ccomplex_t IT_0033 = -IT_0027 + (-0.5)*IT_0032;
    const ccomplex_t IT_0034 = IT_0004*IT_0031;
    const ccomplex_t IT_0035 = (-0.5)*IT_0034;
    const ccomplex_t IT_0036 = N_B1*e_em*U_stau_11;
    const ccomplex_t IT_0037 = IT_0007*IT_0036;
    const ccomplex_t IT_0038 = 1.4142135623731*IT_0037;
    const ccomplex_t IT_0039 = N_d1*e_em*m_tau*IT_0019*U_stau_01;
    const ccomplex_t IT_0040 = IT_0018*IT_0039;
    const ccomplex_t IT_0041 = 1.4142135623731*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*(IT_0038 + 0.5*IT_0041);
    const ccomplex_t IT_0043 = -IT_0042;
    const ccomplex_t IT_0044 = IT_0029*IT_0043;
    const ccomplex_t IT_0045 = (0 + _Complex_I*1)*IT_0044;
    const ccomplex_t IT_0046 = IT_0004*IT_0045;
    const ccomplex_t IT_0047 = 0.5*IT_0046;
    const ccomplex_t IT_0048 = IT_0005*IT_0043;
    const ccomplex_t IT_0049 = IT_0003*IT_0048;
    const ccomplex_t IT_0050 = (0 + _Complex_I*1)*IT_0049;
    const ccomplex_t IT_0051 = IT_0028*IT_0045;
    const ccomplex_t IT_0052 = IT_0050 + 0.5*IT_0051;
    const ccomplex_t IT_0053 = (-2)*s_13;
    const ccomplex_t IT_0054 = s_34*IT_0053;
    const ccomplex_t IT_0055 = IT_0001*IT_0053;
    const ccomplex_t IT_0056 = 0.5*IT_0032;
    const ccomplex_t IT_0057 = 0.5*IT_0050;
    const ccomplex_t IT_0058 = s_14*m_N_1*m_tau;
    const ccomplex_t IT_0059 = (-0.5)*IT_0027;
    const ccomplex_t IT_0060 = s_13*s_14;
    const ccomplex_t IT_0061 = s_34*IT_0000;
    const ccomplex_t IT_0062 = (-0.5)*IT_0061;
    const ccomplex_t IT_0063 = IT_0060 + IT_0062;
    const ccomplex_t IT_0064 = s_14*IT_0053;
    const ccomplex_t IT_0065 = pow(s_13, 2);
    const ccomplex_t IT_0066 = (-2)*IT_0065;
    const ccomplex_t IT_0067 = (-0.5)*IT_0051;
    const ccomplex_t IT_0068 = m_N_1*m_tau;
    const ccomplex_t IT_0069 = (-2)*s_14;
    const ccomplex_t IT_0070 = (-2)*s_34;
    const ccomplex_t IT_0071 = (-2)*IT_0000;
    const ccomplex_t IT_0072 = (-2)*IT_0001;
    const ccomplex_t IT_0073 = IT_0053*(conj(IT_0052)*IT_0056 + IT_0052*conj
      (IT_0056) + conj(IT_0033)*IT_0067 + IT_0033*conj(IT_0067)) + (conj(IT_0047
      )*IT_0056 + IT_0047*conj(IT_0056) + conj(IT_0035)*IT_0067 + IT_0035*conj
      (IT_0067))*IT_0069 + (conj(IT_0033)*IT_0047 + IT_0033*conj(IT_0047) + conj
      (IT_0035)*IT_0052 + IT_0035*conj(IT_0052))*IT_0070 + (conj(IT_0056)
      *IT_0067 + IT_0056*conj(IT_0067))*IT_0071 + (conj(IT_0033)*IT_0052 +
       IT_0033*conj(IT_0052))*IT_0072;
    const ccomplex_t IT_0074 = s_34*m_N_1*m_tau;
    const ccomplex_t IT_0075 = (-2)*conj(IT_0033);
    const ccomplex_t IT_0076 = s_14*IT_0001;
    const ccomplex_t IT_0077 = (-2)*conj(IT_0052);
    const ccomplex_t IT_0078 = s_14*s_34;
    const ccomplex_t IT_0079 = (-2)*IT_0052;
    const ccomplex_t IT_0080 = (-0.5)*IT_0076;
    const ccomplex_t IT_0081 = (-2)*IT_0033;
    const ccomplex_t IT_0082 = (-0.5)*IT_0074;
    const ccomplex_t IT_0083 = (conj(IT_0033)*IT_0035 + IT_0033*conj(IT_0035) 
      + conj(IT_0047)*IT_0052 + IT_0047*conj(IT_0052))*IT_0054 + (IT_0033*conj
      (IT_0033) + IT_0052*conj(IT_0052))*IT_0055 + (-2)*conj(IT_0056)*((-0.5)
      *IT_0000*IT_0053*IT_0056 + IT_0057*IT_0058 + 2*IT_0059*IT_0063 + (-0.5)
      *IT_0035*IT_0064 + (-0.5)*IT_0033*IT_0066) + IT_0056*(conj(IT_0035)
      *IT_0064 + conj(IT_0033)*IT_0066) + (conj(IT_0047)*IT_0064 + conj(IT_0052)
      *IT_0066)*IT_0067 + (-2)*(IT_0058*IT_0059 + 2*IT_0057*IT_0063 + (-0.5)
      *IT_0047*IT_0064 + (-0.5)*IT_0052*IT_0066 + (-0.5)*IT_0000*IT_0053*IT_0067
      )*conj(IT_0067) + IT_0068*IT_0073 + IT_0059*(IT_0075*IT_0076 + IT_0074
      *IT_0077) + IT_0057*(IT_0074*IT_0075 + IT_0076*IT_0077) + (-4)*conj
      (IT_0059)*(IT_0056*IT_0063 + 1./2*IT_0058*IT_0067 + (-2)*IT_0059*IT_0078 +
       1./2*IT_0080*IT_0081 + 1./2*IT_0079*IT_0082) + (-2)*conj(IT_0057)*
      (IT_0056*IT_0058 + 2*IT_0063*IT_0067 + (-4)*IT_0057*IT_0078 + IT_0079
      *IT_0080 + IT_0081*IT_0082);
    return create_ccomplex_return(IT_0083);
}

