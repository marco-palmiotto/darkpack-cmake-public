#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_2_su_R_to_d_W.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_2_su_R_to_d_W(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_d = param->m_d;
    const creal_t m_u = param->m_u;
    const creal_t V_ud = param->V_ud;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_su_R = param->m_su_R;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_c1 = param->Gamma_c1;
    const creal_t Gamma_c2 = param->Gamma_c2;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t IT_0000 = pow(m_d, 2);
    const ccomplex_t IT_0001 = pow(m_su_R, 2);
    const ccomplex_t IT_0002 = cpow((-2)*s_23 + IT_0000 + IT_0001 + -m_C_2*
      (m_C_2 + (0 + _Complex_I*-1)*Gamma_c2) + reg_prop, -1);
    const ccomplex_t IT_0003 = pow(m_W, -1);
    const ccomplex_t IT_0004 = sin(beta);
    const ccomplex_t IT_0005 = cpow(IT_0004, -1);
    const ccomplex_t IT_0006 = sin(theta_W);
    const ccomplex_t IT_0007 = cpow(IT_0006, -1);
    const ccomplex_t IT_0008 = (0 + _Complex_I*1.4142135623731)*m_u*conj(V_u2)
      *V_ud*e_em*IT_0003*IT_0005*IT_0007;
    const ccomplex_t IT_0009 = (-0.5)*IT_0008;
    const ccomplex_t IT_0010 = conj(N_W2)*e_em*V_Wp2;
    const ccomplex_t IT_0011 = IT_0007*IT_0010;
    const ccomplex_t IT_0012 = conj(N_u2)*V_u2*e_em;
    const ccomplex_t IT_0013 = IT_0007*IT_0012;
    const ccomplex_t IT_0014 = 1.4142135623731*IT_0013;
    const ccomplex_t IT_0015 = (0 + _Complex_I*1)*(IT_0011 + (-0.5)*IT_0014);
    const ccomplex_t IT_0016 = -IT_0015;
    const ccomplex_t IT_0017 = IT_0009*IT_0016;
    const ccomplex_t IT_0018 = IT_0002*IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1)*IT_0018;
    const ccomplex_t IT_0020 = (0 + _Complex_I*1.4142135623731)*m_u*conj(V_u1)
      *V_ud*e_em*IT_0003*IT_0005*IT_0007;
    const ccomplex_t IT_0021 = (-0.5)*IT_0020;
    const ccomplex_t IT_0022 = conj(N_W2)*e_em*V_Wp1;
    const ccomplex_t IT_0023 = IT_0007*IT_0022;
    const ccomplex_t IT_0024 = conj(N_u2)*V_u1*e_em;
    const ccomplex_t IT_0025 = IT_0007*IT_0024;
    const ccomplex_t IT_0026 = 1.4142135623731*IT_0025;
    const ccomplex_t IT_0027 = (0 + _Complex_I*1)*(IT_0023 + (-0.5)*IT_0026);
    const ccomplex_t IT_0028 = -IT_0027;
    const ccomplex_t IT_0029 = IT_0021*IT_0028;
    const ccomplex_t IT_0030 = cpow((-2)*s_23 + IT_0000 + IT_0001 + -m_C_1*
      (m_C_1 + (0 + _Complex_I*-1)*Gamma_c1) + reg_prop, -1);
    const ccomplex_t IT_0031 = IT_0029*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1)*IT_0031;
    const ccomplex_t IT_0033 = (-2)*IT_0019 + (-2)*IT_0032;
    const ccomplex_t IT_0034 = N_W2*e_em*conj(U_Wm1);
    const ccomplex_t IT_0035 = IT_0007*IT_0034;
    const ccomplex_t IT_0036 = N_d2*conj(U_d1)*e_em;
    const ccomplex_t IT_0037 = IT_0007*IT_0036;
    const ccomplex_t IT_0038 = 1.4142135623731*IT_0037;
    const ccomplex_t IT_0039 = (0 + _Complex_I*1)*(IT_0035 + 0.5*IT_0038);
    const ccomplex_t IT_0040 = -IT_0039;
    const ccomplex_t IT_0041 = IT_0021*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*m_C_1;
    const ccomplex_t IT_0043 = IT_0030*IT_0041*IT_0042;
    const ccomplex_t IT_0044 = N_W2*e_em*conj(U_Wm2);
    const ccomplex_t IT_0045 = IT_0007*IT_0044;
    const ccomplex_t IT_0046 = N_d2*conj(U_d2)*e_em;
    const ccomplex_t IT_0047 = IT_0007*IT_0046;
    const ccomplex_t IT_0048 = 1.4142135623731*IT_0047;
    const ccomplex_t IT_0049 = (0 + _Complex_I*1)*(IT_0045 + 0.5*IT_0048);
    const ccomplex_t IT_0050 = -IT_0049;
    const ccomplex_t IT_0051 = IT_0009*IT_0050;
    const ccomplex_t IT_0052 = (0 + _Complex_I*1)*m_C_2;
    const ccomplex_t IT_0053 = IT_0002*IT_0051*IT_0052;
    const ccomplex_t IT_0054 = pow(m_u, 2);
    const ccomplex_t IT_0055 = pow(m_N_2, 2);
    const ccomplex_t IT_0056 = cpow((-2)*s_12 + -IT_0001 + IT_0054 + -IT_0055 
      + -reg_prop, -1);
    const ccomplex_t IT_0057 = (0 + _Complex_I*1)*m_u;
    const ccomplex_t IT_0058 = cos(theta_W);
    const ccomplex_t IT_0059 = cpow(IT_0058, -1);
    const ccomplex_t IT_0060 = (0 + _Complex_I*1.4142135623731)*N_B2*e_em
      *IT_0059;
    const ccomplex_t IT_0061 = 0.666666666666667*IT_0060;
    const ccomplex_t IT_0062 = (0 + _Complex_I*1.4142135623731)*V_ud*e_em
      *IT_0007;
    const ccomplex_t IT_0063 = 0.5*IT_0062;
    const ccomplex_t IT_0064 = IT_0061*IT_0063;
    const ccomplex_t IT_0065 = IT_0056*IT_0057*IT_0064;
    const ccomplex_t IT_0066 = IT_0043 + IT_0053 + -IT_0065;
    const ccomplex_t IT_0067 = pow(m_W, -2);
    const ccomplex_t IT_0068 = s_14*s_34*IT_0067;
    const ccomplex_t IT_0069 = -IT_0068;
    const ccomplex_t IT_0070 = s_13 + IT_0069;
    const ccomplex_t IT_0071 = m_N_2*IT_0070;
    const ccomplex_t IT_0072 = (0 + _Complex_I*1.4142135623731)*m_u*conj(N_u2)
      *e_em*IT_0003*IT_0005*IT_0007;
    const ccomplex_t IT_0073 = 0.5*IT_0072;
    const ccomplex_t IT_0074 = IT_0063*IT_0073;
    const ccomplex_t IT_0075 = IT_0056*IT_0074;
    const ccomplex_t IT_0076 = (0 + _Complex_I*1)*IT_0075;
    const ccomplex_t IT_0077 = m_d*IT_0076;
    const ccomplex_t IT_0078 = -IT_0077;
    const ccomplex_t IT_0079 = pow(s_14, 2);
    const ccomplex_t IT_0080 = IT_0067*IT_0079;
    const ccomplex_t IT_0081 = -IT_0055;
    const ccomplex_t IT_0082 = IT_0080 + IT_0081;
    const ccomplex_t IT_0083 = m_d*IT_0082;
    const ccomplex_t IT_0084 = IT_0019 + IT_0032;
    const ccomplex_t IT_0085 = m_N_2*IT_0084;
    const ccomplex_t IT_0086 = -IT_0085;
    const ccomplex_t IT_0087 = s_13*IT_0082;
    const ccomplex_t IT_0088 = 2*IT_0076;
    const ccomplex_t IT_0089 = s_13*IT_0070;
    const ccomplex_t IT_0090 = s_13*s_14;
    const ccomplex_t IT_0091 = s_34*IT_0055;
    const ccomplex_t IT_0092 = -IT_0091;
    const ccomplex_t IT_0093 = IT_0090 + IT_0092;
    const ccomplex_t IT_0094 = conj(IT_0076) + conj(IT_0084);
    const ccomplex_t IT_0095 = 6*IT_0033;
    const ccomplex_t IT_0096 = 6*conj(IT_0033);
    const ccomplex_t IT_0097 = pow(s_34, 2);
    const ccomplex_t IT_0098 = IT_0067*IT_0097;
    const ccomplex_t IT_0099 = -IT_0098;
    const ccomplex_t IT_0100 = IT_0000 + IT_0099;
    const ccomplex_t IT_0101 = m_N_2*IT_0100;
    const ccomplex_t IT_0102 = (-6)*conj(IT_0066);
    const ccomplex_t IT_0103 = s_13*IT_0100;
    const ccomplex_t IT_0104 = m_d*IT_0070;
    const ccomplex_t IT_0105 = s_14*IT_0000;
    const ccomplex_t IT_0106 = s_13*s_34;
    const ccomplex_t IT_0107 = -IT_0106;
    const ccomplex_t IT_0108 = IT_0105 + IT_0107;
    const ccomplex_t IT_0109 = (-6)*IT_0088;
    const ccomplex_t IT_0110 = pow(m_W, 2);
    const ccomplex_t IT_0111 = s_14*IT_0110;
    const ccomplex_t IT_0112 = IT_0067*IT_0111;
    const ccomplex_t IT_0113 = (-0.25)*IT_0112;
    const ccomplex_t IT_0114 = s_14 + IT_0113;
    const ccomplex_t IT_0115 = m_d*IT_0114;
    const ccomplex_t IT_0116 = (-4)*IT_0115;
    const ccomplex_t IT_0117 = pow(m_W, 4);
    const ccomplex_t IT_0118 = s_13*IT_0117;
    const ccomplex_t IT_0119 = IT_0067*IT_0118;
    const ccomplex_t IT_0120 = 6*IT_0119;
    const ccomplex_t IT_0121 = s_14*s_34;
    const ccomplex_t IT_0122 = 24*IT_0121;
    const ccomplex_t IT_0123 = s_13*IT_0110;
    const ccomplex_t IT_0124 = (-12)*IT_0123;
    const ccomplex_t IT_0125 = IT_0120 + IT_0122 + IT_0124;
    const ccomplex_t IT_0126 = (-6)*IT_0119;
    const ccomplex_t IT_0127 = IT_0122 + IT_0126;
    const ccomplex_t IT_0128 = s_34*IT_0110;
    const ccomplex_t IT_0129 = IT_0067*IT_0128;
    const ccomplex_t IT_0130 = 0.5*IT_0129;
    const ccomplex_t IT_0131 = s_34 + IT_0130;
    const ccomplex_t IT_0132 = m_N_2*IT_0131;
    const ccomplex_t IT_0133 = 2*IT_0132;
    const ccomplex_t IT_0134 = conj(IT_0066) + conj(IT_0086);
    const ccomplex_t IT_0135 = (-6)*conj(IT_0088);
    const ccomplex_t IT_0136 = 0.5*IT_0112;
    const ccomplex_t IT_0137 = s_14 + IT_0136;
    const ccomplex_t IT_0138 = m_d*IT_0137;
    const ccomplex_t IT_0139 = 2*IT_0138;
    const ccomplex_t IT_0140 = (-0.25)*IT_0129;
    const ccomplex_t IT_0141 = s_34 + IT_0140;
    const ccomplex_t IT_0142 = m_N_2*IT_0141;
    const ccomplex_t IT_0143 = (-4)*IT_0142;
    const ccomplex_t IT_0144 = m_d*m_N_2;
    const ccomplex_t IT_0145 = (-2)*IT_0121;
    const ccomplex_t IT_0146 = IT_0123 + IT_0145;
    const ccomplex_t IT_0147 = IT_0067*IT_0146;
    const ccomplex_t IT_0148 = (-6)*IT_0147;
    const ccomplex_t IT_0149 = 12*s_13;
    const ccomplex_t IT_0150 = IT_0148 + IT_0149;
    const ccomplex_t IT_0151 = 6*conj(IT_0076);
    const ccomplex_t IT_0152 = (-6)*conj(IT_0084);
    const ccomplex_t IT_0153 = (-18)*conj(IT_0078)*IT_0144;
    const ccomplex_t IT_0154 = IT_0133*IT_0151;
    const ccomplex_t IT_0155 = IT_0143*IT_0152;
    const ccomplex_t IT_0156 = IT_0134*IT_0150;
    const ccomplex_t IT_0157 = 6*IT_0033*(conj(IT_0066)*IT_0071 + -conj
      (IT_0078)*IT_0083) + (IT_0071*conj(IT_0086) + conj(IT_0033)*IT_0087 + conj
      (IT_0088)*IT_0089 + IT_0093*IT_0094)*IT_0095 + IT_0088*(IT_0089*IT_0096 +
       IT_0101*IT_0102) + (conj(IT_0086)*IT_0101 + conj(IT_0088)*IT_0103 + conj
      (IT_0078)*IT_0104 + IT_0094*IT_0108)*IT_0109 + 6*IT_0076*
      (0.166666666666667*IT_0093*IT_0096 + conj(IT_0078)*IT_0116 +
       0.166666666666667*conj(IT_0076)*IT_0125 + 0.166666666666667*conj(IT_0084)
      *IT_0127 + IT_0133*IT_0134 + 0.166666666666667*IT_0108*IT_0135) + (-6)
      *IT_0084*((-0.166666666666667)*IT_0093*IT_0096 + (-0.166666666666667)*conj
      (IT_0084)*IT_0125 + (-0.166666666666667)*conj(IT_0076)*IT_0127 + (
      -0.166666666666667)*IT_0108*IT_0135 + conj(IT_0078)*IT_0139 + (conj
      (IT_0086) + (-0.166666666666667)*IT_0102)*IT_0143) + (-6)*IT_0078*(conj
      (IT_0033)*IT_0083 + -1./6*IT_0104*IT_0135 + -1./6*((-18)*conj(IT_0066) + (
      -18)*conj(IT_0086))*IT_0144 + -1./6*conj(IT_0078)*IT_0150 + -1./6*IT_0116
      *IT_0151 + -1./6*IT_0139*IT_0152) + IT_0086*(IT_0071*IT_0096 + IT_0101
      *IT_0135 + IT_0153 + IT_0154 + IT_0155 + IT_0156) + 6*IT_0066*(conj
      (IT_0033)*IT_0071 + -conj(IT_0088)*IT_0101 + 0.166666666666667*IT_0153 +
       0.166666666666667*IT_0154 + 0.166666666666667*IT_0155 + 0.166666666666667
      *IT_0156);
    return create_ccomplex_return(IT_0157);
}

