#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_3_snu_tau_to_anti_A0_nu_tau.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_3_snu_tau_to_anti_A0_nu_tau(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_A0 = param->m_A0;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t IT_0000 = cos(beta);
    const ccomplex_t IT_0001 = cos(theta_W);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = IT_0000*IT_0002;
    const ccomplex_t IT_0004 = N_B4*N_u3*e_em;
    const ccomplex_t IT_0005 = IT_0003*IT_0004;
    const ccomplex_t IT_0006 = sin(beta);
    const ccomplex_t IT_0007 = IT_0002*IT_0006;
    const ccomplex_t IT_0008 = N_B4*N_d3*e_em;
    const ccomplex_t IT_0009 = IT_0007*IT_0008;
    const ccomplex_t IT_0010 = sin(theta_W);
    const ccomplex_t IT_0011 = cpow(IT_0010, -1);
    const ccomplex_t IT_0012 = IT_0000*IT_0011;
    const ccomplex_t IT_0013 = N_W4*N_u3*e_em;
    const ccomplex_t IT_0014 = IT_0012*IT_0013;
    const ccomplex_t IT_0015 = IT_0006*IT_0011;
    const ccomplex_t IT_0016 = N_W4*N_d3*e_em;
    const ccomplex_t IT_0017 = IT_0015*IT_0016;
    const ccomplex_t IT_0018 = N_B3*N_u4*e_em;
    const ccomplex_t IT_0019 = IT_0003*IT_0018;
    const ccomplex_t IT_0020 = N_B3*N_d4*e_em;
    const ccomplex_t IT_0021 = IT_0007*IT_0020;
    const ccomplex_t IT_0022 = N_W3*N_u4*e_em;
    const ccomplex_t IT_0023 = IT_0012*IT_0022;
    const ccomplex_t IT_0024 = N_W3*N_d4*e_em;
    const ccomplex_t IT_0025 = IT_0015*IT_0024;
    const ccomplex_t IT_0026 = (-0.5)*IT_0005 + 0.5*IT_0009 + 0.5*IT_0014 + (
      -0.5)*IT_0017 + (-0.5)*IT_0019 + 0.5*IT_0021 + 0.5*IT_0023 + (-0.5)*IT_0025;
    const ccomplex_t IT_0027 = m_N_3*IT_0026;
    const ccomplex_t IT_0028 = conj(N_B3)*conj(N_u4)*e_em;
    const ccomplex_t IT_0029 = IT_0003*IT_0028;
    const ccomplex_t IT_0030 = conj(N_B3)*conj(N_d4)*e_em;
    const ccomplex_t IT_0031 = IT_0007*IT_0030;
    const ccomplex_t IT_0032 = conj(N_W3)*conj(N_u4)*e_em;
    const ccomplex_t IT_0033 = IT_0012*IT_0032;
    const ccomplex_t IT_0034 = conj(N_W3)*conj(N_d4)*e_em;
    const ccomplex_t IT_0035 = IT_0015*IT_0034;
    const ccomplex_t IT_0036 = conj(N_B4)*conj(N_u3)*e_em;
    const ccomplex_t IT_0037 = IT_0003*IT_0036;
    const ccomplex_t IT_0038 = conj(N_B4)*conj(N_d3)*e_em;
    const ccomplex_t IT_0039 = IT_0007*IT_0038;
    const ccomplex_t IT_0040 = conj(N_W4)*conj(N_u3)*e_em;
    const ccomplex_t IT_0041 = IT_0012*IT_0040;
    const ccomplex_t IT_0042 = conj(N_W4)*conj(N_d3)*e_em;
    const ccomplex_t IT_0043 = IT_0015*IT_0042;
    const ccomplex_t IT_0044 = 0.5*IT_0029 + (-0.5)*IT_0031 + (-0.5)*IT_0033 +
       0.5*IT_0035 + 0.5*IT_0037 + (-0.5)*IT_0039 + (-0.5)*IT_0041 + 0.5*IT_0043;
    const ccomplex_t IT_0045 = m_N_4*IT_0044;
    const ccomplex_t IT_0046 = IT_0027 + IT_0045;
    const ccomplex_t IT_0047 = conj(N_B4)*e_em;
    const ccomplex_t IT_0048 = IT_0002*IT_0047;
    const ccomplex_t IT_0049 = 1.4142135623731*IT_0048;
    const ccomplex_t IT_0050 = conj(N_W4)*e_em;
    const ccomplex_t IT_0051 = IT_0011*IT_0050;
    const ccomplex_t IT_0052 = 1.4142135623731*IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*(IT_0049 + -IT_0052);
    const ccomplex_t IT_0054 = (-0.5)*IT_0053;
    const ccomplex_t IT_0055 = pow(m_A0, 2);
    const ccomplex_t IT_0056 = pow(m_N_3, 2);
    const ccomplex_t IT_0057 = cpow((-2)*s_13 + IT_0055 + IT_0056 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0058 = IT_0054*IT_0057;
    const ccomplex_t IT_0059 = (0 + _Complex_I*1)*IT_0058;
    const ccomplex_t IT_0060 = IT_0046*IT_0059;
    const ccomplex_t IT_0061 = N_B3*N_u1*e_em;
    const ccomplex_t IT_0062 = IT_0003*IT_0061;
    const ccomplex_t IT_0063 = N_B3*N_d1*e_em;
    const ccomplex_t IT_0064 = IT_0007*IT_0063;
    const ccomplex_t IT_0065 = N_W3*N_u1*e_em;
    const ccomplex_t IT_0066 = IT_0012*IT_0065;
    const ccomplex_t IT_0067 = N_W3*N_d1*e_em;
    const ccomplex_t IT_0068 = IT_0015*IT_0067;
    const ccomplex_t IT_0069 = N_B1*N_u3*e_em;
    const ccomplex_t IT_0070 = IT_0003*IT_0069;
    const ccomplex_t IT_0071 = N_B1*N_d3*e_em;
    const ccomplex_t IT_0072 = IT_0007*IT_0071;
    const ccomplex_t IT_0073 = N_W1*N_u3*e_em;
    const ccomplex_t IT_0074 = IT_0012*IT_0073;
    const ccomplex_t IT_0075 = N_W1*N_d3*e_em;
    const ccomplex_t IT_0076 = IT_0015*IT_0075;
    const ccomplex_t IT_0077 = (-0.5)*IT_0062 + 0.5*IT_0064 + 0.5*IT_0066 + (
      -0.5)*IT_0068 + (-0.5)*IT_0070 + 0.5*IT_0072 + 0.5*IT_0074 + (-0.5)*IT_0076;
    const ccomplex_t IT_0078 = m_N_3*IT_0077;
    const ccomplex_t IT_0079 = conj(N_B1)*conj(N_u3)*e_em;
    const ccomplex_t IT_0080 = IT_0003*IT_0079;
    const ccomplex_t IT_0081 = conj(N_B1)*conj(N_d3)*e_em;
    const ccomplex_t IT_0082 = IT_0007*IT_0081;
    const ccomplex_t IT_0083 = conj(N_W1)*conj(N_u3)*e_em;
    const ccomplex_t IT_0084 = IT_0012*IT_0083;
    const ccomplex_t IT_0085 = conj(N_W1)*conj(N_d3)*e_em;
    const ccomplex_t IT_0086 = IT_0015*IT_0085;
    const ccomplex_t IT_0087 = conj(N_B3)*conj(N_u1)*e_em;
    const ccomplex_t IT_0088 = IT_0003*IT_0087;
    const ccomplex_t IT_0089 = conj(N_B3)*conj(N_d1)*e_em;
    const ccomplex_t IT_0090 = IT_0007*IT_0089;
    const ccomplex_t IT_0091 = conj(N_W3)*conj(N_u1)*e_em;
    const ccomplex_t IT_0092 = IT_0012*IT_0091;
    const ccomplex_t IT_0093 = conj(N_W3)*conj(N_d1)*e_em;
    const ccomplex_t IT_0094 = IT_0015*IT_0093;
    const ccomplex_t IT_0095 = 0.5*IT_0080 + (-0.5)*IT_0082 + (-0.5)*IT_0084 +
       0.5*IT_0086 + 0.5*IT_0088 + (-0.5)*IT_0090 + (-0.5)*IT_0092 + 0.5*IT_0094;
    const ccomplex_t IT_0096 = m_N_1*IT_0095;
    const ccomplex_t IT_0097 = IT_0078 + IT_0096;
    const ccomplex_t IT_0098 = conj(N_B1)*e_em;
    const ccomplex_t IT_0099 = IT_0002*IT_0098;
    const ccomplex_t IT_0100 = 1.4142135623731*IT_0099;
    const ccomplex_t IT_0101 = conj(N_W1)*e_em;
    const ccomplex_t IT_0102 = IT_0011*IT_0101;
    const ccomplex_t IT_0103 = 1.4142135623731*IT_0102;
    const ccomplex_t IT_0104 = (0 + _Complex_I*1)*(IT_0100 + -IT_0103);
    const ccomplex_t IT_0105 = (-0.5)*IT_0104;
    const ccomplex_t IT_0106 = cpow((-2)*s_13 + IT_0055 + IT_0056 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0107 = IT_0105*IT_0106;
    const ccomplex_t IT_0108 = (0 + _Complex_I*1)*IT_0107;
    const ccomplex_t IT_0109 = IT_0097*IT_0108;
    const ccomplex_t IT_0110 = N_B3*N_u2*e_em;
    const ccomplex_t IT_0111 = IT_0003*IT_0110;
    const ccomplex_t IT_0112 = N_B3*N_d2*e_em;
    const ccomplex_t IT_0113 = IT_0007*IT_0112;
    const ccomplex_t IT_0114 = N_W3*N_u2*e_em;
    const ccomplex_t IT_0115 = IT_0012*IT_0114;
    const ccomplex_t IT_0116 = N_W3*N_d2*e_em;
    const ccomplex_t IT_0117 = IT_0015*IT_0116;
    const ccomplex_t IT_0118 = N_B2*N_u3*e_em;
    const ccomplex_t IT_0119 = IT_0003*IT_0118;
    const ccomplex_t IT_0120 = N_B2*N_d3*e_em;
    const ccomplex_t IT_0121 = IT_0007*IT_0120;
    const ccomplex_t IT_0122 = N_W2*N_u3*e_em;
    const ccomplex_t IT_0123 = IT_0012*IT_0122;
    const ccomplex_t IT_0124 = N_W2*N_d3*e_em;
    const ccomplex_t IT_0125 = IT_0015*IT_0124;
    const ccomplex_t IT_0126 = (-0.5)*IT_0111 + 0.5*IT_0113 + 0.5*IT_0115 + (
      -0.5)*IT_0117 + (-0.5)*IT_0119 + 0.5*IT_0121 + 0.5*IT_0123 + (-0.5)*IT_0125;
    const ccomplex_t IT_0127 = m_N_3*IT_0126;
    const ccomplex_t IT_0128 = conj(N_B2)*conj(N_u3)*e_em;
    const ccomplex_t IT_0129 = IT_0003*IT_0128;
    const ccomplex_t IT_0130 = conj(N_B2)*conj(N_d3)*e_em;
    const ccomplex_t IT_0131 = IT_0007*IT_0130;
    const ccomplex_t IT_0132 = conj(N_W2)*conj(N_u3)*e_em;
    const ccomplex_t IT_0133 = IT_0012*IT_0132;
    const ccomplex_t IT_0134 = conj(N_W2)*conj(N_d3)*e_em;
    const ccomplex_t IT_0135 = IT_0015*IT_0134;
    const ccomplex_t IT_0136 = conj(N_B3)*conj(N_u2)*e_em;
    const ccomplex_t IT_0137 = IT_0003*IT_0136;
    const ccomplex_t IT_0138 = conj(N_B3)*conj(N_d2)*e_em;
    const ccomplex_t IT_0139 = IT_0007*IT_0138;
    const ccomplex_t IT_0140 = conj(N_W3)*conj(N_u2)*e_em;
    const ccomplex_t IT_0141 = IT_0012*IT_0140;
    const ccomplex_t IT_0142 = conj(N_W3)*conj(N_d2)*e_em;
    const ccomplex_t IT_0143 = IT_0015*IT_0142;
    const ccomplex_t IT_0144 = 0.5*IT_0129 + (-0.5)*IT_0131 + (-0.5)*IT_0133 +
       0.5*IT_0135 + 0.5*IT_0137 + (-0.5)*IT_0139 + (-0.5)*IT_0141 + 0.5*IT_0143;
    const ccomplex_t IT_0145 = m_N_2*IT_0144;
    const ccomplex_t IT_0146 = IT_0127 + IT_0145;
    const ccomplex_t IT_0147 = conj(N_B2)*e_em;
    const ccomplex_t IT_0148 = IT_0002*IT_0147;
    const ccomplex_t IT_0149 = 1.4142135623731*IT_0148;
    const ccomplex_t IT_0150 = conj(N_W2)*e_em;
    const ccomplex_t IT_0151 = IT_0011*IT_0150;
    const ccomplex_t IT_0152 = 1.4142135623731*IT_0151;
    const ccomplex_t IT_0153 = (0 + _Complex_I*1)*(IT_0149 + -IT_0152);
    const ccomplex_t IT_0154 = (-0.5)*IT_0153;
    const ccomplex_t IT_0155 = cpow((-2)*s_13 + IT_0055 + IT_0056 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0156 = IT_0154*IT_0155;
    const ccomplex_t IT_0157 = (0 + _Complex_I*1)*IT_0156;
    const ccomplex_t IT_0158 = IT_0146*IT_0157;
    const ccomplex_t IT_0159 = N_B3*N_u3*e_em;
    const ccomplex_t IT_0160 = IT_0003*IT_0159;
    const ccomplex_t IT_0161 = N_B3*N_d3*e_em;
    const ccomplex_t IT_0162 = IT_0007*IT_0161;
    const ccomplex_t IT_0163 = N_W3*N_u3*e_em;
    const ccomplex_t IT_0164 = IT_0012*IT_0163;
    const ccomplex_t IT_0165 = N_W3*N_d3*e_em;
    const ccomplex_t IT_0166 = IT_0015*IT_0165;
    const ccomplex_t IT_0167 = -IT_0160 + IT_0162 + IT_0164 + -IT_0166;
    const ccomplex_t IT_0168 = m_N_3*IT_0167;
    const ccomplex_t IT_0169 = conj(N_B3)*conj(N_u3)*e_em;
    const ccomplex_t IT_0170 = IT_0003*IT_0169;
    const ccomplex_t IT_0171 = conj(N_B3)*conj(N_d3)*e_em;
    const ccomplex_t IT_0172 = IT_0007*IT_0171;
    const ccomplex_t IT_0173 = conj(N_W3)*conj(N_u3)*e_em;
    const ccomplex_t IT_0174 = IT_0012*IT_0173;
    const ccomplex_t IT_0175 = conj(N_W3)*conj(N_d3)*e_em;
    const ccomplex_t IT_0176 = IT_0015*IT_0175;
    const ccomplex_t IT_0177 = IT_0170 + -IT_0172 + -IT_0174 + IT_0176;
    const ccomplex_t IT_0178 = m_N_3*IT_0177;
    const ccomplex_t IT_0179 = IT_0168 + IT_0178;
    const ccomplex_t IT_0180 = conj(N_B3)*e_em;
    const ccomplex_t IT_0181 = IT_0002*IT_0180;
    const ccomplex_t IT_0182 = 1.4142135623731*IT_0181;
    const ccomplex_t IT_0183 = conj(N_W3)*e_em;
    const ccomplex_t IT_0184 = IT_0011*IT_0183;
    const ccomplex_t IT_0185 = 1.4142135623731*IT_0184;
    const ccomplex_t IT_0186 = (0 + _Complex_I*1)*(IT_0182 + -IT_0185);
    const ccomplex_t IT_0187 = (-0.5)*IT_0186;
    const ccomplex_t IT_0188 = cpow((-2)*s_13 + IT_0055 + (0 + _Complex_I*1)
      *m_N_3*Gamma_o3 + reg_prop, -1);
    const ccomplex_t IT_0189 = IT_0187*IT_0188;
    const ccomplex_t IT_0190 = (0 + _Complex_I*1)*IT_0189;
    const ccomplex_t IT_0191 = IT_0179*IT_0190;
    const ccomplex_t IT_0192 = -IT_0060 + -IT_0109 + -IT_0158 + -IT_0191;
    const ccomplex_t IT_0193 = IT_0077*IT_0108;
    const ccomplex_t IT_0194 = IT_0026*IT_0059;
    const ccomplex_t IT_0195 = IT_0167*IT_0190;
    const ccomplex_t IT_0196 = IT_0126*IT_0157;
    const ccomplex_t IT_0197 = IT_0193 + IT_0194 + IT_0195 + IT_0196;
    const ccomplex_t IT_0198 = s_34*m_N_3;
    const ccomplex_t IT_0199 = s_14*IT_0055;
    const ccomplex_t IT_0200 = s_13*s_34;
    return create_ccomplex_return(2*IT_0192*(s_14*conj(IT_0192) + conj(IT_0197
      )*IT_0198) + 2*IT_0197*(conj(IT_0192)*IT_0198 + -conj(IT_0197)*(IT_0199 + 
      (-2)*IT_0200)));
}

