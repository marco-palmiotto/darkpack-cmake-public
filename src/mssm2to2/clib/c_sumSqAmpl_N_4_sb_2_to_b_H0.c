#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_4_sb_2_to_b_H0.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_4_sb_2_to_b_H0(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t A_b = param->A_b;
    const creal_t m_W = param->m_W;
    const creal_t m_b = param->m_b;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_H0 = param->m_H0;
    const creal_t mu_h = param->mu_h;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t alpha = param->alpha;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_sb_1 = param->m_sb_1;
    const creal_t m_sb_2 = param->m_sb_2;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_b1 = param->Gamma_b1;
    const creal_t Gamma_b2 = param->Gamma_b2;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t V_tb = param->V_tb;
    const ccomplex_t U_sb_00 = param->U_sb_00;
    const ccomplex_t U_sb_01 = param->U_sb_01;
    const ccomplex_t U_sb_10 = param->U_sb_10;
    const ccomplex_t U_sb_11 = param->U_sb_11;
    const ccomplex_t IT_0000 = cos(alpha);
    const ccomplex_t IT_0001 = cos(theta_W);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = IT_0000*IT_0002;
    const ccomplex_t IT_0004 = N_B4*N_d3*e_em;
    const ccomplex_t IT_0005 = IT_0003*IT_0004;
    const ccomplex_t IT_0006 = sin(alpha);
    const ccomplex_t IT_0007 = sin(theta_W);
    const ccomplex_t IT_0008 = cpow(IT_0007, -1);
    const ccomplex_t IT_0009 = IT_0006*IT_0008;
    const ccomplex_t IT_0010 = N_W4*N_u3*e_em;
    const ccomplex_t IT_0011 = IT_0009*IT_0010;
    const ccomplex_t IT_0012 = N_B3*N_d4*e_em;
    const ccomplex_t IT_0013 = IT_0003*IT_0012;
    const ccomplex_t IT_0014 = N_W3*N_u4*e_em;
    const ccomplex_t IT_0015 = IT_0009*IT_0014;
    const ccomplex_t IT_0016 = IT_0005 + IT_0011 + IT_0013 + IT_0015;
    const ccomplex_t IT_0017 = IT_0002*IT_0006;
    const ccomplex_t IT_0018 = N_B4*N_u3*e_em;
    const ccomplex_t IT_0019 = IT_0017*IT_0018;
    const ccomplex_t IT_0020 = IT_0000*IT_0008;
    const ccomplex_t IT_0021 = N_W4*N_d3*e_em;
    const ccomplex_t IT_0022 = IT_0020*IT_0021;
    const ccomplex_t IT_0023 = N_B3*N_u4*e_em;
    const ccomplex_t IT_0024 = IT_0017*IT_0023;
    const ccomplex_t IT_0025 = N_W3*N_d4*e_em;
    const ccomplex_t IT_0026 = IT_0020*IT_0025;
    const ccomplex_t IT_0027 = -IT_0019 + -IT_0022 + -IT_0024 + -IT_0026;
    const ccomplex_t IT_0028 = IT_0016 + IT_0027;
    const ccomplex_t IT_0029 = (0 + _Complex_I*1)*IT_0028;
    const ccomplex_t IT_0030 = 0.5*IT_0029;
    const ccomplex_t IT_0031 = m_N_3*IT_0030;
    const ccomplex_t IT_0032 = conj(N_B3)*conj(N_d4)*e_em;
    const ccomplex_t IT_0033 = IT_0003*IT_0032;
    const ccomplex_t IT_0034 = conj(N_W3)*conj(N_u4)*e_em;
    const ccomplex_t IT_0035 = IT_0009*IT_0034;
    const ccomplex_t IT_0036 = conj(N_B4)*conj(N_d3)*e_em;
    const ccomplex_t IT_0037 = IT_0003*IT_0036;
    const ccomplex_t IT_0038 = conj(N_W4)*conj(N_u3)*e_em;
    const ccomplex_t IT_0039 = IT_0009*IT_0038;
    const ccomplex_t IT_0040 = IT_0033 + IT_0035 + IT_0037 + IT_0039;
    const ccomplex_t IT_0041 = conj(N_B3)*conj(N_u4)*e_em;
    const ccomplex_t IT_0042 = IT_0017*IT_0041;
    const ccomplex_t IT_0043 = conj(N_W3)*conj(N_d4)*e_em;
    const ccomplex_t IT_0044 = IT_0020*IT_0043;
    const ccomplex_t IT_0045 = conj(N_B4)*conj(N_u3)*e_em;
    const ccomplex_t IT_0046 = IT_0017*IT_0045;
    const ccomplex_t IT_0047 = conj(N_W4)*conj(N_d3)*e_em;
    const ccomplex_t IT_0048 = IT_0020*IT_0047;
    const ccomplex_t IT_0049 = -IT_0042 + -IT_0044 + -IT_0046 + -IT_0048;
    const ccomplex_t IT_0050 = IT_0040 + IT_0049;
    const ccomplex_t IT_0051 = (0 + _Complex_I*1)*IT_0050;
    const ccomplex_t IT_0052 = 0.5*IT_0051;
    const ccomplex_t IT_0053 = m_N_4*IT_0052;
    const ccomplex_t IT_0054 = IT_0031 + IT_0053;
    const ccomplex_t IT_0055 = N_B3*e_em*U_sb_11;
    const ccomplex_t IT_0056 = IT_0002*IT_0055;
    const ccomplex_t IT_0057 = 1.4142135623731*IT_0056;
    const ccomplex_t IT_0058 = cos(beta);
    const ccomplex_t IT_0059 = cpow(IT_0058, -1);
    const ccomplex_t IT_0060 = IT_0008*IT_0059;
    const ccomplex_t IT_0061 = pow(m_W, -1);
    const ccomplex_t IT_0062 = m_b*N_d3*e_em*IT_0061*U_sb_01;
    const ccomplex_t IT_0063 = IT_0060*IT_0062;
    const ccomplex_t IT_0064 = 1.4142135623731*IT_0063;
    const ccomplex_t IT_0065 = (0 + _Complex_I*1)*(IT_0057 + 1.5*IT_0064);
    const ccomplex_t IT_0066 = 0.666666666666667*IT_0065;
    const ccomplex_t IT_0067 = 1.5*IT_0066;
    const ccomplex_t IT_0068 = 0.666666666666667*IT_0067;
    const ccomplex_t IT_0069 = 1.5*IT_0068;
    const ccomplex_t IT_0070 = 0.666666666666667*IT_0069;
    const ccomplex_t IT_0071 = 1.5*IT_0070;
    const ccomplex_t IT_0072 = (-0.333333333333333)*IT_0071;
    const ccomplex_t IT_0073 = pow(m_b, 2);
    const ccomplex_t IT_0074 = pow(m_sb_2, 2);
    const ccomplex_t IT_0075 = cpow((-2)*s_23 + IT_0073 + IT_0074 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0076 = IT_0072*IT_0075;
    const ccomplex_t IT_0077 = (0 + _Complex_I*1)*IT_0076;
    const ccomplex_t IT_0078 = IT_0054*IT_0077;
    const ccomplex_t IT_0079 = pow(m_N_4, 2);
    const ccomplex_t IT_0080 = cpow((-2)*s_13 + IT_0073 + IT_0079 + -m_sb_2*
      (m_sb_2 + (0 + _Complex_I*-1)*Gamma_b2) + reg_prop, -1);
    const ccomplex_t IT_0081 = cpow(V_tb, 2);
    const ccomplex_t IT_0082 = m_b*mu_h*IT_0061;
    const ccomplex_t IT_0083 = sin(beta);
    const ccomplex_t IT_0084 = -IT_0006*IT_0083;
    const ccomplex_t IT_0085 = IT_0000*IT_0058;
    const ccomplex_t IT_0086 = IT_0084 + IT_0085;
    const ccomplex_t IT_0087 = cpow(IT_0001, -2);
    const ccomplex_t IT_0088 = (-2)*U_sb_01*((IT_0000*IT_0008*IT_0059*IT_0061
      *IT_0073 + (-0.5)*m_W*IT_0086*(IT_0008 + 1./3*IT_0007*IT_0087))*conj
      (U_sb_01) + -1./2*IT_0006*IT_0008*IT_0059*IT_0082*conj(U_sb_11)) + U_sb_11
      *(IT_0006*IT_0008*IT_0059*IT_0082*conj(U_sb_01) + (-2)*(IT_0000*IT_0008
      *IT_0059*IT_0061*IT_0073 + -1./3*m_W*IT_0007*IT_0086*IT_0087)*conj(U_sb_11
      ));
    const ccomplex_t IT_0089 = (0 + _Complex_I*-1)*(e_em*IT_0088 + (
      -1.4142135623731)*A_b*IT_0000*IT_0081*(conj(U_sb_01)*U_sb_11 + U_sb_01
      *conj(U_sb_11)));
    const ccomplex_t IT_0090 = (-0.5)*IT_0089;
    const ccomplex_t IT_0091 = N_B4*e_em*U_sb_11;
    const ccomplex_t IT_0092 = IT_0002*IT_0091;
    const ccomplex_t IT_0093 = 1.4142135623731*IT_0092;
    const ccomplex_t IT_0094 = m_b*N_d4*e_em*IT_0061*U_sb_01;
    const ccomplex_t IT_0095 = IT_0060*IT_0094;
    const ccomplex_t IT_0096 = 1.4142135623731*IT_0095;
    const ccomplex_t IT_0097 = (0 + _Complex_I*1)*(IT_0093 + 1.5*IT_0096);
    const ccomplex_t IT_0098 = 0.666666666666667*IT_0097;
    const ccomplex_t IT_0099 = 1.5*IT_0098;
    const ccomplex_t IT_0100 = (-0.333333333333333)*IT_0099;
    const ccomplex_t IT_0101 = IT_0090*IT_0100;
    const ccomplex_t IT_0102 = IT_0080*IT_0101;
    const ccomplex_t IT_0103 = (0 + _Complex_I*1)*IT_0102;
    const ccomplex_t IT_0104 = cpow((-2)*s_13 + IT_0073 + IT_0079 + -m_sb_1*
      (m_sb_1 + (0 + _Complex_I*-1)*Gamma_b1) + reg_prop, -1);
    const ccomplex_t IT_0105 = N_B4*e_em*U_sb_10;
    const ccomplex_t IT_0106 = IT_0002*IT_0105;
    const ccomplex_t IT_0107 = 1.4142135623731*IT_0106;
    const ccomplex_t IT_0108 = m_b*N_d4*e_em*IT_0061*U_sb_00;
    const ccomplex_t IT_0109 = IT_0060*IT_0108;
    const ccomplex_t IT_0110 = 1.4142135623731*IT_0109;
    const ccomplex_t IT_0111 = (0 + _Complex_I*1)*(IT_0107 + 1.5*IT_0110);
    const ccomplex_t IT_0112 = 0.666666666666667*IT_0111;
    const ccomplex_t IT_0113 = 1.5*IT_0112;
    const ccomplex_t IT_0114 = (-0.333333333333333)*IT_0113;
    const ccomplex_t IT_0115 = (-2)*conj(U_sb_00)*((IT_0000*IT_0008*IT_0059
      *IT_0061*IT_0073 + (-0.5)*m_W*IT_0086*(IT_0008 + 1./3*IT_0007*IT_0087))
      *U_sb_01 + -1./2*IT_0006*IT_0008*IT_0059*IT_0082*U_sb_11) + conj(U_sb_10)*
      (IT_0006*IT_0008*IT_0059*IT_0082*U_sb_01 + (-2)*(IT_0000*IT_0008*IT_0059
      *IT_0061*IT_0073 + -1./3*m_W*IT_0007*IT_0086*IT_0087)*U_sb_11);
    const ccomplex_t IT_0116 = (0 + _Complex_I*-1)*(e_em*IT_0115 + (
      -1.4142135623731)*A_b*IT_0000*IT_0081*(U_sb_01*conj(U_sb_10) + conj
      (U_sb_00)*U_sb_11));
    const ccomplex_t IT_0117 = (-0.5)*IT_0116;
    const ccomplex_t IT_0118 = IT_0114*IT_0117;
    const ccomplex_t IT_0119 = IT_0104*IT_0118;
    const ccomplex_t IT_0120 = (0 + _Complex_I*1)*IT_0119;
    const ccomplex_t IT_0121 = cpow((-2)*s_12 + IT_0073 + -IT_0074 + -IT_0079 
      + -reg_prop, -1);
    const ccomplex_t IT_0122 = (0 + _Complex_I*1)*m_b*e_em*IT_0000*IT_0008
      *IT_0059*IT_0061;
    const ccomplex_t IT_0123 = (-0.5)*IT_0122;
    const ccomplex_t IT_0124 = IT_0100*IT_0123;
    const ccomplex_t IT_0125 = IT_0121*IT_0124;
    const ccomplex_t IT_0126 = (0 + _Complex_I*1)*IT_0125;
    const ccomplex_t IT_0127 = m_b*IT_0126;
    const ccomplex_t IT_0128 = N_B4*N_d4*e_em;
    const ccomplex_t IT_0129 = IT_0003*IT_0128;
    const ccomplex_t IT_0130 = N_B4*N_u4*e_em;
    const ccomplex_t IT_0131 = IT_0017*IT_0130;
    const ccomplex_t IT_0132 = N_W4*N_d4*e_em;
    const ccomplex_t IT_0133 = IT_0020*IT_0132;
    const ccomplex_t IT_0134 = N_W4*N_u4*e_em;
    const ccomplex_t IT_0135 = IT_0009*IT_0134;
    const ccomplex_t IT_0136 = (0 + _Complex_I*1)*(IT_0129 + -IT_0131 + 
      -IT_0133 + IT_0135);
    const ccomplex_t IT_0137 = m_N_4*IT_0136;
    const ccomplex_t IT_0138 = conj(N_B4)*conj(N_d4)*e_em;
    const ccomplex_t IT_0139 = IT_0003*IT_0138;
    const ccomplex_t IT_0140 = conj(N_B4)*conj(N_u4)*e_em;
    const ccomplex_t IT_0141 = IT_0017*IT_0140;
    const ccomplex_t IT_0142 = conj(N_W4)*conj(N_d4)*e_em;
    const ccomplex_t IT_0143 = IT_0020*IT_0142;
    const ccomplex_t IT_0144 = conj(N_W4)*conj(N_u4)*e_em;
    const ccomplex_t IT_0145 = IT_0009*IT_0144;
    const ccomplex_t IT_0146 = (0 + _Complex_I*1)*(IT_0139 + -IT_0141 + 
      -IT_0143 + IT_0145);
    const ccomplex_t IT_0147 = m_N_4*IT_0146;
    const ccomplex_t IT_0148 = IT_0137 + IT_0147;
    const ccomplex_t IT_0149 = cpow((-2)*s_23 + IT_0073 + IT_0074 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0150 = IT_0100*IT_0149;
    const ccomplex_t IT_0151 = (0 + _Complex_I*1)*IT_0150;
    const ccomplex_t IT_0152 = IT_0148*IT_0151;
    const ccomplex_t IT_0153 = N_B1*e_em*U_sb_11;
    const ccomplex_t IT_0154 = IT_0002*IT_0153;
    const ccomplex_t IT_0155 = 1.4142135623731*IT_0154;
    const ccomplex_t IT_0156 = m_b*N_d1*e_em*IT_0061*U_sb_01;
    const ccomplex_t IT_0157 = IT_0060*IT_0156;
    const ccomplex_t IT_0158 = 1.4142135623731*IT_0157;
    const ccomplex_t IT_0159 = (0 + _Complex_I*1)*(IT_0155 + 1.5*IT_0158);
    const ccomplex_t IT_0160 = (-0.333333333333333)*IT_0159;
    const ccomplex_t IT_0161 = cpow((-2)*s_23 + IT_0073 + IT_0074 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0162 = IT_0160*IT_0161;
    const ccomplex_t IT_0163 = (0 + _Complex_I*1)*IT_0162;
    const ccomplex_t IT_0164 = N_B4*N_d1*e_em;
    const ccomplex_t IT_0165 = IT_0003*IT_0164;
    const ccomplex_t IT_0166 = N_W4*N_u1*e_em;
    const ccomplex_t IT_0167 = IT_0009*IT_0166;
    const ccomplex_t IT_0168 = N_B1*N_d4*e_em;
    const ccomplex_t IT_0169 = IT_0003*IT_0168;
    const ccomplex_t IT_0170 = N_W1*N_u4*e_em;
    const ccomplex_t IT_0171 = IT_0009*IT_0170;
    const ccomplex_t IT_0172 = IT_0165 + IT_0167 + IT_0169 + IT_0171;
    const ccomplex_t IT_0173 = N_B4*N_u1*e_em;
    const ccomplex_t IT_0174 = IT_0017*IT_0173;
    const ccomplex_t IT_0175 = N_W4*N_d1*e_em;
    const ccomplex_t IT_0176 = IT_0020*IT_0175;
    const ccomplex_t IT_0177 = N_B1*N_u4*e_em;
    const ccomplex_t IT_0178 = IT_0017*IT_0177;
    const ccomplex_t IT_0179 = N_W1*N_d4*e_em;
    const ccomplex_t IT_0180 = IT_0020*IT_0179;
    const ccomplex_t IT_0181 = -IT_0174 + -IT_0176 + -IT_0178 + -IT_0180;
    const ccomplex_t IT_0182 = IT_0172 + IT_0181;
    const ccomplex_t IT_0183 = (0 + _Complex_I*1)*IT_0182;
    const ccomplex_t IT_0184 = 0.5*IT_0183;
    const ccomplex_t IT_0185 = m_N_1*IT_0184;
    const ccomplex_t IT_0186 = conj(N_B1)*conj(N_d4)*e_em;
    const ccomplex_t IT_0187 = IT_0003*IT_0186;
    const ccomplex_t IT_0188 = conj(N_W1)*conj(N_u4)*e_em;
    const ccomplex_t IT_0189 = IT_0009*IT_0188;
    const ccomplex_t IT_0190 = conj(N_B4)*conj(N_d1)*e_em;
    const ccomplex_t IT_0191 = IT_0003*IT_0190;
    const ccomplex_t IT_0192 = conj(N_W4)*conj(N_u1)*e_em;
    const ccomplex_t IT_0193 = IT_0009*IT_0192;
    const ccomplex_t IT_0194 = IT_0187 + IT_0189 + IT_0191 + IT_0193;
    const ccomplex_t IT_0195 = conj(N_B1)*conj(N_u4)*e_em;
    const ccomplex_t IT_0196 = IT_0017*IT_0195;
    const ccomplex_t IT_0197 = conj(N_W1)*conj(N_d4)*e_em;
    const ccomplex_t IT_0198 = IT_0020*IT_0197;
    const ccomplex_t IT_0199 = conj(N_B4)*conj(N_u1)*e_em;
    const ccomplex_t IT_0200 = IT_0017*IT_0199;
    const ccomplex_t IT_0201 = conj(N_W4)*conj(N_d1)*e_em;
    const ccomplex_t IT_0202 = IT_0020*IT_0201;
    const ccomplex_t IT_0203 = -IT_0196 + -IT_0198 + -IT_0200 + -IT_0202;
    const ccomplex_t IT_0204 = IT_0194 + IT_0203;
    const ccomplex_t IT_0205 = (0 + _Complex_I*1)*IT_0204;
    const ccomplex_t IT_0206 = 0.5*IT_0205;
    const ccomplex_t IT_0207 = m_N_4*IT_0206;
    const ccomplex_t IT_0208 = IT_0185 + IT_0207;
    const ccomplex_t IT_0209 = IT_0163*IT_0208;
    const ccomplex_t IT_0210 = N_B2*e_em*U_sb_11;
    const ccomplex_t IT_0211 = IT_0002*IT_0210;
    const ccomplex_t IT_0212 = 1.4142135623731*IT_0211;
    const ccomplex_t IT_0213 = m_b*N_d2*e_em*IT_0061*U_sb_01;
    const ccomplex_t IT_0214 = IT_0060*IT_0213;
    const ccomplex_t IT_0215 = 1.4142135623731*IT_0214;
    const ccomplex_t IT_0216 = (0 + _Complex_I*1)*(IT_0212 + 1.5*IT_0215);
    const ccomplex_t IT_0217 = 0.666666666666667*IT_0216;
    const ccomplex_t IT_0218 = 1.5*IT_0217;
    const ccomplex_t IT_0219 = (-0.333333333333333)*IT_0218;
    const ccomplex_t IT_0220 = cpow((-2)*s_23 + IT_0073 + IT_0074 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0221 = IT_0219*IT_0220;
    const ccomplex_t IT_0222 = (0 + _Complex_I*1)*IT_0221;
    const ccomplex_t IT_0223 = N_B4*N_d2*e_em;
    const ccomplex_t IT_0224 = IT_0003*IT_0223;
    const ccomplex_t IT_0225 = N_W4*N_u2*e_em;
    const ccomplex_t IT_0226 = IT_0009*IT_0225;
    const ccomplex_t IT_0227 = N_B2*N_d4*e_em;
    const ccomplex_t IT_0228 = IT_0003*IT_0227;
    const ccomplex_t IT_0229 = N_W2*N_u4*e_em;
    const ccomplex_t IT_0230 = IT_0009*IT_0229;
    const ccomplex_t IT_0231 = IT_0224 + IT_0226 + IT_0228 + IT_0230;
    const ccomplex_t IT_0232 = N_B4*N_u2*e_em;
    const ccomplex_t IT_0233 = IT_0017*IT_0232;
    const ccomplex_t IT_0234 = N_W4*N_d2*e_em;
    const ccomplex_t IT_0235 = IT_0020*IT_0234;
    const ccomplex_t IT_0236 = N_B2*N_u4*e_em;
    const ccomplex_t IT_0237 = IT_0017*IT_0236;
    const ccomplex_t IT_0238 = N_W2*N_d4*e_em;
    const ccomplex_t IT_0239 = IT_0020*IT_0238;
    const ccomplex_t IT_0240 = -IT_0233 + -IT_0235 + -IT_0237 + -IT_0239;
    const ccomplex_t IT_0241 = IT_0231 + IT_0240;
    const ccomplex_t IT_0242 = (0 + _Complex_I*1)*IT_0241;
    const ccomplex_t IT_0243 = 0.5*IT_0242;
    const ccomplex_t IT_0244 = m_N_2*IT_0243;
    const ccomplex_t IT_0245 = conj(N_B2)*conj(N_d4)*e_em;
    const ccomplex_t IT_0246 = IT_0003*IT_0245;
    const ccomplex_t IT_0247 = conj(N_W2)*conj(N_u4)*e_em;
    const ccomplex_t IT_0248 = IT_0009*IT_0247;
    const ccomplex_t IT_0249 = conj(N_B4)*conj(N_d2)*e_em;
    const ccomplex_t IT_0250 = IT_0003*IT_0249;
    const ccomplex_t IT_0251 = conj(N_W4)*conj(N_u2)*e_em;
    const ccomplex_t IT_0252 = IT_0009*IT_0251;
    const ccomplex_t IT_0253 = IT_0246 + IT_0248 + IT_0250 + IT_0252;
    const ccomplex_t IT_0254 = conj(N_B2)*conj(N_u4)*e_em;
    const ccomplex_t IT_0255 = IT_0017*IT_0254;
    const ccomplex_t IT_0256 = conj(N_W2)*conj(N_d4)*e_em;
    const ccomplex_t IT_0257 = IT_0020*IT_0256;
    const ccomplex_t IT_0258 = conj(N_B4)*conj(N_u2)*e_em;
    const ccomplex_t IT_0259 = IT_0017*IT_0258;
    const ccomplex_t IT_0260 = conj(N_W4)*conj(N_d2)*e_em;
    const ccomplex_t IT_0261 = IT_0020*IT_0260;
    const ccomplex_t IT_0262 = -IT_0255 + -IT_0257 + -IT_0259 + -IT_0261;
    const ccomplex_t IT_0263 = IT_0253 + IT_0262;
    const ccomplex_t IT_0264 = (0 + _Complex_I*1)*IT_0263;
    const ccomplex_t IT_0265 = 0.5*IT_0264;
    const ccomplex_t IT_0266 = m_N_4*IT_0265;
    const ccomplex_t IT_0267 = IT_0244 + IT_0266;
    const ccomplex_t IT_0268 = IT_0222*IT_0267;
    const ccomplex_t IT_0269 = IT_0078 + IT_0103 + IT_0120 + (-2)*IT_0127 +
       IT_0152 + IT_0209 + IT_0268;
    const ccomplex_t IT_0270 = m_N_4*IT_0030;
    const ccomplex_t IT_0271 = m_N_3*IT_0052;
    const ccomplex_t IT_0272 = IT_0270 + IT_0271;
    const ccomplex_t IT_0273 = m_b*conj(N_d3)*e_em*IT_0061*U_sb_11;
    const ccomplex_t IT_0274 = IT_0060*IT_0273;
    const ccomplex_t IT_0275 = 1.4142135623731*IT_0274;
    const ccomplex_t IT_0276 = conj(N_W3)*e_em*U_sb_01;
    const ccomplex_t IT_0277 = IT_0008*IT_0276;
    const ccomplex_t IT_0278 = 1.4142135623731*IT_0277;
    const ccomplex_t IT_0279 = conj(N_B3)*e_em*U_sb_01;
    const ccomplex_t IT_0280 = IT_0002*IT_0279;
    const ccomplex_t IT_0281 = 1.4142135623731*IT_0280;
    const ccomplex_t IT_0282 = (0 + _Complex_I*1)*(IT_0275 + -IT_0278 +
       0.333333333333333*IT_0281);
    const ccomplex_t IT_0283 = 3*IT_0282;
    const ccomplex_t IT_0284 = 0.166666666666667*IT_0283;
    const ccomplex_t IT_0285 = IT_0075*IT_0284;
    const ccomplex_t IT_0286 = (0 + _Complex_I*1)*IT_0285;
    const ccomplex_t IT_0287 = IT_0272*IT_0286;
    const ccomplex_t IT_0288 = m_N_4*IT_0243;
    const ccomplex_t IT_0289 = m_N_2*IT_0265;
    const ccomplex_t IT_0290 = IT_0288 + IT_0289;
    const ccomplex_t IT_0291 = m_b*conj(N_d2)*e_em*IT_0061*U_sb_11;
    const ccomplex_t IT_0292 = IT_0060*IT_0291;
    const ccomplex_t IT_0293 = 1.4142135623731*IT_0292;
    const ccomplex_t IT_0294 = conj(N_W2)*e_em*U_sb_01;
    const ccomplex_t IT_0295 = IT_0008*IT_0294;
    const ccomplex_t IT_0296 = 1.4142135623731*IT_0295;
    const ccomplex_t IT_0297 = conj(N_B2)*e_em*U_sb_01;
    const ccomplex_t IT_0298 = IT_0002*IT_0297;
    const ccomplex_t IT_0299 = 1.4142135623731*IT_0298;
    const ccomplex_t IT_0300 = (0 + _Complex_I*1)*(IT_0293 + -IT_0296 +
       0.333333333333333*IT_0299);
    const ccomplex_t IT_0301 = -IT_0300;
    const ccomplex_t IT_0302 = (-3)*IT_0301;
    const ccomplex_t IT_0303 = 0.166666666666667*IT_0302;
    const ccomplex_t IT_0304 = IT_0220*IT_0303;
    const ccomplex_t IT_0305 = (0 + _Complex_I*1)*IT_0304;
    const ccomplex_t IT_0306 = IT_0290*IT_0305;
    const ccomplex_t IT_0307 = (-2)*U_sb_01*((IT_0000*IT_0008*IT_0059*IT_0061
      *IT_0073 + (-0.5)*m_W*IT_0086*(IT_0008 + 1./3*IT_0007*IT_0087))*conj
      (U_sb_01) + -1./2*IT_0006*IT_0008*IT_0059*IT_0082*conj(U_sb_11)) + U_sb_11
      *(IT_0006*IT_0008*IT_0059*IT_0082*conj(U_sb_01) + (-2)*(IT_0000*IT_0008
      *IT_0059*IT_0061*IT_0073 + -1./3*m_W*IT_0007*IT_0086*IT_0087)*conj(U_sb_11
      ));
    const ccomplex_t IT_0308 = (0 + _Complex_I*-1)*(e_em*IT_0307 + (
      -1.4142135623731)*A_b*IT_0000*IT_0081*(conj(U_sb_01)*U_sb_11 + U_sb_01
      *conj(U_sb_11)));
    const ccomplex_t IT_0309 = (-0.5)*IT_0308;
    const ccomplex_t IT_0310 = m_b*conj(N_d4)*e_em*IT_0061*U_sb_11;
    const ccomplex_t IT_0311 = IT_0060*IT_0310;
    const ccomplex_t IT_0312 = 1.4142135623731*IT_0311;
    const ccomplex_t IT_0313 = conj(N_B4)*e_em*U_sb_01;
    const ccomplex_t IT_0314 = IT_0002*IT_0313;
    const ccomplex_t IT_0315 = 1.4142135623731*IT_0314;
    const ccomplex_t IT_0316 = conj(N_W4)*e_em*U_sb_01;
    const ccomplex_t IT_0317 = IT_0008*IT_0316;
    const ccomplex_t IT_0318 = 1.4142135623731*IT_0317;
    const ccomplex_t IT_0319 = (0 + _Complex_I*1)*(IT_0312 + 0.333333333333333
      *IT_0315 + -IT_0318);
    const ccomplex_t IT_0320 = 3*IT_0319;
    const ccomplex_t IT_0321 = 0.166666666666667*IT_0320;
    const ccomplex_t IT_0322 = IT_0309*IT_0321;
    const ccomplex_t IT_0323 = IT_0080*IT_0322;
    const ccomplex_t IT_0324 = (0 + _Complex_I*1)*IT_0323;
    const ccomplex_t IT_0325 = m_b*conj(N_d4)*e_em*IT_0061*U_sb_10;
    const ccomplex_t IT_0326 = IT_0060*IT_0325;
    const ccomplex_t IT_0327 = 1.4142135623731*IT_0326;
    const ccomplex_t IT_0328 = conj(N_W4)*e_em*U_sb_00;
    const ccomplex_t IT_0329 = IT_0008*IT_0328;
    const ccomplex_t IT_0330 = 1.4142135623731*IT_0329;
    const ccomplex_t IT_0331 = conj(N_B4)*e_em*U_sb_00;
    const ccomplex_t IT_0332 = IT_0002*IT_0331;
    const ccomplex_t IT_0333 = 1.4142135623731*IT_0332;
    const ccomplex_t IT_0334 = (0 + _Complex_I*1)*(IT_0327 + -IT_0330 +
       0.333333333333333*IT_0333);
    const ccomplex_t IT_0335 = 3*IT_0334;
    const ccomplex_t IT_0336 = 0.333333333333333*IT_0335;
    const ccomplex_t IT_0337 = 3*IT_0336;
    const ccomplex_t IT_0338 = 0.166666666666667*IT_0337;
    const ccomplex_t IT_0339 = (-2)*conj(U_sb_00)*((IT_0000*IT_0008*IT_0059
      *IT_0061*IT_0073 + (-0.5)*m_W*IT_0086*(IT_0008 + 1./3*IT_0007*IT_0087))
      *U_sb_01 + -1./2*IT_0006*IT_0008*IT_0059*IT_0082*U_sb_11) + conj(U_sb_10)*
      (IT_0006*IT_0008*IT_0059*IT_0082*U_sb_01 + (-2)*(IT_0000*IT_0008*IT_0059
      *IT_0061*IT_0073 + -1./3*m_W*IT_0007*IT_0086*IT_0087)*U_sb_11);
    const ccomplex_t IT_0340 = (0 + _Complex_I*-1)*(e_em*IT_0339 + (
      -1.4142135623731)*A_b*IT_0000*IT_0081*(U_sb_01*conj(U_sb_10) + conj
      (U_sb_00)*U_sb_11));
    const ccomplex_t IT_0341 = (-0.5)*IT_0340;
    const ccomplex_t IT_0342 = IT_0338*IT_0341;
    const ccomplex_t IT_0343 = IT_0104*IT_0342;
    const ccomplex_t IT_0344 = (0 + _Complex_I*1)*IT_0343;
    const ccomplex_t IT_0345 = IT_0123*IT_0321;
    const ccomplex_t IT_0346 = IT_0121*IT_0345;
    const ccomplex_t IT_0347 = (0 + _Complex_I*1)*IT_0346;
    const ccomplex_t IT_0348 = m_b*IT_0347;
    const ccomplex_t IT_0349 = IT_0149*IT_0321;
    const ccomplex_t IT_0350 = (0 + _Complex_I*1)*IT_0349;
    const ccomplex_t IT_0351 = IT_0148*IT_0350;
    const ccomplex_t IT_0352 = m_N_4*IT_0184;
    const ccomplex_t IT_0353 = m_N_1*IT_0206;
    const ccomplex_t IT_0354 = IT_0352 + IT_0353;
    const ccomplex_t IT_0355 = m_b*conj(N_d1)*e_em*IT_0061*U_sb_11;
    const ccomplex_t IT_0356 = IT_0060*IT_0355;
    const ccomplex_t IT_0357 = 1.4142135623731*IT_0356;
    const ccomplex_t IT_0358 = conj(N_W1)*e_em*U_sb_01;
    const ccomplex_t IT_0359 = IT_0008*IT_0358;
    const ccomplex_t IT_0360 = 1.4142135623731*IT_0359;
    const ccomplex_t IT_0361 = conj(N_B1)*e_em*U_sb_01;
    const ccomplex_t IT_0362 = IT_0002*IT_0361;
    const ccomplex_t IT_0363 = 1.4142135623731*IT_0362;
    const ccomplex_t IT_0364 = (0 + _Complex_I*1)*(IT_0357 + -IT_0360 +
       0.333333333333333*IT_0363);
    const ccomplex_t IT_0365 = 3*IT_0364;
    const ccomplex_t IT_0366 = 0.166666666666667*IT_0365;
    const ccomplex_t IT_0367 = IT_0161*IT_0366;
    const ccomplex_t IT_0368 = (0 + _Complex_I*1)*IT_0367;
    const ccomplex_t IT_0369 = IT_0354*IT_0368;
    const ccomplex_t IT_0370 = -IT_0287 + -IT_0306 + -IT_0324 + -IT_0344 + 2
      *IT_0348 + -IT_0351 + -IT_0369;
    const ccomplex_t IT_0371 = 6*s_13;
    const ccomplex_t IT_0372 = m_b*m_N_4;
    const ccomplex_t IT_0373 = 6*IT_0269;
    const ccomplex_t IT_0374 = IT_0184*IT_0368;
    const ccomplex_t IT_0375 = IT_0136*IT_0350;
    const ccomplex_t IT_0376 = IT_0243*IT_0305;
    const ccomplex_t IT_0377 = IT_0030*IT_0286;
    const ccomplex_t IT_0378 = -IT_0126 + IT_0374 + IT_0375 + IT_0376 + IT_0377;
    const ccomplex_t IT_0379 = m_b*s_14;
    const ccomplex_t IT_0380 = IT_0222*IT_0265;
    const ccomplex_t IT_0381 = IT_0146*IT_0151;
    const ccomplex_t IT_0382 = IT_0163*IT_0206;
    const ccomplex_t IT_0383 = IT_0052*IT_0077;
    const ccomplex_t IT_0384 = IT_0347 + -IT_0380 + -IT_0381 + -IT_0382 + 
      -IT_0383;
    const ccomplex_t IT_0385 = s_34*m_N_4;
    const ccomplex_t IT_0386 = 6*conj(IT_0269);
    const ccomplex_t IT_0387 = 6*conj(IT_0370);
    const ccomplex_t IT_0388 = pow(m_H0, 2);
    const ccomplex_t IT_0389 = m_b*m_N_4*IT_0388;
    const ccomplex_t IT_0390 = s_13*IT_0388;
    const ccomplex_t IT_0391 = s_14*s_34;
    const ccomplex_t IT_0392 = (-2)*IT_0391;
    const ccomplex_t IT_0393 = IT_0390 + IT_0392;
    const ccomplex_t IT_0394 = 6*IT_0370;
    const ccomplex_t IT_0395 = 0.166666666666667*IT_0385;
    const ccomplex_t IT_0396 = (IT_0269*conj(IT_0269) + IT_0370*conj(IT_0370))
      *IT_0371 + conj(IT_0370)*IT_0372*IT_0373 + (IT_0370*IT_0372 + IT_0378
      *IT_0379 + IT_0384*IT_0385)*IT_0386 + (IT_0379*IT_0384 + IT_0378*IT_0385)
      *IT_0387 + 6*conj(IT_0384)*(IT_0378*IT_0389 + -IT_0384*IT_0393 +
       0.166666666666667*IT_0379*IT_0394 + IT_0373*IT_0395) + 6*conj(IT_0378)*
      (0.166666666666667*IT_0373*IT_0379 + IT_0384*IT_0389 + -IT_0378*IT_0393 +
       IT_0394*IT_0395);
    return create_ccomplex_return(IT_0396);
}

