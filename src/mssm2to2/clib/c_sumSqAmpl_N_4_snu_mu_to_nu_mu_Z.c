#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_4_snu_mu_to_nu_mu_Z.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_4_snu_mu_to_nu_mu_Z(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_Z = param->m_Z;
    const creal_t e_em = param->e_em;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_snu_mu = param->m_snu_mu;
    const creal_t reg_prop = param->reg_prop;
    const creal_t Gamma_numl = param->Gamma_numl;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t IT_0000 = pow(m_Z, -2);
    const ccomplex_t IT_0001 = pow(m_Z, 2);
    const ccomplex_t IT_0002 = s_13*IT_0001;
    const ccomplex_t IT_0003 = s_14*s_34;
    const ccomplex_t IT_0004 = (-2)*IT_0003;
    const ccomplex_t IT_0005 = IT_0002 + IT_0004;
    const ccomplex_t IT_0006 = IT_0000*IT_0005;
    const ccomplex_t IT_0007 = (-0.5)*IT_0006;
    const ccomplex_t IT_0008 = s_13 + IT_0007;
    const ccomplex_t IT_0009 = cos(theta_W);
    const ccomplex_t IT_0010 = cpow(IT_0009, -1);
    const ccomplex_t IT_0011 = conj(N_B4)*e_em;
    const ccomplex_t IT_0012 = IT_0010*IT_0011;
    const ccomplex_t IT_0013 = 1.4142135623731*IT_0012;
    const ccomplex_t IT_0014 = sin(theta_W);
    const ccomplex_t IT_0015 = cpow(IT_0014, -1);
    const ccomplex_t IT_0016 = conj(N_W4)*e_em;
    const ccomplex_t IT_0017 = IT_0015*IT_0016;
    const ccomplex_t IT_0018 = 1.4142135623731*IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1)*(IT_0013 + -IT_0018);
    const ccomplex_t IT_0020 = (-0.5)*IT_0019;
    const ccomplex_t IT_0021 = pow(m_snu_mu, 2);
    const ccomplex_t IT_0022 = cpow(s_23 + (-0.5)*IT_0021 + 0.5*m_N_4*(m_N_4 +
       (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0023 = IT_0020*IT_0022;
    const ccomplex_t IT_0024 = (0 + _Complex_I*1)*IT_0023;
    const ccomplex_t IT_0025 = IT_0010*IT_0014;
    const ccomplex_t IT_0026 = N_d4*conj(N_d4)*e_em;
    const ccomplex_t IT_0027 = IT_0025*IT_0026;
    const ccomplex_t IT_0028 = IT_0009*IT_0015;
    const ccomplex_t IT_0029 = IT_0026*IT_0028;
    const ccomplex_t IT_0030 = N_u4*conj(N_u4)*e_em;
    const ccomplex_t IT_0031 = IT_0028*IT_0030;
    const ccomplex_t IT_0032 = IT_0025*IT_0030;
    const ccomplex_t IT_0033 = (0 + _Complex_I*1)*(IT_0027 + IT_0029 + 
      -IT_0031 + -IT_0032);
    const ccomplex_t IT_0034 = -IT_0033;
    const ccomplex_t IT_0035 = -IT_0034;
    const ccomplex_t IT_0036 = -IT_0035;
    const ccomplex_t IT_0037 = 0.5*IT_0036;
    const ccomplex_t IT_0038 = m_N_4*IT_0037;
    const ccomplex_t IT_0039 = (-0.5)*IT_0036;
    const ccomplex_t IT_0040 = m_N_4*IT_0039;
    const ccomplex_t IT_0041 = -IT_0040;
    const ccomplex_t IT_0042 = IT_0038 + IT_0041;
    const ccomplex_t IT_0043 = IT_0024*IT_0042;
    const ccomplex_t IT_0044 = conj(N_u1)*N_u4*e_em;
    const ccomplex_t IT_0045 = IT_0028*IT_0044;
    const ccomplex_t IT_0046 = IT_0025*IT_0044;
    const ccomplex_t IT_0047 = conj(N_d1)*N_d4*e_em;
    const ccomplex_t IT_0048 = IT_0028*IT_0047;
    const ccomplex_t IT_0049 = IT_0025*IT_0047;
    const ccomplex_t IT_0050 = (0 + _Complex_I*1)*(IT_0045 + IT_0046 + 
      -IT_0048 + -IT_0049);
    const ccomplex_t IT_0051 = -IT_0050;
    const ccomplex_t IT_0052 = -IT_0051;
    const ccomplex_t IT_0053 = -IT_0052;
    const ccomplex_t IT_0054 = (-0.5)*IT_0053;
    const ccomplex_t IT_0055 = m_N_1*IT_0054;
    const ccomplex_t IT_0056 = conj(N_B1)*e_em;
    const ccomplex_t IT_0057 = IT_0010*IT_0056;
    const ccomplex_t IT_0058 = 1.4142135623731*IT_0057;
    const ccomplex_t IT_0059 = conj(N_W1)*e_em;
    const ccomplex_t IT_0060 = IT_0015*IT_0059;
    const ccomplex_t IT_0061 = 1.4142135623731*IT_0060;
    const ccomplex_t IT_0062 = (0 + _Complex_I*1)*(IT_0058 + -IT_0061);
    const ccomplex_t IT_0063 = (-0.5)*IT_0062;
    const ccomplex_t IT_0064 = cpow(s_23 + (-0.5)*IT_0021 + 0.5*m_N_1*(m_N_1 +
       (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0065 = IT_0063*IT_0064;
    const ccomplex_t IT_0066 = (0 + _Complex_I*1)*IT_0065;
    const ccomplex_t IT_0067 = IT_0055*IT_0066;
    const ccomplex_t IT_0068 = conj(N_u2)*N_u4*e_em;
    const ccomplex_t IT_0069 = IT_0025*IT_0068;
    const ccomplex_t IT_0070 = IT_0028*IT_0068;
    const ccomplex_t IT_0071 = conj(N_d2)*N_d4*e_em;
    const ccomplex_t IT_0072 = IT_0028*IT_0071;
    const ccomplex_t IT_0073 = IT_0025*IT_0071;
    const ccomplex_t IT_0074 = (0 + _Complex_I*1)*(IT_0069 + IT_0070 + 
      -IT_0072 + -IT_0073);
    const ccomplex_t IT_0075 = -IT_0074;
    const ccomplex_t IT_0076 = -IT_0075;
    const ccomplex_t IT_0077 = -IT_0076;
    const ccomplex_t IT_0078 = (-0.5)*IT_0077;
    const ccomplex_t IT_0079 = m_N_2*IT_0078;
    const ccomplex_t IT_0080 = N_d2*conj(N_d4)*e_em;
    const ccomplex_t IT_0081 = IT_0025*IT_0080;
    const ccomplex_t IT_0082 = IT_0028*IT_0080;
    const ccomplex_t IT_0083 = N_u2*conj(N_u4)*e_em;
    const ccomplex_t IT_0084 = IT_0028*IT_0083;
    const ccomplex_t IT_0085 = IT_0025*IT_0083;
    const ccomplex_t IT_0086 = (0 + _Complex_I*1)*(IT_0081 + IT_0082 + 
      -IT_0084 + -IT_0085);
    const ccomplex_t IT_0087 = -IT_0086;
    const ccomplex_t IT_0088 = (-0.5)*IT_0087;
    const ccomplex_t IT_0089 = m_N_4*IT_0088;
    const ccomplex_t IT_0090 = -IT_0089;
    const ccomplex_t IT_0091 = IT_0079 + IT_0090;
    const ccomplex_t IT_0092 = conj(N_B2)*e_em;
    const ccomplex_t IT_0093 = IT_0010*IT_0092;
    const ccomplex_t IT_0094 = 1.4142135623731*IT_0093;
    const ccomplex_t IT_0095 = conj(N_W2)*e_em;
    const ccomplex_t IT_0096 = IT_0015*IT_0095;
    const ccomplex_t IT_0097 = 1.4142135623731*IT_0096;
    const ccomplex_t IT_0098 = (0 + _Complex_I*1)*(IT_0094 + -IT_0097);
    const ccomplex_t IT_0099 = (-0.5)*IT_0098;
    const ccomplex_t IT_0100 = cpow(s_23 + (-0.5)*IT_0021 + 0.5*m_N_2*(m_N_2 +
       (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0101 = IT_0099*IT_0100;
    const ccomplex_t IT_0102 = (0 + _Complex_I*1)*IT_0101;
    const ccomplex_t IT_0103 = IT_0091*IT_0102;
    const ccomplex_t IT_0104 = conj(N_u3)*N_u4*e_em;
    const ccomplex_t IT_0105 = IT_0025*IT_0104;
    const ccomplex_t IT_0106 = IT_0028*IT_0104;
    const ccomplex_t IT_0107 = conj(N_d3)*N_d4*e_em;
    const ccomplex_t IT_0108 = IT_0025*IT_0107;
    const ccomplex_t IT_0109 = IT_0028*IT_0107;
    const ccomplex_t IT_0110 = (0 + _Complex_I*1)*(IT_0105 + IT_0106 + 
      -IT_0108 + -IT_0109);
    const ccomplex_t IT_0111 = -IT_0110;
    const ccomplex_t IT_0112 = (-0.5)*IT_0111;
    const ccomplex_t IT_0113 = m_N_3*IT_0112;
    const ccomplex_t IT_0114 = conj(N_B3)*e_em;
    const ccomplex_t IT_0115 = IT_0010*IT_0114;
    const ccomplex_t IT_0116 = 1.4142135623731*IT_0115;
    const ccomplex_t IT_0117 = conj(N_W3)*e_em;
    const ccomplex_t IT_0118 = IT_0015*IT_0117;
    const ccomplex_t IT_0119 = 1.4142135623731*IT_0118;
    const ccomplex_t IT_0120 = (0 + _Complex_I*1)*(IT_0116 + -IT_0119);
    const ccomplex_t IT_0121 = (-0.5)*IT_0120;
    const ccomplex_t IT_0122 = cpow(s_23 + (-0.5)*IT_0021 + 0.5*m_N_3*(m_N_3 +
       (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0123 = IT_0121*IT_0122;
    const ccomplex_t IT_0124 = (0 + _Complex_I*1)*IT_0123;
    const ccomplex_t IT_0125 = IT_0113*IT_0124;
    const ccomplex_t IT_0126 = (-0.25)*IT_0043 + (-0.5)*IT_0067 + (-0.5)
      *IT_0103 + (-0.5)*IT_0125;
    const ccomplex_t IT_0127 = N_u3*conj(N_u4)*e_em;
    const ccomplex_t IT_0128 = IT_0025*IT_0127;
    const ccomplex_t IT_0129 = IT_0028*IT_0127;
    const ccomplex_t IT_0130 = N_d3*conj(N_d4)*e_em;
    const ccomplex_t IT_0131 = IT_0025*IT_0130;
    const ccomplex_t IT_0132 = IT_0028*IT_0130;
    const ccomplex_t IT_0133 = (0 + _Complex_I*1)*(IT_0128 + IT_0129 + 
      -IT_0131 + -IT_0132);
    const ccomplex_t IT_0134 = (-0.5)*IT_0133;
    const ccomplex_t IT_0135 = m_N_4*IT_0134;
    const ccomplex_t IT_0136 = IT_0124*IT_0135;
    const ccomplex_t IT_0137 = N_u1*conj(N_u4)*e_em;
    const ccomplex_t IT_0138 = IT_0025*IT_0137;
    const ccomplex_t IT_0139 = IT_0028*IT_0137;
    const ccomplex_t IT_0140 = N_d1*conj(N_d4)*e_em;
    const ccomplex_t IT_0141 = IT_0025*IT_0140;
    const ccomplex_t IT_0142 = IT_0028*IT_0140;
    const ccomplex_t IT_0143 = (0 + _Complex_I*1)*(IT_0138 + IT_0139 + 
      -IT_0141 + -IT_0142);
    const ccomplex_t IT_0144 = -IT_0143;
    const ccomplex_t IT_0145 = -IT_0144;
    const ccomplex_t IT_0146 = (-0.5)*IT_0145;
    const ccomplex_t IT_0147 = m_N_4*IT_0146;
    const ccomplex_t IT_0148 = IT_0066*IT_0147;
    const ccomplex_t IT_0149 = (-0.25)*IT_0043 + 0.5*IT_0136 + 0.5*IT_0148;
    const ccomplex_t IT_0150 = IT_0126 + IT_0149;
    const ccomplex_t IT_0151 = conj(IT_0126) + conj(IT_0149);
    const ccomplex_t IT_0152 = s_14*s_34*IT_0000;
    const ccomplex_t IT_0153 = -IT_0152;
    const ccomplex_t IT_0154 = s_13 + IT_0153;
    const ccomplex_t IT_0155 = s_13*IT_0154;
    const ccomplex_t IT_0156 = (0 + _Complex_I*1)*e_em*IT_0010*IT_0015;
    const ccomplex_t IT_0157 = -IT_0156;
    const ccomplex_t IT_0158 = pow(m_N_4, 2);
    const ccomplex_t IT_0159 = cpow(s_13 + (-0.5)*IT_0158 + (-0.5)*reg_prop +
       0.5*m_snu_mu*(m_snu_mu + (0 + _Complex_I*-1)*Gamma_numl), -1);
    const ccomplex_t IT_0160 = IT_0020*IT_0159;
    const ccomplex_t IT_0161 = (0 + _Complex_I*1)*IT_0160;
    const ccomplex_t IT_0162 = IT_0157*IT_0161;
    const ccomplex_t IT_0163 = cpow(s_12 + 0.5*IT_0021 + 0.5*IT_0158 + 0.5
      *reg_prop, -1);
    const ccomplex_t IT_0164 = e_em*IT_0025;
    const ccomplex_t IT_0165 = e_em*IT_0028;
    const ccomplex_t IT_0166 = (0 + _Complex_I*1)*(IT_0164 + IT_0165);
    const ccomplex_t IT_0167 = 0.5*IT_0166;
    const ccomplex_t IT_0168 = IT_0020*IT_0167;
    const ccomplex_t IT_0169 = IT_0163*IT_0168;
    const ccomplex_t IT_0170 = (0 + _Complex_I*1)*IT_0169;
    const ccomplex_t IT_0171 = (-0.5)*IT_0162 + -IT_0170;
    const ccomplex_t IT_0172 = IT_0088*IT_0102;
    const ccomplex_t IT_0173 = IT_0024*IT_0039;
    const ccomplex_t IT_0174 = IT_0172 + 0.5*IT_0173;
    const ccomplex_t IT_0175 = 2*IT_0174;
    const ccomplex_t IT_0176 = IT_0124*IT_0134;
    const ccomplex_t IT_0177 = IT_0024*IT_0037;
    const ccomplex_t IT_0178 = IT_0066*IT_0146;
    const ccomplex_t IT_0179 = 0.5*IT_0162 + -IT_0176 + 0.5*IT_0177 + -IT_0178;
    const ccomplex_t IT_0180 = (-2)*IT_0179;
    const ccomplex_t IT_0181 = IT_0175 + IT_0180;
    const ccomplex_t IT_0182 = 2*conj(IT_0174);
    const ccomplex_t IT_0183 = (-2)*conj(IT_0179);
    const ccomplex_t IT_0184 = IT_0182 + IT_0183;
    const ccomplex_t IT_0185 = m_N_4*IT_0154;
    const ccomplex_t IT_0186 = s_34*IT_0001;
    const ccomplex_t IT_0187 = IT_0000*IT_0186;
    const ccomplex_t IT_0188 = (-0.25)*IT_0187;
    const ccomplex_t IT_0189 = s_34 + IT_0188;
    const ccomplex_t IT_0190 = m_N_4*IT_0189;
    const ccomplex_t IT_0191 = (-4)*IT_0190;
    const ccomplex_t IT_0192 = (-0.5)*IT_0172 + (-0.25)*IT_0173;
    const ccomplex_t IT_0193 = 0.5*IT_0176 + (-0.25)*IT_0177 + 0.5*IT_0178;
    const ccomplex_t IT_0194 = 2*IT_0126;
    const ccomplex_t IT_0195 = 2*IT_0149;
    const ccomplex_t IT_0196 = IT_0194 + IT_0195;
    const ccomplex_t IT_0197 = 2*conj(IT_0126);
    const ccomplex_t IT_0198 = 2*conj(IT_0149);
    const ccomplex_t IT_0199 = IT_0197 + IT_0198;
    const ccomplex_t IT_0200 = pow(s_34, 2);
    const ccomplex_t IT_0201 = s_13*IT_0000*IT_0200;
    const ccomplex_t IT_0202 = 2*IT_0171;
    const ccomplex_t IT_0203 = m_N_4*IT_0000*IT_0200;
    const ccomplex_t IT_0204 = 2*conj(IT_0171);
    const ccomplex_t IT_0205 = s_13*s_34;
    const ccomplex_t IT_0206 = (-0.5)*IT_0170;
    const ccomplex_t IT_0207 = conj(IT_0192) + conj(IT_0206);
    const ccomplex_t IT_0208 = IT_0192 + IT_0206;
    const ccomplex_t IT_0209 = pow(m_Z, 4);
    const ccomplex_t IT_0210 = s_13*IT_0209;
    const ccomplex_t IT_0211 = IT_0000*IT_0210;
    const ccomplex_t IT_0212 = (-0.5)*IT_0211;
    const ccomplex_t IT_0213 = IT_0002 + IT_0004 + IT_0212;
    const ccomplex_t IT_0214 = -conj(IT_0193);
    const ccomplex_t IT_0215 = (-0.25)*IT_0211;
    const ccomplex_t IT_0216 = IT_0003 + IT_0215;
    const ccomplex_t IT_0217 = 0.5*IT_0187;
    const ccomplex_t IT_0218 = s_34 + IT_0217;
    const ccomplex_t IT_0219 = m_N_4*IT_0218;
    const ccomplex_t IT_0220 = 2*IT_0219;
    const ccomplex_t IT_0221 = 0.125*IT_0220;
    const ccomplex_t IT_0222 = pow(s_14, 2);
    const ccomplex_t IT_0223 = IT_0000*IT_0222;
    const ccomplex_t IT_0224 = -IT_0158;
    const ccomplex_t IT_0225 = IT_0223 + IT_0224;
    const ccomplex_t IT_0226 = s_13*IT_0225;
    const ccomplex_t IT_0227 = (-2)*conj(IT_0174);
    const ccomplex_t IT_0228 = 2*IT_0179;
    const ccomplex_t IT_0229 = (-0.5)*IT_0228;
    const ccomplex_t IT_0230 = s_13*s_14;
    const ccomplex_t IT_0231 = s_34*IT_0158;
    const ccomplex_t IT_0232 = -IT_0231;
    const ccomplex_t IT_0233 = IT_0230 + IT_0232;
    const ccomplex_t IT_0234 = 4*IT_0008*IT_0150*IT_0151 + IT_0155*(conj
      (IT_0171)*IT_0181 + IT_0171*IT_0184) + (IT_0151*IT_0181 + IT_0150*IT_0184)
      *IT_0185 + (-2)*IT_0191*(IT_0151*IT_0192 + IT_0150*conj(IT_0192) + (-0.5)
      *conj(IT_0193)*IT_0196 + (-0.5)*IT_0193*IT_0199) + conj(IT_0171)*IT_0201
      *IT_0202 + IT_0203*(IT_0151*IT_0202 + IT_0150*IT_0204) + (-2)*IT_0205*
      (conj(IT_0171)*IT_0193 + IT_0171*conj(IT_0193) + (-0.5)*IT_0202*IT_0207 + 
      (-0.5)*IT_0204*IT_0208) + (-4)*IT_0213*(IT_0206*conj(IT_0206) + (IT_0192 +
       -IT_0193)*(conj(IT_0192) + IT_0214)) + 8*conj(IT_0206)*((IT_0192 + 
      -IT_0193)*IT_0216 + IT_0196*IT_0221) + 8*IT_0206*((conj(IT_0192) + IT_0214
      )*IT_0216 + IT_0199*IT_0221) + IT_0226*(conj(IT_0174)*IT_0175 + IT_0179
      *IT_0227 + (-2)*conj(IT_0179)*(IT_0174 + IT_0229)) + (IT_0181*IT_0207 +
       IT_0184*IT_0208 + 2*IT_0193*(conj(IT_0179) + 0.5*IT_0227) + 2*IT_0214*
      (IT_0174 + IT_0229))*IT_0233;
    return create_ccomplex_return(IT_0234);
}

