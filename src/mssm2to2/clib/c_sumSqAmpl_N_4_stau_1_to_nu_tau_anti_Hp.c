#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_N_4_stau_1_to_nu_tau_anti_Hp.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_N_4_stau_1_to_nu_tau_anti_Hp(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t mu_h = param->mu_h;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t A_tau = param->A_tau;
    const creal_t m_C_1 = param->m_C_1;
    const creal_t m_C_2 = param->m_C_2;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_c1 = param->Gamma_c1;
    const creal_t Gamma_c2 = param->Gamma_c2;
    const creal_t m_stau_1 = param->m_stau_1;
    const creal_t reg_prop = param->reg_prop;
    const creal_t m_snu_tau = param->m_snu_tau;
    const creal_t Gamma_nutaul = param->Gamma_nutaul;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t U_d1 = param->U_d1;
    const ccomplex_t U_d2 = param->U_d2;
    const ccomplex_t V_u1 = param->V_u1;
    const ccomplex_t V_u2 = param->V_u2;
    const ccomplex_t U_Wm1 = param->U_Wm1;
    const ccomplex_t U_Wm2 = param->U_Wm2;
    const ccomplex_t V_Wp1 = param->V_Wp1;
    const ccomplex_t V_Wp2 = param->V_Wp2;
    const ccomplex_t U_stau_00 = param->U_stau_00;
    const ccomplex_t U_stau_10 = param->U_stau_10;
    const ccomplex_t IT_0000 = (0 + _Complex_I*1)*m_C_2;
    const ccomplex_t IT_0001 = pow(m_stau_1, 2);
    const ccomplex_t IT_0002 = cpow(s_23 + (-0.5)*IT_0001 + 0.5*m_C_2*(m_C_2 +
       (0 + _Complex_I*-1)*Gamma_c2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0003 = sin(theta_W);
    const ccomplex_t IT_0004 = cpow(IT_0003, -1);
    const ccomplex_t IT_0005 = e_em*conj(U_Wm2)*U_stau_00;
    const ccomplex_t IT_0006 = IT_0004*IT_0005;
    const ccomplex_t IT_0007 = cos(beta);
    const ccomplex_t IT_0008 = cpow(IT_0007, -1);
    const ccomplex_t IT_0009 = IT_0004*IT_0008;
    const ccomplex_t IT_0010 = pow(m_W, -1);
    const ccomplex_t IT_0011 = conj(U_d2)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0012 = IT_0009*IT_0011;
    const ccomplex_t IT_0013 = 1.4142135623731*IT_0012;
    const ccomplex_t IT_0014 = (0 + _Complex_I*1)*(IT_0006 + (-0.5)*IT_0013);
    const ccomplex_t IT_0015 = cos(theta_W);
    const ccomplex_t IT_0016 = cpow(IT_0015, -1);
    const ccomplex_t IT_0017 = IT_0007*IT_0016;
    const ccomplex_t IT_0018 = conj(N_B4)*conj(V_u2)*e_em;
    const ccomplex_t IT_0019 = IT_0017*IT_0018;
    const ccomplex_t IT_0020 = 1.4142135623731*IT_0019;
    const ccomplex_t IT_0021 = IT_0004*IT_0007;
    const ccomplex_t IT_0022 = conj(N_W4)*conj(V_u2)*e_em;
    const ccomplex_t IT_0023 = IT_0021*IT_0022;
    const ccomplex_t IT_0024 = 1.4142135623731*IT_0023;
    const ccomplex_t IT_0025 = IT_0020 + IT_0024;
    const ccomplex_t IT_0026 = conj(N_u4)*e_em*conj(V_Wp2);
    const ccomplex_t IT_0027 = IT_0021*IT_0026;
    const ccomplex_t IT_0028 = 2*IT_0027;
    const ccomplex_t IT_0029 = IT_0025 + IT_0028;
    const ccomplex_t IT_0030 = (0 + _Complex_I*1)*IT_0029;
    const ccomplex_t IT_0031 = 0.5*IT_0030;
    const ccomplex_t IT_0032 = IT_0014*IT_0031;
    const ccomplex_t IT_0033 = IT_0000*IT_0002*IT_0032;
    const ccomplex_t IT_0034 = conj(N_B4)*e_em;
    const ccomplex_t IT_0035 = IT_0016*IT_0034;
    const ccomplex_t IT_0036 = 1.4142135623731*IT_0035;
    const ccomplex_t IT_0037 = conj(N_W4)*e_em;
    const ccomplex_t IT_0038 = IT_0004*IT_0037;
    const ccomplex_t IT_0039 = 1.4142135623731*IT_0038;
    const ccomplex_t IT_0040 = (0 + _Complex_I*1)*(IT_0036 + -IT_0039);
    const ccomplex_t IT_0041 = (-0.5)*IT_0040;
    const ccomplex_t IT_0042 = sin(beta);
    const ccomplex_t IT_0043 = (0 + _Complex_I*1)*(A_tau*IT_0042*U_stau_10 + (
      -1.4142135623731)*e_em*IT_0004*(m_W*IT_0007*IT_0042*U_stau_00 + (-0.5)
      *m_tau*IT_0010*(m_tau*IT_0008*IT_0042*U_stau_00 + mu_h*U_stau_10)));
    const ccomplex_t IT_0044 = IT_0041*IT_0043;
    const ccomplex_t IT_0045 = pow(m_N_4, 2);
    const ccomplex_t IT_0046 = cpow(s_13 + (-0.5)*IT_0045 + (-0.5)*reg_prop +
       0.5*m_snu_tau*(m_snu_tau + (0 + _Complex_I*-1)*Gamma_nutaul), -1);
    const ccomplex_t IT_0047 = IT_0044*IT_0046;
    const ccomplex_t IT_0048 = (0 + _Complex_I*1)*IT_0047;
    const ccomplex_t IT_0049 = pow(m_tau, 2);
    const ccomplex_t IT_0050 = cpow(s_12 + 0.5*IT_0001 + 0.5*IT_0045 + (-0.5)
      *IT_0049 + 0.5*reg_prop, -1);
    const ccomplex_t IT_0051 = (0 + _Complex_I*1)*m_tau;
    const ccomplex_t IT_0052 = (0 + _Complex_I*1.4142135623731)*e_em*m_tau
      *IT_0004*IT_0008*IT_0010*IT_0042;
    const ccomplex_t IT_0053 = 0.5*IT_0052;
    const ccomplex_t IT_0054 = conj(N_B4)*e_em*U_stau_00;
    const ccomplex_t IT_0055 = IT_0016*IT_0054;
    const ccomplex_t IT_0056 = 1.4142135623731*IT_0055;
    const ccomplex_t IT_0057 = conj(N_W4)*e_em*U_stau_00;
    const ccomplex_t IT_0058 = IT_0004*IT_0057;
    const ccomplex_t IT_0059 = 1.4142135623731*IT_0058;
    const ccomplex_t IT_0060 = conj(N_d4)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0061 = IT_0009*IT_0060;
    const ccomplex_t IT_0062 = 1.4142135623731*IT_0061;
    const ccomplex_t IT_0063 = (0 + _Complex_I*1)*(IT_0056 + IT_0059 + 
      -IT_0062);
    const ccomplex_t IT_0064 = (-0.5)*IT_0063;
    const ccomplex_t IT_0065 = IT_0053*IT_0064;
    const ccomplex_t IT_0066 = IT_0050*IT_0051*IT_0065;
    const ccomplex_t IT_0067 = N_B4*e_em*U_stau_10;
    const ccomplex_t IT_0068 = IT_0016*IT_0067;
    const ccomplex_t IT_0069 = 1.4142135623731*IT_0068;
    const ccomplex_t IT_0070 = N_d4*e_em*m_tau*IT_0010*U_stau_00;
    const ccomplex_t IT_0071 = IT_0009*IT_0070;
    const ccomplex_t IT_0072 = 1.4142135623731*IT_0071;
    const ccomplex_t IT_0073 = (0 + _Complex_I*1)*(IT_0069 + 0.5*IT_0072);
    const ccomplex_t IT_0074 = 2*IT_0073;
    const ccomplex_t IT_0075 = 0.5*IT_0074;
    const ccomplex_t IT_0076 = -IT_0075;
    const ccomplex_t IT_0077 = IT_0053*IT_0076;
    const ccomplex_t IT_0078 = IT_0050*IT_0077;
    const ccomplex_t IT_0079 = (0 + _Complex_I*1)*IT_0078;
    const ccomplex_t IT_0080 = m_N_4*IT_0079;
    const ccomplex_t IT_0081 = conj(N_u4)*e_em*conj(V_Wp1);
    const ccomplex_t IT_0082 = IT_0021*IT_0081;
    const ccomplex_t IT_0083 = 2*IT_0082;
    const ccomplex_t IT_0084 = conj(N_B4)*conj(V_u1)*e_em;
    const ccomplex_t IT_0085 = IT_0017*IT_0084;
    const ccomplex_t IT_0086 = 1.4142135623731*IT_0085;
    const ccomplex_t IT_0087 = conj(N_W4)*conj(V_u1)*e_em;
    const ccomplex_t IT_0088 = IT_0021*IT_0087;
    const ccomplex_t IT_0089 = 1.4142135623731*IT_0088;
    const ccomplex_t IT_0090 = IT_0086 + IT_0089;
    const ccomplex_t IT_0091 = IT_0083 + IT_0090;
    const ccomplex_t IT_0092 = (0 + _Complex_I*1)*IT_0091;
    const ccomplex_t IT_0093 = 0.5*IT_0092;
    const ccomplex_t IT_0094 = e_em*conj(U_Wm1)*U_stau_00;
    const ccomplex_t IT_0095 = IT_0004*IT_0094;
    const ccomplex_t IT_0096 = conj(U_d1)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0097 = IT_0009*IT_0096;
    const ccomplex_t IT_0098 = 1.4142135623731*IT_0097;
    const ccomplex_t IT_0099 = (0 + _Complex_I*1)*(IT_0095 + (-0.5)*IT_0098);
    const ccomplex_t IT_0100 = IT_0093*IT_0099;
    const ccomplex_t IT_0101 = (0 + _Complex_I*1)*m_C_1;
    const ccomplex_t IT_0102 = cpow(s_23 + (-0.5)*IT_0001 + 0.5*m_C_1*(m_C_1 +
       (0 + _Complex_I*-1)*Gamma_c1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0103 = IT_0100*IT_0101*IT_0102;
    const ccomplex_t IT_0104 = (-0.5)*IT_0033 + 0.5*IT_0048 + (-0.5)*IT_0066 +
       0.5*IT_0080 + (-0.5)*IT_0103;
    const ccomplex_t IT_0105 = IT_0004*IT_0042;
    const ccomplex_t IT_0106 = N_d4*e_em*U_Wm1;
    const ccomplex_t IT_0107 = IT_0105*IT_0106;
    const ccomplex_t IT_0108 = (-2)*IT_0107;
    const ccomplex_t IT_0109 = IT_0016*IT_0042;
    const ccomplex_t IT_0110 = N_B4*U_d1*e_em;
    const ccomplex_t IT_0111 = IT_0109*IT_0110;
    const ccomplex_t IT_0112 = 1.4142135623731*IT_0111;
    const ccomplex_t IT_0113 = N_W4*U_d1*e_em;
    const ccomplex_t IT_0114 = IT_0105*IT_0113;
    const ccomplex_t IT_0115 = 1.4142135623731*IT_0114;
    const ccomplex_t IT_0116 = IT_0112 + IT_0115;
    const ccomplex_t IT_0117 = IT_0108 + IT_0116;
    const ccomplex_t IT_0118 = (0 + _Complex_I*1)*IT_0117;
    const ccomplex_t IT_0119 = 0.5*IT_0118;
    const ccomplex_t IT_0120 = IT_0099*IT_0119;
    const ccomplex_t IT_0121 = IT_0102*IT_0120;
    const ccomplex_t IT_0122 = (0 + _Complex_I*1)*IT_0121;
    const ccomplex_t IT_0123 = N_d4*e_em*U_Wm2;
    const ccomplex_t IT_0124 = IT_0105*IT_0123;
    const ccomplex_t IT_0125 = (-2)*IT_0124;
    const ccomplex_t IT_0126 = N_B4*U_d2*e_em;
    const ccomplex_t IT_0127 = IT_0109*IT_0126;
    const ccomplex_t IT_0128 = 1.4142135623731*IT_0127;
    const ccomplex_t IT_0129 = N_W4*U_d2*e_em;
    const ccomplex_t IT_0130 = IT_0105*IT_0129;
    const ccomplex_t IT_0131 = 1.4142135623731*IT_0130;
    const ccomplex_t IT_0132 = IT_0128 + IT_0131;
    const ccomplex_t IT_0133 = IT_0125 + IT_0132;
    const ccomplex_t IT_0134 = (0 + _Complex_I*1)*IT_0133;
    const ccomplex_t IT_0135 = 0.5*IT_0134;
    const ccomplex_t IT_0136 = IT_0014*IT_0135;
    const ccomplex_t IT_0137 = IT_0002*IT_0136;
    const ccomplex_t IT_0138 = (0 + _Complex_I*1)*IT_0137;
    const ccomplex_t IT_0139 = 0.5*IT_0079 + (-0.5)*IT_0122 + (-0.5)*IT_0138;
    const ccomplex_t IT_0140 = s_23*m_N_4;
    const ccomplex_t IT_0141 = s_12*s_23;
    const ccomplex_t IT_0142 = s_13*IT_0001;
    return create_ccomplex_return(2*IT_0104*(s_13*conj(IT_0104) + conj(IT_0139
      )*IT_0140) + 2*IT_0139*(conj(IT_0104)*IT_0140 + 2*conj(IT_0139)*(IT_0141 +
       (-0.5)*IT_0142)));
}

