#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_sc_L_anti_ss_L_to_anti_mu_nu_mu.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_sc_L_anti_ss_L_to_anti_mu_nu_mu(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t m_c = param->m_c;
    const creal_t m_s = param->m_s;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_Hp = param->m_Hp;
    const creal_t m_mu = param->m_mu;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_34 = param->s_34;
    const creal_t m_sc_L = param->m_sc_L;
    const creal_t m_ss_L = param->m_ss_L;
    const creal_t Gamma_W = param->Gamma_W;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_Hp = param->Gamma_Hp;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t V_cs = param->V_cs;
    const ccomplex_t IT_0000 = pow(m_W, -1);
    const ccomplex_t IT_0001 = cos(beta);
    const ccomplex_t IT_0002 = cpow(IT_0001, -1);
    const ccomplex_t IT_0003 = sin(beta);
    const ccomplex_t IT_0004 = sin(theta_W);
    const ccomplex_t IT_0005 = cpow(IT_0004, -1);
    const ccomplex_t IT_0006 = (0 + _Complex_I*1.4142135623731)*e_em*m_mu
      *IT_0000*IT_0002*IT_0003*IT_0005;
    const ccomplex_t IT_0007 = 0.5*IT_0006;
    const ccomplex_t IT_0008 = pow(m_s, 2);
    const ccomplex_t IT_0009 = pow(m_c, 2);
    const ccomplex_t IT_0010 = cpow(IT_0003, -1);
    const ccomplex_t IT_0011 = (0 + _Complex_I*1.4142135623731)*conj(V_cs)
      *e_em*IT_0005*(IT_0000*IT_0002*IT_0003*IT_0008 + (-2)*IT_0001*(m_W*IT_0003
       + (-0.5)*IT_0000*IT_0009*IT_0010));
    const ccomplex_t IT_0012 = 0.5*IT_0011;
    const ccomplex_t IT_0013 = IT_0007*IT_0012;
    const ccomplex_t IT_0014 = pow(m_sc_L, 2);
    const ccomplex_t IT_0015 = pow(m_ss_L, 2);
    const ccomplex_t IT_0016 = cpow((-2)*s_12 + -IT_0014 + -IT_0015 + m_Hp*
      (m_Hp + (0 + _Complex_I*-1)*Gamma_Hp) + -reg_prop, -1);
    const ccomplex_t IT_0017 = IT_0013*IT_0016;
    const ccomplex_t IT_0018 = (0 + _Complex_I*1)*IT_0017;
    const ccomplex_t IT_0019 = (0 + _Complex_I*1.4142135623731)*e_em*IT_0005;
    const ccomplex_t IT_0020 = 0.5*IT_0019;
    const ccomplex_t IT_0021 = cpow((-2)*s_12 + m_W*(m_W + (0 + _Complex_I*-1)
      *Gamma_W) + -IT_0014 + -IT_0015 + -reg_prop, -1);
    const ccomplex_t IT_0022 = IT_0020*IT_0021;
    const ccomplex_t IT_0023 = (0 + _Complex_I*1)*IT_0022;
    const ccomplex_t IT_0024 = (0 + _Complex_I*1.4142135623731)*conj(V_cs)
      *e_em*IT_0005;
    const ccomplex_t IT_0025 = (-0.5)*IT_0024;
    const ccomplex_t IT_0026 = m_mu*IT_0025;
    const ccomplex_t IT_0027 = -IT_0024;
    const ccomplex_t IT_0028 = m_mu*IT_0027;
    const ccomplex_t IT_0029 = -IT_0028;
    const ccomplex_t IT_0030 = IT_0026 + IT_0029;
    const ccomplex_t IT_0031 = IT_0023*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1.4142135623731)*e_em*m_mu
      *IT_0000*IT_0005;
    const ccomplex_t IT_0033 = (-0.5)*IT_0032;
    const ccomplex_t IT_0034 = cpow(IT_0001, 2);
    const ccomplex_t IT_0035 = cpow(IT_0003, 2);
    const ccomplex_t IT_0036 = (0 + _Complex_I*(-1.4142135623731))*conj(V_cs)
      *e_em*IT_0005*(IT_0000*(IT_0008 + -IT_0009) + -m_W*(IT_0034 + -IT_0035));
    const ccomplex_t IT_0037 = 0.5*IT_0036;
    const ccomplex_t IT_0038 = IT_0033*IT_0037;
    const ccomplex_t IT_0039 = pow(m_W, 2);
    const ccomplex_t IT_0040 = cpow((-2)*s_12 + -IT_0014 + -IT_0015 + IT_0039 
      + -reg_prop, -1);
    const ccomplex_t IT_0041 = IT_0038*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*IT_0041;
    const ccomplex_t IT_0043 = -IT_0018 + IT_0031 + -IT_0042;
    const ccomplex_t IT_0044 = IT_0023*IT_0027;
    const ccomplex_t IT_0045 = -IT_0044;
    const ccomplex_t IT_0046 = m_mu*s_14;
    const ccomplex_t IT_0047 = s_13*s_14;
    const ccomplex_t IT_0048 = s_34*IT_0014;
    return create_ccomplex_return(6*IT_0043*(s_34*conj(IT_0043) + -conj
      (IT_0045)*IT_0046) + (-6)*IT_0045*(conj(IT_0043)*IT_0046 + (
      -0.166666666666667)*conj(IT_0045)*(12*IT_0047 + (-6)*IT_0048)));
}

