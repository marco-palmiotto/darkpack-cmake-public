#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_sd_L_anti_sc_L_to_anti_c_d.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_sd_L_anti_sc_L_to_anti_c_d(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t g_s = param->g_s;
    const creal_t m_W = param->m_W;
    const creal_t m_c = param->m_c;
    const creal_t m_d = param->m_d;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t m_Hp = param->m_Hp;
    const creal_t m_sG = param->m_sG;
    const creal_t s_12 = param->s_12;
    const creal_t s_13 = param->s_13;
    const creal_t s_14 = param->s_14;
    const creal_t s_23 = param->s_23;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_sc_L = param->m_sc_L;
    const creal_t m_sd_L = param->m_sd_L;
    const creal_t Gamma_W = param->Gamma_W;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_Hp = param->Gamma_Hp;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t N_u1 = param->N_u1;
    const ccomplex_t N_u2 = param->N_u2;
    const ccomplex_t N_u3 = param->N_u3;
    const ccomplex_t N_u4 = param->N_u4;
    const ccomplex_t V_cd = param->V_cd;
    const ccomplex_t IT_0000 = cos(theta_W);
    const ccomplex_t IT_0001 = cpow(IT_0000, -1);
    const ccomplex_t IT_0002 = N_B1*e_em;
    const ccomplex_t IT_0003 = IT_0001*IT_0002;
    const ccomplex_t IT_0004 = 1.4142135623731*IT_0003;
    const ccomplex_t IT_0005 = sin(theta_W);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = N_W1*e_em;
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = 1.4142135623731*IT_0008;
    const ccomplex_t IT_0010 = (0 + _Complex_I*1)*(IT_0004 + 3*IT_0009);
    const ccomplex_t IT_0011 = 0.166666666666667*IT_0010;
    const ccomplex_t IT_0012 = pow(m_W, -1);
    const ccomplex_t IT_0013 = cos(beta);
    const ccomplex_t IT_0014 = cpow(IT_0013, -1);
    const ccomplex_t IT_0015 = (0 + _Complex_I*1.4142135623731)*m_d*N_d1*e_em
      *IT_0006*IT_0012*IT_0014;
    const ccomplex_t IT_0016 = (-0.5)*IT_0015;
    const ccomplex_t IT_0017 = IT_0011*IT_0016;
    const ccomplex_t IT_0018 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0019 = pow(m_c, 2);
    const ccomplex_t IT_0020 = pow(m_sc_L, 2);
    const ccomplex_t IT_0021 = cpow((-2)*s_23 + IT_0019 + IT_0020 + -m_N_1*
      (m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + reg_prop, -1);
    const ccomplex_t IT_0022 = IT_0017*IT_0018*IT_0021;
    const ccomplex_t IT_0023 = N_B2*e_em;
    const ccomplex_t IT_0024 = IT_0001*IT_0023;
    const ccomplex_t IT_0025 = 1.4142135623731*IT_0024;
    const ccomplex_t IT_0026 = N_W2*e_em;
    const ccomplex_t IT_0027 = IT_0006*IT_0026;
    const ccomplex_t IT_0028 = 1.4142135623731*IT_0027;
    const ccomplex_t IT_0029 = (0 + _Complex_I*1)*(IT_0025 + 3*IT_0028);
    const ccomplex_t IT_0030 = 0.166666666666667*IT_0029;
    const ccomplex_t IT_0031 = (0 + _Complex_I*1.4142135623731)*m_d*N_d2*e_em
      *IT_0006*IT_0012*IT_0014;
    const ccomplex_t IT_0032 = (-0.5)*IT_0031;
    const ccomplex_t IT_0033 = IT_0030*IT_0032;
    const ccomplex_t IT_0034 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0035 = cpow((-2)*s_23 + IT_0019 + IT_0020 + -m_N_2*
      (m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + reg_prop, -1);
    const ccomplex_t IT_0036 = IT_0033*IT_0034*IT_0035;
    const ccomplex_t IT_0037 = N_B3*e_em;
    const ccomplex_t IT_0038 = IT_0001*IT_0037;
    const ccomplex_t IT_0039 = 1.4142135623731*IT_0038;
    const ccomplex_t IT_0040 = N_W3*e_em;
    const ccomplex_t IT_0041 = IT_0006*IT_0040;
    const ccomplex_t IT_0042 = 1.4142135623731*IT_0041;
    const ccomplex_t IT_0043 = (0 + _Complex_I*1)*(IT_0039 + 3*IT_0042);
    const ccomplex_t IT_0044 = 0.166666666666667*IT_0043;
    const ccomplex_t IT_0045 = (0 + _Complex_I*1.4142135623731)*m_d*N_d3*e_em
      *IT_0006*IT_0012*IT_0014;
    const ccomplex_t IT_0046 = (-0.5)*IT_0045;
    const ccomplex_t IT_0047 = IT_0044*IT_0046;
    const ccomplex_t IT_0048 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0049 = cpow((-2)*s_23 + IT_0019 + IT_0020 + -m_N_3*
      (m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + reg_prop, -1);
    const ccomplex_t IT_0050 = IT_0047*IT_0048*IT_0049;
    const ccomplex_t IT_0051 = N_B4*e_em;
    const ccomplex_t IT_0052 = IT_0001*IT_0051;
    const ccomplex_t IT_0053 = 1.4142135623731*IT_0052;
    const ccomplex_t IT_0054 = N_W4*e_em;
    const ccomplex_t IT_0055 = IT_0006*IT_0054;
    const ccomplex_t IT_0056 = 1.4142135623731*IT_0055;
    const ccomplex_t IT_0057 = (0 + _Complex_I*1)*(IT_0053 + 3*IT_0056);
    const ccomplex_t IT_0058 = 0.166666666666667*IT_0057;
    const ccomplex_t IT_0059 = (0 + _Complex_I*1.4142135623731)*m_d*N_d4*e_em
      *IT_0006*IT_0012*IT_0014;
    const ccomplex_t IT_0060 = (-0.5)*IT_0059;
    const ccomplex_t IT_0061 = IT_0058*IT_0060;
    const ccomplex_t IT_0062 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0063 = cpow((-2)*s_23 + IT_0019 + IT_0020 + -m_N_4*
      (m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + reg_prop, -1);
    const ccomplex_t IT_0064 = IT_0061*IT_0062*IT_0063;
    const ccomplex_t IT_0065 = conj(N_B1)*e_em;
    const ccomplex_t IT_0066 = IT_0001*IT_0065;
    const ccomplex_t IT_0067 = 1.4142135623731*IT_0066;
    const ccomplex_t IT_0068 = conj(N_W1)*e_em;
    const ccomplex_t IT_0069 = IT_0006*IT_0068;
    const ccomplex_t IT_0070 = 1.4142135623731*IT_0069;
    const ccomplex_t IT_0071 = (0 + _Complex_I*1)*(IT_0067 + (-3)*IT_0070);
    const ccomplex_t IT_0072 = 0.166666666666667*IT_0071;
    const ccomplex_t IT_0073 = IT_0011*IT_0072;
    const ccomplex_t IT_0074 = IT_0021*IT_0073;
    const ccomplex_t IT_0075 = (0 + _Complex_I*1)*IT_0074;
    const ccomplex_t IT_0076 = m_d*IT_0075;
    const ccomplex_t IT_0077 = conj(N_B2)*e_em;
    const ccomplex_t IT_0078 = IT_0001*IT_0077;
    const ccomplex_t IT_0079 = 1.4142135623731*IT_0078;
    const ccomplex_t IT_0080 = conj(N_W2)*e_em;
    const ccomplex_t IT_0081 = IT_0006*IT_0080;
    const ccomplex_t IT_0082 = 1.4142135623731*IT_0081;
    const ccomplex_t IT_0083 = (0 + _Complex_I*1)*(IT_0079 + (-3)*IT_0082);
    const ccomplex_t IT_0084 = 0.166666666666667*IT_0083;
    const ccomplex_t IT_0085 = IT_0030*IT_0084;
    const ccomplex_t IT_0086 = IT_0035*IT_0085;
    const ccomplex_t IT_0087 = (0 + _Complex_I*1)*IT_0086;
    const ccomplex_t IT_0088 = m_d*IT_0087;
    const ccomplex_t IT_0089 = conj(N_B3)*e_em;
    const ccomplex_t IT_0090 = IT_0001*IT_0089;
    const ccomplex_t IT_0091 = 1.4142135623731*IT_0090;
    const ccomplex_t IT_0092 = conj(N_W3)*e_em;
    const ccomplex_t IT_0093 = IT_0006*IT_0092;
    const ccomplex_t IT_0094 = 1.4142135623731*IT_0093;
    const ccomplex_t IT_0095 = (0 + _Complex_I*1)*(IT_0091 + (-3)*IT_0094);
    const ccomplex_t IT_0096 = 0.166666666666667*IT_0095;
    const ccomplex_t IT_0097 = IT_0044*IT_0096;
    const ccomplex_t IT_0098 = IT_0049*IT_0097;
    const ccomplex_t IT_0099 = (0 + _Complex_I*1)*IT_0098;
    const ccomplex_t IT_0100 = m_d*IT_0099;
    const ccomplex_t IT_0101 = conj(N_B4)*e_em;
    const ccomplex_t IT_0102 = IT_0001*IT_0101;
    const ccomplex_t IT_0103 = 1.4142135623731*IT_0102;
    const ccomplex_t IT_0104 = conj(N_W4)*e_em;
    const ccomplex_t IT_0105 = IT_0006*IT_0104;
    const ccomplex_t IT_0106 = 1.4142135623731*IT_0105;
    const ccomplex_t IT_0107 = (0 + _Complex_I*1)*(IT_0103 + (-3)*IT_0106);
    const ccomplex_t IT_0108 = 0.166666666666667*IT_0107;
    const ccomplex_t IT_0109 = IT_0058*IT_0108;
    const ccomplex_t IT_0110 = IT_0063*IT_0109;
    const ccomplex_t IT_0111 = (0 + _Complex_I*1)*IT_0110;
    const ccomplex_t IT_0112 = m_d*IT_0111;
    const ccomplex_t IT_0113 = pow(m_sG, 2);
    const ccomplex_t IT_0114 = cpow((-2)*s_23 + IT_0019 + IT_0020 + -IT_0113 +
       reg_prop, -1);
    const ccomplex_t IT_0115 = (0 + _Complex_I*1.4142135623731)*g_s;
    const ccomplex_t IT_0116 = cpow(IT_0115, 2);
    const ccomplex_t IT_0117 = (0 + _Complex_I*1)*IT_0116;
    const ccomplex_t IT_0118 = IT_0114*IT_0117;
    const ccomplex_t IT_0119 = m_d*IT_0118;
    const ccomplex_t IT_0120 = -IT_0022 + -IT_0036 + -IT_0050 + -IT_0064 +
       IT_0076 + IT_0088 + IT_0100 + IT_0112 + (-0.166666666666667)*IT_0119;
    const ccomplex_t IT_0121 = (0 + _Complex_I*1.4142135623731)*conj(V_cd)
      *e_em*IT_0006;
    const ccomplex_t IT_0122 = 0.5*IT_0121;
    const ccomplex_t IT_0123 = pow(m_sd_L, 2);
    const ccomplex_t IT_0124 = cpow((-2)*s_12 + m_W*(m_W + (0 + _Complex_I*-1)
      *Gamma_W) + -IT_0020 + -IT_0123 + -reg_prop, -1);
    const ccomplex_t IT_0125 = IT_0122*IT_0124;
    const ccomplex_t IT_0126 = (0 + _Complex_I*1)*IT_0125;
    const ccomplex_t IT_0127 = (0 + _Complex_I*1.4142135623731)*V_cd*e_em
      *IT_0006;
    const ccomplex_t IT_0128 = (-0.5)*IT_0127;
    const ccomplex_t IT_0129 = m_d*IT_0128;
    const ccomplex_t IT_0130 = -IT_0127;
    const ccomplex_t IT_0131 = m_d*IT_0130;
    const ccomplex_t IT_0132 = -IT_0131;
    const ccomplex_t IT_0133 = IT_0129 + IT_0132;
    const ccomplex_t IT_0134 = IT_0126*IT_0133;
    const ccomplex_t IT_0135 = pow(m_d, 2);
    const ccomplex_t IT_0136 = sin(beta);
    const ccomplex_t IT_0137 = cpow(IT_0136, -1);
    const ccomplex_t IT_0138 = (0 + _Complex_I*1.4142135623731)*V_cd*e_em
      *IT_0006*(IT_0012*IT_0014*IT_0135*IT_0136 + (-2)*IT_0013*(m_W*IT_0136 + (
      -0.5)*IT_0012*IT_0019*IT_0137));
    const ccomplex_t IT_0139 = 0.5*IT_0138;
    const ccomplex_t IT_0140 = (0 + _Complex_I*1.4142135623731)*m_d*conj(V_cd)
      *e_em*IT_0006*IT_0012*IT_0014*IT_0136;
    const ccomplex_t IT_0141 = 0.5*IT_0140;
    const ccomplex_t IT_0142 = IT_0139*IT_0141;
    const ccomplex_t IT_0143 = cpow((-2)*s_12 + -IT_0020 + -IT_0123 + m_Hp*
      (m_Hp + (0 + _Complex_I*-1)*Gamma_Hp) + -reg_prop, -1);
    const ccomplex_t IT_0144 = IT_0142*IT_0143;
    const ccomplex_t IT_0145 = (0 + _Complex_I*1)*IT_0144;
    const ccomplex_t IT_0146 = (0 + _Complex_I*1.4142135623731)*m_d*conj(V_cd)
      *e_em*IT_0006*IT_0012;
    const ccomplex_t IT_0147 = (-0.5)*IT_0146;
    const ccomplex_t IT_0148 = cpow(IT_0013, 2);
    const ccomplex_t IT_0149 = cpow(IT_0136, 2);
    const ccomplex_t IT_0150 = (0 + _Complex_I*1.4142135623731)*V_cd*e_em
      *IT_0006*(IT_0012*(IT_0019 + -IT_0135) + m_W*(IT_0148 + -IT_0149));
    const ccomplex_t IT_0151 = 0.5*IT_0150;
    const ccomplex_t IT_0152 = IT_0147*IT_0151;
    const ccomplex_t IT_0153 = pow(m_W, 2);
    const ccomplex_t IT_0154 = cpow((-2)*s_12 + -IT_0020 + -IT_0123 + IT_0153 
      + -reg_prop, -1);
    const ccomplex_t IT_0155 = IT_0152*IT_0154;
    const ccomplex_t IT_0156 = (0 + _Complex_I*1)*IT_0155;
    const ccomplex_t IT_0157 = 0.5*IT_0119 + -IT_0134 + -IT_0145 + -IT_0156;
    const ccomplex_t IT_0158 = (0 + _Complex_I*1.4142135623731)*m_c*conj(N_u1)
      *e_em*IT_0006*IT_0012*IT_0137;
    const ccomplex_t IT_0159 = (-0.5)*IT_0158;
    const ccomplex_t IT_0160 = IT_0072*IT_0159;
    const ccomplex_t IT_0161 = IT_0018*IT_0021*IT_0160;
    const ccomplex_t IT_0162 = (0 + _Complex_I*1.4142135623731)*m_c*conj(N_u2)
      *e_em*IT_0006*IT_0012*IT_0137;
    const ccomplex_t IT_0163 = (-0.5)*IT_0162;
    const ccomplex_t IT_0164 = IT_0084*IT_0163;
    const ccomplex_t IT_0165 = IT_0034*IT_0035*IT_0164;
    const ccomplex_t IT_0166 = (0 + _Complex_I*1.4142135623731)*m_c*conj(N_u3)
      *e_em*IT_0006*IT_0012*IT_0137;
    const ccomplex_t IT_0167 = (-0.5)*IT_0166;
    const ccomplex_t IT_0168 = IT_0096*IT_0167;
    const ccomplex_t IT_0169 = IT_0048*IT_0049*IT_0168;
    const ccomplex_t IT_0170 = (0 + _Complex_I*1.4142135623731)*m_c*conj(N_u4)
      *e_em*IT_0006*IT_0012*IT_0137;
    const ccomplex_t IT_0171 = (-0.5)*IT_0170;
    const ccomplex_t IT_0172 = IT_0108*IT_0171;
    const ccomplex_t IT_0173 = IT_0062*IT_0063*IT_0172;
    const ccomplex_t IT_0174 = IT_0016*IT_0159;
    const ccomplex_t IT_0175 = IT_0021*IT_0174;
    const ccomplex_t IT_0176 = (0 + _Complex_I*1)*IT_0175;
    const ccomplex_t IT_0177 = m_d*IT_0176;
    const ccomplex_t IT_0178 = IT_0032*IT_0163;
    const ccomplex_t IT_0179 = IT_0035*IT_0178;
    const ccomplex_t IT_0180 = (0 + _Complex_I*1)*IT_0179;
    const ccomplex_t IT_0181 = m_d*IT_0180;
    const ccomplex_t IT_0182 = IT_0046*IT_0167;
    const ccomplex_t IT_0183 = IT_0049*IT_0182;
    const ccomplex_t IT_0184 = (0 + _Complex_I*1)*IT_0183;
    const ccomplex_t IT_0185 = m_d*IT_0184;
    const ccomplex_t IT_0186 = IT_0060*IT_0171;
    const ccomplex_t IT_0187 = IT_0063*IT_0186;
    const ccomplex_t IT_0188 = (0 + _Complex_I*1)*IT_0187;
    const ccomplex_t IT_0189 = m_d*IT_0188;
    const ccomplex_t IT_0190 = -IT_0161 + -IT_0165 + -IT_0169 + -IT_0173 +
       IT_0177 + IT_0181 + IT_0185 + IT_0189;
    const ccomplex_t IT_0191 = (0 + _Complex_I*1.4142135623731)*m_c*conj(V_cd)
      *e_em*IT_0006*IT_0012*IT_0013*IT_0137;
    const ccomplex_t IT_0192 = 0.5*IT_0191;
    const ccomplex_t IT_0193 = IT_0139*IT_0192;
    const ccomplex_t IT_0194 = IT_0143*IT_0193;
    const ccomplex_t IT_0195 = (0 + _Complex_I*1)*IT_0194;
    const ccomplex_t IT_0196 = m_c*IT_0128;
    const ccomplex_t IT_0197 = m_c*IT_0130;
    const ccomplex_t IT_0198 = -IT_0197;
    const ccomplex_t IT_0199 = IT_0196 + IT_0198;
    const ccomplex_t IT_0200 = IT_0126*IT_0199;
    const ccomplex_t IT_0201 = (0 + _Complex_I*1.4142135623731)*m_c*conj(V_cd)
      *e_em*IT_0006*IT_0012;
    const ccomplex_t IT_0202 = 0.5*IT_0201;
    const ccomplex_t IT_0203 = IT_0151*IT_0202;
    const ccomplex_t IT_0204 = IT_0154*IT_0203;
    const ccomplex_t IT_0205 = (0 + _Complex_I*1)*IT_0204;
    const ccomplex_t IT_0206 = -IT_0195 + IT_0200 + -IT_0205;
    const ccomplex_t IT_0207 = 6*s_34;
    const ccomplex_t IT_0208 = 18*s_34;
    const ccomplex_t IT_0209 = s_34*IT_0123;
    const ccomplex_t IT_0210 = (-6)*IT_0209;
    const ccomplex_t IT_0211 = s_13*s_14;
    const ccomplex_t IT_0212 = 12*IT_0211;
    const ccomplex_t IT_0213 = IT_0210 + IT_0212;
    const ccomplex_t IT_0214 = IT_0126*IT_0130;
    const ccomplex_t IT_0215 = -IT_0214;
    const ccomplex_t IT_0216 = -IT_0087 + -IT_0099 + -IT_0111;
    const ccomplex_t IT_0217 = -IT_0075 + 0.166666666666667*IT_0118;
    const ccomplex_t IT_0218 = IT_0216 + IT_0217;
    const ccomplex_t IT_0219 = conj(IT_0216) + conj(IT_0217);
    const ccomplex_t IT_0220 = (-0.5)*IT_0118;
    const ccomplex_t IT_0221 = -IT_0220;
    const ccomplex_t IT_0222 = (-18)*IT_0209;
    const ccomplex_t IT_0223 = 36*IT_0211;
    const ccomplex_t IT_0224 = IT_0222 + IT_0223;
    const ccomplex_t IT_0225 = -IT_0176 + -IT_0180 + -IT_0184 + -IT_0188;
    const ccomplex_t IT_0226 = m_c*m_d*IT_0123;
    const ccomplex_t IT_0227 = (-18)*conj(IT_0217);
    const ccomplex_t IT_0228 = 6*IT_0220;
    const ccomplex_t IT_0229 = (-6)*conj(IT_0215);
    const ccomplex_t IT_0230 = (-18)*conj(IT_0216) + IT_0227 + IT_0228 +
       IT_0229;
    const ccomplex_t IT_0231 = (-6)*IT_0220;
    const ccomplex_t IT_0232 = m_c*m_d;
    const ccomplex_t IT_0233 = (-18)*IT_0120;
    const ccomplex_t IT_0234 = (-6)*IT_0157;
    const ccomplex_t IT_0235 = IT_0233 + IT_0234;
    const ccomplex_t IT_0236 = (-18)*conj(IT_0120);
    const ccomplex_t IT_0237 = (-6)*conj(IT_0157);
    const ccomplex_t IT_0238 = IT_0236 + IT_0237;
    const ccomplex_t IT_0239 = (-18)*IT_0157;
    const ccomplex_t IT_0240 = (-0.166666666666667)*IT_0239;
    const ccomplex_t IT_0241 = m_d*s_13;
    const ccomplex_t IT_0242 = 6*IT_0157;
    const ccomplex_t IT_0243 = 18*IT_0120;
    const ccomplex_t IT_0244 = 6*conj(IT_0157);
    const ccomplex_t IT_0245 = 18*conj(IT_0120);
    const ccomplex_t IT_0246 = 6*conj(IT_0120);
    const ccomplex_t IT_0247 = 18*conj(IT_0157);
    const ccomplex_t IT_0248 = (IT_0120 + 3*IT_0157)*conj(IT_0215) +
       0.166666666666667*(18*conj(IT_0190) + 6*conj(IT_0206))*IT_0225 +
       0.166666666666667*(18*IT_0190 + 6*IT_0206)*conj(IT_0225) +
       0.166666666666667*IT_0219*(IT_0242 + IT_0243) + 0.166666666666667*IT_0218
      *(IT_0244 + IT_0245) + 0.166666666666667*IT_0215*(IT_0246 + IT_0247) +
       IT_0221*(IT_0120 + IT_0240 + (-0.166666666666667)*IT_0246 + (
      -0.166666666666667)*IT_0247);
    const ccomplex_t IT_0249 = m_c*s_14;
    const ccomplex_t IT_0250 = (-6)*IT_0206;
    const ccomplex_t IT_0251 = (-18)*conj(IT_0190);
    const ccomplex_t IT_0252 = (-6)*conj(IT_0206);
    const ccomplex_t IT_0253 = (-18)*conj(IT_0206);
    const ccomplex_t IT_0254 = (-0.166666666666667)*IT_0253;
    const ccomplex_t IT_0255 = (-18)*IT_0206*conj(IT_0215) + IT_0190*IT_0230 +
       conj(IT_0225)*IT_0235 + IT_0225*IT_0238 + IT_0219*IT_0250 + IT_0218*
      (IT_0251 + IT_0252) + (-6)*IT_0215*(conj(IT_0190) + IT_0254) + IT_0231*
      (conj(IT_0190) + (-3)*IT_0206 + IT_0254);
    const ccomplex_t IT_0256 = (conj(IT_0120)*IT_0157 + IT_0120*conj(IT_0157) 
      + conj(IT_0190)*IT_0206 + IT_0190*conj(IT_0206))*IT_0207 + (IT_0120*conj
      (IT_0120) + IT_0157*conj(IT_0157) + IT_0190*conj(IT_0190) + IT_0206*conj
      (IT_0206))*IT_0208 + IT_0213*(conj(IT_0215)*IT_0218 + IT_0215*IT_0219 + 
      (IT_0216 + -conj(IT_0216) + IT_0217 + -conj(IT_0217))*IT_0221) + IT_0224*(
      (IT_0216 + IT_0217)*IT_0219 + (IT_0215 + IT_0220)*(conj(IT_0215) + IT_0221
      ) + IT_0225*conj(IT_0225)) + IT_0226*(IT_0225*IT_0230 + (-6)*conj(IT_0225)
      *(IT_0215 + 3*IT_0216 + 3*IT_0217 + -1./6*IT_0231)) + (-6)*IT_0232*((conj
      (IT_0120) + 3*conj(IT_0157))*IT_0206 + (-0.166666666666667)*conj(IT_0190)
      *IT_0235 + (-0.166666666666667)*IT_0190*IT_0238 + conj(IT_0206)*(IT_0120 +
       IT_0240)) + 6*IT_0241*IT_0248 + IT_0249*IT_0255;
    return create_ccomplex_return(IT_0256);
}

