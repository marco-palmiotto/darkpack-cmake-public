#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_stau_1_stau_2_to_tau_tau.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_stau_1_stau_2_to_tau_tau(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t s_24 = param->s_24;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_stau_1 = param->m_stau_1;
    const creal_t m_stau_2 = param->m_stau_2;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t U_stau_00 = param->U_stau_00;
    const ccomplex_t U_stau_01 = param->U_stau_01;
    const ccomplex_t U_stau_10 = param->U_stau_10;
    const ccomplex_t U_stau_11 = param->U_stau_11;
    const ccomplex_t IT_0000 = cos(theta_W);
    const ccomplex_t IT_0001 = cpow(IT_0000, -1);
    const ccomplex_t IT_0002 = N_B3*e_em*U_stau_10;
    const ccomplex_t IT_0003 = IT_0001*IT_0002;
    const ccomplex_t IT_0004 = 1.4142135623731*IT_0003;
    const ccomplex_t IT_0005 = cos(beta);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = sin(theta_W);
    const ccomplex_t IT_0008 = cpow(IT_0007, -1);
    const ccomplex_t IT_0009 = IT_0006*IT_0008;
    const ccomplex_t IT_0010 = pow(m_W, -1);
    const ccomplex_t IT_0011 = N_d3*e_em*m_tau*IT_0010*U_stau_00;
    const ccomplex_t IT_0012 = IT_0009*IT_0011;
    const ccomplex_t IT_0013 = 1.4142135623731*IT_0012;
    const ccomplex_t IT_0014 = (0 + _Complex_I*1)*(IT_0004 + 0.5*IT_0013);
    const ccomplex_t IT_0015 = -IT_0014;
    const ccomplex_t IT_0016 = conj(N_B3)*e_em*U_stau_01;
    const ccomplex_t IT_0017 = IT_0001*IT_0016;
    const ccomplex_t IT_0018 = 1.4142135623731*IT_0017;
    const ccomplex_t IT_0019 = conj(N_W3)*e_em*U_stau_01;
    const ccomplex_t IT_0020 = IT_0008*IT_0019;
    const ccomplex_t IT_0021 = 1.4142135623731*IT_0020;
    const ccomplex_t IT_0022 = conj(N_d3)*e_em*m_tau*IT_0010*U_stau_11;
    const ccomplex_t IT_0023 = IT_0009*IT_0022;
    const ccomplex_t IT_0024 = 1.4142135623731*IT_0023;
    const ccomplex_t IT_0025 = (0 + _Complex_I*1)*(IT_0018 + IT_0021 + 
      -IT_0024);
    const ccomplex_t IT_0026 = (-0.5)*IT_0025;
    const ccomplex_t IT_0027 = IT_0015*IT_0026;
    const ccomplex_t IT_0028 = pow(m_tau, 2);
    const ccomplex_t IT_0029 = pow(m_stau_2, 2);
    const ccomplex_t IT_0030 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_3*(m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0031 = IT_0027*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1)*IT_0031;
    const ccomplex_t IT_0033 = N_B4*e_em*U_stau_10;
    const ccomplex_t IT_0034 = IT_0001*IT_0033;
    const ccomplex_t IT_0035 = 1.4142135623731*IT_0034;
    const ccomplex_t IT_0036 = N_d4*e_em*m_tau*IT_0010*U_stau_00;
    const ccomplex_t IT_0037 = IT_0009*IT_0036;
    const ccomplex_t IT_0038 = 1.4142135623731*IT_0037;
    const ccomplex_t IT_0039 = (0 + _Complex_I*1)*(IT_0035 + 0.5*IT_0038);
    const ccomplex_t IT_0040 = -IT_0039;
    const ccomplex_t IT_0041 = conj(N_B4)*e_em*U_stau_01;
    const ccomplex_t IT_0042 = IT_0001*IT_0041;
    const ccomplex_t IT_0043 = 1.4142135623731*IT_0042;
    const ccomplex_t IT_0044 = conj(N_W4)*e_em*U_stau_01;
    const ccomplex_t IT_0045 = IT_0008*IT_0044;
    const ccomplex_t IT_0046 = 1.4142135623731*IT_0045;
    const ccomplex_t IT_0047 = conj(N_d4)*e_em*m_tau*IT_0010*U_stau_11;
    const ccomplex_t IT_0048 = IT_0009*IT_0047;
    const ccomplex_t IT_0049 = 1.4142135623731*IT_0048;
    const ccomplex_t IT_0050 = (0 + _Complex_I*1)*(IT_0043 + IT_0046 + 
      -IT_0049);
    const ccomplex_t IT_0051 = (-0.5)*IT_0050;
    const ccomplex_t IT_0052 = IT_0040*IT_0051;
    const ccomplex_t IT_0053 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_4*(m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0054 = IT_0052*IT_0053;
    const ccomplex_t IT_0055 = (0 + _Complex_I*1)*IT_0054;
    const ccomplex_t IT_0056 = pow(m_stau_1, 2);
    const ccomplex_t IT_0057 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0056 +
       0.5*m_N_1*(m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0058 = N_B1*e_em*U_stau_11;
    const ccomplex_t IT_0059 = IT_0001*IT_0058;
    const ccomplex_t IT_0060 = 1.4142135623731*IT_0059;
    const ccomplex_t IT_0061 = N_d1*e_em*m_tau*IT_0010*U_stau_01;
    const ccomplex_t IT_0062 = IT_0009*IT_0061;
    const ccomplex_t IT_0063 = 1.4142135623731*IT_0062;
    const ccomplex_t IT_0064 = (0 + _Complex_I*1)*(IT_0060 + 0.5*IT_0063);
    const ccomplex_t IT_0065 = -IT_0064;
    const ccomplex_t IT_0066 = conj(N_B1)*e_em*U_stau_00;
    const ccomplex_t IT_0067 = IT_0001*IT_0066;
    const ccomplex_t IT_0068 = 1.4142135623731*IT_0067;
    const ccomplex_t IT_0069 = conj(N_W1)*e_em*U_stau_00;
    const ccomplex_t IT_0070 = IT_0008*IT_0069;
    const ccomplex_t IT_0071 = 1.4142135623731*IT_0070;
    const ccomplex_t IT_0072 = conj(N_d1)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0073 = IT_0009*IT_0072;
    const ccomplex_t IT_0074 = 1.4142135623731*IT_0073;
    const ccomplex_t IT_0075 = (0 + _Complex_I*1)*(IT_0068 + IT_0071 + 
      -IT_0074);
    const ccomplex_t IT_0076 = (-0.5)*IT_0075;
    const ccomplex_t IT_0077 = IT_0065*IT_0076;
    const ccomplex_t IT_0078 = IT_0057*IT_0077;
    const ccomplex_t IT_0079 = (0 + _Complex_I*1)*IT_0078;
    const ccomplex_t IT_0080 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_1*(m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0081 = conj(N_B1)*e_em*U_stau_01;
    const ccomplex_t IT_0082 = IT_0001*IT_0081;
    const ccomplex_t IT_0083 = 1.4142135623731*IT_0082;
    const ccomplex_t IT_0084 = conj(N_W1)*e_em*U_stau_01;
    const ccomplex_t IT_0085 = IT_0008*IT_0084;
    const ccomplex_t IT_0086 = 1.4142135623731*IT_0085;
    const ccomplex_t IT_0087 = conj(N_d1)*e_em*m_tau*IT_0010*U_stau_11;
    const ccomplex_t IT_0088 = IT_0009*IT_0087;
    const ccomplex_t IT_0089 = 1.4142135623731*IT_0088;
    const ccomplex_t IT_0090 = (0 + _Complex_I*1)*(IT_0083 + IT_0086 + 
      -IT_0089);
    const ccomplex_t IT_0091 = (-0.5)*IT_0090;
    const ccomplex_t IT_0092 = N_B1*e_em*U_stau_10;
    const ccomplex_t IT_0093 = IT_0001*IT_0092;
    const ccomplex_t IT_0094 = 1.4142135623731*IT_0093;
    const ccomplex_t IT_0095 = N_d1*e_em*m_tau*IT_0010*U_stau_00;
    const ccomplex_t IT_0096 = IT_0009*IT_0095;
    const ccomplex_t IT_0097 = 1.4142135623731*IT_0096;
    const ccomplex_t IT_0098 = (0 + _Complex_I*1)*(IT_0094 + 0.5*IT_0097);
    const ccomplex_t IT_0099 = -IT_0098;
    const ccomplex_t IT_0100 = IT_0091*IT_0099;
    const ccomplex_t IT_0101 = IT_0080*IT_0100;
    const ccomplex_t IT_0102 = (0 + _Complex_I*1)*IT_0101;
    const ccomplex_t IT_0103 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_2*(m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0104 = conj(N_B2)*e_em*U_stau_01;
    const ccomplex_t IT_0105 = IT_0001*IT_0104;
    const ccomplex_t IT_0106 = 1.4142135623731*IT_0105;
    const ccomplex_t IT_0107 = conj(N_W2)*e_em*U_stau_01;
    const ccomplex_t IT_0108 = IT_0008*IT_0107;
    const ccomplex_t IT_0109 = 1.4142135623731*IT_0108;
    const ccomplex_t IT_0110 = conj(N_d2)*e_em*m_tau*IT_0010*U_stau_11;
    const ccomplex_t IT_0111 = IT_0009*IT_0110;
    const ccomplex_t IT_0112 = 1.4142135623731*IT_0111;
    const ccomplex_t IT_0113 = (0 + _Complex_I*1)*(IT_0106 + IT_0109 + 
      -IT_0112);
    const ccomplex_t IT_0114 = (-0.5)*IT_0113;
    const ccomplex_t IT_0115 = N_B2*e_em*U_stau_10;
    const ccomplex_t IT_0116 = IT_0001*IT_0115;
    const ccomplex_t IT_0117 = 1.4142135623731*IT_0116;
    const ccomplex_t IT_0118 = N_d2*e_em*m_tau*IT_0010*U_stau_00;
    const ccomplex_t IT_0119 = IT_0009*IT_0118;
    const ccomplex_t IT_0120 = 1.4142135623731*IT_0119;
    const ccomplex_t IT_0121 = (0 + _Complex_I*1)*(IT_0117 + 0.5*IT_0120);
    const ccomplex_t IT_0122 = -IT_0121;
    const ccomplex_t IT_0123 = IT_0114*IT_0122;
    const ccomplex_t IT_0124 = IT_0103*IT_0123;
    const ccomplex_t IT_0125 = (0 + _Complex_I*1)*IT_0124;
    const ccomplex_t IT_0126 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0056 +
       0.5*m_N_2*(m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0127 = N_B2*e_em*U_stau_11;
    const ccomplex_t IT_0128 = IT_0001*IT_0127;
    const ccomplex_t IT_0129 = 1.4142135623731*IT_0128;
    const ccomplex_t IT_0130 = N_d2*e_em*m_tau*IT_0010*U_stau_01;
    const ccomplex_t IT_0131 = IT_0009*IT_0130;
    const ccomplex_t IT_0132 = 1.4142135623731*IT_0131;
    const ccomplex_t IT_0133 = (0 + _Complex_I*1)*(IT_0129 + 0.5*IT_0132);
    const ccomplex_t IT_0134 = -IT_0133;
    const ccomplex_t IT_0135 = conj(N_B2)*e_em*U_stau_00;
    const ccomplex_t IT_0136 = IT_0001*IT_0135;
    const ccomplex_t IT_0137 = 1.4142135623731*IT_0136;
    const ccomplex_t IT_0138 = conj(N_W2)*e_em*U_stau_00;
    const ccomplex_t IT_0139 = IT_0008*IT_0138;
    const ccomplex_t IT_0140 = 1.4142135623731*IT_0139;
    const ccomplex_t IT_0141 = conj(N_d2)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0142 = IT_0009*IT_0141;
    const ccomplex_t IT_0143 = 1.4142135623731*IT_0142;
    const ccomplex_t IT_0144 = (0 + _Complex_I*1)*(IT_0137 + IT_0140 + 
      -IT_0143);
    const ccomplex_t IT_0145 = (-0.5)*IT_0144;
    const ccomplex_t IT_0146 = IT_0134*IT_0145;
    const ccomplex_t IT_0147 = IT_0126*IT_0146;
    const ccomplex_t IT_0148 = (0 + _Complex_I*1)*IT_0147;
    const ccomplex_t IT_0149 = conj(N_B3)*e_em*U_stau_00;
    const ccomplex_t IT_0150 = IT_0001*IT_0149;
    const ccomplex_t IT_0151 = 1.4142135623731*IT_0150;
    const ccomplex_t IT_0152 = conj(N_W3)*e_em*U_stau_00;
    const ccomplex_t IT_0153 = IT_0008*IT_0152;
    const ccomplex_t IT_0154 = 1.4142135623731*IT_0153;
    const ccomplex_t IT_0155 = conj(N_d3)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0156 = IT_0009*IT_0155;
    const ccomplex_t IT_0157 = 1.4142135623731*IT_0156;
    const ccomplex_t IT_0158 = (0 + _Complex_I*1)*(IT_0151 + IT_0154 + 
      -IT_0157);
    const ccomplex_t IT_0159 = (-0.5)*IT_0158;
    const ccomplex_t IT_0160 = N_B3*e_em*U_stau_11;
    const ccomplex_t IT_0161 = IT_0001*IT_0160;
    const ccomplex_t IT_0162 = 1.4142135623731*IT_0161;
    const ccomplex_t IT_0163 = N_d3*e_em*m_tau*IT_0010*U_stau_01;
    const ccomplex_t IT_0164 = IT_0009*IT_0163;
    const ccomplex_t IT_0165 = 1.4142135623731*IT_0164;
    const ccomplex_t IT_0166 = (0 + _Complex_I*1)*(IT_0162 + 0.5*IT_0165);
    const ccomplex_t IT_0167 = -IT_0166;
    const ccomplex_t IT_0168 = IT_0159*IT_0167;
    const ccomplex_t IT_0169 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0056 +
       0.5*m_N_3*(m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0170 = IT_0168*IT_0169;
    const ccomplex_t IT_0171 = (0 + _Complex_I*1)*IT_0170;
    const ccomplex_t IT_0172 = conj(N_B4)*e_em*U_stau_00;
    const ccomplex_t IT_0173 = IT_0001*IT_0172;
    const ccomplex_t IT_0174 = 1.4142135623731*IT_0173;
    const ccomplex_t IT_0175 = conj(N_W4)*e_em*U_stau_00;
    const ccomplex_t IT_0176 = IT_0008*IT_0175;
    const ccomplex_t IT_0177 = 1.4142135623731*IT_0176;
    const ccomplex_t IT_0178 = conj(N_d4)*e_em*m_tau*IT_0010*U_stau_10;
    const ccomplex_t IT_0179 = IT_0009*IT_0178;
    const ccomplex_t IT_0180 = 1.4142135623731*IT_0179;
    const ccomplex_t IT_0181 = (0 + _Complex_I*1)*(IT_0174 + IT_0177 + 
      -IT_0180);
    const ccomplex_t IT_0182 = (-0.5)*IT_0181;
    const ccomplex_t IT_0183 = N_B4*e_em*U_stau_11;
    const ccomplex_t IT_0184 = IT_0001*IT_0183;
    const ccomplex_t IT_0185 = 1.4142135623731*IT_0184;
    const ccomplex_t IT_0186 = N_d4*e_em*m_tau*IT_0010*U_stau_01;
    const ccomplex_t IT_0187 = IT_0009*IT_0186;
    const ccomplex_t IT_0188 = 1.4142135623731*IT_0187;
    const ccomplex_t IT_0189 = (0 + _Complex_I*1)*(IT_0185 + 0.5*IT_0188);
    const ccomplex_t IT_0190 = -IT_0189;
    const ccomplex_t IT_0191 = IT_0182*IT_0190;
    const ccomplex_t IT_0192 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0056 +
       0.5*m_N_4*(m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0193 = IT_0191*IT_0192;
    const ccomplex_t IT_0194 = (0 + _Complex_I*1)*IT_0193;
    const ccomplex_t IT_0195 = (-0.5)*IT_0032 + (-0.5)*IT_0055 + 0.5*IT_0079 +
       (-0.5)*IT_0102 + (-0.5)*IT_0125 + 0.5*IT_0148 + 0.5*IT_0171 + 0.5*IT_0194;
    const ccomplex_t IT_0196 = IT_0123*IT_0126;
    const ccomplex_t IT_0197 = (0 + _Complex_I*1)*IT_0196;
    const ccomplex_t IT_0198 = IT_0027*IT_0169;
    const ccomplex_t IT_0199 = (0 + _Complex_I*1)*IT_0198;
    const ccomplex_t IT_0200 = IT_0052*IT_0192;
    const ccomplex_t IT_0201 = (0 + _Complex_I*1)*IT_0200;
    const ccomplex_t IT_0202 = IT_0057*IT_0100;
    const ccomplex_t IT_0203 = (0 + _Complex_I*1)*IT_0202;
    const ccomplex_t IT_0204 = IT_0077*IT_0080;
    const ccomplex_t IT_0205 = (0 + _Complex_I*1)*IT_0204;
    const ccomplex_t IT_0206 = IT_0030*IT_0168;
    const ccomplex_t IT_0207 = (0 + _Complex_I*1)*IT_0206;
    const ccomplex_t IT_0208 = IT_0053*IT_0191;
    const ccomplex_t IT_0209 = (0 + _Complex_I*1)*IT_0208;
    const ccomplex_t IT_0210 = IT_0103*IT_0146;
    const ccomplex_t IT_0211 = (0 + _Complex_I*1)*IT_0210;
    const ccomplex_t IT_0212 = 0.5*IT_0197 + 0.5*IT_0199 + 0.5*IT_0201 + 0.5
      *IT_0203 + (-0.5)*IT_0205 + (-0.5)*IT_0207 + (-0.5)*IT_0209 + (-0.5)
      *IT_0211;
    const ccomplex_t IT_0213 = IT_0028*IT_0029;
    const ccomplex_t IT_0214 = s_23*s_24;
    const ccomplex_t IT_0215 = s_34*IT_0029;
    const ccomplex_t IT_0216 = (-0.5)*IT_0215;
    const ccomplex_t IT_0217 = IT_0214 + IT_0216;
    const ccomplex_t IT_0218 = s_23*m_tau;
    const ccomplex_t IT_0219 = (0 + _Complex_I*1)*m_N_2;
    const ccomplex_t IT_0220 = IT_0114*IT_0145;
    const ccomplex_t IT_0221 = IT_0126*IT_0219*IT_0220;
    const ccomplex_t IT_0222 = (0 + _Complex_I*1)*m_N_4;
    const ccomplex_t IT_0223 = IT_0051*IT_0182;
    const ccomplex_t IT_0224 = IT_0053*IT_0222*IT_0223;
    const ccomplex_t IT_0225 = m_tau*IT_0171;
    const ccomplex_t IT_0226 = m_tau*IT_0194;
    const ccomplex_t IT_0227 = (0 + _Complex_I*1)*m_N_1;
    const ccomplex_t IT_0228 = IT_0076*IT_0091;
    const ccomplex_t IT_0229 = IT_0080*IT_0227*IT_0228;
    const ccomplex_t IT_0230 = m_tau*IT_0211;
    const ccomplex_t IT_0231 = m_tau*IT_0205;
    const ccomplex_t IT_0232 = m_tau*IT_0079;
    const ccomplex_t IT_0233 = m_tau*IT_0148;
    const ccomplex_t IT_0234 = m_tau*IT_0207;
    const ccomplex_t IT_0235 = m_tau*IT_0209;
    const ccomplex_t IT_0236 = (-0.5)*IT_0221 + (-0.5)*IT_0224 + 0.5*IT_0225 +
       0.5*IT_0226 + (-0.5)*IT_0229 + 0.5*IT_0230 + 0.5*IT_0231 + 0.5*IT_0232 +
       0.5*IT_0233 + 0.5*IT_0234 + 0.5*IT_0235;
    const ccomplex_t IT_0237 = IT_0103*IT_0219*IT_0220;
    const ccomplex_t IT_0238 = (0 + _Complex_I*1)*m_N_3;
    const ccomplex_t IT_0239 = IT_0026*IT_0159;
    const ccomplex_t IT_0240 = IT_0169*IT_0238*IT_0239;
    const ccomplex_t IT_0241 = IT_0030*IT_0238*IT_0239;
    const ccomplex_t IT_0242 = IT_0192*IT_0222*IT_0223;
    const ccomplex_t IT_0243 = IT_0057*IT_0227*IT_0228;
    const ccomplex_t IT_0244 = 0.5*IT_0237 + 0.5*IT_0240 + 0.5*IT_0241 + 0.5
      *IT_0242 + 0.5*IT_0243;
    const ccomplex_t IT_0245 = IT_0236 + -IT_0244;
    const ccomplex_t IT_0246 = -conj(IT_0244);
    const ccomplex_t IT_0247 = conj(IT_0236) + IT_0246;
    const ccomplex_t IT_0248 = IT_0122*IT_0134;
    const ccomplex_t IT_0249 = IT_0126*IT_0219*IT_0248;
    const ccomplex_t IT_0250 = IT_0015*IT_0167;
    const ccomplex_t IT_0251 = IT_0169*IT_0238*IT_0250;
    const ccomplex_t IT_0252 = IT_0030*IT_0238*IT_0250;
    const ccomplex_t IT_0253 = IT_0040*IT_0190;
    const ccomplex_t IT_0254 = IT_0053*IT_0222*IT_0253;
    const ccomplex_t IT_0255 = IT_0065*IT_0099;
    const ccomplex_t IT_0256 = IT_0080*IT_0227*IT_0255;
    const ccomplex_t IT_0257 = 0.5*IT_0249 + 0.5*IT_0251 + 0.5*IT_0252 + 0.5
      *IT_0254 + 0.5*IT_0256;
    const ccomplex_t IT_0258 = 2*IT_0257;
    const ccomplex_t IT_0259 = IT_0103*IT_0219*IT_0248;
    const ccomplex_t IT_0260 = IT_0192*IT_0222*IT_0253;
    const ccomplex_t IT_0261 = m_tau*IT_0197;
    const ccomplex_t IT_0262 = m_tau*IT_0125;
    const ccomplex_t IT_0263 = m_tau*IT_0199;
    const ccomplex_t IT_0264 = m_tau*IT_0032;
    const ccomplex_t IT_0265 = m_tau*IT_0201;
    const ccomplex_t IT_0266 = m_tau*IT_0055;
    const ccomplex_t IT_0267 = IT_0057*IT_0227*IT_0255;
    const ccomplex_t IT_0268 = m_tau*IT_0203;
    const ccomplex_t IT_0269 = m_tau*IT_0102;
    const ccomplex_t IT_0270 = (-0.5)*IT_0259 + (-0.5)*IT_0260 + 0.5*IT_0261 +
       0.5*IT_0262 + 0.5*IT_0263 + 0.5*IT_0264 + 0.5*IT_0265 + 0.5*IT_0266 + (
      -0.5)*IT_0267 + 0.5*IT_0268 + 0.5*IT_0269;
    const ccomplex_t IT_0271 = (-2)*IT_0270;
    const ccomplex_t IT_0272 = IT_0258 + IT_0271;
    const ccomplex_t IT_0273 = 2*conj(IT_0257);
    const ccomplex_t IT_0274 = (-2)*conj(IT_0270);
    const ccomplex_t IT_0275 = IT_0273 + IT_0274;
    const ccomplex_t IT_0276 = 2*IT_0270;
    const ccomplex_t IT_0277 = (-2)*IT_0257;
    const ccomplex_t IT_0278 = IT_0276 + IT_0277;
    const ccomplex_t IT_0279 = 2*conj(IT_0270);
    const ccomplex_t IT_0280 = (-2)*conj(IT_0257);
    const ccomplex_t IT_0281 = IT_0279 + IT_0280;
    const ccomplex_t IT_0282 = s_24*m_tau;
    const ccomplex_t IT_0283 = 2*s_34;
    const ccomplex_t IT_0284 = (-2)*s_34;
    const ccomplex_t IT_0285 = (-2)*conj(IT_0195)*(IT_0212*IT_0213 + (-2)
      *IT_0195*IT_0217) + (-2)*conj(IT_0212)*(IT_0195*IT_0213 + (-2)*IT_0212
      *IT_0217) + (-2)*IT_0218*(conj(IT_0195)*IT_0245 + IT_0195*IT_0247 + (-0.5)
      *conj(IT_0212)*IT_0272 + (-0.5)*IT_0212*IT_0275) + IT_0028*(conj(IT_0236)
      *IT_0272 + IT_0236*IT_0275 + conj(IT_0244)*IT_0278 + IT_0244*IT_0281) + 2*
      (conj(IT_0212)*IT_0245 + IT_0212*IT_0247 + 0.5*conj(IT_0195)*IT_0278 + 0.5
      *IT_0195*IT_0281)*IT_0282 + (IT_0236*conj(IT_0236) + IT_0244*conj(IT_0244)
       + IT_0257*conj(IT_0257) + IT_0270*conj(IT_0270))*IT_0283 + (conj(IT_0236)
      *IT_0244 + IT_0236*conj(IT_0244) + conj(IT_0257)*IT_0270 + IT_0257*conj
      (IT_0270))*IT_0284;
    return create_ccomplex_return(IT_0285);
}

