#include "clooptools.h"
#include "marty/core/looptools_init.h"
#include <math.h>
#include "stdbool.h"
#include "c_sumSqAmpl_stau_2_stau_2_to_tau_tau.h"
#include "ccommon.h"

#include "cparams.h"

ccomplex_return_t c_sumSqAmpl_stau_2_stau_2_to_tau_tau(
        cparam_t const *param
        )
{
    clearcache();
    const creal_t m_W = param->m_W;
    const creal_t beta = param->beta;
    const creal_t e_em = param->e_em;
    const creal_t s_13 = param->s_13;
    const creal_t s_23 = param->s_23;
    const creal_t s_24 = param->s_24;
    const creal_t s_34 = param->s_34;
    const creal_t m_N_1 = param->m_N_1;
    const creal_t m_N_2 = param->m_N_2;
    const creal_t m_N_3 = param->m_N_3;
    const creal_t m_N_4 = param->m_N_4;
    const creal_t m_tau = param->m_tau;
    const creal_t theta_W = param->theta_W;
    const creal_t Gamma_o1 = param->Gamma_o1;
    const creal_t Gamma_o2 = param->Gamma_o2;
    const creal_t Gamma_o3 = param->Gamma_o3;
    const creal_t Gamma_o4 = param->Gamma_o4;
    const creal_t m_stau_2 = param->m_stau_2;
    const creal_t reg_prop = param->reg_prop;
    const ccomplex_t N_B1 = param->N_B1;
    const ccomplex_t N_B2 = param->N_B2;
    const ccomplex_t N_B3 = param->N_B3;
    const ccomplex_t N_B4 = param->N_B4;
    const ccomplex_t N_W1 = param->N_W1;
    const ccomplex_t N_W2 = param->N_W2;
    const ccomplex_t N_W3 = param->N_W3;
    const ccomplex_t N_W4 = param->N_W4;
    const ccomplex_t N_d1 = param->N_d1;
    const ccomplex_t N_d2 = param->N_d2;
    const ccomplex_t N_d3 = param->N_d3;
    const ccomplex_t N_d4 = param->N_d4;
    const ccomplex_t U_stau_01 = param->U_stau_01;
    const ccomplex_t U_stau_11 = param->U_stau_11;
    const ccomplex_t IT_0000 = cos(theta_W);
    const ccomplex_t IT_0001 = cpow(IT_0000, -1);
    const ccomplex_t IT_0002 = conj(N_B4)*e_em*U_stau_01;
    const ccomplex_t IT_0003 = IT_0001*IT_0002;
    const ccomplex_t IT_0004 = 1.4142135623731*IT_0003;
    const ccomplex_t IT_0005 = sin(theta_W);
    const ccomplex_t IT_0006 = cpow(IT_0005, -1);
    const ccomplex_t IT_0007 = conj(N_W4)*e_em*U_stau_01;
    const ccomplex_t IT_0008 = IT_0006*IT_0007;
    const ccomplex_t IT_0009 = 1.4142135623731*IT_0008;
    const ccomplex_t IT_0010 = cos(beta);
    const ccomplex_t IT_0011 = cpow(IT_0010, -1);
    const ccomplex_t IT_0012 = IT_0006*IT_0011;
    const ccomplex_t IT_0013 = pow(m_W, -1);
    const ccomplex_t IT_0014 = conj(N_d4)*e_em*m_tau*IT_0013*U_stau_11;
    const ccomplex_t IT_0015 = IT_0012*IT_0014;
    const ccomplex_t IT_0016 = 1.4142135623731*IT_0015;
    const ccomplex_t IT_0017 = (0 + _Complex_I*1)*(IT_0004 + IT_0009 + 
      -IT_0016);
    const ccomplex_t IT_0018 = (-0.5)*IT_0017;
    const ccomplex_t IT_0019 = N_B4*e_em*U_stau_11;
    const ccomplex_t IT_0020 = IT_0001*IT_0019;
    const ccomplex_t IT_0021 = 1.4142135623731*IT_0020;
    const ccomplex_t IT_0022 = N_d4*e_em*m_tau*IT_0013*U_stau_01;
    const ccomplex_t IT_0023 = IT_0012*IT_0022;
    const ccomplex_t IT_0024 = 1.4142135623731*IT_0023;
    const ccomplex_t IT_0025 = (0 + _Complex_I*1)*(IT_0021 + 0.5*IT_0024);
    const ccomplex_t IT_0026 = -IT_0025;
    const ccomplex_t IT_0027 = IT_0018*IT_0026;
    const ccomplex_t IT_0028 = pow(m_tau, 2);
    const ccomplex_t IT_0029 = pow(m_stau_2, 2);
    const ccomplex_t IT_0030 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_4*(m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0031 = IT_0027*IT_0030;
    const ccomplex_t IT_0032 = (0 + _Complex_I*1)*IT_0031;
    const ccomplex_t IT_0033 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_4*(m_N_4 + (0 + _Complex_I*-1)*Gamma_o4) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0034 = IT_0027*IT_0033;
    const ccomplex_t IT_0035 = (0 + _Complex_I*1)*IT_0034;
    const ccomplex_t IT_0036 = N_B2*e_em*U_stau_11;
    const ccomplex_t IT_0037 = IT_0001*IT_0036;
    const ccomplex_t IT_0038 = 1.4142135623731*IT_0037;
    const ccomplex_t IT_0039 = N_d2*e_em*m_tau*IT_0013*U_stau_01;
    const ccomplex_t IT_0040 = IT_0012*IT_0039;
    const ccomplex_t IT_0041 = 1.4142135623731*IT_0040;
    const ccomplex_t IT_0042 = (0 + _Complex_I*1)*(IT_0038 + 0.5*IT_0041);
    const ccomplex_t IT_0043 = -IT_0042;
    const ccomplex_t IT_0044 = conj(N_B2)*e_em*U_stau_01;
    const ccomplex_t IT_0045 = IT_0001*IT_0044;
    const ccomplex_t IT_0046 = 1.4142135623731*IT_0045;
    const ccomplex_t IT_0047 = conj(N_W2)*e_em*U_stau_01;
    const ccomplex_t IT_0048 = IT_0006*IT_0047;
    const ccomplex_t IT_0049 = 1.4142135623731*IT_0048;
    const ccomplex_t IT_0050 = conj(N_d2)*e_em*m_tau*IT_0013*U_stau_11;
    const ccomplex_t IT_0051 = IT_0012*IT_0050;
    const ccomplex_t IT_0052 = 1.4142135623731*IT_0051;
    const ccomplex_t IT_0053 = (0 + _Complex_I*1)*(IT_0046 + IT_0049 + 
      -IT_0052);
    const ccomplex_t IT_0054 = (-0.5)*IT_0053;
    const ccomplex_t IT_0055 = IT_0043*IT_0054;
    const ccomplex_t IT_0056 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_2*(m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0057 = IT_0055*IT_0056;
    const ccomplex_t IT_0058 = (0 + _Complex_I*1)*IT_0057;
    const ccomplex_t IT_0059 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_2*(m_N_2 + (0 + _Complex_I*-1)*Gamma_o2) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0060 = IT_0055*IT_0059;
    const ccomplex_t IT_0061 = (0 + _Complex_I*1)*IT_0060;
    const ccomplex_t IT_0062 = N_B3*e_em*U_stau_11;
    const ccomplex_t IT_0063 = IT_0001*IT_0062;
    const ccomplex_t IT_0064 = 1.4142135623731*IT_0063;
    const ccomplex_t IT_0065 = N_d3*e_em*m_tau*IT_0013*U_stau_01;
    const ccomplex_t IT_0066 = IT_0012*IT_0065;
    const ccomplex_t IT_0067 = 1.4142135623731*IT_0066;
    const ccomplex_t IT_0068 = (0 + _Complex_I*1)*(IT_0064 + 0.5*IT_0067);
    const ccomplex_t IT_0069 = -IT_0068;
    const ccomplex_t IT_0070 = conj(N_B3)*e_em*U_stau_01;
    const ccomplex_t IT_0071 = IT_0001*IT_0070;
    const ccomplex_t IT_0072 = 1.4142135623731*IT_0071;
    const ccomplex_t IT_0073 = conj(N_W3)*e_em*U_stau_01;
    const ccomplex_t IT_0074 = IT_0006*IT_0073;
    const ccomplex_t IT_0075 = 1.4142135623731*IT_0074;
    const ccomplex_t IT_0076 = conj(N_d3)*e_em*m_tau*IT_0013*U_stau_11;
    const ccomplex_t IT_0077 = IT_0012*IT_0076;
    const ccomplex_t IT_0078 = 1.4142135623731*IT_0077;
    const ccomplex_t IT_0079 = (0 + _Complex_I*1)*(IT_0072 + IT_0075 + 
      -IT_0078);
    const ccomplex_t IT_0080 = (-0.5)*IT_0079;
    const ccomplex_t IT_0081 = IT_0069*IT_0080;
    const ccomplex_t IT_0082 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_3*(m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0083 = IT_0081*IT_0082;
    const ccomplex_t IT_0084 = (0 + _Complex_I*1)*IT_0083;
    const ccomplex_t IT_0085 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_3*(m_N_3 + (0 + _Complex_I*-1)*Gamma_o3) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0086 = IT_0081*IT_0085;
    const ccomplex_t IT_0087 = (0 + _Complex_I*1)*IT_0086;
    const ccomplex_t IT_0088 = cpow(s_13 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_1*(m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0089 = conj(N_B1)*e_em*U_stau_01;
    const ccomplex_t IT_0090 = IT_0001*IT_0089;
    const ccomplex_t IT_0091 = 1.4142135623731*IT_0090;
    const ccomplex_t IT_0092 = conj(N_W1)*e_em*U_stau_01;
    const ccomplex_t IT_0093 = IT_0006*IT_0092;
    const ccomplex_t IT_0094 = 1.4142135623731*IT_0093;
    const ccomplex_t IT_0095 = conj(N_d1)*e_em*m_tau*IT_0013*U_stau_11;
    const ccomplex_t IT_0096 = IT_0012*IT_0095;
    const ccomplex_t IT_0097 = 1.4142135623731*IT_0096;
    const ccomplex_t IT_0098 = (0 + _Complex_I*1)*(IT_0091 + IT_0094 + 
      -IT_0097);
    const ccomplex_t IT_0099 = (-0.5)*IT_0098;
    const ccomplex_t IT_0100 = N_B1*e_em*U_stau_11;
    const ccomplex_t IT_0101 = IT_0001*IT_0100;
    const ccomplex_t IT_0102 = 1.4142135623731*IT_0101;
    const ccomplex_t IT_0103 = N_d1*e_em*m_tau*IT_0013*U_stau_01;
    const ccomplex_t IT_0104 = IT_0012*IT_0103;
    const ccomplex_t IT_0105 = 1.4142135623731*IT_0104;
    const ccomplex_t IT_0106 = (0 + _Complex_I*1)*(IT_0102 + 0.5*IT_0105);
    const ccomplex_t IT_0107 = -IT_0106;
    const ccomplex_t IT_0108 = IT_0099*IT_0107;
    const ccomplex_t IT_0109 = IT_0088*IT_0108;
    const ccomplex_t IT_0110 = (0 + _Complex_I*1)*IT_0109;
    const ccomplex_t IT_0111 = cpow(s_23 + (-0.5)*IT_0028 + (-0.5)*IT_0029 +
       0.5*m_N_1*(m_N_1 + (0 + _Complex_I*-1)*Gamma_o1) + (-0.5)*reg_prop, -1);
    const ccomplex_t IT_0112 = IT_0108*IT_0111;
    const ccomplex_t IT_0113 = (0 + _Complex_I*1)*IT_0112;
    const ccomplex_t IT_0114 = 0.5*IT_0032 + (-0.5)*IT_0035 + 0.5*IT_0058 + (
      -0.5)*IT_0061 + 0.5*IT_0084 + (-0.5)*IT_0087 + 0.5*IT_0110 + (-0.5)*IT_0113;
    const ccomplex_t IT_0115 = IT_0028*IT_0029;
    const ccomplex_t IT_0116 = s_23*s_24;
    const ccomplex_t IT_0117 = s_34*IT_0029;
    const ccomplex_t IT_0118 = (-0.5)*IT_0117;
    const ccomplex_t IT_0119 = IT_0116 + IT_0118;
    const ccomplex_t IT_0120 = s_23*m_tau;
    const ccomplex_t IT_0121 = cpow(IT_0054, 2);
    const ccomplex_t IT_0122 = (0 + _Complex_I*1)*m_N_2*IT_0121;
    const ccomplex_t IT_0123 = IT_0056*IT_0122;
    const ccomplex_t IT_0124 = IT_0059*IT_0122;
    const ccomplex_t IT_0125 = cpow(IT_0080, 2);
    const ccomplex_t IT_0126 = (0 + _Complex_I*1)*m_N_3*IT_0125;
    const ccomplex_t IT_0127 = IT_0082*IT_0126;
    const ccomplex_t IT_0128 = IT_0085*IT_0126;
    const ccomplex_t IT_0129 = cpow(IT_0018, 2);
    const ccomplex_t IT_0130 = (0 + _Complex_I*1)*m_N_4*IT_0129;
    const ccomplex_t IT_0131 = IT_0030*IT_0130;
    const ccomplex_t IT_0132 = IT_0033*IT_0130;
    const ccomplex_t IT_0133 = cpow(IT_0099, 2);
    const ccomplex_t IT_0134 = (0 + _Complex_I*1)*m_N_1*IT_0133;
    const ccomplex_t IT_0135 = IT_0088*IT_0134;
    const ccomplex_t IT_0136 = IT_0111*IT_0134;
    const ccomplex_t IT_0137 = 0.5*IT_0123 + 0.5*IT_0124 + 0.5*IT_0127 + 0.5
      *IT_0128 + 0.5*IT_0131 + 0.5*IT_0132 + 0.5*IT_0135 + 0.5*IT_0136;
    const ccomplex_t IT_0138 = IT_0032 + IT_0035 + IT_0058 + IT_0061 + IT_0084
       + IT_0087 + IT_0110 + IT_0113;
    const ccomplex_t IT_0139 = m_tau*IT_0138;
    const ccomplex_t IT_0140 = 0.5*IT_0139;
    const ccomplex_t IT_0141 = IT_0137 + -IT_0140;
    const ccomplex_t IT_0142 = -conj(IT_0140);
    const ccomplex_t IT_0143 = conj(IT_0137) + IT_0142;
    const ccomplex_t IT_0144 = cpow(IT_0107, 2);
    const ccomplex_t IT_0145 = (0 + _Complex_I*1)*m_N_1*IT_0144;
    const ccomplex_t IT_0146 = IT_0088*IT_0145;
    const ccomplex_t IT_0147 = IT_0111*IT_0145;
    const ccomplex_t IT_0148 = cpow(IT_0043, 2);
    const ccomplex_t IT_0149 = (0 + _Complex_I*1)*m_N_2*IT_0148;
    const ccomplex_t IT_0150 = IT_0056*IT_0149;
    const ccomplex_t IT_0151 = IT_0059*IT_0149;
    const ccomplex_t IT_0152 = cpow(IT_0069, 2);
    const ccomplex_t IT_0153 = (0 + _Complex_I*1)*m_N_3*IT_0152;
    const ccomplex_t IT_0154 = IT_0082*IT_0153;
    const ccomplex_t IT_0155 = cpow(IT_0026, 2);
    const ccomplex_t IT_0156 = (0 + _Complex_I*1)*m_N_4*IT_0155;
    const ccomplex_t IT_0157 = IT_0030*IT_0156;
    const ccomplex_t IT_0158 = IT_0033*IT_0156;
    const ccomplex_t IT_0159 = IT_0085*IT_0153;
    const ccomplex_t IT_0160 = 0.5*IT_0146 + 0.5*IT_0147 + 0.5*IT_0150 + 0.5
      *IT_0151 + 0.5*IT_0154 + 0.5*IT_0157 + 0.5*IT_0158 + 0.5*IT_0159;
    const ccomplex_t IT_0161 = 2*IT_0160;
    const ccomplex_t IT_0162 = (-2)*IT_0140;
    const ccomplex_t IT_0163 = IT_0161 + IT_0162;
    const ccomplex_t IT_0164 = 2*conj(IT_0160);
    const ccomplex_t IT_0165 = (-2)*conj(IT_0140);
    const ccomplex_t IT_0166 = IT_0164 + IT_0165;
    const ccomplex_t IT_0167 = 2*IT_0140;
    const ccomplex_t IT_0168 = (-2)*IT_0160;
    const ccomplex_t IT_0169 = IT_0167 + IT_0168;
    const ccomplex_t IT_0170 = 2*conj(IT_0140);
    const ccomplex_t IT_0171 = (-2)*conj(IT_0160);
    const ccomplex_t IT_0172 = IT_0170 + IT_0171;
    const ccomplex_t IT_0173 = s_24*m_tau;
    const ccomplex_t IT_0174 = 2*s_34;
    const ccomplex_t IT_0175 = (-2)*s_34;
    const ccomplex_t IT_0176 = (-4)*conj(IT_0114)*(IT_0114*IT_0115 + (-2)
      *IT_0114*IT_0119) + 2*IT_0120*(conj(IT_0114)*IT_0141 + IT_0114*IT_0143 +
       0.5*conj(IT_0114)*IT_0163 + 0.5*IT_0114*IT_0166) + IT_0028*(conj(IT_0140)
      *IT_0163 + IT_0140*IT_0166 + conj(IT_0137)*IT_0169 + IT_0137*IT_0172) + (
      -2)*(conj(IT_0114)*IT_0141 + IT_0114*IT_0143 + (-0.5)*conj(IT_0114)
      *IT_0169 + (-0.5)*IT_0114*IT_0172)*IT_0173 + (IT_0137*conj(IT_0137) + 2
      *IT_0140*conj(IT_0140) + IT_0160*conj(IT_0160))*IT_0174 + (conj(IT_0137)
      *IT_0140 + IT_0137*conj(IT_0140) + conj(IT_0140)*IT_0160 + IT_0140*conj
      (IT_0160))*IT_0175;
    return create_ccomplex_return(IT_0176);
}

