...
SpectrumOutput updateDiagonalization(SpectrumInput const &inputs)
{
    ...
    Diagonalizer::applyBiDiagonalization(
        ...
        {&outputs.m_C_1, &outputs.m_C_2, }
        );

    Diagonalizer::applyDiagonalization(
        {
            std::pow(m_t, 2) + std::pow(M_q3L, 2) + (-0.5)*std::pow(m_W, 2)*std::pow(std::sin(beta), 2) + (-0.5)*std::pow(m_W, 2)*(1 + -std::pow(std::sin(beta), 2)) + (-0.166667)*std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2) + 0.166667*std::pow(m_Z, 2)*(1 + -std::pow(std::sin(beta), 2))*std::pow(std::sin(theta_W), 2),
            (-2)*m_t*mu_h*std::cos(beta)/std::sin(beta) + 2.82843*A_t*m_W*std::sin(beta)*std::sin(theta_W)/e_em,
            0,
            std::pow(m_t, 2) + std::pow(M_qtR, 2) + (-0.666667)*std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2) + 0.666667*std::pow(m_Z, 2)*(1 + -std::pow(std::sin(beta), 2))*std::pow(std::sin(theta_W), 2),
        },
        {&outputs.U_st_00, &outputs.U_st_01, &outputs.U_st_10, &outputs.U_st_11, },
        {&outputs.m_st_1, &outputs.m_st_2, }
        );
    if (0 > outputs.m_st_1) {
    ...
    Diagonalizer::applyDiagonalization(
        {
            1.45924e-05*std::pow(M_q1L, 2) + std::pow(V_cb, 2)*std::pow(M_q1L, 2) + V_tb*std::conj(V_tb)*std::pow(M_q3L, 2) + (-0.5)*std::pow(m_W, 2)*std::pow(std::sin(beta), 2) + std::pow(m_b, 2)*std::pow(std::cos(beta), -2)*(1 + -std::pow(std::sin(beta), 2)) + (-0.166667)*std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2),
            (-2)*m_b*mu_h*std::sin(beta)/std::cos(beta) + 2.82843*A_b*m_W*V_tb*std::conj(V_tb)*std::cos(beta)*std::sin(theta_W)/e_em,
            0,
            V_tb*std::conj(V_tb)*std::pow(M_qbR, 2) + 1.45924e-05*std::pow(M_qdR, 2) + std::pow(V_cb, 2)*std::pow(M_qdR, 2) + std::pow(m_b, 2)*std::pow(std::cos(beta), -2)*(1 + -std::pow(std::sin(beta), 2)) + 0.333333*std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2) + (-0.333333)*std::pow(m_Z, 2)*(1 + -std::pow(std::sin(beta), 2))*std::pow(std::sin(theta_W), 2),
        },
        {&outputs.U_sb_00, &outputs.U_sb_01, &outputs.U_sb_10, &outputs.U_sb_11, },
        {&outputs.m_sb_1, &outputs.m_sb_2, }
        );
    if (0 > outputs.m_sb_1) {
    ...
    Diagonalizer::applyDiagonalization(
        {
            std::pow(M_tauL, 2) + (-0.5)*std::pow(m_W, 2)*std::pow(std::sin(beta), 2) + (-0.5)*std::pow(m_W, 2)*(1 + -std::pow(std::sin(beta), 2)) + std::pow(m_tau, 2)*std::pow(std::cos(beta), -2)*(1 + -std::pow(std::sin(beta), 2)) + 0.5*std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2) + (-0.5)*std::pow(m_Z, 2)*(1 + -std::pow(std::sin(beta), 2))*std::pow(std::sin(theta_W), 2),
            (-2)*mu_h*m_tau*std::sin(beta)/std::cos(beta) + 2.82843*m_W*A_tau*std::cos(beta)*std::sin(theta_W)/e_em,
            0,
            std::pow(M_tauR, 2) + std::pow(m_tau, 2)*std::pow(std::cos(beta), -2)*(1 + -std::pow(std::sin(beta), 2)) + std::pow(m_Z, 2)*std::pow(std::sin(beta), 2)*std::pow(std::sin(theta_W), 2) + -std::pow(m_Z, 2)*(1 + -std::pow(std::sin(beta), 2))*std::pow(std::sin(theta_W), 2),
        },
        {&outputs.U_stau_00, &outputs.U_stau_01, &outputs.U_stau_10, &outputs.U_stau_11, },
        {&outputs.m_stau_1, &outputs.m_stau_2, }
        );
